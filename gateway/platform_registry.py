...
class PlatformEntry:
    ...
    # status displays (``fulilian setup``, ``fulilian status``, the dashboard
    ...
    # Env vars this platform needs (for ``fulilian setup`` display).
    ...
    # built-ins).  Used by ``fulilian gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved FULILIAN_HOME and overlay the
        ...
        # plugin-discovery time added several seconds to *every* `fulilian`
        # invocation -- including plain `fulilian chat`, which never touches any
        ...
        # `fulilian setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``fulilian setup``/``gateway status``, channel
