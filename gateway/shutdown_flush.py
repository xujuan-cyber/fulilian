...
1. ``flush_pending_to_file()`` — called BEFORE ``_pending_messages.clear()``
   ...
   under ``<fulilian_home>/pending_messages/``.
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active FULILIAN_HOME."""
    from fulilian_constants import get_fulilian_home

    flush_dir = get_fulilian_home() / "pending_messages"
...
def spool_dropped_transcript_message(
    ...
    ``<fulilian_home>/pending_messages/``), so a runtime cap rotation no
