...
There are two ways to add a platform to the Fulilian gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.fulilian/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Fulilian code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `fulilian gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `fulilian_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Fulilian core.
...
And add it to the `fulilian-gateway` composite:

```python
"fulilian-gateway": {
    "includes": [..., "fulilian-your-platform"]
}
```
...
## 12. Status Display (`fulilian_cli/status.py`)
...
## 13. Gateway Setup Wizard (`fulilian_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ fulilian_cli/ toolsets.py \
  --include="*.py" -l | sort -u
