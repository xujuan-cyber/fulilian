...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.fulilian to the active profile home."""
    ...
    if raw.startswith("~/.fulilian/"):
        return fulilian_home / raw.removeprefix("~/.fulilian/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return fulilian_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under FULILIAN_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from fulilian_constants import get_fulilian_home

    scripts_root = (get_fulilian_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.fulilian" or raw_text.startswith("~/.fulilian/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__fulilian_ignore__") is True
        ):
            return False, None
