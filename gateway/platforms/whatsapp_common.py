...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *FuLiLian*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to FULILIAN_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/fulilian), this function
    mirrors the bridge source to a writable FULILIAN_HOME location and returns that
    ...
    # Try FULILIAN_HOME location first
    fulilian_home = get_fulilian_home()
    fulilian_home_bridge = fulilian_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to FULILIAN_HOME if needed
    if fulilian_home_bridge.exists():
        return fulilian_home_bridge

    # Mirror the bridge source to FULILIAN_HOME
    try:
        fulilian_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            fulilian_home_bridge,
            dirs_exist_ok=False,
        )
        return fulilian_home_bridge
    except Exception:
