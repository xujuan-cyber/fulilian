...
Defenses 1 and 2 (the ``_FULILIAN_GATEWAY`` guard on ``fulilian gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.fulilian.gateway")``,
...
State lives in ``<FULILIAN_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_fulilian_home() / "gateway" / "restart_loop.json"
