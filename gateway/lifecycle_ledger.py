...
``<FULILIAN_HOME>/state/gateway.lifecycle.json``:
...
def _process_fulilian_home() -> Path:
    """FULILIAN_HOME for process-level identity files (ignore task overrides)."""
...
def get_lifecycle_sentinel_path(home: Optional[Path] = None) -> Path:
    """Return ``<FULILIAN_HOME>/state/gateway.lifecycle.json``."""
...
def read_prior_exit_label(profile_home: Path) -> str:
    ...
    ``fulilian_cli.container_boot`` to annotate ``container-boot.log``.
    """
    try:
