...
def render_skill_page(
    ...
    fulilian_meta = (fm.get("metadata") or {}).get("fulilian") or {}
    tags = fulilian_meta.get("tags") or []
    related = fulilian_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `fulilian skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with FuLiLian"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Fulilian ships with a large built-in skill library copied into `~/.fulilian/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Fulilian also syncs bundled skills on `fulilian update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.fulilian/skills/` tree, it is still shipped with Fulilian; restore it with `fulilian skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with fulilian-agent — install via fulilian skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with fulilian-agent under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
