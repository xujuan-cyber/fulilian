...
ACP 适配器将 Fulilian 的同步 `AIAgent` 封装为异步 JSON-RPC stdio 服务器。
...
`acp_adapter/tools.py` 将 Fulilian 工具映射到 ACP 工具类型，并构建面向编辑器的内容。
...
而是复用 Fulilian 的运行时解析器：

- `acp_adapter/auth.py`
- `fulilian_cli/runtime_provider.py`

因此 ACP 通告并使用当前配置的 Fulilian provider/凭据。它还始终通告一个终端 setup 认证方法（`fulilian-setup`，参数 `--setup`），以便首次运行的 ACP 客户端在启动正常 ACP 会话前可以打开 Fulilian 的交互式模型/provider 配置。
...
- ACP 会话持久化至共享的 `~/.fulilian/state.db`（SessionDB），在进程重启后透明恢复；它们会出现在 `session_search` 中
...
- `tests/acp/` — ACP 测试套件
- `toolsets.py` — `fulilian-acp` toolset 定义
- `fulilian_cli/main.py` — `fulilian acp` CLI 子命令
- `pyproject.toml` — `[acp]` 可选依赖 + `fulilian-acp` 脚本