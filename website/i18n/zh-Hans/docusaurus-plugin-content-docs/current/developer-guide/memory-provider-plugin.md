...
description: "如何为 FuLiLian 构建 memory provider 插件"
---

# 构建 Memory Provider 插件

Memory provider 插件为 FuLiLian 提供跨会话的持久化知识，超越内置的 MEMORY.md 和 USER.md。本指南介绍如何构建一个 memory provider 插件。

:::tip
Memory provider 是两种 **provider 插件**类型之一。另一种是 [Context Engine 插件](/developer-guide/context-engine-plugin)，用于替换内置的上下文压缩器。两者遵循相同的模式：单选、配置驱动、通过 `fulilian plugins` 管理。
:::
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs 始终包含：
          fulilian_home (str): 当前活跃的 FULILIAN_HOME 路径。用于存储数据。
        """
...
| 方法 | 用途 | 是否必须实现？ |
|--------|---------|-----------------|
| `get_config_schema()` | 为 `fulilian memory setup` 声明配置字段 | **是** |
| `save_config(values, fulilian_home)` | 将非敏感配置写入原生位置 | **是**（除非仅使用环境变量） |
...
`get_config_schema()` 返回一个字段描述符列表，供 `fulilian memory setup` 使用：
...
`get_config_schema()` 中的每个字段都会在 `fulilian memory setup` 期间提示用户输入。选项较多的 provider 应保持 schema 精简——只包含用户**必须**配置的字段（API key、必要凭证）。可选配置请在配置文件参考文档中说明（例如 `$FULILIAN_HOME/myprovider.json`），而不是在 setup 向导中逐一提示。这样既能保持 setup 流程简洁，又支持高级配置。可参考 Supermemory provider 的实现——它只提示输入 API key，其余选项均位于 `supermemory.json` 中。
:::
...
所有存储路径**必须**使用 `initialize()` 中的 `fulilian_home` kwarg，而不是硬编码的 `~/.fulilian`：
...
Memory provider 插件可以注册自己的 CLI 子命令树（例如 `fulilian my-provider status`、`fulilian my-provider config`）。这套系统基于约定发现，无需修改核心文件。
...
4. 你的命令以 `fulilian <provider-name> <subcommand>` 的形式出现

**仅对活跃 provider 开放：** 你的 CLI 命令只在你的 provider 是配置中活跃的 `memory.provider` 时才会出现。如果用户尚未配置你的 provider，你的命令不会显示在 `fulilian --help` 中。
...
def register_cli(subparser) -> None:
    """构建 fulilian my-provider 的 argparse 树。
