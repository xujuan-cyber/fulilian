...
sidebar_position: 13
title: "Webhooks"
description: "接收来自 GitHub、GitLab 等服务的事件以触发 Fulilian agent 运行"
---

# Webhooks

接收来自外部服务（GitHub、GitLab、JIRA、Stripe 等）的事件，并自动触发 Fulilian agent 运行。Webhook 适配器运行一个 HTTP 服务器，接受 POST 请求、验证 HMAC 签名、将 payload（载荷）转换为 agent prompt（提示词），并将响应路由回来源或其他已配置的平台。
...
1. 通过 `fulilian gateway setup` 或环境变量启用
2. 在 `config.yaml` 中定义路由，**或**使用 `fulilian webhook subscribe` 动态创建
...
| `script` | 否 | 位于 `~/.fulilian/scripts/` 下的过滤/转换脚本。webhook payload 以 JSON 形式通过 stdin 传入。stdout 为 JSON 对象时会在模板渲染前替换 payload；文本 stdout 以 `script_output` 形式暴露；空 stdout、`[SILENT]` 或非零退出码会忽略该 webhook。 |
...
当声明式过滤器不够用时，使用 `script`。脚本必须位于当前 profile 的 `~/.fulilian/scripts/` 目录下；相对路径在该目录内解析，且禁止路径穿越到目录之外。`.sh` 和 `.bash` 脚本用 bash 运行，其他扩展名用当前 Python 解释器运行。
...
- 空 stdout、精确的 `[SILENT]`、`{"__fulilian_ignore__": true}`、超时、脚本缺失或非零退出码，均返回 HTTP 200 及 `{"status":"ignored","reason":"script"}`。
...
按照上方示例，将 `github-pr` 路由添加到 `~/.fulilian/config.yaml`。
...
在仓库中打开一个 pull request。webhook 触发后，Fulilian 处理事件并在 PR 上发布审查评论。
...
除了 `config.yaml` 中的静态路由，还可以使用 `fulilian webhook` CLI 命令动态创建 webhook 订阅。当 agent 本身需要设置事件驱动触发器时，这尤为有用。
...
- 订阅存储在 `~/.fulilian/webhook_subscriptions.json`
...
agent 可通过 terminal 工具在 `webhook-subscriptions` skill 的引导下创建订阅。向 agent 请求"为 GitHub issues 设置 webhook"，它将运行相应的 `fulilian webhook subscribe` 命令。
...
- 在前台运行 gateway 以查看日志：`fulilian gateway run`
- 检查 prompt 模板是否正确渲染
- 验证投递目标已配置并连接
