...
- 需要逐步执行某个函数并观察集合的变化
- 长期运行的进程（fulilian gateway、tui_gateway）出现异常且无法重启
...
fulilian 测试运行器和 pytest 均支持以下方式：
...
适用于长期运行的进程：Fulilian gateway、tui_gateway、daemon，或已出现异常且无法干净重启的进程。

### 安装

```bash
source /home/bb/fulilian-agent/.venv/bin/activate
pip install debugpy
```
...
最简便的终端侧 DAP 客户端是 VS Code CLI 或一个小脚本。在 Fulilian 内部有两个实用选项：
...
```json
{
  ...
  "justMyCode": false,
  "pathMappings": [
    { "localRoot": "${workspaceFolder}", "remoteRoot": "/home/bb/fulilian-agent" }
  ]
}
```
...
最简单：在可疑行附近添加 `breakpoint()`，然后正常运行 `fulilian`。控制权将在暂停点返回到你的终端。

### `tui_gateway` 子进程（由 `fulilian --tui` 启动）
...
debugpy.wait_for_client()
```
启动 `fulilian --tui`。TUI 将显示为冻结状态（其后端正在等待）。附加客户端后，执行在你 `continue` 时恢复。
...
9. **fork / 多进程。** pdb 不会跟随 fork。每个子进程需要自己的 `breakpoint()` 或 `set_trace()`。对于 Fulilian 子 agent，每次只调试一个进程。
