...
sidebar_position: 3
title: "常见问题与故障排查"
description: "FuLiLian 常见问题解答及常见问题解决方案"
---
...
### Fulilian 支持哪些 LLM 提供商？

FuLiLian 可与任何兼容 OpenAI 的 API 配合使用。支持的提供商包括：
...
- **Anthropic** — Claude 模型（直接 API、通过 `fulilian auth add anthropic` 进行 OAuth、OpenRouter 或任何兼容代理）
...
使用 `fulilian model` 设置提供商，或直接编辑 `~/.fulilian/.env`。所有提供商 key 请参阅[环境变量](./environment-variables.md)参考文档。
...
- 让 Fulilian 使用生成的 MCP 浏览器工具

这比强制 Fulilian 核心浏览器传输直接跨越 WSL2/Windows 边界进行附加更为可靠。

参见：

- [在 Fulilian 中使用 MCP](../guides/use-mcp-with-fulilian.md#wsl2-bridge-fulilian-in-wsl-to-windows-chrome)
- [浏览器自动化](../user-guide/features/browser.md#wsl2--windows-chrome-prefer-mcp-over-browser-connect)

### 支持 Android / Termux 吗？

支持 — Fulilian 现已为 Android 手机提供经过测试的 Termux 安装路径。
...
API 调用**仅发送至您配置的 LLM 提供商**（例如 OpenRouter、您本地的 Ollama 实例）。FuLiLian 不收集遥测数据、使用数据或分析数据。您的对话、记忆和技能均存储在本地 `~/.fulilian/` 目录中。

### 可以离线使用 / 使用本地模型吗？

可以。运行 `fulilian model`，选择**自定义端点**，然后输入您服务器的 URL：
...
Fulilian 会将端点、提供商和 base URL 持久化到 `config.yaml`，重启后仍然有效。如果您的本地服务器只加载了一个模型，`/model custom` 会自动检测到它。您也可以在 config.yaml 中设置 `provider: custom` — 这是一个一等提供商，不是其他任何东西的别名。

此方式适用于 Ollama、vLLM、llama.cpp server、SGLang、LocalAI 等。详情请参阅[配置指南](../user-guide/configuration.md)。

:::tip Ollama 用户
如果您在 Ollama 中设置了自定义 `num_ctx`（例如 `ollama run --num_ctx 16384`），请确保在 Fulilian 中设置匹配的上下文长度 — Ollama 的 `/api/show` 报告的是模型的*最大*上下文，而非您配置的实际 `num_ctx`。
:::

:::tip 本地模型超时问题
Fulilian 会自动检测本地端点并放宽流式传输超时（读取超时从 120s 提升至 1800s，禁用停滞流检测）。如果在非常大的上下文下仍然超时，请在 `.env` 中设置 `FULILIAN_STREAM_READ_TIMEOUT=1800`。详情请参阅[本地 LLM 指南](../guides/local-llm-on-mac.md#timeouts)。
:::

### 费用是多少？

FuLiLian 本身**免费且开源**（MIT 许可证）。您只需为所选提供商的 LLM API 用量付费。本地模型完全免费运行。

### 多人可以使用同一个实例吗？

可以。[消息网关](../user-guide/messaging/index.md)允许多个用户通过 Telegram、Discord、Slack、WhatsApp 或 Home Assistant 与同一个 FuLiLian 实例交互。访问权限通过白名单（特定用户 ID）和私信配对（第一个发消息的用户获得访问权）来控制。
...
可以。导入 `AIAgent` 类，以编程方式使用 Fulilian：
...
#### 安装后出现 `fulilian: command not found`
...
**原因：** Fulilian 需要 Python 3.11 或更新版本。
...
**原因：** Fulilian 在启动时通过运行一次 `bash -l` 构建每个会话的环境快照。bash 登录 shell 会读取 `/etc/profile`、`~/.bash_profile` 和 `~/.profile`，但**不会 source `~/.bashrc`** — 因此在 `~/.bashrc` 中安装自身的工具（`nvm`、`asdf`、`pyenv`、`cargo`、自定义 `PATH` 导出）对快照不可见。当 Fulilian 在 systemd 下运行或在未预加载交互式 shell 配置的最小 shell 中运行时，此问题最为常见。

**解决方案：** Fulilian 默认自动 source `~/.bashrc`。如果这还不够 — 例如您是 zsh 用户，PATH 在 `~/.zshrc` 中，或者您从独立文件初始化 `nvm` — 请在 `~/.fulilian/config.yaml` 中列出需要额外 source 的文件：
...
**解决方案：** 退出当前会话，在终端中使用 `fulilian model` 添加新提供商：

```bash
# 先退出 Fulilian 聊天会话（Ctrl+C 或 /quit）
...
通过 `fulilian model` 添加新提供商后，启动新的聊天会话 — `/model` 将显示所有已配置的提供商。

:::tip 快速参考
| 目标 | 使用方式 |
|-----------|-----|
| 添加新提供商 | `fulilian model`（从终端） |
| 输入/更改 API key | `fulilian model`（从终端） |
...
:::warning
请确保 key 与提供商匹配。OpenAI 的 key 无法用于 OpenRouter，反之亦然。检查 `~/.fulilian/.env` 中是否有冲突条目。
:::
...
- 升级您的提供商套餐
- 切换到其他模型或提供商
- 使用 `fulilian chat --provider <alternative>` 路由到其他后端

#### 上下文长度超限

**原因：** 对话内容超出模型的上下文窗口，或 Fulilian 检测到的模型上下文长度有误。
...
如果在第一次长对话时就出现此问题，Fulilian 可能检测到了错误的模型上下文长度。检查检测结果：
...
**原因：** Fulilian 检测到潜在的破坏性命令（例如 `rm -rf`、`DROP TABLE`）。这是一项安全功能。
...
:::tip
这是预期行为 — Fulilian 绝不会静默执行破坏性命令。审批提示会向您显示将要执行的确切内容。
:::
...
**解决方案：**
- 使用 `fulilian gateway setup` 验证您的 bot token 是否有效
- 检查网关日志：`cat ~/.fulilian/logs/gateway.log | tail -50`
...
在 `~/.fulilian/config.yaml` 中您的网关设置下进行配置。请参阅[消息文档](../user-guide/messaging/index.md)。
...
```bash
# 安装核心消息网关依赖项
cd ~/.fulilian/fulilian-agent && uv pip install -e ".[messaging]"  # Telegram、Discord、Slack 及共享网关依赖
...
#### WSL：网关持续断开连接或 `fulilian gateway start` 失败
...
1. 创建一个任务，运行 `wsl -d Ubuntu -- bash -lc 'fulilian gateway run'`
2. 设置在用户登录时触发
:::
...
**解决方案：** 网关在您运行 `fulilian gateway install` 时会捕获您的 shell PATH。如果您在设置网关后安装了新工具，请重新运行 install 以捕获更新后的 PATH：

```bash
fulilian gateway install    # 重新快照当前 PATH
fulilian gateway start      # 检测到更新的 plist 并重新加载
```
...
**解决方案：**
- 尝试更快/更小的模型：`fulilian chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- 减少激活的工具集：`fulilian chat -t "terminal"`
- 检查到提供商的网络延迟
...
验证您的 `~/.fulilian/config.yaml` 中的 MCP 配置：
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
# 更改配置后重启 Fulilian 或重新加载 MCP
fulilian chat
```

另请参阅：
- [MCP（模型上下文协议）](/user-guide/features/mcp)
- [在 Fulilian 中使用 MCP](/guides/use-mcp-with-fulilian)
...
:::warning
如果 MCP 服务器在请求中途崩溃，Fulilian 会报告超时。请检查服务器自身的日志（而非仅 Fulilian 日志）以诊断根本原因。
:::
...
### Profiles 与直接设置 FULILIAN_HOME 有何不同？

Profiles 是构建在 `FULILIAN_HOME` 之上的托管层。您*可以*在每次命令前手动设置 `FULILIAN_HOME=/some/path`，但 profiles 会为您处理所有底层工作：创建目录结构、生成 shell 别名（`fulilian-work`）、在 `~/.fulilian/active_profile` 中跟踪活动 profile，以及自动跨所有 profiles 同步技能更新。它们还与 tab 补全集成，让您无需记忆路径。
...
不共享。每个 profile 都有自己独立的记忆存储、会话数据库和技能目录，完全隔离。如果您想用现有的记忆和会话创建新 profile，请使用 `fulilian profile create newname --clone-all` 从当前 profile 复制所有内容，或添加 `--clone-from <profile>` 从指定源 profile 复制。

### 运行 `fulilian update` 时会发生什么？

`fulilian update` 拉取最新代码并重新安装依赖项**一次**（不是每个 profile 各一次）。然后自动将更新的技能同步到所有 profiles。您只需运行一次 `fulilian update` — 它覆盖机器上的每个 profile。

### 可以运行多少个 profiles？

没有硬性限制。每个 profile 只是 `~/.fulilian/profiles/` 下的一个目录。实际限制取决于您的磁盘空间以及系统能处理多少个并发网关（每个网关是一个轻量级 Python 进程）。运行数十个 profiles 完全没问题；每个空闲的 profile 不占用任何资源。
...
**解决方案：委托配置。** Fulilian 可以自动将子智能体路由到不同的模型。在 `~/.fulilian/config.yaml` 中设置：
...
现在当您告诉 Fulilian "帮我写一个关于 X 的 Twitter 帖子"并生成 `delegate_task` 子智能体时，该子智能体将在 Gemini 上运行，而非您的主模型。您的主对话仍在 GPT-5.4 上进行。
...
**场景：** 在 OpenClaw 中，您可以将多个独立智能体绑定到特定的 WhatsApp 聊天 — 一个用于家庭购物清单群组，另一个用于您的私聊。Fulilian 能做到吗？

**当前限制：** Fulilian 的每个 profile 都需要自己的 WhatsApp 号码/会话。您无法将多个 profiles 绑定到同一个 WhatsApp 号码上的不同聊天 — WhatsApp bridge（Baileys）每个号码使用一个已认证的会话。
...
**场景：** 您在 Telegram 中看到了网关执行日志、Fulilian 推理过程和工具调用详情，而不是最终输出。
...
**场景：** Telegram 有 100 个 slash 命令的限制，您的技能数量已超过此限制。您想禁用 Telegram 上不需要的技能，但 `fulilian skills config` 设置似乎没有生效。

**解决方案：** 使用 `fulilian skills config` 按平台禁用技能。这会写入 `config.yaml`：
...
更改后，**重启网关**（`fulilian gateway restart` 或终止并重新启动）。Telegram bot 命令菜单在启动时重建。
...
**当前行为：** Fulilian 在大多数平台上按用户 ID 创建会话，因此每个人都有自己的对话上下文。这是出于隐私和上下文隔离的设计考量。
...
2. 在**源机器**上创建完整备份：
   ...
   fulilian backup
   ```
   这会将您整个 `~/.fulilian/` 目录（配置、API key、记忆、技能、会话和 profiles）打包为 zip 文件，保存到主目录 `~/fulilian-backup-<timestamp>.zip`。
...
4. 在新机器上运行 `fulilian setup` 以验证 API key 和提供商配置是否正常工作。
...
### `fulilian backup` 与 `fulilian profile export` 的对比

| 功能 | `fulilian backup` | `fulilian profile export` |
| :--- | :--- | :--- |
| **使用场景** | **整机迁移** | **移植/共享特定 profile** |
| **范围** | 全局（整个 `~/.fulilian` 目录） | 局部（单个 profile 目录） |
...
:::tip
`fulilian backup` 即使在 Fulilian 正在运行时也能生成一致的快照。还原的归档文件不包含机器本地的运行时文件，如 `gateway.pid` 和 `cron.pid`。
:::

### 安装后重新加载 shell 时出现权限拒绝

**场景：** 运行 Fulilian 安装程序后，`source ~/.zshrc` 提示权限拒绝错误。

**原因：** 这通常发生在 `~/.zshrc`（或 `~/.bashrc`）文件权限不正确，或安装程序无法干净写入时。这不是 Fulilian 特有的问题 — 而是 shell 配置权限问题。
...
1. **搜索现有 issue：** [GitHub Issues](https://github.com/xujuan-cyber/fulilian/issues)
2. **向社区提问：** [Nous Research Discord](https://discord.gg/nousresearch)
3. **提交 bug 报告：** 请包含您的操作系统、Python 版本（`python3 --version`）、Fulilian 版本（`fulilian --version`）以及完整的错误信息