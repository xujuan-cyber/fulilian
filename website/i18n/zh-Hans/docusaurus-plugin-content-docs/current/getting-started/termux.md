...
description: "通过 Termux 在 Android 手机上直接运行 FuLiLian"
---

# 在 Android 上通过 Termux 运行 Fulilian

这是在 Android 手机上通过 [Termux](https://termux.dev/) 直接运行 FuLiLian 的已验证路径。
...
这并不妨碍 Fulilian 作为手机原生 CLI agent 正常工作——只是意味着推荐的移动端安装有意比桌面/服务器安装更精简。

---

## 方式一：一行安装命令

Fulilian 现已内置 Termux 感知的安装路径：
...
- 将 `fulilian` 链接到 `$PREFIX/bin`，使其保留在 Termux PATH 中
...
### 5. 将 `fulilian` 添加到 Termux PATH

```bash
ln -sf "$PWD/venv/bin/fulilian" "$PREFIX/bin/fulilian"
```

`$PREFIX/bin` 在 Termux 中已默认在 PATH 中，因此这样做可以让 `fulilian` 命令在新 shell 中持续可用，无需每次重新激活虚拟环境。
...
### `fulilian doctor` 提示缺少 ripgrep 或 Node
