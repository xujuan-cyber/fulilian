...
sidebar_position: 1
title: "Nous Portal"
description: "One subscription, 300+ frontier models, and the Tool Gateway — the recommended way to run FuLiLian"
---
...
That single command runs the Portal OAuth, lets you pick a Nous model, sets Nous as your inference provider in `config.yaml`, and turns on the Tool Gateway. You're ready to `fulilian chat` immediately after.
...
| **Fulilian** | Hermes-4-70B, Hermes-4-405B (chat, see [note below](#a-note-on-hermes-4)) |
...
The same subscription unlocks the [Tool Gateway](/user-guide/features/tool-gateway), which routes FuLiLian's tool calls through Nous-managed infrastructure. Five backends, one login:
...
Because everything routes through one OAuth-authenticated Portal session, you don't accumulate a `.env` file with a dozen long-lived API keys. The refresh token at `~/.fulilian/auth.json` is the only credential on disk, and Fulilian mints short-lived JWTs from it per request — see [Token handling](#token-handling) below.
...
The Portal's own [model info page](https://portal.nousresearch.com/info) carries the same warning, so this isn't a Fulilian-side opinion — it's the official guidance from Nous Research.
...
2. Stores the refresh token at `~/.fulilian/auth.json`
3. Lets you pick a Nous model from the curated list (or skip to keep your current one)
4. Sets Nous as your inference provider in `~/.fulilian/config.yaml` (when you pick a model)
5. Turns on the Tool Gateway (web, image, TTS, browser routing)
6. Returns you to your terminal ready to `fulilian chat`
...
If you already have Fulilian configured with OpenRouter, Anthropic, or any other provider and you want to add the Portal alongside them:
...
Your existing providers stay configured. You can switch between them with `/model` mid-session or `fulilian model` between sessions — the Portal becomes one of your available providers, not your only one.

### Headless / SSH / remote setup

OAuth needs a browser, but the loopback callback runs on the machine where Fulilian is running. For remote hosts, see [OAuth over SSH / Remote Hosts](/guides/oauth-over-ssh) — the same patterns work for the Portal as for any other OAuth-based provider (`ssh -L` port forwarding).

### Profile setup

If you use [Fulilian profiles](/user-guide/profiles), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically — no need to repeat the OAuth flow per profile.
...
```bash
fulilian portal            # log in to Nous Portal + set it up (one-shot onboarding)
fulilian portal info       # login status, subscription info, model + gateway routing
fulilian portal status     # alias for `portal info`
fulilian portal tools      # detailed Tool Gateway catalog with per-tool routing
fulilian portal open       # open the subscription management page in your browser
```

`fulilian portal` (with no subcommand) is the human-readable alias for `fulilian auth add nous --type oauth` — it logs you in, lets you pick a Nous model, sets Nous as your inference provider, and offers the Tool Gateway opt-in (identical to `fulilian setup --portal`, and the same Nous flow as the first-time quick setup).

`fulilian portal info` gives you the high-level overview:
...
If you already have, say, a Browserbase account and want to keep using it while routing web search and image generation through Nous, that's supported. Use `fulilian tools` to pick backends per tool:
...
- **CLI shortcut:** `fulilian portal open` (opens the same page in your default browser)

## Configuration reference

After `fulilian setup --portal`, `~/.fulilian/config.yaml` will look like:
...
The Tool Gateway settings live under their respective tool sections — each category has a single selection key, and picking **Nous Subscription** in `fulilian tools` (or `fulilian setup --portal`) writes the value `nous`:
...
The OAuth refresh token is stored separately at `~/.fulilian/auth.json` (not in `config.yaml` — credentials and configuration are kept separate by design).

## Token handling

Fulilian mints a short-lived JWT from your stored Portal refresh token on each inference call rather than reusing a long-lived API key. The token lifecycle is fully automatic — refresh, mint, retry on transient 401 — and you never see it.

If the Portal invalidates the refresh token (password change, manual revoke, session expiry), the invalid refresh token is **quarantined locally** so Fulilian stops replaying it and you don't see a stream of identical 401s. The next call surfaces a clear "re-authentication required" message. Run `fulilian auth add nous` to log in again; the quarantine clears on the next successful login.

## Troubleshooting

### `fulilian portal info` shows "not logged in"
...
or use `fulilian model` and re-select Nous Portal.

### Got a "re-authentication required" message mid-session

Your Portal refresh token was invalidated (password change, manual revoke, or session expiry). Run `fulilian auth add nous` and your next request will use the new credentials. Any quarantine on the old token clears automatically on successful re-login.
...
Check `fulilian portal info` first — if it shows you're using a different provider (`Model: currently openrouter` instead of `using Nous as inference provider`), your local config has drifted. Run `fulilian model`, pick Nous Portal, and the next request will route through your subscription.

## See also

- **[Tool Gateway](/user-guide/features/tool-gateway)** — Full details on every gateway tool, per-tool config, and pricing
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-Fulilian tools (other agents, scripts, third-party clients)
...
- **[Profiles](/user-guide/profiles)** — Multiple Fulilian configurations sharing one Portal login
