...
Fulilian calls this **no-agent mode**. It's the cron system minus the LLM.
...
The real win of no-agent mode is that the agent itself can set up the watchdog for you — no editor, no shell, no remembering CLI flags. You describe what you want, Fulilian writes the script, schedules it, and tells you when it'll fire.

### Example transcript

> **You:** ping me on telegram if RAM is over 85% every 5 minutes
>
> **Fulilian:** *(writes `~/.fulilian/scripts/memory-watchdog.sh`, then calls `cronjob(...)` with `no_agent=true`)*
>
...
write_file(
    path="~/.fulilian/scripts/memory-watchdog.sh",
...
>
> **Fulilian:** *(calls `cronjob(action='pause', job_id='abc123')`)*
>
> Paused. Resume with "turn it back on" or via `fulilian cron resume abc123`.

> **You:** change it to every 15 minutes
>
> **Fulilian:** *(calls `cronjob(action='update', job_id='abc123', schedule='every 15m')`)*
...
```bash
# 1. Write your script
cat > ~/.fulilian/scripts/memory-watchdog.sh <<'EOF'
#!/usr/bin/env bash
...
EOF
chmod +x ~/.fulilian/scripts/memory-watchdog.sh
...
# 3. Verify
fulilian cron list
fulilian cron run <job_id>    # fire it once to test
```
...
Scripts must live in `~/.fulilian/scripts/`. This is enforced at both job-creation time and run time — absolute paths, `~/` expansion, and path-traversal patterns (`../`) are rejected. The same directory is shared with the pre-check script gate used by LLM jobs.
...
```bash
fulilian cron create "every 5m"        # interval
fulilian cron create "every 2h"
fulilian cron create "0 9 * * *"       # standard cron: 9am daily
fulilian cron create "30m"             # one-shot: run once in 30 minutes
```
...
--deliver local                          # just save to ~/.fulilian/cron/output/
```

No running gateway is required at script-run time for bot-token platforms (Telegram, Discord, Slack, Signal, SMS, WhatsApp) — the tool calls each platform's REST endpoint directly using the credentials already in `~/.fulilian/.env` / `~/.fulilian/config.yaml`.

## Editing and Lifecycle

```bash
fulilian cron list                                    # see all jobs
fulilian cron pause <job_id>                          # stop firing, keep definition
fulilian cron resume <job_id>
fulilian cron edit <job_id> --schedule "every 10m"    # adjust cadence
fulilian cron edit <job_id> --agent                   # flip to LLM mode
fulilian cron edit <job_id> --no-agent --script …     # flip back
fulilian cron remove <job_id>                         # delete it
```
...
```bash
cat > ~/.fulilian/scripts/disk-alert.sh <<'EOF'
#!/usr/bin/env bash
...
'
EOF
chmod +x ~/.fulilian/scripts/disk-alert.sh

fulilian cron create "*/15 * * * *" \
  --no-agent \
...
|----------|-----------|-------------|
| `cronjob --no-agent` (this page) | Your script on Fulilian' schedule | Recurring watchdogs / alerts / metrics that don't need reasoning |
| `cronjob` (default, LLM) | Agent with optional pre-check script | When the message content requires reasoning over data |
| OS cron + `curl` to a [webhook subscription](/user-guide/messaging/webhooks) | Your script on the OS schedule | When Fulilian might be unhealthy (the thing you're monitoring) |

For critical system-health watchdogs that must fire *even when the gateway is down*, use OS-level cron with a plain `curl` to a Fulilian webhook subscription (or any external alerting endpoint) — those run as independent OS processes and don't depend on Fulilian being up. The in-gateway scheduler is the right choice when the thing being monitored is external.
