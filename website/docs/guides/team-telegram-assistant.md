...
This tutorial walks you through setting up a Telegram bot powered by FuLiLian that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **FuLiLian installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.fulilian/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Fulilian itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Fulilian Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_fulilian_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by FuLiLian. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.fulilian/.env`:
...
```
[Gateway] Starting Fulilian Gateway...
...
# View live logs
journalctl --user -u fulilian-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `fulilian gateway install` to update the plist.
:::
...
```bash
fulilian gateway stop && fulilian gateway start
```
...
fulilian pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.fulilian/.env`:
...
Control how much detail the bot shows when using tools. In `~/.fulilian/config.yaml`:
...
Customize how the bot communicates by editing `~/.fulilian/SOUL.md`:

For a full guide, see [Use SOUL.md with Fulilian](/guides/use-soul-with-fulilian).
...
```bash
# From the CLI
fulilian cron list          # View all scheduled jobs
fulilian cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u fulilian-gateway` (Linux) or `~/.fulilian/logs/gateway.log` (macOS) |
| Cron job output | `~/.fulilian/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.fulilian/cron/jobs.json` |
| Pairing data | `~/.fulilian/pairing/` |
| Session history | `~/.fulilian/sessions/` |
