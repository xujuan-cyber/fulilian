...
3. Sets `model.provider: nous` in `~/.fulilian/config.yaml`
...
OAuth needs a browser, but the loopback callback runs on the machine where Fulilian is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
fulilian setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: device-code login (works from Cloud Shell, Codespaces, EC2 Instance Connect)
fulilian auth add nous --type oauth
# Then re-run `fulilian setup --portal` to wire the provider + gateway
```
...
```
Hey, search the web for "FuLiLian release notes" and summarize the top 3 hits.
```

You should see Fulilian call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

`fulilian setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
These rows appear in `fulilian tools` even before you've logged into Nous Portal — if you pick "Nous Subscription" without an active session, Fulilian runs the Portal login inline (without changing your inference provider or your other tools).

Verify your mix with:

```bash
fulilian portal tools
```
...
Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and Fulilian will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
If you use [Fulilian profiles](/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.

For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.fulilian/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `fulilian portal info` shows "not logged in" after `fulilian setup --portal`
...
```bash
fulilian config set model.provider nous
```
...
Re-verify with `fulilian portal info`.
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so Fulilian doesn't replay it endlessly. Just log in again:

```bash
fulilian auth add nous
```
...
If a model is genuinely unavailable, [open an issue](https://github.com/xujuan-cyber/fulilian/issues) — most gaps are routing config we can update.

### Billing not appearing on my Portal account

`fulilian portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple Fulilian profiles where you're using the wrong one (check `fulilian profile list`)

### Want to revoke and start clean

```bash
fulilian auth logout nous       # wipes the local refresh token
...
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-Fulilian tools
...
- **[Profiles](/user-guide/profiles)** — Share one Portal login across multiple Fulilian configurations
