...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `fulilian send`."
---

# Pipe Script Output to Messaging Platforms

`fulilian send` is a small, scriptable CLI that pushes a message to any
messaging platform Fulilian is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | fulilian send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `fulilian
...
fulilian send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
fulilian send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
fulilian send --list

# Filter by platform
fulilian send --list telegram
```
...
Any platform Fulilian ships adapters for works as a target:
...
`fulilian send` resolves the message body in this order:

1. **Positional argument** — `fulilian send --to telegram "hi"`
2. **`--file PATH`** — `fulilian send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | fulilian send --to telegram`

When stdin is a TTY (no pipe), Fulilian does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  fulilian send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `fulilian send` reuses your Fulilian config, the same script works on
any host where Fulilian is installed — no need to export bot tokens into
...
`fulilian send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  fulilian send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | fulilian send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.fulilian/bin/fulilian send \
...
```bash
./train.py --epochs 200 && \
  fulilian send --to telegram "training done" || \
  fulilian send --to telegram "training failed (exit $?)"
```
...
fulilian send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(fulilian send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `fulilian send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `fulilian send` calls
the platform's REST endpoint directly using credentials from
`~/.fulilian/.env` and `~/.fulilian/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `fulilian gateway start` and retry.
...
# Machine-readable
fulilian send --list --json
```

The listing is built from `~/.fulilian/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`fulilian gateway
...
| Approach | Multi-platform | Reuses Fulilian creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `fulilian send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`fulilian send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `fulilian send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `fulilian send` shares with cron delivery.
