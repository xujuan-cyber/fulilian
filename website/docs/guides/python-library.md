---
sidebar_position: 5
title: "Using Fulilian as a Python Library"
description: "Embed AIAgent in your own Python scripts, web apps, or automation pipelines — no CLI required"
---

# Using Fulilian as a Python Library

Fulilian isn't just a CLI tool. You can import `AIAgent` directly and use it programmatically in your own Python scripts, web applications, or automation pipelines. This guide shows you how.
...
Run your application with `uv run python your_app.py` from that checkout. Fulilian does not publish a supported wheel or source distribution for `requirements.txt` installs.

:::tip
The same environment variables used by the CLI are required when using Fulilian as a library. At minimum, set `OPENROUTER_API_KEY` (or `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` if using direct provider access).
:::
...
The simplest way to use Fulilian is the `chat()` method — pass a message, get a string back:
...
:::warning
Always set `quiet_mode=True` when embedding Fulilian in your own code. Without it, the agent prints CLI spinners, progress indicators, and other terminal output that will clutter your application's output.
:::
...
For running many prompts in parallel, Fulilian includes `batch_runner.py`. It manages concurrent `AIAgent` instances with proper resource isolation:
...
async def on_message(message):
    if message.author == client.user:
        return
    if message.content.startswith("!fulilian "):
...
|-----------|------|---------|-------------|
| `model` | `str` | `""` | Model in OpenRouter format (defaults to empty; resolved from your fulilian config at runtime) |
