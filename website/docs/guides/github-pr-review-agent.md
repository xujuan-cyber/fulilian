...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  FuLiLian  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Fulilian instantly when PRs are opened or updated.
:::
...
- **FuLiLian installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  fulilian gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.fulilian/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Fulilian can access GitHub. Start a chat:
...
Still in the chat, ask Fulilian to review a real PR:
...
A skill gives Fulilian consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.fulilian/skills/code-review
```

Create `~/.fulilian/skills/code-review/SKILL.md`:
...
Verify it loaded — start `fulilian` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Fulilian your team's standards:
...
2. Teach Fulilian your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
fulilian gateway status    # Is the gateway running?
fulilian cron list         # Is the job enabled?
```
