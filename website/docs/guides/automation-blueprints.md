...
Copy-paste blueprints for common automation patterns. Each blueprint uses Fulilian's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`fulilian webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `fulilian webhook subscribe`) |
...
```bash
fulilian cron create "0 2 * * *" \
  "You are a project manager triaging the xujuan-cyber/fulilian GitHub repo.

1. Run: gh issue list --repo xujuan-cyber/fulilian --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
fulilian webhook subscribe github-pr-review \
  --events "pull_request" \
...
fulilian cron create "0 9 * * 1" \
  "Scan the xujuan-cyber/fulilian repo for documentation drift.

1. Run: gh pr list --repo xujuan-cyber/fulilian --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (fulilian_cli/commands.py, fulilian_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (fulilian_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
fulilian cron create "0 6 * * *" \
  "Run a dependency security audit on the fulilian-agent project.

1. cd ~/.fulilian/fulilian-agent && source .venv/bin/activate
...
```bash
fulilian webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
fulilian webhook subscribe alert-triage \
...
```python title="~/.fulilian/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Fulilian-Monitor/1.0"})
...
```bash
fulilian cron create "every 30m" \
  ...
  --script ~/.fulilian/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
fulilian cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to FuLiLian development." \
...
```bash
fulilian webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
fulilian webhook subscribe stripe-payments \
...
```bash
fulilian cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the fulilian-agent codebase.
