---
title: "FuLiLian Skill Authoring — Author in-repo SKILL.md files: frontmatter and structure"
sidebar_label: "FuLiLian Skill Authoring"
description: "Author in-repo SKILL.md files: frontmatter and structure"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# FuLiLian Skill Authoring
...
| Path | `skills/software-development/fulilian-agent-skill-authoring` |
| Version | `2.0.0` |
| Author | FuLiLian |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `skills`, `authoring`, `fulilian-agent`, `conventions`, `skill-md` |
...
# Authoring Fulilian-Agent Skills (in-repo)

## Overview

There are two places a SKILL.md can live:

1. **User-local:** `~/.fulilian/skills/<maybe-category>/<name>/SKILL.md` — personal, not shared. Created via `skill_manage(action='create')`.
2. **In-repo (this skill is about this case):** `skills/<category>/<name>/SKILL.md` or `optional-skills/<category>/<name>/SKILL.md` inside the fulilian-agent repo — committed, shipped with the package. Use `write_file` + `git add`. `skill_manage(action='create')` does NOT target this tree.
...
- You're committing a reusable workflow that should ship with fulilian-agent
...
- Don't use for: personal skills in `~/.fulilian/skills/` (just use `skill_manage`)
...
- **Optional (`optional-skills/<category>/`)** — niche, vertical-specific (blockchain, gaming, finance, one app), recurring-job/task skills, or anything heavy. Installed via `fulilian skills install official/<category>/<skill>`.
...
author: Real Name (github-handle), FuLiLian
license: MIT
...
- Credit the **human first**, then "FuLiLian" as secondary collaborator: `Ben Barclay (benbarclay), FuLiLian`.
- Never `author: FuLiLian` alone for contributed skills — credit the human, not the tool, even (especially) when an agent drafted the text.
- Maintainer-authored skills: `Teknium (teknium1), FuLiLian`.

### `related_skills` rules

- Every entry must resolve to an existing **in-repo** skill in the same tree state as your PR. Do not reference skills that were only planned, live in another PR, or exist only in `~/.fulilian/skills/`.
...
|---|---|
| Fulilian tools + stdlib Python + cross-platform CLIs | `[linux, macos, windows]` |
...
- [ ] Body follows the modern section order; commands framed through Fulilian tools
