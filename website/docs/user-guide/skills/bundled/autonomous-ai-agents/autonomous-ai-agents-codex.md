...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`fulilian-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-fulilian-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Fulilian terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Fulilian itself, `model.provider: openai-codex` uses Fulilian-managed Codex
OAuth from `~/.fulilian/auth.json` after `fulilian auth add openai-codex`. For the
...
## Fulilian Gateway Caveat

When invoking the Codex CLI from a Fulilian gateway/service context (for example,
