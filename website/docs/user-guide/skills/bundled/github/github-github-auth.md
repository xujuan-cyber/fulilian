...
- Give it a name like "fulilian-agent"
- Select scopes:
...
- Give it a title like "fulilian-agent-&lt;machine-name>"
...
uv run python3 "${FULILIAN_HOME:-$HOME/.fulilian}/skills/github/github-auth/scripts/git-credential-token.py"
```
...
elif _fulilian_env="${FULILIAN_HOME:-$HOME/.fulilian}/.env"; [ -f "$_fulilian_env" ] && grep -q "^GITHUB_TOKEN=" "$_fulilian_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_fulilian_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
  export GITHUB_TOKEN=$(uv run python3 "${FULILIAN_HOME:-$HOME/.fulilian}/skills/github/github-auth/scripts/git-credential-token.py")
  echo "AUTH_METHOD=curl"
else
