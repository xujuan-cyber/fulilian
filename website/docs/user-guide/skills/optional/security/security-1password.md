...
| | |
|---|---|
| Source | Optional — install with `fulilian skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by FuLiLian |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Fulilian terminal calls (desktop app flow only)
...
### Service Account (recommended for Fulilian)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${FULILIAN_HOME:-~/.fulilian}/.env` (the skill will prompt for this on first load).
...
## Fulilian Execution Pattern (desktop app flow)

Fulilian terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/fulilian-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/fulilian-op.sock"
