...
| | |
|---|---|
| Source | Optional — install with `fulilian skills install official/productivity/siyuan` |
...
3. Store it in `${FULILIAN_HOME:-~/.fulilian}/.env`:
   ```
...
```yaml
# In ~/.fulilian/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
