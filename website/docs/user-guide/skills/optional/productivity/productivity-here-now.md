...
| | |
|---|---|
| Source | Optional — install with `fulilian skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${FULILIAN_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${FULILIAN_SKILL_DIR}/scripts/drive.sh` for private Drive storage
...
| `--client {name}`      | Agent name for attribution (e.g. `fulilian`)    |
