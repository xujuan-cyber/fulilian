---
title: "Honcho — Configure and troubleshoot Honcho memory for Fulilian"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Fulilian"
---
...
Configure and troubleshoot Honcho memory for Fulilian.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `fulilian skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Fulilian

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Fulilian profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/fulilian#running-honcho-locally-with-fulilian

### Verify

```bash
fulilian honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Fulilian profile's AI peer
...
Honcho models conversations as interactions between **peers**. Fulilian creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Fulilian instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Fulilian run |
| `global` | Single session across all directories |

Manual override: `fulilian honcho map my-project-name`
...
Each Fulilian profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block fulilian.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `fulilian.coder` host block in `honcho.json`
...
```bash
fulilian honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Fulilian when Honcho memory is active.
...
Config file: `$FULILIAN_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `fulilian honcho setup`. Ensure `memory.provider: honcho` is in `~/.fulilian/config.yaml`.

### Memory not persisting across sessions
Check `fulilian honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `fulilian profile create <name> --clone`. For existing profiles: `fulilian honcho sync`.
...
| Command | Description |
|---------|-------------|
| `fulilian honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `fulilian honcho status` | Show resolved config, connection test, peer info for active profile |
| `fulilian honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `fulilian honcho disable` | Disable Honcho for the active profile |
| `fulilian honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `fulilian honcho peers` | Show peer identities across all profiles |
| `fulilian honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `fulilian honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `fulilian honcho sessions` | List known directory-to-session-name mappings |
| `fulilian honcho map <name>` | Map current working directory to a Honcho session name |
| `fulilian honcho identity` | Seed AI peer identity or show both peer representations |
| `fulilian honcho sync` | Create host blocks for all Fulilian profiles that don't have one yet |
| `fulilian honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Fulilian + Honcho |
| `fulilian memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `fulilian memory status` | Show active memory provider and config |
| `fulilian memory off` | Disable external memory provider |
