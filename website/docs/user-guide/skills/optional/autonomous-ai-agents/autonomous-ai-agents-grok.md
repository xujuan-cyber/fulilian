...
| | |
|---|---|
| Source | Optional — install with `fulilian skills install official/autonomous-ai-agents/grok` |
| Path | `optional-skills/autonomous-ai-agents/grok` |
| Version | `0.1.1` |
| Author | Matt Maximo (MattMaximo), FuLiLian |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Coding-Agent`, `Grok`, `xAI`, `Code-Review`, `Refactoring`, `Automation` |
| Related skills | [`codex`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-codex), [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`fulilian-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-fulilian-agent) |
...
# Grok Build CLI — Fulilian Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Fulilian terminal. Grok
...
1. Prepare stable input files first with Fulilian tools (`read_file`,
...
2. **Don't conflate Fulilian' xAI auth with the `grok` CLI's auth.** Fulilian'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Fulilian' xAI auth carries
   over.
