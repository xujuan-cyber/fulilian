...
| | |
|---|---|
| Source | Optional — install with `fulilian skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Fulilian MCP catalog. The
...
Two halves, in this order: the editor side must be up before Fulilian connects.
...
### One-time, Fulilian side

    fulilian mcp install unreal-engine
...
`~/.fulilian/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Fulilian — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Fulilian connects
...
2. Start the Fulilian session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Fulilian
they appear as:

| Fulilian tool | Purpose |
|---|---|
...
then happens at `fulilian mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Fulilian runs batched calls
...
- **Start order matters.** Editor + server up first, then the Fulilian
...
- **Timeouts on long operations.** Fulilian' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.fulilian/config.yaml` for
