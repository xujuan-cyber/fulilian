...
| | |
|---|---|
| Source | Optional — install with `fulilian skills install official/creative/kanban-video-orchestrator` |
...
short to a music video to an ASCII loop — in a Fulilian Kanban pipeline that
...
3. **Generates** a setup script that creates Fulilian profiles, project workspace, and the initial kanban task
...
3. Creates each Fulilian profile via `fulilian profile create --clone`
...
```bash
fulilian kanban watch --tenant <project-tenant>     # live events
fulilian kanban list  --tenant <project-tenant>     # board snapshot
fulilian dashboard                                   # visual board UI
```
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   a `fulilian kanban create` call to a specialist profile. The kanban
...
8. **Verify API keys BEFORE firing.** External APIs (TTS, image-gen,
   image-to-video) need keys in `${FULILIAN_HOME:-~/.fulilian}/.env` or the user's secret store.
