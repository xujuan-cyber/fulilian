...
| | |
|---|---|
| Source | Optional — install with `fulilian skills install official/mcp/fastmcp` |
...
| Related skills | [`fulilian-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-fulilian-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Fulilian or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Fulilian. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.fulilian/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.fulilian/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Fulilian integration, either:

- configure the server in `~/.fulilian/config.yaml` using the `native-mcp` skill, or
...
### Fulilian cannot see the deployed server

The server-building part may be correct while the Fulilian config is not. Load the `native-mcp` skill and configure the server in `~/.fulilian/config.yaml`, then restart Fulilian.
