---
title: "Actual Setup — Set up Actual Computer (actual.inc) inference in Fulilian"
sidebar_label: "Actual Setup"
description: "Set up Actual Computer (actual.inc) inference in Fulilian"
---
...
Set up Actual Computer (actual.inc) inference in Fulilian.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `fulilian skills install official/devops/actual-setup` |
...
Sets up [actual.inc](https://actual.inc) (Actual Computer) as a Fulilian inference
...
- User has an `ac_` key and wants Fulilian routed through their Actual cluster.
...
- Fulilian has **first-class `actual` provider support** (provider id `actual`,
  aliases `actual-computer`, `actualcomputer`, `aci`). Do NOT configure Actual
  as a `custom_providers` / `providers.actual.*` entry on current Fulilian — the
...
1. Put the key in `.env` (secrets only — never config.yaml):
   append `ACTUAL_API_KEY=ac_...` to `~/.fulilian/.env`.
...
3. Select provider + model:
   ```bash
   fulilian config set model.provider actual
   fulilian config set model.default "MODEL_ID_FROM_DISCOVERY"
   ```
4. Verify end-to-end:
   ```bash
   fulilian chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL_ID
   ```
...
2. Download and load a model (scriptable once authorized):
   ...
   ```
3. Point Fulilian at the daemon. `ACTUAL_BASE_URL` with a loopback host flips the
   built-in provider into local no-auth mode automatically — no key needed:
   append `ACTUAL_BASE_URL=http://127.0.0.1:8080` to `~/.fulilian/.env`, then:
   ```bash
   fulilian config set model.provider actual
   fulilian config set model.default "INSTALLED_MODEL_NAME"
   ```
4. Verify (reduced toolset — see context-window pitfall below):
   ```bash
   fulilian chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m INSTALLED_NAME -t file,web
   ```
...
1. **reasoning_effort trap (handled by Fulilian since the first-class provider).**
   ...
   request still 400s this way on an old Fulilian, set a per-model cap:
   `agent.reasoning_overrides.<model>: high` in config.yaml.
2. **Context-window overflow on small local models.** Fulilian' default toolset
   ...
   servers emit a bare `data: [DONE]` — Fulilian reports
...
5. **Do not create a custom provider named `actual`.** Older setup guides
   ...
   current Fulilian the built-in provider wins the name; stale custom blocks
...
```bash
# Relay:
fulilian chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL
# Local (small model — reduced toolset):
fulilian chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m MODEL -t file,web
