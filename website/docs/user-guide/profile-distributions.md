...
A **profile distribution** packages a complete Fulilian agent — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## Two ways to share a profile

Fulilian has two sharing paths, and they answer different questions. Distributions are the durable one; export files are the quick one.

| | **Distribution** (git repo) | **Export file** (`.tar.gz`) |
|---|---|---|
| Ship it by | `fulilian profile install <repo>` | Send a file — chat, AirDrop, USB, email |
| Recipient needs | git, and access to the repo | The file |
| Updates | `fulilian profile update` pulls new versions | Re-send the file |
...
| Made with | `fulilian profile install` / `update` | `/export` and `/import`, or `fulilian profile export` / `import` |
...
Before distributions, sharing a Fulilian agent meant sending someone:
...
```bash
fulilian profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `fulilian profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running Fulilian in 2026, that's already true.
...
:::caution
**Fulilian does not control git.** The file exclusions described on this page are applied by the **installer** when someone runs `fulilian profile install` or `fulilian profile update`. They are **not** applied when you run `git add` or `git commit`.
:::
...
```bash
fulilian profile create research-bot
research-bot setup                    # configure model, API keys
# Edit ~/.fulilian/profiles/research-bot/SOUL.md
...
Create `~/.fulilian/profiles/research-bot/distribution.yaml`:
...
Create `~/.fulilian/profiles/research-bot/.gitignore` with at minimum:
...
```bash
cd ~/.fulilian/profiles/research-bot
git init
git add .
git commit -m "v1.0.0"
...
Recipients who run `fulilian profile update research-bot` will pull the latest.
...
```bash
fulilian profile install github.com/you/research-bot --alias
```
...
5. Copies distribution-owned files into `~/.fulilian/profiles/research-bot/` (or wherever the manifest's `name` resolves). The [hard-excluded paths](#whats-not-in-a-distribution-ever) are stripped during this copy, even if the author accidentally left them in the repo.
...
```bash
# GitHub shorthand
fulilian profile install github.com/you/research-bot

# Full HTTPS
fulilian profile install https://github.com/you/research-bot.git

# SSH
fulilian profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
fulilian profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
fulilian profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
fulilian profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
fulilian profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
fulilian profile install github.com/acme/support-bot --name support-eu --alias
```
...
```
# Environment variables required by this Fulilian distribution.
...
```bash
cp ~/.fulilian/profiles/research-bot/.env.EXAMPLE ~/.fulilian/profiles/research-bot/.env
...
```bash
fulilian profile info research-bot
```
...
`fulilian profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
fulilian profile update research-bot
```
...
```bash
fulilian profile delete research-bot
```
...
Path:    ~/.fulilian/profiles/research-bot
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `fulilian profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
cd ~/.fulilian/profiles/pr-reviewer
# ... build and tune ...
git init && git add . && git status   # confirm no secrets staged
...
# Each engineer
fulilian profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `fulilian profile update pr-reviewer` and everyone's on the new version within minutes.
...
cd ~/.fulilian/profiles/polymarket-trader
# Write a solid README.md at the repo root — GitHub shows it on the repo page
git init && git add . && git status   # confirm no secrets staged
git commit -m "v1.0"
git tag v1.0.0
# Publish to a public GitHub repo
git remote add origin https://github.com/you/fulilian-polymarket-trader.git
git push -u origin main --tags

# Anyone
fulilian profile install github.com/you/fulilian-polymarket-trader --alias
```
...
You built Fulilian-on-top — maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
# Each on-call
fulilian profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
fulilian profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `fulilian profile info <name>` and hold off on updates until you're ready.
:::
...
fulilian profile info research-bot | grep Version
...
```yaml
# ~/.fulilian/profiles/research-bot/local/my-overrides.yaml
...
fulilian profile delete research-bot --yes
fulilian profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
fulilian profile update research-bot --force-config --yes
```
...
fulilian profile install github.com/yourname/forked-research-bot --alias

# Iterate locally in ~/.fulilian/profiles/forked-research-bot/
...
fulilian profile install ~/.fulilian/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
fulilian profile delete research-bot-test --yes
fulilian profile install ~/.fulilian/profiles/research-bot --name research-bot-test
```
...
```bash
fulilian profile export research-bot
fulilian profile export research-bot -o ./research-bot.tar.gz
```
...
```bash
fulilian profile import ./research-bot.tar.gz
fulilian profile import ./research-bot.tar.gz --name research-bot-2
```

The profile name is inferred from the archive unless you pass `--name`. Importing over an existing profile is refused — rename or delete the old one first. A shell wrapper (`research-bot` → `fulilian -p research-bot`) is created when the name doesn't collide with an existing command.
...
:::note
You cannot import as `default` — that name is the built-in root profile (`~/.fulilian`). Pass `--name something-else`.
:::
...
**The default profile** (`~/.fulilian`) is exported through an allow-list — only known Fulilian artifacts, so an unrelated file sitting in your home directory can't get swept in:
...
**A named profile** (`~/.fulilian/profiles/<name>`) copies the whole directory minus `auth.json` / `.env`. That's broader — if the profile has `state.db`, logs, or caches, they go in the archive too, and the file gets big.
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `fulilian -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `fulilian profile` — not a parallel command tree.
...
- Reserved profile names (`fulilian`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`fulilian profile export` / `import`](../reference/profile-commands.md#fulilian-profile-export) — the CLI form of [export files](#export-and-import-a-profile-file)
- [Slash Commands reference](../reference/slash-commands.md) — `/export`, `/import`, and every other in-chat command
- [Using SOUL with Fulilian](../guides/use-soul-with-fulilian.md) — authoring personalities
