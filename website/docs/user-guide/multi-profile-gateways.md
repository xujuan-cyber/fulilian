...
If you only run one Fulilian agent, you don't need this page — see
...
[Connecting Desktop to Many Fulilian Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more Fulilian agents that should all
...
(`ai.fulilian.gateway-<name>.plist`) or systemd user service
(`fulilian-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
fulilian profile create coder
fulilian profile create personal-bot
fulilian profile create research
...
```bash
fulilian config set gateway.multiplex_profiles true
fulilian gateway restart
```

Equivalently, in the default profile's `~/.fulilian/config.yaml`:
...
You do **not** run `fulilian gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `fulilian gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.fulilian/profiles/coder/.env`; the default listener key is rejected.
...
home). `fulilian status` reports the multiplexer and the profiles it serves;
`fulilian status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/fulilian-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: fulilian-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    fulilian gateway "$action"
  else
    fulilian -p "$profile" gateway "$action"
  fi
}
...
```bash
fulilian-gateways start      # start every configured profile
fulilian-gateways stop       # stop every configured profile
fulilian-gateways restart    # restart all
fulilian-gateways status     # status across all
fulilian-gateways list       # delegates to `fulilian gateway list`
```

:::tip
The `default` profile is targeted with `fulilian gateway <action>` (no `-p`),
not `fulilian -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `fulilian -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.fulilian.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/fulilian-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.fulilian.gateway.plist` /
...
```bash
# Default profile
tail -f ~/.fulilian/logs/gateway.log
tail -f ~/.fulilian/logs/gateway.error.log

# Named profile
tail -f ~/.fulilian/profiles/<name>/logs/gateway.log
tail -f ~/.fulilian/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.fulilian/logs/gateway.log ~/.fulilian/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
fulilian logs -f                  # follow default profile
fulilian -p coder logs -f         # follow one profile
fulilian logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
fulilian profile list             # profiles + model + gateway state
fulilian-gateways status          # full status across every profile
launchctl list | grep fulilian    # macOS — PIDs and labels
systemctl --user list-units 'fulilian-gateway-*'   # Linux — units
```
...
```
~/.fulilian/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.fulilian/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
fulilian config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.fulilian/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=fulilian --why="gateways running" \
  sleep infinity &
...
`fulilian-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.fulilian/.env ~/.fulilian/profiles/*/.env
```

## Updating the code

`fulilian update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `fulilian gateway start` after a previous `fulilian gateway stop`. The
...
```bash
ps -ef | grep "fulilian_cli.*-p <profile>"
cat ~/.fulilian/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.fulilian.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.fulilian.gateway-<profile>.plist

# Linux
systemctl --user restart fulilian-gateway-<profile>.service
```

### Health check

```bash
fulilian doctor                  # default profile
fulilian -p <profile> doctor     # one profile
```
