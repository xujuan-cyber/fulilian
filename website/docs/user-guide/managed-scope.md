...
`~/.fulilian/config.yaml`, `~/.fulilian/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/fulilian`:

```text
/etc/fulilian/
├── config.yaml     # managed config layer (wins over ~/.fulilian/config.yaml)
└── .env            # managed env layer (wins over ~/.fulilian/.env + shell)
```
...
The location can be relocated with the `FULILIAN_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `FULILIAN_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Fulilian.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export FULILIAN_MANAGED_DIR=/opt/org/fulilian-policy
```

:::warning
A user who can set `FULILIAN_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `fulilian doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/fulilian/config.yaml` (managed) | `/etc/fulilian/.env` (managed) |
| 2 | `~/.fulilian/config.yaml` (user) | `~/.fulilian/.env` (user) |
...
```bash
fulilian config        # shows a header naming the managed source + the pinned keys
fulilian doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Fulilian refuses and names the source:

```bash
$ fulilian config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/fulilian/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `fulilian config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/fulilian

# Pin some config values for every user on this machine
sudo tee /etc/fulilian/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/fulilian/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/fulilian
sudo chmod 0644 /etc/fulilian/config.yaml /etc/fulilian/.env
```

Changes take effect on the next Fulilian start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`fulilian doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Fulilian as `root`), managed scope is advisory.
