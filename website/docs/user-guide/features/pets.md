...
Fulilian can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<FULILIAN_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Fulilian renders the sprite at full fidelity when your
...
fulilian pets list
fulilian pets list cat

# Install a pet and make it active in one step
fulilian pets install boba --select

# Preview / animate it in your terminal (Ctrl+C to stop)
fulilian pets show

# Check your setup
fulilian pets doctor
```

## `fulilian pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `fulilian pets list [query] [--limit N]` |
| List installed pets | `fulilian pets list --installed` |
| Install a pet | `fulilian pets install <slug> [--select] [--force]` |
| Set the active pet | `fulilian pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `fulilian pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `fulilian pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `fulilian pets off` |
| Remove an installed pet | `fulilian pets remove <slug>` |
| Diagnose setup | `fulilian pets doctor` |

`fulilian pets show` flags:
...
Beyond installing pre-made pets from the gallery, Fulilian can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `fulilian pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `fulilian tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `FULILIAN_PET_IMAGE_PROVIDER` env var (e.g. `FULILIAN_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Fulilian is
...
Run `fulilian pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Fulilian uses its own
  profile-scoped `<FULILIAN_HOME>/pets/` instead — install through `fulilian pets`.

## See also

- The [`fulilian-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-fulilian-agent.md)
