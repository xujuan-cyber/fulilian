...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Fulilian's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Fulilian instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
fulilian memory setup    # select "honcho" from the provider list
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$FULILIAN_HOME/honcho.json` (profile-local). The setup wizard handles this for you.

### Self-Hosted Honcho with Authentication

When pointing Fulilian at a self-hosted Honcho server, `fulilian honcho setup` (and `fulilian memory setup`) ask for a **local JWT / bearer token** after the base URL. Paste a JWT signed with the server's `AUTH_JWT_SECRET` (the Honcho compose env var) to enable authenticated access; leave it blank for servers running with `AUTH_USE_AUTH=false`. The local token is stored under the host block (`hosts.<host>.apiKey` in `honcho.json`), separate from any cloud `apiKey`, so you can flip the `Cloud or local?` prompt back to `cloud` later without losing either credential.
...
- `per-session` — each `fulilian` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Fulilian syncs them back at session init.
...
The `fulilian honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `fulilian memory setup honcho` (or run `fulilian memory setup` and pick it from the list); the `fulilian honcho` subcommand then appears on the next invocation.

```bash
fulilian memory setup honcho    # Configure Honcho directly (works before activation)
fulilian honcho status          # Connection status, config, and key settings
fulilian honcho setup           # Redirects to `fulilian memory setup` (post-activation alias)
fulilian honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
fulilian honcho peer            # Show or update peer names + dialectic reasoning level
fulilian honcho mode            # Show or set recall mode (hybrid/context/tools)
fulilian honcho tokens          # Show or set token budget for context and dialectic
fulilian honcho identity        # Seed or show the AI peer's Honcho identity
fulilian honcho sync            # Sync Honcho config to all existing profiles
fulilian honcho peers           # Show peer identities across all profiles
fulilian honcho sessions        # List known Honcho session mappings
fulilian honcho map             # Map current directory to a Honcho session name
fulilian honcho enable          # Enable Honcho for the active profile
fulilian honcho disable         # Disable Honcho for the active profile
fulilian honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `fulilian honcho`

If you previously used the standalone `fulilian honcho setup`:
...
No re-login or re-setup needed. Run `fulilian memory setup` and select "honcho" — the wizard detects your existing config.
