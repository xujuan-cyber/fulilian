...
description: "Durable SQLite-backed task board for coordinating multiple Fulilian profiles"
---
...
Fulilian Kanban is a durable task board, shared across all your Fulilian profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.fulilian/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.

### Two surfaces: the model talks through tools, you talk through the CLI

The board has two front doors, both backed by the same `~/.fulilian/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_request_review`, `kanban_request_changes`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_attach`, `kanban_attach_url`, `kanban_attachments`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `fulilian kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `fulilian kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
For the full design rationale, comparative analysis against Cline Kanban / Paperclip / NanoClaw / Google Gemini Enterprise, and the eight canonical collaboration patterns, see `docs/fulilian-kanban-v1-spec.pdf` in the repository.
...
- **Dispatcher** — a long-lived loop that, every N seconds (default 60): reclaims stale claims, reclaims crashed workers (PID gone but TTL not yet expired), promotes ready tasks, atomically claims, spawns assigned profiles. Runs **inside the gateway** by default (`kanban.dispatch_in_gateway: true`). One dispatcher sweeps all boards per tick; workers are spawned with `FULILIAN_KANBAN_BOARD` pinned so they can't see other boards. After `kanban.failure_limit` consecutive spawn failures on the same task (default: 2) the dispatcher auto-blocks it with the last error as the reason — prevents thrashing on tasks whose profile doesn't exist, workspace can't mount, etc.
...
fulilian kanban boards list

# Create a new board.
fulilian kanban boards create atm10-server \
    --name "ATM10 Server" \
...
fulilian kanban --board atm10-server list
fulilian kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
fulilian kanban boards switch atm10-server
fulilian kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
fulilian kanban boards rename atm10-server "ATM10 (Prod)"
...
2. `FULILIAN_KANBAN_BOARD` env var (set by the dispatcher when spawning a
   worker, so workers can't see other boards).
3. `~/.fulilian/kanban/current` — the slug persisted by `fulilian kanban
   boards switch`.
4. `default`.
...
`fulilian dashboard` → Kanban tab shows a board switcher at the top as soon
...
- **Storage** — files land under
  `<fulilian-home>/kanban/attachments/<task_id>/` for the default board, or
  `<fulilian-home>/kanban/boards/<slug>/attachments/<task_id>/` for a named
  board. Set `FULILIAN_KANBAN_ATTACHMENTS_ROOT` to pin a custom location.
...
fulilian kanban init

# 2. Start the gateway (hosts the embedded dispatcher)
fulilian gateway start

# 3. Create a task (you — or an orchestrator agent via kanban_create)
fulilian kanban create "research AI funding landscape" --assignee researcher

# 4. Watch activity live (you)
fulilian kanban watch

# 5. See the board (you)
fulilian kanban list
fulilian kanban stats
```

When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `fulilian kanban show t_abcd`.
...
Override the config flag at runtime via `FULILIAN_KANBAN_DISPATCH_IN_GATEWAY=0`
for debugging. Standard gateway supervision applies: run `fulilian gateway
...
until one comes up — `fulilian kanban create` warns about this at creation
time.

Running `fulilian kanban daemon` as a separate process is **deprecated**;
...
fulilian kanban create "nightly ops review" \
    --assignee ops \
...
```bash
fulilian kanban complete t_abc t_def t_hij --result "batch wrap"
fulilian kanban archive  t_abc t_def t_hij
fulilian kanban unblock  t_abc t_def
fulilian kanban block    t_abc "need input" --ids t_def t_hij
```
...
**Workers do not shell out to `fulilian kanban`.** When the dispatcher spawns a worker it sets `FULILIAN_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `fulilian kanban` CLI.
...
kanban_show()                                     # no args — uses FULILIAN_KANBAN_TASK
...
### Why tools instead of shelling to `fulilian kanban`

Three reasons:

1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `fulilian kanban complete` *inside* the container, where `fulilian` isn't installed and `~/.fulilian/kanban.db` isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.fulilian/kanban.db` regardless of terminal backend.
...
**Zero schema footprint on normal sessions.** A regular `fulilian chat` session has zero `kanban_*` tools in its schema unless the active profile explicitly enables the `kanban` toolset for orchestrator work. Dispatcher-spawned task workers get task-scoped tools because `FULILIAN_KANBAN_TASK` is set; orchestrator profiles get the broader routing surface through config. No tool bloat for users who never touch kanban.
...
```json
{
  ...
  "verification": ["pytest tests/fulilian_cli/test_kanban_db.py -q"],
...
2. `cd $FULILIAN_KANBAN_WORKSPACE` (via the terminal tool) and do the work there.
...
**Agent-side prevention:** Before the worker exits, Fulilian injects up to two
...
guard is active only for dispatcher-spawned workers (`FULILIAN_KANBAN_TASK` is
set) and can be disabled with `FULILIAN_KANBAN_STOP_NUDGE=0`.
...
```bash
fulilian kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

fulilian kanban create "audit auth flow" \
    --assignee reviewer \
...
The dispatcher emits one `--skills <name>` flag per skill listed, so the worker spawns with all of them loaded on top of the auto-injected kanban guidance. The skill names must match skills that are actually installed on the assignee's profile (run `fulilian skills list` to see what's available); there's no runtime install.
...
```bash
# At creation
fulilian kanban create "hard refactor" --assignee coder \
    --model claude-opus-4.6 --provider anthropic

# Or later — takes effect on the next dispatch
fulilian kanban set-model t_abcd claude-opus-4.6 --provider anthropic
fulilian kanban set-model t_abcd none    # clear the override
```
...
Kanban's per-profile configs make the planner/worker cost split natural. Decomposing a project into well-scoped cards takes frontier-level judgment; executing a card that already carries a clear goal, context, and handoff evidence usually doesn't — and the workers are where the vast majority of tokens are spent, so the worker model is where the cost lives. Run your orchestrator/dispatcher profile on a frontier model and point worker profiles at inexpensive models. Each profile has its own `config.yaml` under `~/.fulilian/profiles/<name>/`, and the dispatcher injects the profile-scoped `FULILIAN_HOME` when it spawns `fulilian -p <assignee>`, so each worker reads its own profile's model settings:

```yaml
# ~/.fulilian/config.yaml (orchestrator / dispatcher profile)
model:
  default: "your-frontier-model"

# ~/.fulilian/profiles/coder/config.yaml (worker profile)
model:
  default: "your-inexpensive-model"

# ~/.fulilian/profiles/researcher/config.yaml (another worker profile)
model:
  default: "your-inexpensive-model"
```

For the occasional quality-sensitive card, pin just that task back to a stronger model with the [per-task model override](#per-task-model-override) (`--model`/`--provider` at create time, `fulilian kanban set-model` later, or the dashboard's model dropdown) — no profile edits needed.
...
```bash
fulilian kanban create "Translate the docs site to French" \
...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `fulilian kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.
...
Flip between the two modes from the **Orchestration: Auto/Manual** pill at the top of the kanban page (emerald = Auto, muted gray = Manual), or by editing `config.yaml` directly. Both modes coexist with `fulilian kanban specify` — that's still available as a single-task spec rewrite when you don't want fan-out.

The decomposer's routing decisions depend on profile descriptions, which is a per-profile labeling primitive you set with `fulilian profile create --description "..."`, `fulilian profile describe <name> --text "..."`, `fulilian profile describe <name> --auto` (LLM-generates from the profile's installed skills + model), or the dashboard's per-profile editor in the expanded **Orchestration settings** panel. Profiles without a description still appear in the roster — they're routable by name, just less precisely. The decomposer NEVER lands a child task with `assignee=None`: when the LLM picks an unknown profile, the child gets routed to `kanban.default_assignee` (or the active default profile if that's unset).
...
Config knobs (all under `kanban:` in `~/.fulilian/config.yaml`):
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `fulilian profile describe --auto`). |
...
└──────────┬─────────────┘                                    │
           ...
           ▼                                                  │
┌────────────────────────┐                                    │
│  ~/.fulilian/kanban.db   │ ───── append task_events ──────────┘
│  (WAL, shared)         │
└────────────────────────┘
```
...
The plugin uses the standard Fulilian dashboard plugin contract — see [Extending the Dashboard](./extending-the-dashboard) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
```
fulilian kanban init                                     # create kanban.db + print daemon hint
fulilian kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
fulilian kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived]
        ...
        [--json]
fulilian kanban show <id> [--json]
fulilian kanban assign <id> <profile>                    # or 'none' to unassign
fulilian kanban reassign <id>... <profile>               # bulk re-assign tasks to a profile
fulilian kanban edit <id> [--title ...] [--body ...]     # edit task title / body / priority in place
        [--priority N]
fulilian kanban promote <id>...                          # move todo/blocked tasks to ready (recovery)
fulilian kanban schedule <id> --at <ISO8601>             # set/clear a task's scheduled_at start time
fulilian kanban diagnostics [--json]                     # board health snapshot (alias: diag)
fulilian kanban link <parent_id> <child_id>
fulilian kanban unlink <parent_id> <child_id>
fulilian kanban claim <id> [--ttl SECONDS]
fulilian kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
fulilian kanban complete <id>... [--result "..."]
fulilian kanban block <id> "<reason>" [--ids <id>...]
fulilian kanban unblock <id>...
fulilian kanban archive <id>...

fulilian kanban request-review <id> [--summary "..."] [--metadata JSON] [--reviewer PROFILE]
fulilian kanban request-changes <id> "<required changes>"               # active reviewer -> implementer
fulilian kanban reopen-review  <id>... [--reason "..."]                 # changes requested: 'review' -> ready/todo

fulilian kanban tail <id>                                # follow a single task's event stream
fulilian kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
fulilian kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
fulilian kanban runs <id> [--json]                       # attempt history (one row per run)
fulilian kanban assignees [--json]                       # profiles on disk + per-assignee task counts
fulilian kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
fulilian kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `fulilian gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
fulilian kanban stats [--json]                           # per-status + per-assignee counts
fulilian kanban log <id> [--tail BYTES]                  # worker log from ~/.fulilian/kanban/logs/
fulilian kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
        [--chat-type dm|group|channel|thread] [--delivery-mode notify|notify+wake|wake]
fulilian kanban notify-list [<id>] [--json]
fulilian kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
fulilian kanban context <id>                             # what a worker sees
fulilian kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
fulilian kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
```bash
fulilian kanban create "nightly backup audit" \
...
Every `fulilian kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `fulilian chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `fulilian_cli.kanban.run_slash()` entry point that reuses the `fulilian kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `fulilian kanban`. You don't have to leave the chat to drive the board.
...
The gateway normally queues slash commands and user messages while an agent is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.fulilian/kanban.db`, not in the running agent's state, so reads (`list`, `show`, `context`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`fulilian kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `fulilian kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |

For worked examples of each, see `docs/fulilian-kanban-v1-spec.pdf`.
...
_metadata_: `{"changed_files": ["fulilian_cli/retry.py", "tests/test_retry.py"], "decisions": ["capped backoff at 60s", "jitter = full"]}`
```
...
fulilian kanban create "Fix CI failure from t_impl: test_retry flakes on 3.11" \
    --assignee coder \
    --parent t_impl \
    --body "$(cat <<'EOF'
...
```
hotspot: fulilian_cli/kanban_db.py — third conflicting edit to the dispatch loop this wave
```
...
```bash
fulilian kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7 \
    --chat-type group --delivery-mode notify+wake
fulilian kanban notify-list
fulilian kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
fulilian kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
fulilian kanban runs t_abcd
...
**Bulk close caveat.** `fulilian kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`fulilian kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `fulilian kanban heartbeat $TASK` to signal liveness during long operations. |
...
`fulilian kanban tail <id>` shows these for a single task. `fulilian kanban watch` streams them board-wide.

## Out of scope

Kanban is deliberately single-host. `~/.fulilian/kanban.db` is a local SQLite file and the dispatcher spawns workers on the same machine. Running a shared board across two hosts is not supported — there's no coordination primitive for "worker X on host A, worker Y on host B," and the crash-detection path assumes PIDs are host-local. If you need multi-host, run an independent board per host and use `delegate_task` / a message queue to bridge them.

## Design spec

The complete design — architecture, concurrency correctness, comparison with other systems, implementation plan, risks, open questions — lives in `docs/fulilian-kanban-v1-spec.pdf`. Read that before filing any behavior-change PR.
