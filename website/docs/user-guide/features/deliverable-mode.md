...
When FuLiLian runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.fulilian/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.fulilian/config.yaml` (switchable per session
via `/personality`).
...
If you use Fulilian' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.fulilian/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. FuLiLian's deliverable
