...
sidebar_position: 10
title: "Skins & Themes"
description: "Customize the Fulilian CLI with built-in and user-defined skins"
---

# Skins & Themes

Skins control the **visual presentation** of the Fulilian CLI: banner colors, spinner faces and verbs, response-box labels, branding text, and the tool activity prefix.
...
/skin mytheme        # switch to a custom skin from ~/.fulilian/skins/mytheme.yaml
```

Or set the default skin in `~/.fulilian/config.yaml`:
...
|------|-------------|----------------|------------------|
| `default` | Classic Fulilian — gold and kawaii | `FuLiLian` | Warm gold borders, cornsilk text, kawaii faces in spinners. The familiar caduceus banner. Clean and inviting. |
...
| `mono` | Monochrome — clean grayscale | `FuLiLian` | All grays — no color. Borders are `#555555`, text is `#c9d1d9`. Ideal for minimal terminal setups or screen recordings. |
| `slate` | Cool blue — developer-focused | `FuLiLian` | Royal blue borders (`#4169e1`), soft blue text. Calm and professional. No custom spinner — uses default faces. |
| `daylight` | Light theme for bright terminals with dark text and cool blue accents | `FuLiLian` | Designed for white or bright terminals. Dark slate text with blue borders, pale status surfaces, and a light completion menu that stays readable in light terminal profiles. |
| `warm-lightmode` | Warm brown/gold text for light terminal backgrounds | `FuLiLian` | Warm parchment tones for light terminals. Dark brown text with saddle-brown accents, cream-colored status surfaces. An earthy alternative to the cooler daylight theme. |
...
|-----|-------------|---------|
| `agent_name` | Name shown in banner title and status display | `FuLiLian` |
| `welcome` | Welcome message shown at CLI startup | `Welcome to FuLiLian! Type your message or /help for commands.` |
| `goodbye` | Message shown on exit | `Goodbye! ⚕` |
| `response_label` | Label on the response box header | ` ⚕ Fulilian ` |
...
| `banner_logo` | string | Rich-markup ASCII art logo (replaces the default FULILIAN_AGENT banner) | `""` |
| `banner_hero` | string | Rich-markup hero art (replaces the default caduceus art) | `""` |

## Custom skins

Create YAML files under `~/.fulilian/skins/`. User skins inherit missing values from the built-in `default` skin, so you only need to specify the keys you want to change.

### Full custom skin YAML template

```yaml
# ~/.fulilian/skins/mytheme.yaml
...
## Fulilian Mod — Visual Skin Editor

[Fulilian Mod](https://github.com/cocktailpeanut/fulilian-mod) is a community-built web UI for creating and managing skins visually. Instead of writing YAML by hand, you get a point-and-click editor with live preview.

![Fulilian Mod skin editor](https://raw.githubusercontent.com/cocktailpeanut/fulilian-mod/master/nous.png)

**What it does:**

- Lists all built-in and custom skins
- Opens any skin into a visual editor with all Fulilian skin fields (colors, spinner, branding, tool prefix, tool emojis)
...
- Saves directly to `~/.fulilian/skins/`
- Activates a skin by updating `~/.fulilian/config.yaml`
...
```bash
git clone https://github.com/cocktailpeanut/fulilian-mod.git
cd fulilian-mod/app
npm install
npm start
```
...
6. Click **Save** to write the skin YAML to `~/.fulilian/skins/`.
7. Click **Activate** to set it as the current skin (updates `display.skin` in `config.yaml`).

Fulilian Mod respects the `FULILIAN_HOME` environment variable, so it works with [profiles](/user-guide/profiles) too.

## Operational notes

- Built-in skins load from `fulilian_cli/skin_engine.py`.
- Unknown skins automatically fall back to `default`.
- `/skin` updates the active CLI theme immediately for the current session.
- User skins in `~/.fulilian/skins/` take precedence over built-in skins with the same name.
