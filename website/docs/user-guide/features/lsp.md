...
Fulilian runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Fulilian
...
1. Fulilian captures a baseline of current diagnostics for the file.
...
…). Fulilian auto-detects the binary on PATH or in
`<FULILIAN_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Fulilian at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<FULILIAN_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`fulilian lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Fulilian installs both packages together when you
run `fulilian lsp install typescript` or auto-install fires on first
use.

## CLI

```
fulilian lsp status          # service state + per-server install status
fulilian lsp list            # registry, optionally --installed-only
fulilian lsp install <id>    # eagerly install one server
fulilian lsp install-all     # try every server with a known recipe
fulilian lsp restart         # tear down running clients
fulilian lsp which <id>      # print resolved binary path
```

`fulilian lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <FULILIAN_HOME>/lsp/bin
...
When `install_strategy: auto`, Fulilian installs binaries into
`<FULILIAN_HOME>/lsp/bin/`. NPM packages land in
`<FULILIAN_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Fulilian-owned and is
...
**`fulilian lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<FULILIAN_HOME>/lsp/bin/`. Run
`fulilian lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `fulilian lsp status`**
...
When `fulilian lsp status` shows a `Backend warnings` section, install
...
`~/.fulilian/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.fulilian/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `fulilian lsp restart` to clear the set;
