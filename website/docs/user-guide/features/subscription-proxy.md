...
chat completions — use your Fulilian-managed provider subscription as their
...
| Use case | "Use Fulilian as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Fulilian stores
the refresh token in `~/.fulilian/auth.json` — the same place all Fulilian
...
interface in `fulilian_cli/proxy/adapters/`.
...
If you see `not logged in`, run `fulilian portal`. If you see
...
`fulilian_cli/proxy/adapters/<provider>.py` and registering it in
