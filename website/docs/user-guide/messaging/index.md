...
description: "Chat with Fulilian from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---
...
For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Fulilian](/guides/use-voice-mode-with-fulilian).
...
```mermaid
flowchart TB
    subgraph Gateway["Fulilian Gateway"]
...
For group chats, hooks, and automation flows, Fulilian supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Fulilian keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Fulilian does not hide failures just because the text resembles a silence token.
...
```bash
fulilian gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
fulilian gateway              # Run in foreground
fulilian gateway setup        # Configure messaging platforms interactively
fulilian gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo fulilian gateway install --system   # Linux only: install a boot-time system service
fulilian gateway start        # Start the default service
fulilian gateway stop         # Stop the default service
fulilian gateway status       # Check default service status
fulilian gateway status --system         # Linux only: inspect the system service explicitly
```
...
```yaml title="~/.fulilian/config.yaml"
gateway:
...
```bash
fulilian gateway install --force
```

A positive value makes the generated unit use `Type=notify`,
`NotifyAccess=main`, and the matching `WatchdogSec`. Fulilian sends heartbeats
...
| `/update` | Update FuLiLian to the latest version |
...
with the `session_reset` section in `~/.fulilian/config.yaml`:
...
Configure per-platform overrides in `~/.fulilian/gateway.json`:
...
Different channels can run different models and personas from a **single gateway** — e.g. a cheap fast model in `#daily` and a frontier model with a specialist prompt in `#dev`. Configure `channel_overrides` under the platform in `~/.fulilian/gateway-config.yaml`:
...
fulilian pairing approve telegram XKGH5N7P

# Other pairing commands:
fulilian pairing list          # View pending + approved users
fulilian pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Fulilian appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.fulilian/config.yaml`:
...
Setting `display.tool_progress: log` sends **no** progress bubbles to chat. Instead, each tool call is appended as a line to `~/.fulilian/logs/tool_calls.log` — a rotating audit file (5 MB × 3 backups) run through the same secret-redacting formatter as regular logs, so credentials never land on disk. Use it when you want a full tool-call trail without any chat noise.

### Configurable status phrases

Long-running gateway status lines ("still working…"-style heartbeats) draw from a phrase catalog. Built-in defaults ship in `gateway/assets/status_phrases.yaml`; you can add your own with profile-portable files under `FULILIAN_HOME`:

- `~/.fulilian/status_phrases.yaml` or any `*.yaml` in `~/.fulilian/status_phrases/` (conventional paths, auto-loaded), or
- point config at a relative path:

```yaml
display:
  status_phrases:
    path: status_phrases/whatsapp.yaml  # relative to FULILIAN_HOME
...
Off by default. When enabled, Fulilian prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.fulilian/config.yaml`:
...
```bash
FULILIAN_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
fulilian gateway install               # Install as user service
fulilian gateway start                 # Start the service
fulilian gateway stop                  # Stop the service
fulilian gateway status                # Check status
journalctl --user -u fulilian-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo fulilian gateway install --system
sudo fulilian gateway start --system
sudo fulilian gateway status --system
journalctl -u fulilian-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Fulilian installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit fulilian-gateway` (or `sudo systemctl edit fulilian-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `fulilian update`. When `fulilian update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart fulilian-gateway` command (it never blocks on an interactive password prompt).
...
```bash
fulilian gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `fulilian update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo fulilian update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/fulilian-gateway`:

```
fulilian ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed fulilian-gateway*, /usr/bin/systemctl --no-ask-password start fulilian-gateway*, /usr/bin/systemctl --no-ask-password restart fulilian-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Fulilian will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Fulilian installations on the same machine (with different `FULILIAN_HOME` directories), each gets its own systemd service name. The default `~/.fulilian` uses `fulilian-gateway`; other installations use `fulilian-gateway-<hash>`. The `fulilian gateway` commands automatically target the correct service for your current `FULILIAN_HOME`.
:::

### macOS (launchd)

```bash
fulilian gateway install               # Install as launchd agent
fulilian gateway start                 # Start the service
fulilian gateway stop                  # Stop the service
fulilian gateway status                # Check status
tail -f ~/.fulilian/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.fulilian.gateway.plist`. It includes three environment variables:
...
- **FULILIAN_HOME** — scopes the gateway to your Fulilian installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `fulilian gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `FULILIAN_HOME` directory gets its own launchd label. The default `~/.fulilian` uses `ai.fulilian.gateway`; other installations use `ai.fulilian.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `fulilian-cli` | Full access |
| Telegram | `fulilian-telegram` | Full tools including terminal |
| Discord | `fulilian-discord` | Full tools including terminal |
| WhatsApp | `fulilian-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `fulilian-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `fulilian-slack` | Full tools including terminal |
| Google Chat | `fulilian-google_chat` | Full tools including terminal |
| Signal | `fulilian-signal` | Full tools including terminal |
| SMS | `fulilian-sms` | Full tools including terminal |
| Email | `fulilian-email` | Full tools including terminal |
| Home Assistant | `fulilian-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `fulilian-mattermost` | Full tools including terminal |
| Matrix | `fulilian-matrix` | Full tools including terminal |
| DingTalk | `fulilian-dingtalk` | Full tools including terminal |
| Feishu/Lark | `fulilian-feishu` | Full tools including terminal |
| WeCom | `fulilian-wecom` | Full tools including terminal |
| WeCom Callback | `fulilian-wecom-callback` | Full tools including terminal |
| Weixin | `fulilian-weixin` | Full tools including terminal |
| BlueBubbles | `fulilian-bluebubbles` | Full tools including terminal |
| QQBot | `fulilian-qqbot` | Full tools including terminal |
| Yuanbao | `fulilian-yuanbao` | Full tools including terminal |
| Microsoft Teams | `fulilian-teams` | Full tools including terminal |
| API Server | `fulilian-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `fulilian-webhook` | Full tools including terminal |
| Raft | `fulilian-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.fulilian/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
