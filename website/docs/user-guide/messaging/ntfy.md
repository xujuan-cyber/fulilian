...
ntfy makes a great lightweight push channel for Fulilian: subscribe to a topic from the [ntfy mobile app](https://ntfy.sh/docs/subscribe/phone/), send messages to the topic to talk to the agent, get the response back on your phone.

> Run `fulilian gateway setup` and pick **ntfy** for a guided walk-through.

## Prerequisites

- A topic name (any unique string — `fulilian-myname-2026` works fine)
...
That's it. No SDK, no daemon, no Node.js. The adapter uses `httpx` which is already a Fulilian dependency.
...
Add these to `~/.fulilian/.env`:

```
NTFY_TOPIC=fulilian-myname-2026
NTFY_ALLOWED_USERS=fulilian-myname-2026
NTFY_HOME_CHANNEL=fulilian-myname-2026
```
...
ntfy has no native authenticated user identity. The `title` field on a published message is **publisher-controlled** and can be anything the sender wants. The Fulilian adapter does NOT use `title` for authorization — it would let any publisher who knows the topic spoof an allowed user.
...
1. Pick a topic name: `fulilian-myname-2026`
2. On your phone: install the [ntfy app](https://ntfy.sh/docs/subscribe/phone/), tap **+**, enter `fulilian-myname-2026`
3. On the host:
   ```bash
   echo 'NTFY_TOPIC=fulilian-myname-2026' >> ~/.fulilian/.env
   echo 'NTFY_ALLOWED_USERS=fulilian-myname-2026' >> ~/.fulilian/.env
...
Or target a specific topic explicitly via the cron job's `deliver:` field, or from a shell script with the [`fulilian send` CLI](/guides/pipe-script-output):

```bash
fulilian send ntfy:alerts-channel "Done!"
```
...
ntfy clients render markdown when the publisher sets the `X-Markdown: true` header. To enable for outgoing Fulilian replies:
...
If you only want Fulilian to *push* notifications to ntfy (cron summaries, alerts) and never accept messages back, set both `NTFY_TOPIC` and `NTFY_PUBLISH_TOPIC` to the same value and skip `NTFY_ALLOWED_USERS` entirely. With no allowlist, the agent never responds to inbound messages — your phone gets the pushes, but the conversation is one-way.

## Limits

- **Message size**: ntfy caps message bodies at 4096 chars. Fulilian truncates with a warning when this is exceeded.
