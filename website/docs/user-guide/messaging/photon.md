...
Connect Fulilian to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Fulilian — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Fulilian runs it in a
...
fulilian photon setup --phone +15551234567
```
...
2. **Finds or creates** the `FuLiLian` project on your account.
...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.fulilian/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/.fulilian/.env`
...
(device token, dashboard project id) lives in `~/.fulilian/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Fulilian
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Fulilian replies with a pairing code. Approve it with:

```bash
fulilian pairing approve photon <CODE>
```

Use `fulilian pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.fulilian/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.fulilian/.env`):
...
By default Fulilian responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Fulilian` and
`@Fulilian agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Fulilian will reply.

## Status & troubleshooting

```bash
fulilian photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line Fulilian uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run fulilian photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `fulilian photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `fulilian photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `fulilian photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Fulilian sends images, voice
  ...
  media.
- **Native polls are supported.** Fulilian sends poll content through
  spectrum-ts' `poll()` builder via the sidecar's `/send-poll` endpoint.
- **Message effects are supported.** Fulilian sends text with native iMessage
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `fulilian send`, the dashboard) reuse the sidecar the
  gateway spawned — they read its port/token from
  `<fulilian-home>/runtime/photon-sidecar.json`, written once the sidecar
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when Fulilian is set
...
| `PHOTON_MENTION_PATTERNS` | Fulilian wake words  | JSON list / comma / newline regex patterns for group mentions |
