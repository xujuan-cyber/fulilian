...
description: "Set up FuLiLian as a WhatsApp bot via Meta's official Business Cloud API"
---

# WhatsApp Business Cloud API Setup

Fulilian can connect to WhatsApp through Meta's **official** WhatsApp Business Cloud API. This is the production-grade path: no Node.js bridge subprocess, no QR codes, no account-ban risk.
...
- The Fulilian gateway needs a **public HTTPS URL** so Meta can deliver inbound messages via webhook.
- Replies more than 24 hours after the user's last message require a pre-approved **template** (this is Meta's "customer service window" rule, not a Fulilian limit).
...
4. **Optional but recommended**: ffmpeg on `PATH` so outbound voice messages render as native WhatsApp voice-note bubbles (green waveform) instead of MP3 audio attachments. Fulilian degrades gracefully if absent.
...
2. **Add** → name (e.g. `fulilian-bot`) → role: **Admin**.
...
6. Copy the token → update `WHATSAPP_CLOUD_ACCESS_TOKEN` in `~/.fulilian/.env` → restart the gateway.

System User tokens don't expire unless you explicitly revoke them.

---

## Exposing Fulilian to the internet

The Cloud API delivers inbound messages by HTTPS POST to your webhook URL — that means the Fulilian gateway has to be reachable from Meta's servers.  Three common ways:
...
2. Generate a **Verify Token** — the wizard does this for you with `secrets.token_urlsafe(32)`; if you're configuring manually, run:
   ...
   ```
   Save it as `WHATSAPP_CLOUD_VERIFY_TOKEN` in `~/.fulilian/.env`.
3. Start the Fulilian gateway: `fulilian gateway`.
...
## Allowlist (Fulilian-side)

In addition to Meta's recipient whitelist, Fulilian has its own per-platform allowlist that controls **which incoming messages the agent processes**.  Add to `~/.fulilian/.env`:
...
The `fulilian whatsapp-cloud` wizard prints these links at the end of setup. None of this is required for the bot to work — it's pure polish for how your bot appears to users.

---

## Configuration reference

All settings live in `~/.fulilian/.env`.  Required values are in **bold**.
...
When the agent invokes any of these flows, Fulilian uses WhatsApp's native interactive messages — tap-to-answer buttons instead of "reply with the number" prompts:
...
Fulilian acknowledges inbound messages immediately:
...
Fulilian TTS produces MP3. Two paths:
...
Fulilian warns the agent about this window in its system prompt, so the model knows to mention it when scheduling delayed messages.

Message-template support (the workaround for outside-window sends) is not yet implemented in Fulilian. If you need it, please [open an issue](https://github.com/NousResearch/hermes-agent/issues) — it's planned but waiting on a clear demand signal.

### Group chats

The Cloud API has limited group support (capability-tier gated by Meta).  Fulilian's `whatsapp_cloud` adapter currently handles **direct messages only** in v1.  If you need group chats, use the Baileys bridge.

### Outbound rate limit

Meta's default throughput is **80 messages/second per business phone number**, with upgrades available.  Fulilian doesn't currently enforce this client-side — extremely high-volume sends could hit Meta's limit.
...
- **Verify token mismatch** — the token in `~/.fulilian/.env`'s `WHATSAPP_CLOUD_VERIFY_TOKEN` must match exactly what you typed into Meta's dashboard.  Run the curl probe above to confirm the gateway's verify handshake works locally first.
- **Gateway not running** — check `fulilian gateway` is up.
- **App Secret not set** — without it, Fulilian refuses inbound POSTs with 503.  Meta interprets that as "can't validate."
...
- Wait for template support to land in Fulilian.
...
Common cause: the toolset configured for `whatsapp_cloud` is missing the tools the agent wants to call.  Check `fulilian tools list` and verify the platform is using `fulilian-whatsapp` (the default Cloud adapter toolset, same as Baileys).

If the model emits tool-call-shaped text instead of a structured call, it usually means the toolset was effectively empty.  See `fulilian_cli/platforms.py` for the platform → default toolset mapping.

### STT (voice note transcription) returns empty / "could not transcribe"

The default `stt.provider: local` requires `pip install faster-whisper`.  If you're a Nous subscriber, you can route STT through the managed gateway instead — select **Nous Subscription** for speech-to-text in `fulilian tools`, or set it directly:

```bash
fulilian config set stt.provider nous
...
- **Treat the App Secret like a password** — anyone with it can forge webhook payloads that Fulilian will accept as authentic.
...
| | Baileys (`fulilian whatsapp`) | Cloud API (`fulilian whatsapp-cloud`) |
|---|---|---|
...
Most users running Fulilian for personal projects prefer Baileys. Most users running customer-facing bots prefer Cloud API.
