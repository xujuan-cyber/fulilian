...
description: Control your smart home with FuLiLian via Home Assistant integration.
...
FuLiLian integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "FuLiLian"
5. Copy the token
...
FuLiLian registers four tools for smart home control:
...
Configure which events the agent sees in `~/.fulilian/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "FuLiLian".
...
The adapter reads credentials from `~/.fulilian/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Fulilian profile
...
running Fulilian — `curl -H "Authorization: Bearer <token>" <url>/api/` should
