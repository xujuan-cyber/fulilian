...
Connect Fulilian to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Fulilian supports two WeCom integration modes:
...
> Run `fulilian gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Fulilian decrypts the message, queues it for the agent
...
(Use `fulilian gateway start` only after `fulilian gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Fulilian and the token the
...
in `~/.fulilian/.env` values around `=` will also break signature checks. After
fixing, restart `fulilian gateway run`.
...
Check `fulilian gateway run` logs for the bound host/port. If the adapter bound to
