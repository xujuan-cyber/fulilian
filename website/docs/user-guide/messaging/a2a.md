...
[A2A](https://a2a-protocol.org) is the open Agent2Agent protocol (v1.0, stewarded by the Linux Foundation) for communication between independent AI agents. The Fulilian A2A plugin works in **both directions**: your agent can call other A2A agents as tools, and other agents can send tasks to your Fulilian over HTTP.

It interoperates with any A2A-compliant peer — another Fulilian, LangChain, CrewAI, Google ADK agents, or anything built on the official `a2a-sdk`.

## When to use A2A

- **Fulilian ↔ Fulilian across machines** — let your desktop agent hand tasks to a Fulilian on a server, or vice versa, each with its own memory, tools, and credentials.
...
- **Being a callable service** — expose your Fulilian so other frameworks' agents can send it tasks.
...
```bash
fulilian gateway setup      # pick A2A
```
...
```bash
fulilian tools enable a2a --platform cli        # CLI/TUI sessions
fulilian tools enable a2a --platform telegram   # or any messaging platform
fulilian tools enable a2a --platform a2a        # let inbound A2A tasks call peers (agent chaining)
```
...
With the platform enabled, Fulilian serves:
...
- **Audit log** — every exchange appends to `~/.fulilian/a2a_audit.jsonl`.
