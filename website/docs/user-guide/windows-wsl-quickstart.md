...
description: "Run FuLiLian on Windows via WSL2 — setup, filesystem access between Windows and Linux, networking, and common pitfalls"
...
FuLiLian now supports **both** native Windows and WSL2.  This page covers the WSL2 path; for the native PowerShell install see the dedicated **[Windows (Native) Guide](./windows-native.md)**.
...
- You're doing POSIX-heavy development work and want your Fulilian sessions to share the same filesystem / paths as your dev tools.
- You already have a WSL2 environment and don't want to maintain a second install.

**When native is fine (or better):**
- Interactive chat, gateway (Telegram/Discord/etc.), cron scheduler, browser tool, MCP servers, and most Fulilian features all run natively on Windows.
...
This guide covers the parts of that split that specifically affect Fulilian: installing WSL2, getting files back and forth between Windows and Linux, networking in both directions, and the pitfalls people actually hit.
...
The native Windows install runs in Windows directly: your Windows terminal (PowerShell, Windows Terminal, etc.), Windows filesystem paths (`C:\Users\…`), and Windows processes.  Fulilian uses Git Bash to run shell commands, which is how Claude Code and other agents handle Windows today — it sidesteps the POSIX-vs-Windows gap without a full rewrite.

WSL2 runs a real Linux kernel in a lightweight VM, so Fulilian inside it is essentially identical to running on Ubuntu.  That's valuable when you want a real POSIX environment: `fork`, `/tmp`, UNIX sockets, signal semantics, PTY-backed terminals, shells like `bash`/`zsh`, and tools like `rg`, `git`, `ffmpeg` that behave the way they do on Linux.

Practical consequences of WSL2:

- The Fulilian CLI, gateway, sessions, memory, skills, and tool runtimes all live inside the Linux VM.
- Windows programs (browsers, native apps, Chrome with your logged-in profile) live outside it.
- Every time you want the two to talk — share files, open URLs, control Chrome, hit a local model server, expose the Fulilian gateway to your phone — you cross a boundary. Those boundaries are what this guide is about.
...
Fulilian does not work reliably on WSL1 — WSL1 translates Linux syscalls on the fly and some behaviors (procfs, signals, network) diverge from real Linux.
...
The fulilian gateway (and anything else you want to keep running) is easier to manage with systemd. On modern WSL, enable it once inside your distro:
...
### Where to put Fulilian and your projects

**Rule of thumb: keep everything Linux-ish inside the Linux filesystem.**

- Your Fulilian install (`~/.fulilian/`) — Linux side. The installer already does this.
...
Clone inside WSL. Always, unless you have a specific reason not to. A typical Fulilian workflow (`fulilian chat`, tool calls that `rg`/`ripgrep` the repo, file watchers, background gateway) will be dramatically faster and more reliable against `~/code/myrepo` than `/mnt/c/Users/you/myrepo`.

One exception: **MCP bridges that launch Windows binaries.** If you're using `chrome-devtools-mcp` through `cmd.exe` (see [MCP guide: WSL → Windows Chrome](/guides/use-mcp-with-fulilian#wsl2-bridge-fulilian-in-wsl-to-windows-chrome)), Windows may complain with a `UNC` warning if Fulilian's current working directory is `~`. In that case, start Fulilian from somewhere under `/mnt/c/` so the Windows process has a drive-letter cwd.
...
### Case 1 — Fulilian in WSL talks to a service on Windows

Most common: you're running **Ollama, LM Studio, or a llama-server on Windows**, and Fulilian (inside WSL) needs to hit it.
...
### Case 2 — Something on Windows (or your LAN) talks to Fulilian in WSL

This is the reverse direction and is less documented elsewhere, but it's what you need for:

- Using the Fulilian **web dashboard** from a Windows browser.
- Using the **OpenAI-compatible API server** (exposed by `fulilian gateway` when `API_SERVER_ENABLED=true`) from a Windows-side tool. See the [API Server feature page](/user-guide/features/api-server).
...
On **NAT mode** (Windows 10 / older Windows 11), the default "localhost forwarding" in WSL2 will generally forward Linux-side `127.0.0.1` binds to Windows `localhost`, so a Fulilian service started with `--host 127.0.0.1` is usually reachable as `http://localhost:PORT` from Windows. If it isn't:
...
2. **Port-forward Windows → WSL VM.** In mirrored mode this is automatic. In NAT mode you have to do it yourself, per port, in Admin PowerShell:
   ...
   New-NetFirewallRule -DisplayName "Fulilian WSL 8080" `
...
## Running Fulilian services long-term on Windows

The Fulilian [Tool Gateway](/user-guide/features/tool-gateway) and the API server are long-lived processes. In WSL2 you have a few options for keeping them up.

### Desktop shortcut for opening Fulilian quickly

If you just want a double-click launcher for an interactive Fulilian shell, create
...
2. For the target, use your distro name (replace `Ubuntu` if needed):
   ...
   ```text
   wt.exe -w 0 -p "Ubuntu" wsl.exe -d Ubuntu --cd ~ -- bash -ic "fulilian"
   ```

3. Name it something obvious like `Fulilian`.

That opens Windows Terminal, starts your WSL distro, drops you in your Linux
home directory, and launches Fulilian. If `fulilian` is not on PATH yet, open WSL
once manually and run `source ~/.bashrc`, or replace the command with
`uv run fulilian` inside your project checkout.

Optional polish:

- **Custom icon:** open **Properties -> Change Icon** and point it at an `.ico`
  file, such as the Fulilian favicon from the repo.
...
If you enabled systemd per the setup section above, `fulilian gateway` and the API server work the way they do on any Linux machine. Use the gateway setup wizard:
...
AMD ROCm and Intel Arc support inside WSL2 is still evolving and outside Fulilian's test matrix — it may work with current drivers but we don't have a recipe to recommend.
...
**Massive slowness on `git status` / `fulilian chat` in a repo.**
...
Fulilian's cwd is inside the Linux filesystem, and Windows `cmd.exe` doesn't know what to do with it. Start Fulilian from `/mnt/c/...` for that session, or use a wrapper that `cd`s to a Windows-reachable path before invoking the Windows executable.
...
**`fulilian` not found after running the installer.**
...
- **[MCP guide → WSL → Windows Chrome](/guides/use-mcp-with-fulilian#wsl2-bridge-fulilian-in-wsl-to-windows-chrome)** — controlling your signed-in Windows Chrome from Fulilian in WSL.
