...
sidebar_position: 1
title: "Architecture"
description: "FuLiLian internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of FuLiLian internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ fulilian_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # FulilianCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── fulilian_state.py           # SQLite session/state database with FTS5
├── fulilian_constants.py       # FULILIAN_HOME, profile-aware paths
...
│
├── fulilian_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `fulilian` subcommands (large file)
...
│   ├── skills_config.py      # fulilian skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # fulilian tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # fulilian gateway start/stop
│
...
```text
User input → FulilianCLI.process_input()
...
Three discovery sources: `~/.fulilian/plugins/` (user), `.fulilian/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `fulilian plugins` or `config.yaml`.
...
Exposes Fulilian as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`fulilian -p <name>`) gets its own FULILIAN_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
