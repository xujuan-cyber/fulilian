...
description: "How to build a web-search/extract/crawl backend plugin for FuLiLian"
---
...
When neither key is set, Fulilian auto-detects the backend from whichever API key/URL is present in the environment. `fulilian tools` walks users through selection.
...
class MyBackendWebSearchProvider(WebSearchProvider):
    ...
    def display_name(self) -> str:
        # Human label shown in `fulilian tools`. Defaults to `name`.
        return "My Backend"

    def is_available(self) -> bool:
        # Cheap check — env var present, optional dep importable, etc.
        # MUST NOT make network calls (runs on every `fulilian tools` paint).
...
| `provides_web_providers` | List of provider `name`s this plugin registers — used by the loader to advertise the plugin in `fulilian tools` even before `register()` runs |
| `requires_env` | Interactive credential prompt during `fulilian plugins install` (see [Build a Fulilian Plugin](/developer-guide/plugins#gate-on-environment-variables) for the rich format) |
...
| `display_name` | — | `name` | Label shown in `fulilian tools` |
...
Fulilian routes calls to the right provider based on the `supports_*` flags. A common multi-provider setup:
...
When `web.search_backend` or `web.extract_backend` aren't set, both fall through to `web.backend`. When that's also unset, Fulilian picks the first available provider that supports the requested capability based on env-var presence.
...
## How Fulilian wires it into the tools
...
Errors surface as the tool result; the LLM decides how to explain them. If no provider is registered (or every available one fails the capability gate), the tool returns a helpful error pointing at `fulilian tools`.

## Lazy-installing optional dependencies

If your provider wraps a third-party SDK (like DDGS does with the `ddgs` package), don't `import` it at module top level. Use `tools.lazy_deps.ensure(...)` inside `is_available()` or `search()` — Fulilian will install the package on first use, gated by `security.allow_lazy_installs`. See [Build a Fulilian Plugin → Lazy-install](/developer-guide/plugins#lazy-install-optional-python-dependencies) for the security model.
