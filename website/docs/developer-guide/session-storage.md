...
FuLiLian uses a SQLite database (`~/.fulilian/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `fulilian_state.py`


## Architecture Overview

```
~/.fulilian/state.db (SQLite, WAL mode)
...
Abridged — see `SCHEMA_SQL` in `fulilian_state.py` for the full current column list
...
indexed columns — see `SCHEMA_SQL` in `fulilian_state.py` for the exact SQL.
...
Multiple fulilian processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.fulilian/state.db
...
Default path: `~/.fulilian/state.db`

This is derived from `fulilian_constants.get_fulilian_home()` which resolves to
`~/.fulilian/` by default, or the value of `FULILIAN_HOME` environment variable.
