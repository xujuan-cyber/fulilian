...
| Variable | Meaning |
|----------|---------|
| `FULILIAN_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `FULILIAN_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `FULILIAN_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Fulilian itself — they're private to these helpers. The variables Fulilian *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `fulilian --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_fulilian_root)" || { echo "htui: not in a Fulilian checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$FULILIAN_MAIN_CHECKOUT/.venv/bin/python" -m fulilian_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `FULILIAN_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_fulilian_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `FULILIAN_TUI_DIR` are mutually exclusive
`FULILIAN_TUI_DIR` points Fulilian at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `fulilian --tui --dev` exits with an error. Run `unset FULILIAN_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_fulilian_root)" || { echo "hgui: not in a Fulilian checkout" >&2; return 1; }
  deps="${FULILIAN_GUI_DEPS_CHECKOUT:-$FULILIAN_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _fulilian_link_deps "$desktop" "$deps/apps/desktop"
    _fulilian_link_deps "$root" "$deps"
  else
  ...
  trap '_fulilian_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    FULILIAN_DESKTOP_FULILIAN_ROOT="$root" \
    FULILIAN_DESKTOP_PYTHON="$FULILIAN_MAIN_CHECKOUT/.venv/bin/python" \
    FULILIAN_DESKTOP_IGNORE_EXISTING=1 \
    FULILIAN_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `FULILIAN_DESKTOP_FULILIAN_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `fulilian`. |
| `FULILIAN_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `FULILIAN_DESKTOP_IGNORE_EXISTING` | Ignores any `fulilian` on `PATH` so it can't shadow the worktree. |
| `FULILIAN_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Fulilian checkout.
_fulilian_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/fulilian_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_fulilian_gui_cleanup() {
  local root="$1"
  [[ -n "$root" ]] && pkill -TERM -f "${root}/apps/desktop/node_modules/electron" 2>/dev/null
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'fulilian_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `fulilian --tui --dev` and the `FULILIAN_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/xujuan-cyber/fulilian/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `FULILIAN_*` variable Fulilian reads
