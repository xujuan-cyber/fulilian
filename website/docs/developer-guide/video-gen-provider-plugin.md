...
description: "How to build a video-generation backend plugin for FuLiLian"
---
...
Fulilian scans for video-gen backends in three places:

1. **Bundled** — `<repo>/plugins/video_gen/<name>/` (auto-loaded with `kind: backend`)
2. **User** — `~/.fulilian/plugins/video_gen/<name>/` (opt-in via `plugins.enabled`)
3. **Pip** — packages declaring a `fulilian_agent.plugins` entry point

Each plugin's `register(ctx)` function calls `ctx.register_video_gen_provider(...)`. The active provider is picked by `video_gen.provider` in `config.yaml`; `fulilian tools` → Video Generation walks users through selection. Unlike `image_generate`, there is no in-tree legacy backend — every provider is a plugin.
...
class MyVideoGenProvider(VideoGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            "name": "My Backend",
            "badge": "paid",
            "tag": "Short description shown in `fulilian tools`",
            "env_vars": [
                {
                    "key": "MY_API_KEY",
...
The provider's `capabilities()` advertises which of these are honored. The agent sees the active backend's capabilities in the tool description, dynamically rebuilt when the user changes backend via `fulilian tools`.
...
The user picks `veo3.1` once in `fulilian tools`. The agent never thinks about endpoints — it just passes (or doesn't pass) `image_url`.
...
If your backend returns base64, use `save_b64_video()` to write under `$FULILIAN_HOME/cache/videos/`. For raw bytes from a follow-up HTTP fetch, use `save_bytes_video()`. Otherwise return the upstream URL directly — the gateway resolves remote URLs on delivery.
