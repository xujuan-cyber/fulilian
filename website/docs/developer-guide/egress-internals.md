...
description: "How the iron-proxy egress firewall integrates with Fulilian — module layout, lifecycle, security invariants, and extension points"
---

# Egress proxy internals

This page covers the architecture of the egress credential-injection firewall (`fulilian egress` / iron-proxy) from a contributor / plugin author's perspective. End-user setup + usage docs live at [Egress proxy](../user-guide/egress/iron-proxy.md).
...
fulilian_cli/proxy_cli.py               Wizard + slash command handlers.
                                       `fulilian egress {install,setup,start,stop,
                                       status,disable,config}`.  Wires the
                                       core module into argparse.

fulilian_cli/main.py:_dispatch_egress   Top-level subparser dispatcher.
                                       dest='egress_command' (intentionally
                                       disjoint from the inbound OAuth
                                       `fulilian proxy` subparser, which uses
                                       dest='proxy_command').

fulilian_cli/config.py: proxy schema    The `proxy:` block in DEFAULT_CONFIG.
...
tools/environments/docker.py
  ...
  DockerEnvironment.__init__          Docker-side merge logic: collision
                                       ...
                                       _FULILIAN_EGRESS_NODE_OPTIONS_APPEND
...
tests/test_iron_proxy_e2e.py          Live E2E (gated on FULILIAN_RUN_E2E=1).
...
fulilian egress install
  -> agent.proxy_sources.iron_proxy.install_iron_proxy(force=...)
       ...
       Stage into ~/.fulilian/bin/.iron-proxy_XXXX, chmod 755, os.replace
         to ~/.fulilian/bin/iron-proxy (atomic).
       _VERSION_CACHE.pop(target) so a forced reinstall re-probes
         --version on next call.

fulilian egress setup [--from-bitwarden | --no-bitwarden] [--rotate-tokens]
  -> proxy_cli.cmd_setup
...
| Path | Mode | Test |
|---|---|---|
| `~/.fulilian/proxy/` (dir) | `0o700` | `test_proxy_state_dir_is_0o700` |
...
`merge_mappings(existing, discovered, rotate=False)` MUST return prior tokens for providers that overlap.  Re-running `fulilian egress setup` cannot silently 401 running sandboxes.  `--rotate-tokens` is the explicit opt-in.
...
`cmd_setup` MUST NOT downgrade `credential_source: bitwarden` to `env` on re-run without an explicit `--no-bitwarden` flag.  Running `fulilian egress setup` (no flag) preserves whatever was previously configured.
...
If the provider uses SigV4 / SDK-minted OAuth / request signatures, a static header swap cannot cover it.  Add the env var to `_NON_BEARER_PROVIDERS` so the wizard and `fulilian egress status` warn about it:
...
4. Sets the seven env vars (HTTPS_PROXY, NO_PROXY, REQUESTS_CA_BUNDLE, SSL_CERT_FILE, CURL_CA_BUNDLE, NODE_EXTRA_CA_CERTS, FULILIAN_EGRESS_PROXY) and the per-mapping `FULILIAN_PROXY_TOKEN_<NAME>` vars.
5. Distributes the CA cert into the sandbox at a path the runtime will trust (typically `/etc/ssl/certs/fulilian-egress-ca.crt`).
...
FULILIAN_RUN_E2E=1 scripts/run_tests.sh tests/test_iron_proxy_e2e.py

# Live PTY smoke against `fulilian egress`
FULILIAN_HOME=/tmp/fulilian-egress-test python3 -m fulilian_cli.main egress --help
FULILIAN_HOME=/tmp/fulilian-egress-test python3 -m fulilian_cli.main egress setup --help
```
...
- User-facing setup + troubleshooting: [Egress proxy](https://github.com/xujuan-cyber/fulilian)
- Docker backend internals: [Docker](https://github.com/xujuan-cyber/fulilian)
- Bitwarden Secrets Manager integration: [`fulilian secrets bitwarden`](https://github.com/xujuan-cyber/fulilian)
- CLI command reference: [`fulilian egress`](https://github.com/xujuan-cyber/fulilian)
- Sandbox-injected environment variables: [Egress proxy (sandbox-injected)](https://github.com/xujuan-cyber/fulilian)
