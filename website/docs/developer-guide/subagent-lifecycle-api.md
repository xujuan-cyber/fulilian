...
Plugins can launch and supervise fresh Fulilian child sessions without importing
...
active agent turn fails closed with `No active Fulilian parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Fulilian can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Fulilian's existing unsafe-tool block remains enforced.
