...
#
# 控制侧（ctl）：FULILIAN_CTF_HARD_SOLVE_PROTOCOL **不设**
# 实验侧（exp）：FULILIAN_CTF_HARD_SOLVE_PROTOCOL=1
...
echo "  代码=当前工作树（同一份）; 两侧仅差 FULILIAN_CTF_HARD_SOLVE_PROTOCOL"
...
run_side() {
  ...
  for i in $(seq 1 "$N"); do
    ...
    if [ "$side" = exp ]; then
      export FULILIAN_CTF_HARD_SOLVE_PROTOCOL=1
    else
      unset FULILIAN_CTF_HARD_SOLVE_PROTOCOL
    fi
