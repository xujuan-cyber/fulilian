...
- **冷却从未正确生效**。它依赖"平台判错"能正确记账，而判定的极性是错的
  （历史成因：当时读 CTFd 信封顶层 ``success``，答错时它同样为 True）→ ``wrong`` 计数几乎
...
平台提交路径已整体移除（2026-09-17）：本闸门现在只服务**本地声明式提交**
（写 ``FLAG`` 文件 + 三重校验门）。既然不再打网络请求，就没有"未受理 vs
答错"的区分需求，限流/突发提交的防滥用退避也一并不需要了。

**保留的唯一职责**：校验门已确认通过的 flag 不再重复提交——避免把一个已知
成功的结果再走一遍校验门（纯噪音）。
...
class SubmitGuard:
    """按 key（``work_dir``）隔离的"已确认 flag"闸门。

    本地声明式提交以 ``work_dir`` 为 key，同一个单例在多题之间不会互相干扰。
    """
    ...
    def mark_confirmed(self, key: str, flag: str) -> None:
        ...
        只应由显式成功判定（本地校验门 ``CONFIRMED``）驱动；调用方不得把
        "门无法判定""读取异常"之类的中间态传进来。
        """
        flag = normalize_flag(flag)
        if not flag:
            return
