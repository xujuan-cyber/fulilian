...
> **توجہ فرمائیں:** مقامی ونڈوز (Native Windows) پر فولیلیان بغیر WSL کے چلتا ہے — CLI، گیٹ وے، TUI، اور ٹولز سب مقامی طور پر کام کرتے ہیں۔ اگر آپ WSL2 استعمال کرنا پسند کرتے ہیں، تو اوپر دی گئی لینکس/میک او ایس کی کمانڈ وہاں بھی کام کرے گی۔ کوئی مسئلہ نظر آیا؟ براہ کرم [مسائل (issues) درج کریں](https://github.com/xujuan-cyber/fulilian/issues)۔
...
انسٹالر سب کچھ خود سنبھالتا ہے: uv، Python 3.11، Node.js، ripgrep، ffmpeg، **اور ایک پورٹ ایبل (portable) گٹ بیش (Git Bash)** (یعنی MinGit، جو `%LOCALAPPDATA%\fulilian\git` میں ان پیک ہوتا ہے — اس کے لیے ایڈمن کی اجازت درکار نہیں، اور یہ سسٹم کے کسی بھی گٹ انسٹال سے بالکل الگ ہے)۔ فولیلیان اس بنڈل شدہ گٹ بیش کو شیل کمانڈز چلانے کے لیے استعمال کرتا ہے۔
...
> **اینڈرائیڈ (Android) / ٹرمکس (Termux):** ٹیسٹ کیا گیا مینوئل طریقہ [Termux گائیڈ](https://github.com/xujuan-cyber/fulilian) میں موجود ہے۔ ٹرمکس پر فولیلیان ایک مخصوص `.[termux]` ایکسٹرا انسٹال کرتا ہے کیونکہ مکمل `.[all]` ایکسٹرا میں ایسی وائس ڈیپینڈینسیز شامل ہیں جو اینڈرائیڈ کے ساتھ مطابقت نہیں رکھتیں۔
>
> **ونڈوز (Windows):** مقامی ونڈوز کی مکمل سپورٹ موجود ہے — اوپر دی گئی پاور شیل کی کمانڈ سب کچھ انسٹال کر دیتی ہے۔ اگر آپ WSL2 استعمال کرنا چاہتے ہیں، تو لینکس کی کمانڈ وہاں کام کرتی ہے۔ مقامی ونڈوز میں انسٹالیشن `%LOCALAPPDATA%\fulilian` میں ہوتی ہے؛ جبکہ WSL2 میں لینکس کی طرح `~/.fulilian` میں ہوتی ہے۔ فولیلیان کا وہ واحد فیچر جسے فی الحال خاص طور پر WSL2 کی ضرورت ہے وہ براؤزر پر مبنی ڈیش بورڈ چیٹ پین ہے (یہ POSIX PTY استعمال کرتا ہے — کلاسک CLI اور گیٹ وے دونوں مقامی طور پر چلتے ہیں)۔
...
```bash
fulilian              # انٹرایکٹو CLI — بات چیت شروع کریں
fulilian model        # اپنا LLM پرووائیڈر اور ماڈل منتخب کریں
fulilian tools        # کنفیگر کریں کہ کون سے ٹولز ایکٹو ہیں
fulilian config set   # انفرادی کنفگ (config) ویلیوز سیٹ کریں
fulilian gateway      # میسجنگ گیٹ وے شروع کریں (ٹیلی گرام، ڈسکارڈ، وغیرہ)
fulilian setup        # مکمل سیٹ اپ وزرڈ چلائیں (یہ سب کچھ ایک ساتھ کنفیگر کر دے گا)
fulilian claw migrate # OpenClaw سے مائیگریٹ کریں (اگر آپ OpenClaw سے آ رہے ہیں)
fulilian update       # لیٹسٹ ورژن پر اپ ڈیٹ کریں
fulilian doctor       # کسی بھی مسئلے کی تشخیص کریں
```

</div>

📖 **[مکمل دستاویزات →](https://github.com/xujuan-cyber/fulilian)**

---

## API-کیز اکٹھی کرنے سے بچیں — Nous Portal

فولیلیان آپ کے پسندیدہ پرووائیڈر کے ساتھ کام کرتا ہے — یہ چیز تبدیل نہیں ہو رہی۔ لیکن اگر آپ ماڈل، ویب سرچ، امیج جنریشن، TTS، اور کلاؤڈ براؤزر کے لیے پانچ الگ الگ API کیز جمع نہیں کرنا چاہتے، تو **[Nous Portal](https://portal.nousresearch.com)** ان سب کو ایک ہی سبسکرپشن کے تحت کور کرتا ہے:
...
یہ آپ کو OAuth کے ذریعے لاگ ان کرواتا ہے، Nous کو آپ کا پرووائیڈر مقرر کرتا ہے، اور ٹول گیٹ وے کو آن کر دیتا ہے۔ `fulilian portal info` کمانڈ استعمال کر کے آپ کسی بھی وقت چیک کر سکتے ہیں کہ کون کون سی سروسز منسلک ہیں۔ مکمل تفصیلات [Tool Gateway دستاویزات کے صفحے](https://github.com/xujuan-cyber/fulilian) پر موجود ہیں۔
...
فولیلیان کے دو بنیادی انٹر فیس ہیں: آپ ٹرمینل UI کو `fulilian` کے ساتھ شروع کریں، یا گیٹ وے چلا کر اس کے ساتھ ٹیلی گرام، ڈسکارڈ، سلیک، واٹس ایپ، سگنل، یا ای میل کے ذریعے بات کریں۔ جب آپ کسی بات چیت میں ہوتے ہیں، تو بہت سی سلیش (slash) کمانڈز دونوں انٹرفیسز میں ایک جیسی ہوتی ہیں۔

<div dir="ltr">

| کارروائی (Action)                         | سی ایل آئی (CLI)                              | میسجنگ پلیٹ فارمز (Messaging platforms)                                          |
| --------------------------------------- | --------------------------------------------- | -------------------------------------------------------------------------------- |
| بات چیت شروع کریں                       | `fulilian`                                      | `fulilian gateway setup` اور `fulilian gateway start` چلائیں، پھر بوٹ کو میسج بھیجیں |
...
مکمل کمانڈ لسٹ کے لیے، [CLI گائیڈ](https://github.com/xujuan-cyber/fulilian) اور [میسجنگ گیٹ وے گائیڈ](https://github.com/xujuan-cyber/fulilian) دیکھیں۔

---

## دستاویزات (Documentation)

تمام دستاویزات **[github.com/xujuan-cyber/fulilian](https://github.com/xujuan-cyber/fulilian)** پر موجود ہیں:

<div dir="ltr">

| سیکشن (Section)                                                                                     | تفصیل (What's Covered)                                     |
| --------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- |
| [فوری آغاز (Quickstart)](https://github.com/xujuan-cyber/fulilian)     | انسٹالیشن → سیٹ اپ → 2 منٹ میں پہلی بات چیت شروع کریں       |
| [CLI کا استعمال](https://github.com/xujuan-cyber/fulilian)                         | کمانڈز، کی بائنڈنگز (keybindings)، پرسنلٹیز (personalities)، سیشنز |
| [کنفیگریشن (Configuration)](https://github.com/xujuan-cyber/fulilian)    | کنفگ فائل، پرووائیڈرز، ماڈلز، اور تمام آپشنز               |
| [میسجنگ گیٹ وے](https://github.com/xujuan-cyber/fulilian)                    | ٹیلی گرام، ڈسکارڈ، سلیک، واٹس ایپ، سگنل، ہوم اسسٹنٹ         |
| [سیکیورٹی (Security)](https://github.com/xujuan-cyber/fulilian)              | کمانڈ کی منظوری، DM پیئرنگ (pairing)، کنٹینر آئسولیشن       |
| [ٹولز اور ٹول سیٹس](https://github.com/xujuan-cyber/fulilian)          | 40 سے زائد ٹولز، ٹول سیٹ سسٹم، ٹرمینل بیک اینڈز             |
| [مہارتوں کا سسٹم (Skills System)](https://github.com/xujuan-cyber/fulilian)| پروسیجرل (Procedural) میموری، سکلز ہب، نئی مہارتیں بنانا    |
| [میموری (Memory)](https://github.com/xujuan-cyber/fulilian)            | مستقل میموری، یوزر پروفائلز، بہترین طریقہ کار              |
| [MCP انضمام (Integration)](https://github.com/xujuan-cyber/fulilian)      | صلاحیتوں کو بڑھانے کے لیے کسی بھی MCP سرور کو جوڑیں        |
| [کرون (Cron) شیڈیولنگ](https://github.com/xujuan-cyber/fulilian)         | پلیٹ فارم ڈیلیوری کے ساتھ شیڈول کیے گئے کام                 |
| [کانٹیکسٹ (Context) فائلز](https://github.com/xujuan-cyber/fulilian)| پروجیکٹ کا سیاق و سباق (context) جو ہر بات چیت پر اثر انداز ہوتا ہے |
| [آرکیٹیکچر (Architecture)](https://github.com/xujuan-cyber/fulilian) | پروجیکٹ کا ڈھانچہ، ایجنٹ لوپ، اہم کلاسز                    |
| [تعاون (Contributing)](https://github.com/xujuan-cyber/fulilian)     | ڈیویلپمنٹ سیٹ اپ، PR کا طریقہ کار، کوڈنگ کا انداز          |
| [CLI حوالہ جات (Reference)](https://github.com/xujuan-cyber/fulilian)      | تمام کمانڈز اور فلیگز (flags)                              |
| [انوائرمنٹ ویری ایبلز](https://github.com/xujuan-cyber/fulilian)  | مکمل انوائرمنٹ ویری ایبل حوالہ جات                         |
...
اگر آپ OpenClaw سے منتقل ہو رہے ہیں، تو فولیلیان آپ کی سیٹنگز، یادیں (memories)، مہارتیں (skills)، اور API کیز کو خود بخود امپورٹ کر سکتا ہے۔

**پہلی بار سیٹ اپ کے دوران:** سیٹ اپ وزرڈ (`fulilian setup`) خود بخود `~/.openclaw` کو پہچان لیتا ہے اور کنفیگریشن شروع ہونے سے پہلے مائیگریٹ (migrate) کرنے کا آپشن دیتا ہے۔
...
```bash
fulilian claw migrate              # انٹرایکٹو مائیگریشن (مکمل پری سیٹ)
fulilian claw migrate --dry-run    # جائزہ لیں کہ کیا کیا مائیگریٹ ہوگا
fulilian claw migrate --preset user-data   # حساس معلومات (secrets) کے بغیر مائیگریٹ کریں
fulilian claw migrate --overwrite  # موجودہ متصادم فائلوں کو اوور رائٹ کریں
```
...
- **مہارتیں (Skills)** — صارف کی بنائی گئی مہارتیں → `~/.fulilian/skills/openclaw-imports/`
...
تمام آپشنز دیکھنے کے لیے `fulilian claw migrate --help` استعمال کریں، یا انٹرایکٹو ایجنٹ کی مدد سے مائیگریٹ کرنے کے لیے `openclaw-migration` سکل کا استعمال کریں (جس میں ڈرائی رن (dry-run) پریویوز شامل ہیں)۔

---

## تعاون کریں (Contributing)

ہم آپ کے تعاون کا خیرمقدم کرتے ہیں! ڈیویلپمنٹ سیٹ اپ، کوڈ کے انداز اور PR کے طریقہ کار کے لیے براہ کرم ہماری [Contributing گائیڈ](https://github.com/xujuan-cyber/fulilian) دیکھیں۔

معاونین (contributors) کے لیے فوری آغاز — کلون (clone) کریں اور `setup-fulilian.sh` چلائیں:
...
cd fulilian-agent
./setup-fulilian.sh     # uv کو انسٹال کرتا ہے، venv بناتا ہے، .[all] کو انسٹال کرتا ہے، اور ~/.local/bin/fulilian کا سیم لنک (symlink) بناتا ہے
./fulilian              # خود بخود venv کی شناخت کرتا ہے، پہلے `source` کرنے کی ضرورت نہیں
```
...
- 🐛 [مسائل (Issues)](https://github.com/xujuan-cyber/fulilian/issues)
- 🔌 [computer-use-linux](https://github.com/avifenesh/computer-use-linux) — فولیلیان اور دیگر MCP ہوسٹس کے لیے لینکس (Linux) ڈیسک ٹاپ کنٹرول MCP سرور، جس میں AT-SPI ایکسیسیبلٹی ٹریز، Wayland/X11 ان پٹ، سکرین شاٹس، اور کمپوزیٹر ونڈو ٹارگیٹنگ شامل ہے۔
- 🔌 [FulilianClaw](https://github.com/AaronWong1999/fulilianclaw) — کمیونٹی وی چیٹ (WeChat) برج: فولیلیان ایجنٹ اور OpenClaw کو ایک ہی وی چیٹ اکاؤنٹ پر چلائیں۔
