...
React + Ink terminal UI for Fulilian. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `FULILIAN_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `fulilian setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.fulilian/.fulilian_history` or under `FULILIAN_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `fulilian setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/fulilian-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external fulilian setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.fulilian/tui-parent.log
    ...
    types/
      fulilian-ink.d.ts            type declarations for @fulilian/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent FulilianCLI subprocess for slash commands
```
