...
import { Box, Text } from '@fulilian/ink'
...
/**
 * User widget apps — Fulilian authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$FULILIAN_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.fulilian/plugins/`: files under FULILIAN_HOME execute
...
const widgetsDir = () => join(process.env.FULILIAN_HOME?.trim() || join(homedir(), '.fulilian'), 'tui-widgets')
 ...
 *  every change, so a widget Fulilian writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Fulilian) ever writes must hot-load too — a 10s poll here read
