import type { RunExternalProcess } from '@fulilian/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`fulilian ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching fulilian: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`fulilian ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
