...
import { terminalBackgroundHex } from '@fulilian/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $FULILIAN_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see FULILIAN_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['FULILIAN_TUI_BACKGROUND', process.env.FULILIAN_TUI_BACKGROUND ?? '(unset)'],
            ['FULILIAN_TUI_THEME', process.env.FULILIAN_TUI_THEME ?? '(unset)'],
