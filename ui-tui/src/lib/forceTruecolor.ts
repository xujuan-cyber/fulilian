...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.FULILIAN_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Fulilian on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via FULILIAN_TUI_TRUECOLOR=1.
