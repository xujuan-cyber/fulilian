...
import { isXtermJs } from '@fulilian/ink'
...
/** FULILIAN_TUI_SCROLL_SPEED (or CLAUDE_CODE_SCROLL_SPEED for portability).
 *  Default 1, clamped (0, 20]. */
export function readScrollSpeedBase(): number {
  const n = parseFloat(process.env.FULILIAN_TUI_SCROLL_SPEED ?? process.env.CLAUDE_CODE_SCROLL_SPEED ?? '')
