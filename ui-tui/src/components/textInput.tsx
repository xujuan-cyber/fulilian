import type { InputEvent, Key } from '@fulilian/ink'
import * as Ink from '@fulilian/ink'
 ...
 * but that accepts `key.meta` on macOS — and fulilian-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.FULILIAN_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which fulilian-ink decodes as meta+'d'; without this branch it
