...
import { Box, renderSync } from '@fulilian/ink'
...
describe('INLINE_RE emphasis', () => {
  ...
  it('keeps intraword underscores literal', () => {
    const path = '/home/me/.fulilian/cache/screenshots/browser_screenshot_ecc1c3feab.png'
...
describe('protocol sentinels', () => {
  it('captures MEDIA: paths with surrounding quotes or backticks', () => {
    ...
    expect('  MEDIA: /home/me/.fulilian/cache/screenshots/browser_screenshot_ecc.png  '.match(MEDIA_LINE_RE)?.[1]).toBe(
      '/home/me/.fulilian/cache/screenshots/browser_screenshot_ecc.png'
    )
    expect('`MEDIA:/tmp/a.png`'.match(MEDIA_LINE_RE)?.[1]).toBe('/tmp/a.png')
...
describe('renderTable CJK width alignment', () => {
  it('column starts share the same display offset across CJK rows', async () => {
    const { stringWidth } = await import('@fulilian/ink')
