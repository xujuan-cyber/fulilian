...
// FULILIAN_TUI_THEME=light (or FULILIAN_TUI_BACKGROUND set to something
...
describe('DEFAULT_THEME', () => {
  it('has brand defaults', async () => {
    ...
    expect(DEFAULT_THEME.brand.name).toBe('FuLiLian')
  ...
  it('has color palette', async () => {
    ...
    expect(DEFAULT_THEME.color.primary).toBe('#5DB8F5')
...
describe('detectLightMode', () => {
  ...
  it('honors FULILIAN_TUI_LIGHT on/off', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ FULILIAN_TUI_LIGHT: '1' })).toBe(true)
    expect(detectLightMode({ FULILIAN_TUI_LIGHT: 'true' })).toBe(true)
    expect(detectLightMode({ FULILIAN_TUI_LIGHT: 'on' })).toBe(true)
    expect(detectLightMode({ FULILIAN_TUI_LIGHT: '0' })).toBe(false)
    expect(detectLightMode({ FULILIAN_TUI_LIGHT: 'off' })).toBe(false)
  })
  ...
  it('lets FULILIAN_TUI_LIGHT=0 override a light COLORFGBG', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ COLORFGBG: '0;15', FULILIAN_TUI_LIGHT: '0' })).toBe(false)
  })

  it('honors FULILIAN_TUI_THEME=light/dark as a symmetric explicit override', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ FULILIAN_TUI_THEME: 'light' })).toBe(true)
    expect(detectLightMode({ FULILIAN_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '0;15', FULILIAN_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '15;0', FULILIAN_TUI_THEME: 'light' })).toBe(true)
  })

  it('uses FULILIAN_TUI_BACKGROUND luminance when COLORFGBG is missing', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ FULILIAN_TUI_BACKGROUND: '#ffffff' })).toBe(true)
    expect(detectLightMode({ FULILIAN_TUI_BACKGROUND: '#000000' })).toBe(false)
    expect(detectLightMode({ FULILIAN_TUI_BACKGROUND: '#1e1e1e' })).toBe(false)
    // Three-char hex normalises like CSS.
    expect(detectLightMode({ FULILIAN_TUI_BACKGROUND: '#fff' })).toBe(true)
    // Garbage falls through to the default-dark path.
    expect(detectLightMode({ FULILIAN_TUI_BACKGROUND: 'not-a-colour' })).toBe(false)
  })
  ...
  it('rejects partially-invalid hex instead of silently truncating', async () => {
    ...
    expect(detectLightMode({ FULILIAN_TUI_BACKGROUND: '#fffgff' })).toBe(false)
    expect(detectLightMode({ FULILIAN_TUI_BACKGROUND: 'ffggff' })).toBe(false)
    expect(detectLightMode({ FULILIAN_TUI_BACKGROUND: '#xyz' })).toBe(false)
    // Wrong length also rejected (no implicit padding/truncation).
    expect(detectLightMode({ FULILIAN_TUI_BACKGROUND: '#fffff' })).toBe(false)
    expect(detectLightMode({ FULILIAN_TUI_BACKGROUND: '#fffffff' })).toBe(false)
  })
...
describe('fromSkin', () => {
  ...
  // FULILIAN_TUI_THEME=light would flip the base palette and make these
  ...
  it('derives completion current background from resolved completion background (polarity-compatible)', async () => {
    ...
    const { fromSkin } = await importThemeWithEnv({ FULILIAN_TUI_BACKGROUND: '#ffffff' })
  ...
  it('skinIsLight: the authored background decides; host detection only when absent', async () => {
    const { skinIsLight } = await importThemeWithEnv({ FULILIAN_TUI_BACKGROUND: '#ffffff' })
...
describe('derived tone ladder', () => {
  it('reproduces the authored azure tones from seeds (reverse-engineered knobs)', async () => {
    // The ladder's knobs were grid-search fitted against the pre-refactor
    // hand-tuned gold literals; the azure rebrand re-derived the targets
    // with the SAME knobs. Contract: every derived tone stays within
    // a-few-RGB-units of the authored azure literals (imperceptible), so
    // knob edits that drift the current look fail here instead of
    // shipping as vibes.
    const dark = await importThemeWithCleanEnv()
    const light = await importThemeWithEnv({ FULILIAN_TUI_BACKGROUND: '#ffffff' })

    const cases: Array<[string, string, string]> = [
      [dark.DARK_THEME.color.muted, '#377EB9', 'dark muted'],
      [dark.DARK_THEME.color.label, '#3A86C5', 'dark label'],
      [dark.DARK_THEME.color.statusFg, '#BABABA', 'dark statusFg'],
      ...
      // Light canon = liftForContrast(dark literal, white, 4.5) for the
      // brand seeds; fills derive from the light background.
      [light.LIGHT_THEME.color.muted, '#276EAA', 'light muted'],
      [light.LIGHT_THEME.color.statusFg, '#6D6D6D', 'light statusFg'],
      [light.LIGHT_THEME.color.completionBg, '#F6F8FB', 'light surface'],
      [light.LIGHT_THEME.color.completionCurrentBg, '#C2D6E8', 'light chip'],
      [light.LIGHT_THEME.color.selectionBg, '#D7E5F0', 'light selection']
    ]
...
describe('background-aware adaptation (OSC-11 light terminals)', () => {
  it('renders a dark-authored skin on light like minimumContrastRatio hosts do (the standardized look)', async () => {
    const { contrastRatio, fromSkin } = await importThemeWithEnv({ FULILIAN_TUI_BACKGROUND: '#ffffff' })
  ...
  it('leaves the same skin untouched on a dark background', async () => {
    const { fromSkin } = await importThemeWithEnv({ FULILIAN_TUI_BACKGROUND: '#1e1e2e' })
    const { color } = fromSkin(SLATE_COLORS, {})
  ...
  it('empty skin on a light background resolves to the light base palette', async () => {
    const { fromSkin, LIGHT_THEME } = await importThemeWithEnv({ FULILIAN_TUI_BACKGROUND: '#ffffff' })
  ...
  it('defaultThemeForCurrentBackground follows a late FULILIAN_TUI_BACKGROUND write', async () => {
    ...
    expect(defaultThemeForCurrentBackground({ FULILIAN_TUI_BACKGROUND: '#ffffff' }).color).toEqual(LIGHT_THEME.color)
  })
