...
Let the fulilian agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where fulilian runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `fulilian meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `fulilian meet` CLI |
...
| `cli.py` | `fulilian meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$FULILIAN_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `fulilian meet node {run,list,approve,remove,status,ping}` |
...
fulilian meet install                                      # pip + Chromium
fulilian meet setup                                        # preflight
fulilian meet auth                                         # optional
fulilian meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
fulilian meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.fulilian/.env
fulilian meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
fulilian meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
fulilian meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.fulilian/.env
fulilian meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, fulilian will **not** switch your system audio input automatically — the
...
fulilian plugins enable google_meet
fulilian meet node run --display-name my-mac --host 0.0.0.0 --port 18789
...
fulilian meet node ping my-mac
