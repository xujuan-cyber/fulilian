"""`fulilian meet node ...` subcommand tree.

Wired into the existing ``fulilian meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``fulilian meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="fulilian-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``fulilian meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             fulilian meet node approve <name> ws://<host>:{args.port} {token}")
        try:
