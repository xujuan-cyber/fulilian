...
Wires ``fulilian meet <subcommand>``:
...
def _auth_state_path() -> Path:
    return Path(get_fulilian_home()) / "workspace" / "meetings" / "auth.json"
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    """Build the ``fulilian meet`` argparse tree.
    ...
    join_p.add_argument("--guest-name", default="FuLiLian")
    ...
    try:
        ...
        def _node_unavailable(args):
            print(f"fulilian meet node: module unavailable ({e})")
            return 1
...
def meet_command(args: argparse.Namespace) -> int:
    ...
    if not sub:
        print("usage: fulilian meet {setup,auth,join,status,transcript,say,stop,node}")
        return 2
    if sub == "setup":
        return _cmd_setup()
    if sub == "install":
    ...
    if sub == "node":
        ...
        if fn is None or fn is meet_command:
            print("usage: fulilian meet node {run,list,approve,remove,status,ping}")
            return 2
        return fn(args)
...
def _cmd_setup() -> int:
    ...
    print(
        "  google auth    : "
        + (f"ok ({auth_path})" if auth_ok else "not saved — run: fulilian meet auth")
    )
    ...
    if all_ok:
        print(
            ...
            "fulilian meet join https://meet.google.com/abc-defg-hij"
        )
    else:
    ...
    if realtime:
        ...
        if system == "Linux":
            ...
            print(
                ...
                "  fulilian will not switch your default input for you."
            )
    else:
        print("\n[3/3] skipped (pass --realtime to install audio tooling too)")

    print("\ndone. verify with: fulilian meet setup")
    return 0
...
def _cmd_auth() -> int:
    ...
    try:
        ...
        return 1
    print("saved. you can now run: fulilian meet join <url>")
    return 0
...
def _cmd_stop() -> int:
    res = pm.stop(reason="fulilian meet stop")
    print(json.dumps(res, indent=2))
    return 0 if res.get("ok") else 1


if __name__ == "__main__":  # pragma: no cover
    parser = argparse.ArgumentParser(prog="fulilian meet")
    register_cli(parser)
