"""ntfy platform adapter (Fulilian plugin).
...
a Fulilian dependency.

This adapter ships as a Fulilian platform plugin under
``plugins/platforms/ntfy/``. The Fulilian plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "fulilian-in"              # subscribe topic (incoming)
          publish_topic: "fulilian-out"     # optional — defaults to topic
...
_ECHO_TAG = "fulilian-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``fulilian cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Fulilian dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `fulilian gateway status` without
