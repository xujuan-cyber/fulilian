...
Small Node helper that bridges FuLiLian to Photon's Spectrum SDK
(`spectrum-ts`).  Fulilian is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Fulilian-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Fulilian can receive inbound messages
...
The Fulilian plugin's `fulilian photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Fulilian is Python, so replies go through this sidecar
