// FuLiLian — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Fulilian): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Fulilian -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Fulilian-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `fulilian photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Fulilian can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "fulilian-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Fulilian'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Fulilian allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Fulilian, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-fulilian-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Fulilian supplied them so a known-good
