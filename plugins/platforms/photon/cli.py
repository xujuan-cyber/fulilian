"""
``fulilian photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Fulilian
...
# Writable sidecar runtime dir (mirrors to FULILIAN_HOME on immutable
...
# wiring imports this module for `fulilian --help`).
...
# when called later (gateway start, fulilian status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `fulilian photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'FuLiLian')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``fulilian photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "FuLiLian" project.
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.fulilian/.env,
    ...
    try:
        from fulilian_cli.config import write_platform_config_field
    ...
    print("  Start the gateway:  fulilian gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `fulilian photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`fulilian photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.fulilian/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `fulilian photon telemetry on` / `fulilian photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.fulilian/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  fulilian gateway restart")
    return 0
...
#
# `fulilian gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``fulilian photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `fulilian gateway setup` wizard."""
