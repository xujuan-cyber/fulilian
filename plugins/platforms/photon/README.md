...
This plugin connects FuLiLian to iMessage (and other Spectrum
...
TypeScript-only, Fulilian runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other Fulilian channel,
...
Run `fulilian photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.fulilian/.env` (the same place every other
...
Management metadata lives in `~/.fulilian/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `fulilian photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Fulilian
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Fulilian coalesces
...
- **Native polls are supported.** Fulilian posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `fulilian send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<fulilian-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `fulilian photon status`, a DM and a group roundtrip,
