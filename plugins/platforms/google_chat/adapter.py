...
# ``model_tools`` import time, so ``fulilian status``, ``fulilian chat``, etc.
...
# ``fulilian_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Fulilian is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a fulilian MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "fulilian-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${FULILIAN_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _fulilian_home = _get_fulilian_home()
        except (ModuleNotFoundError, ImportError):
            _fulilian_home = _Path.home() / ".fulilian"
        self._thread_count_store = _ThreadCountStore(
            _fulilian_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Fulilian is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("FULILIAN_HOME", str(_Path.home() / ".fulilian"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${FULILIAN_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a fulilian MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Fulilian is thinking…' marker message.
        ...
        "Fulilian is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Fulilian is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Fulilian is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Fulilian is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``fulilian setup``.

    The setup wizard at ``fulilian_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.fulilian/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. fulilian-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.fulilian/.env")
    print_info("Restart the gateway: fulilian gateway restart")
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `fulilian setup` to install Google Chat support.",
        ...
        # the "Fulilian is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Fulilian is thinking…' marker message "
