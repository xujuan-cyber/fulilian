# A2A — Agent-to-Agent protocol for Fulilian
...
peer (another Fulilian, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
fulilian gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Fulilian serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.fulilian/a2a_audit.jsonl`.
- Conversations persist to `~/.fulilian/a2a_conversations/` — they survive context
