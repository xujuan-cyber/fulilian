...
and injects content-free wake hints into Fulilian' normal gateway session pipeline.
...
class RaftAdapter(BasePlatformAdapter):
    ...
    async def _handle_wake(self, request: "web.Request") -> "web.Response":
        ...
        # Fulilian only verifies that wake hints are content-free.
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        """Accept Raft wake hints without interrupting an active Fulilian turn."""
        if not self._message_handler:
            return
...
def interactive_setup() -> None:
    """Interactive ``fulilian gateway setup`` flow for the Raft platform.

    Lazy-imports CLI helpers so the plugin stays importable in gateway runtime
    and test contexts. The flow persists ``RAFT_PROFILE`` to the Fulilian env
    ...
    print_info("Connect Fulilian to Raft as an external agent.")
