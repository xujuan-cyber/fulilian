"""
Email platform adapter for the Fulilian gateway.

Allows users to interact with Fulilian by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "fulilian-agent" "version" "{_fulilian_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `fulilian config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `fulilian gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "FuLiLian")
        ...
        msg_id = f"<fulilian-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# fulilian_cli/gateway.py, and the _send_email dispatch in
