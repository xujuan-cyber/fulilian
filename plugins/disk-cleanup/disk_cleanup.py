"""disk_cleanup — ephemeral file cleanup for FuLiLian.
...
Rules:
  ...
  - empty dirs    → always delete (under FULILIAN_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly FULILIAN_HOME and /tmp/fulilian-*
Never touches: ~/.fulilian/logs/ or any system directory.
"""
...
try:
    ...
    def get_fulilian_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("FULILIAN_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".fulilian").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$FULILIAN_HOME/logs/``."""
    return get_fulilian_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$FULILIAN_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under FULILIAN_HOME or ``/tmp/fulilian-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    fulilian_home = get_fulilian_home()
    try:
        path.resolve().relative_to(fulilian_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/fulilian-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("fulilian-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "fulilian-agent", "backups", "profiles", ".worktrees",
...
# Paths under $FULILIAN_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so FULILIAN_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        fulilian_home = get_fulilian_home()
        for parent in ("cron", "cronjobs"):
            base = fulilian_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside FULILIAN_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under FULILIAN_HOME, but never recurse into known
    # durable state trees.  Some installs place the Fulilian checkout, venv,
    # and desktop build under FULILIAN_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    fulilian_home = get_fulilian_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in fulilian_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    fulilian_home = get_fulilian_home()
    try:
        rel = path.resolve().relative_to(fulilian_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "fulilian-agent",
        ...
        # Path isn't under FULILIAN_HOME (e.g. /tmp/fulilian-*) — fall through.
        pass
