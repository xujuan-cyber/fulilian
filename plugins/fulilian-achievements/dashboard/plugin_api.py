"""Fulilian Achievements dashboard plugin backend.

Mounted at /api/plugins/fulilian-achievements/ by Fulilian dashboard.
"""
...
try:
    ...
    import os as _os
    def get_fulilian_home() -> Path:  # type: ignore[misc]
        val = (_os.environ.get("FULILIAN_HOME") or "").strip()
...
ACHIEVEMENTS: List[Dict[str, Any]] = [
    ...
    {"id": "let_him_cook", "name": "Let Him Cook", "description": "Let Fulilian run a serious autonomous tool chain in one session.", "category": "Agent Autonomy", "kind": "best_session", "icon": "flame", "threshold_metric": "max_tool_calls_in_session", "tiers": tiers([200, 500, 1200, 3000, 8000])},
    {"id": "autonomous_avalanche", "name": "Autonomous Avalanche", "description": "Accumulate a lifetime avalanche of Fulilian tool calls across sessions.", "category": "Agent Autonomy", "kind": "lifetime", "icon": "avalanche", "threshold_metric": "total_tool_calls", "tiers": tiers([1000, 3000, 8000, 20000, 50000])},
    {"id": "toolchain_maxxer", "name": "Toolchain Maxxer", "description": "Use a wide spread of distinct Fulilian tools in one session.", "category": "Agent Autonomy", "kind": "best_session", "icon": "nodes", "threshold_metric": "max_distinct_tools_in_session", "tiers": tiers([18, 28, 45, 70, 100])},
    ...
    # Fulilian Native
    {"id": "skillsmith", "name": "Skillsmith", "description": "Work with Fulilian skills enough to leave fingerprints.", "category": "Fulilian Native", "kind": "lifetime", "icon": "hammer_scroll", "threshold_metric": "skill_events", "tiers": tiers([5000, 15000, 40000, 100000, 250000])},
    {"id": "skill_issue_skill_created", "name": "Skill Issue? Skill Created.", "description": "Create or patch durable procedures instead of repeating yourself.", "category": "Fulilian Native", "kind": "lifetime", "icon": "anvil", "threshold_metric": "skill_manage_events", "tiers": tiers([25, 75, 200, 600, 1500])},
    {"id": "memory_keeper", "name": "Memory Keeper", "description": "Persist durable knowledge with memory or Mnemosyne.", "category": "Fulilian Native", "kind": "lifetime", "icon": "crystal", "threshold_metric": "memory_events", "tiers": tiers([100, 300, 1000, 3000, 8000])},
    {"id": "memory_palace", "name": "Memory Palace", "description": "Build a serious durable-memory trail.", "category": "Fulilian Native", "kind": "lifetime", "icon": "palace", "threshold_metric": "memory_write_events", "tiers": tiers([100, 300, 1000, 3000, 8000])},
    {"id": "context_dragon", "name": "Context Dragon", "description": "Brush against compression, huge context, or token pressure repeatedly.", "category": "Fulilian Native", "kind": "lifetime", "icon": "dragon", "threshold_metric": "context_events", "tiers": tiers([5000, 15000, 40000, 100000, 250000])},
    {"id": "gateway_dweller", "name": "Gateway Dweller", "description": "Live through gateway-connected Fulilian workflows.", "category": "Fulilian Native", "kind": "lifetime", "icon": "antenna", "threshold_metric": "gateway_events", "tiers": tiers([5000, 15000, 40000, 100000, 250000])},
    {"id": "plugin_goblin", "name": "Plugin Goblin", "description": "Use or develop plugins enough that the dashboard notices.", "category": "Fulilian Native", "kind": "lifetime", "icon": "puzzle", "threshold_metric": "plugin_events", "tiers": tiers([1000, 3000, 8000, 20000, 50000])},
    {"id": "rollback_wizard", "name": "Rollback Wizard", "description": "Invoke rollback/checkpoint recovery magic.", "category": "Fulilian Native", "kind": "lifetime", "icon": "rewind", "secret": True, "threshold_metric": "rollback_events", "tiers": tiers([500, 1500, 4000, 10000, 25000])},
    ...
    {"id": "multi_model_mage", "name": "Multi-Model Mage", "description": "Use a real spread of distinct model names across Fulilian history.", "category": "Model Lore", "kind": "lifetime", "icon": "prism", "threshold_metric": "distinct_model_count", "tiers": tiers([10, 20, 40, 80, 160])},
    ...
    {"id": "provider_polyglot", "name": "Provider Polyglot", "description": "Use models from multiple providers across Fulilian history.", "category": "Model Lore", "kind": "lifetime", "icon": "swap", "threshold_metric": "distinct_provider_count", "tiers": tiers([2, 3, 5, 8, 12])},
    ...
    {"id": "open_weights_pilgrim", "name": "Open Weights Pilgrim", "description": "Actually chat with local/open-weight models through Fulilian session metadata.", "category": "Model Lore", "kind": "lifetime", "icon": "terminal", "threshold_metric": "local_model_chat_sessions", "tiers": tiers([1, 3, 10, 30, 100])},

    # Workflow Intelligence
    {"id": "toolset_cartographer", "name": "Toolset Cartographer", "description": "Navigate Fulilian toolsets deliberately instead of treating tools as a blur.", "category": "Fulilian Native", "kind": "lifetime", "icon": "compass", "threshold_metric": "toolset_events", "tiers": tiers([20, 60, 200, 600, 1500])},
    {"id": "config_surgeon", "name": "Config Surgeon", "description": "Operate on real config files, manifests, env files, and dashboard settings without flinching.", "category": "Fulilian Native", "kind": "lifetime", "icon": "key", "threshold_metric": "config_events", "tiers": tiers([100, 300, 1000, 3000, 10000])},
    ...
    # Lifestyle
    {"id": "marathon_operator", "name": "Marathon Operator", "description": "Accumulate a serious number of Fulilian sessions.", "category": "Lifestyle", "kind": "lifetime", "icon": "marathon", "threshold_metric": "session_count", "tiers": tiers([75, 200, 500, 1500, 5000])},
    {"id": "weekend_warrior", "name": "Weekend Warrior", "description": "Run Fulilian on weekends enough times to make it a lifestyle.", "category": "Lifestyle", "kind": "lifetime", "icon": "calendar", "threshold_metric": "weekend_sessions", "tiers": tiers([25, 75, 200, 600, 1500])},
...
def _data_dir() -> Path:
    """Durable data root (``<fulilian home>/plugin-data/fulilian-achievements/``).

    Was the install tree (``plugins/fulilian-achievements/``) before the
    plugin-data convention existed — state parked there died on
    ``fulilian plugins remove``/``update``. Legacy files migrate on first read.
    """
    try:
        from plugins.plugin_storage import plugin_data_dir

        return plugin_data_dir("fulilian-achievements")
    except Exception:
        # Standalone dashboard import (no plugins package on sys.path):
        # keep the plugin working with the same layout, computed locally.
        root = get_fulilian_home() / "plugin-data" / "fulilian-achievements"
...
def _data_file(name: str) -> Path:
    path = _data_dir() / name
    if not path.exists():
        legacy = get_fulilian_home() / "plugins" / "fulilian-achievements" / name
        if legacy.exists():
            try:
...
def analyze_messages(session_id: str, title: str, messages: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "plugin_events": len(re.findall(r"plugin|dashboard-plugins|__FULILIAN_PLUGIN|manifest\.json", full_text, re.I)),
...
METRIC_LABELS = {
    ...
    "max_distinct_tools_in_session": "distinct Fulilian tools used in one session",
    ...
    "skill_events": "Fulilian skill mentions or tool use",
    ...
    "total_tool_calls": "lifetime Fulilian tool calls",
    ...
    "local_model_chat_sessions": "Fulilian sessions whose model metadata is local/open-weight",
    ...
    "session_count": "Fulilian sessions",
...
def criteria_for(definition: Dict[str, Any]) -> str:
    if definition.get("secret") and definition.get("state") == "secret":
        return "Secret: exact requirement hidden until Fulilian sees the first matching signal. Keep using Fulilian across debugging, tools, memory, skills, plugins, and model workflows to reveal it."
    secret_prefix = ""
    if "threshold_metric" in definition:
        tiers_list = sorted(definition.get("tiers", []), key=lambda t: t["threshold"])
        if not tiers_list:
            return secret_prefix + "Requirement: use Fulilian in the matching workflow."
    ...
    return secret_prefix + "Requirement: complete the matching Fulilian behavior."


def display_achievement(item: Dict[str, Any]) -> Dict[str, Any]:
    clean = dict(item)
    if clean.get("state") == "secret":
        return {**clean, "name": "???", "description": "Secret achievement: hidden until Fulilian detects the first relevant behavior in your session history.", "criteria": criteria_for(clean), "icon": "secret"}
...
def scan_sessions(
    ...
    """Scan Fulilian sessions and build per-session achievement stats.
...
def _start_background_scan() -> None:
    ...
    with _BACKGROUND_SCAN_LOCK:
        ...
        thread = threading.Thread(
            ...
            name="fulilian-achievements-scan",
            daemon=True,
        )
