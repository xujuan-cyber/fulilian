...
- Hosted: API key from [app.supermemory.ai/integrations?connect=fulilian](http://app.supermemory.ai/integrations?connect=fulilian)
...
Before running `fulilian memory setup`, add the local endpoint to
...
Then run `fulilian memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$FULILIAN_HOME/supermemory.json`
...
| `container_tag` | `fulilian` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `fulilian-{identity}` → `fulilian-coder`). |
...
`https://api.supermemory.ai`. Fulilian resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: fulilian`, and document writes stamp
`metadata.sm_source: fulilian`. This is a **functional routing key, not telemetry**:
it groups Fulilian-written memories into a dedicated "Fulilian" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Fulilian profile:

```json
{
  "container_tag": "fulilian-{identity}"
}
```

For a profile named `coder`, this resolves to `fulilian-coder`. The default profile resolves to `fulilian-default`. Without `{identity}`, all profiles share the same container.
