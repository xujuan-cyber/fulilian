"""fulilian-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $FULILIAN_HOME/config.yaml (profile-scoped):
  plugins:
    fulilian-memory-store:
      db_path: $FULILIAN_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "fulilian-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, fulilian_home):
        """Write config to config.yaml under plugins.fulilian-memory-store."""
        from pathlib import Path
        config_path = Path(fulilian_home) / "config.yaml"
        try:
            import yaml
            ...
            existing["plugins"]["fulilian-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_fulilian_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _fulilian_home = str(get_fulilian_home())
        _default_db = _fulilian_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $FULILIAN_HOME in user-supplied paths so config values like
        # "$FULILIAN_HOME/memory_store.db" or "~/.fulilian/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$FULILIAN_HOME", _fulilian_home)
            db_path = db_path.replace("${FULILIAN_HOME}", _fulilian_home)
