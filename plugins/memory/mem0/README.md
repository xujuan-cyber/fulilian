...
```bash
fulilian memory setup    # select "mem0"
```
...
Behavioral settings live in `$FULILIAN_HOME/mem0.json` (set them via `fulilian memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.fulilian/.env`.
...
| `user_id` | `fulilian-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Fulilian session and call `mem0_search` — it connects to your server.
...
```bash
fulilian memory setup mem0 --mode oss \
...
```bash
fulilian memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$FULILIAN_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.fulilian/mem0_qdrant"}}
  }
}
```
...
```bash
fulilian memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$FULILIAN_HOME/mem0.json`).
