...
the ``fulilian`` integration, and applies a chosen manifest to the user's bank
...
from fulilian_cli.urllib_security import open_credentialed_url
...
def fetch_fulilian_templates(url: str | None = None) -> list[dict]:
    """Return catalog entries tagged for the ``fulilian`` integration."""
    catalog = _get_json(url or catalog_url())
    entries = catalog.get("templates", []) if isinstance(catalog, dict) else []
    return [e for e in entries if "fulilian" in (e.get("integrations") or [])]
...
def run_template_step(
    *,
    ...
    """
    try:
        entries = fetch_fulilian_templates()
