...
``fulilian://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Fulilian OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "fulilian-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``fulilian-desktop`` / ``fulilian-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Fulilian.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local FULILIAN_HOME override can't reach.
