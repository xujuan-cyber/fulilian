...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/fulilian>
...
```bash
fulilian memory setup honcho   # configure Honcho directly (works on a fresh install)
fulilian memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
fulilian config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.fulilian/.env
```

> `fulilian honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `fulilian memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `fulilian model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$FULILIAN_HOME/honcho.json` | Profile-local (isolated Fulilian instances) |
| 2 | `~/.fulilian/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Fulilian profile: `fulilian` (default) or `fulilian_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `fulilian honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Fulilian session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`fulilian-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`fulilian`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-fulilian-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening fulilian in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Fulilian session ID (timestamp + hex). Every `fulilian` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Fulilian profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`fulilian`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Fulilian profile: `fulilian` (default) or `fulilian_<profile>` (e.g. `fulilian -p coder` -> host key `fulilian_coder`). Older `fulilian.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `FULILIAN_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `fulilian-agent`) |
...
|---------|-------------|
| `fulilian memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `fulilian honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `fulilian memory setup`) |
| `fulilian honcho status` | Show resolved config for active profile |
| `fulilian honcho enable` / `disable` | Toggle Honcho for active profile |
| `fulilian honcho mode <mode>` | Change recall or observation mode |
| `fulilian honcho peer --user <name>` | Update user peer name |
| `fulilian honcho peer --ai <name>` | Update AI peer name |
| `fulilian honcho tokens --context <N>` | Set context token budget |
| `fulilian honcho tokens --dialectic <N>` | Set dialectic max chars |
| `fulilian honcho map <name>` | Map current directory to a session name |
| `fulilian honcho sync` | Create host blocks for all Fulilian profiles |
