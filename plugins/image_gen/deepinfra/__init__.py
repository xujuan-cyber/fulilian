...
``https://api.deepinfra.com/v1/openai/models?filter=true&sort_by=fulilian``
where each entry's ``metadata.tags`` declares its surface (``image-gen``
here). ``list_models()`` filters that catalog via
:func:`fulilian_cli.models._fetch_deepinfra_models_by_tag` so newly added
models show up in ``fulilian tools`` automatically. No model ids are
hardcoded in this file — if a model is retired upstream, it disappears
from fulilian the next time the catalog is fetched, no patch required.
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "DEEPINFRA_API_KEY not set. Run `fulilian tools` → Image "
                    "Generation → DeepInfra to configure, or `fulilian setup` "
                    "to add the key."
                ),
        ...
        from fulilian_cli.models import deepinfra_base_url
