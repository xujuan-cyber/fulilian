...
three virtual model IDs so the ``fulilian tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `fulilian tools` → Image "
                    "Generation → OpenAI to configure, or `fulilian setup` "
                    "to add the key."
                ),
