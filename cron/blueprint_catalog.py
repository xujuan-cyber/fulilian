...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``fulilian://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``fulilian://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"fulilian://blueprint/{quote(blueprint.key)}{qs}"
