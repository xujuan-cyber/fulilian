...
A *suggestion* is a ready-to-run cron job spec that Fulilian surfaces to the
...
Storage mirrors ``cron/jobs.py``: ``~/.fulilian/cron/suggestions.json``, atomic
...
# profile's cron store. Anchor on get_fulilian_home() (profile home), not the
# shared default root. See cron/jobs.py for the full rationale.
CRON_DIR = get_fulilian_home().resolve() / "cron"
...
def _save_raw(suggestions: List[Dict[str, Any]]) -> None:
    ...
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as f:
            json.dump(
                {"suggestions": suggestions, "updated_at": _fulilian_now().isoformat()},
                f,
                indent=2,
            )
            f.flush()
    ...
    *,
    ...
    with _suggestions_lock:
        ...
        record = {
            ...
            "created_at": _fulilian_now().isoformat(),
        }
    ...
    with _suggestions_lock:
        ...
        for s in suggestions:
            if s.get("id") == suggestion_id:
                s["status"] = status
                s["resolved_at"] = _fulilian_now().isoformat()
                changed = True
                break
        if changed:
            _save_raw(suggestions)
