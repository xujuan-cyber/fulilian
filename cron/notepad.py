...
Write path is the CLI (``fulilian cron notepad <job_id> set <key> <value>``),
...
NOTEPAD_FILE = get_fulilian_home().resolve() / "cron" / "notepad.db"
...
def render_notepad_section(job_id: str) -> str:
    ...
    return (
        ...
        f"`fulilian cron notepad {job_id} set <key> <value>` "
        f"(also: get/delete/list; `fulilian cron notepad {job_id} delete "
