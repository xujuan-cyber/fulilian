"""
Timezone-aware clock for Fulilian.

Provides a single ``now()`` helper that returns a timezone-aware datetime
based on the user's configured IANA timezone (e.g. ``Asia/Kolkata``).

Resolution order:
  1. ``FULILIAN_TIMEZONE`` environment variable
  2. ``timezone`` key in ``~/.fulilian/config.yaml``
  3. Falls back to the server's local time (``datetime.now().astimezone()``)

Invalid timezone values log a warning and fall back safely — Fulilian never
...
from fulilian_constants import get_config_path
...
try:
    ...
    # Python 3.8 fallback (shouldn't be needed — Fulilian requires 3.9+)
...
def _resolve_timezone_name() -> str:
    ...
    tz_env = os.getenv("FULILIAN_TIMEZONE", "").strip()
    if tz_env:
        return tz_env
...
def reset_cache() -> None:
    ...
    config edit or ``FULILIAN_TIMEZONE`` update) to force ``get_timezone()`` /
