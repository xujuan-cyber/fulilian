...
const BACKEND = process.env.FULILIAN_DASHBOARD_URL ?? "http://127.0.0.1:9119";

/**
 * In production the Python `fulilian dashboard` server injects a one-shot
 * session token into `index.html` (see `fulilian_cli/web_server.py`). The
 * Vite dev server serves its own `index.html`, so unless we forward that
 * token, every protected `/api/*` call 401s.
 *
 * This plugin fetches the running dashboard's `index.html` on each dev page
 * load, scrapes the `window.__FULILIAN_SESSION_TOKEN__` assignment, and
 * re-injects it into the dev HTML. No-op in production builds.
 */
function fulilianDevToken(): Plugin {
  const TOKEN_RE = /window\.__FULILIAN_SESSION_TOKEN__\s*=\s*"([^"]+)"/;
  const EMBEDDED_RE =
    /window\.__FULILIAN_DASHBOARD_EMBEDDED_CHAT__\s*=\s*(true|false)/;

  return {
    name: "fulilian:dev-session-token",
    apply: "serve",
    async transformIndexHtml() {
      try {
        ...
        if (!match) {
          console.warn(
            `[fulilian] Could not find session token in ${BACKEND} — ` +
              `is \`fulilian dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        return [
          {
            ...
            injectTo: "head",
            children:
              `window.__FULILIAN_SESSION_TOKEN__="${match[1]}";` +
              `window.__FULILIAN_DASHBOARD_EMBEDDED_CHAT__=${embeddedJs};`,
          },
        ];
      } catch (err) {
        console.warn(
          `[fulilian] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`fulilian dashboard\` or set FULILIAN_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      "@fulilian/shared": path.resolve(__dirname, "../apps/shared/src"),
    },
  ...
  server: {
    proxy: {
      ...
      // Same host as `fulilian dashboard` must serve these; Vite has no
