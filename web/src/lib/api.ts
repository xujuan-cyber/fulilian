import { buildFulilianWebSocketUrl } from "@fulilian/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/fulilian/). The Python backend
// injects ``window.__FULILIAN_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__FULILIAN_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const FULILIAN_BASE_PATH = readBasePath();
const BASE = FULILIAN_BASE_PATH;
...
declare global {
  interface Window {
    __FULILIAN_SESSION_TOKEN__?: string;
    __FULILIAN_BASE_PATH__?: string;
    ...
    __FULILIAN_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-Fulilian-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__FULILIAN_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``fulilian update``, ``fulilian gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__FULILIAN_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__FULILIAN_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__FULILIAN_SESSION_TOKEN__`` is valid, so the next
 ...
 *
 * The dashboard's gated-mode WS auth (``fulilian_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__FULILIAN_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Fulilian-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``fulilian_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__FULILIAN_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export async function buildWsUrl(
  ...
  return buildFulilianWebSocketUrl({
...
export const api = {
  ...
  updateFulilian: () =>
    fetchJSON<ActionResponse>("/api/fulilian/update", { method: "POST" }),
  checkFulilianUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/fulilian/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** fulilian-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``fulilian update``. */
  can_update_fulilian?: boolean;
  config_path: string;
  ...
  /** NS-656: disk-usage rollup for the FULILIAN_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the FULILIAN_HOME volume — no staleness dimension, so no
 * sampled_at. */
