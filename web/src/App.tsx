...
const SIDEBAR_COLLAPSED_KEY = "fulilian-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="fulilian-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="fulilian-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateFulilian = status?.can_update_fulilian === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'fulilian update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "fulilian update";
    return (
      t.status.updateFulilianConfirmMessage ??
      `This will run 'fulilian update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateFulilianConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateFulilian) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateFulilian,
      runningLabel: t.status.updatingFulilian,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Fulilian gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateFulilianConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateFulilianConfirmTitle ?? `${t.status.updateFulilian}?`}
    />
    </>
  );
}
