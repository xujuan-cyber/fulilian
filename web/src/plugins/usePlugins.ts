...
import { api, FULILIAN_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "fulilian:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${FULILIAN_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${FULILIAN_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?fulilian_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-fulilian-plugin", manifest.name);
