 ...
 * `window.__FULILIAN_PLUGIN_SDK__`. Each new key is checked individually so a
...
describe("plugin SDK dialog/toast surface", () => {
  beforeEach(() => {
    ...
    (globalThis as unknown as { window: Record<string, unknown> }).window = {
      __FULILIAN_PLUGINS__: undefined,
      __FULILIAN_PLUGIN_SDK__: undefined,
    };
  });

  it("exposes Dialog + subcomponents on components", () => {
    exposePluginSDK();
    const sdk = (globalThis as unknown as { window: { __FULILIAN_PLUGIN_SDK__: { components: Record<string, unknown>; hooks: Record<string, unknown> } } }).window.__FULILIAN_PLUGIN_SDK__;
