...
If your antivirus (Bitdefender, Windows Defender, etc.) quarantines `uv.exe` from the Fulilian `bin` folder (`%LOCALAPPDATA%\fulilian\bin\uv.exe`), this is a **false positive**. The file is Astral's `uv` — the Rust Python package manager Fulilian bundles to manage its Python environment. ML-based antivirus engines commonly flag unsigned Rust binaries that download and install packages.
...
# Run verification
$uv = "$env:LOCALAPPDATA\fulilian\bin\uv.exe"
...
**To whitelist Fulilian:**
- **Windows Defender:** Run PowerShell as Admin → `Add-MpPreference -ExclusionPath "$env:LOCALAPPDATA\fulilian\bin"`
- **Bitdefender:** Add an exception in the Bitdefender console (Protection > Antivirus > Settings > Manage Exceptions)
- Whitelist the **folder**, not the file hash — Fulilian updates `uv` and the hash changes every version
...
```bash
fulilian              # Interactive CLI — start a conversation
...
fulilian config get   # Print individual config values
fulilian gateway      # Start the messaging gateway (Telegram, Discord, etc.)
fulilian setup        # Run the full setup wizard (configures everything at once)
fulilian claw migrate # Migrate from OpenClaw (if coming from OpenClaw)
fulilian update       # Update to the latest version
...
Fulilian works with whatever provider you want — that's not changing. But if you'd rather not collect five separate API keys for the model, web search, image generation, TTS, and a cloud browser, **[Nous Portal](https://portal.nousresearch.com)** covers all of them under one subscription:
...
Fulilian has two entry points: start the terminal UI with `fulilian`, or run the gateway and talk to it from Telegram, Discord, Slack, WhatsApp, Signal, or Email. Once you're in a conversation, many slash commands are shared across both interfaces.

| Action                         | CLI                                           | Messaging platforms                                                              |
| ------------------------------ | --------------------------------------------- | -------------------------------------------------------------------------------- |
| Start chatting                 | `fulilian`                                      | Run `fulilian gateway setup` + `fulilian gateway start`, then send the bot a message |
...
If you're coming from OpenClaw, Fulilian can automatically import your settings, memories, skills, and API keys.

**During first-time setup:** The setup wizard (`fulilian setup`) automatically detects `~/.openclaw` and offers to migrate before configuration begins.

**Anytime after install:**

```bash
fulilian claw migrate              # Interactive migration (full preset)
...
- **Skills** — user-created skills → `~/.fulilian/skills/openclaw-imports/`
...
See `fulilian claw migrate --help` for all options, or use the `openclaw-migration` skill for an interactive agent-guided migration with dry-run previews.
...
`~/.fulilian/fulilian-agent`). This matches the layout used by `fulilian update`, the
...
source ~/.fulilian/venvs/fulilian-dev/bin/activate
uv pip install -e ".[all,dev]"
scripts/run_tests.sh
```
