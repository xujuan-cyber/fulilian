...
def _is_dispatcher_owned_worker() -> bool:
    """False when FULILIAN_KANBAN_* is present but this execution does not own it
...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # async tools resolve get_fulilian_home() under the active profile.
...
#   - cli.py, fulilian_cli/*      -> inline on startup (no event loop)
    ...
    if quiet_mode:
        ...
        if profile_scope != CHECK_FN_CACHE_BYPASS:
            cache_key = (
                ...
                cfg_fp,
                bool(os.environ.get("FULILIAN_KANBAN_TASK")),
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if (
            ...
            # Dispatcher-spawned workers are scoped by FULILIAN_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like fulilian-cli)
    ...
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                ...
                if toolset_name.startswith("fulilian-") or (get_toolset(toolset_name) or {}).get("posture"):
                    # Platform bundles (fulilian-*) include _FULILIAN_CORE_TOOLS, and
                    ...
                    if (not quiet_mode and toolset_name.startswith("fulilian-")
    ...
    # window). Core Fulilian tools (toolsets._FULILIAN_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from fulilian_cli.config import load_config as _load
        cfg = _load() or {}
...
def _normalize_json_strings_for_schema(value: Any, schema: Any) -> Any:
    ...
    Ported from cline/cline#11803, adapted to fulilian-agent's coercion layer.
...
def _emit_post_tool_call_hook(
    *,
    ...
    tool_may_still_be_running: Optional[bool] = None,
) -> None:
