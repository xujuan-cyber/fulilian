...
#
# `services.fulilian-agent` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  fulilianHome, programs.fulilian-agent (the CLI and
...
let
  ...
  deepConfigType = types.mkOptionType {
    name = "fulilian-config-attrs";
    description = "Fulilian YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    options = {
      ...
      auth = mkOption {
        ...
        description = ''
          ...
          (remote MCP servers). Tokens are stored in $FULILIAN_HOME/mcp-tokens/.
        '';
      };
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "Fulilian Agent";
      ...
      package = mkOption {
        ...
        description = "The fulilian-agent package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The Fulilian configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `fulilian config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $FULILIAN_HOME/.env. Fulilian reads that file at each start, with
          load_fulilian_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."fulilian/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that Fulilian refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          Fulilian reads SOUL.md and memories/ from FULILIAN_HOME, so put those
          files in `fulilianHomeFiles`.
      ...
      fulilianHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into FULILIAN_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          Fulilian reads SOUL.md and the memory files from FULILIAN_HOME and not
          from the working directory. Declare those files here, or Fulilian
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the fulilian plugins
          directory. Each package must contain a plugin.yaml and __init__.py
          at its root. Fulilian discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          fulilian_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as fulilian (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in fulilian-agent's
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `fulilian gateway`.";
      };
      ...
      # ── The backend: `fulilian serve` or `fulilian dashboard` ──────────────
      # `fulilian serve` and `fulilian dashboard` are the same entry point,
      # fulilian_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that Fulilian Desktop
      ...
      # controls an external gateway, with `fulilian gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that Fulilian
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            FULILIAN_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. Fulilian Desktop presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."fulilian/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `fulilian` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.fulilian-agent.installPackage was removed. Fulilian now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.fulilian-agent.enable = ${lib.boolToString (value != false)};  # the fulilian CLI, and FULILIAN_HOME for your shells
        programs.fulilian-agent.desktop.enable = true;  # the desktop application

      `services.fulilian-agent` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "fulilian-config.yaml" (
  ...
  mkDocumentTree =
    { pkgs, documents }:
    pkgs.runCommand "fulilian-documents" { } (
      ''
        mkdir -p $out
      ''
  ...
  mkEnvScript =
    { pkgs, environment }:
    let
      base = pkgs.writeText "fulilian-env-base" (
        lib.concatStringsSep "\n" (lib.mapAttrsToList (k: v: "${k}=${v}") environment)
        + lib.optionalString (environment != { }) "\n"
      );
    in
    pkgs.writeShellScript "fulilian-env-merge" ''
      set -eu
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "fulilian-agent: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # FULILIAN_MANAGED variable of the service. The value tells the shell
    ...
    let
      ...
      homeDocumentTree = mkDocumentTree {
        inherit pkgs;
        documents = cfg.fulilianHomeFiles;
      };
    ...
    in
    ''
      # Directories. The service units and Fulilian make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${fulilianHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. Fulilian
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${fulilianHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${fulilianHome}/config.yaml
            ${run}chmod ${modes.config} ${fulilianHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "fulilian-managed" managedSystem} ${fulilianHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${fulilianHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${fulilianHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${fulilianHome}/auth.json"
        else
          ''
            if [ ! -e ${fulilianHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${fulilianHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree fulilianHome cfg.fulilianHomeFiles}
      ...
      ${run}find ${fulilianHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "fulilian-agent: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${fulilianHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  gatewayArgv =
    cfg:
    [
      "${effectivePackage cfg}/bin/fulilian"
      "gateway"
    ]
    ++ cfg.extraArgs;
  ...
  #
  # `exec` on the last line keeps fulilian as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "fulilian-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "fulilian-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "fulilian-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          FULILIAN_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export FULILIAN_DASHBOARD_SESSION_TOKEN

          if [ -z "$FULILIAN_DASHBOARD_SESSION_TOKEN" ]; then
            echo "fulilian-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "fulilian-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "fulilian-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "fulilian-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "fulilian-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "fulilian-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "fulilian-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "Fulilian Agent web dashboard and desktop backend"
    else
      "Fulilian Agent backend for Fulilian Desktop";

  # The environment that each Fulilian process needs, from either module.
  #
  # managedSystem gives the value of FULILIAN_MANAGED. The CLI reads that
  ...
  processEnvironment =
    ...
    }:
    {
      ...
      FULILIAN_MANAGED = managedSystem;
    };
    ...
    [
      {
        ...
        message = ''
          ...
          To give Fulilian an identity and a memory, use
          ${optionPath}.fulilianHomeFiles instead. Those files go to
          FULILIAN_HOME. Fulilian reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of FULILIAN_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"
