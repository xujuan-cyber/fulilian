# nix/web.nix — Fulilian Web Dashboard (Vite/React) frontend build
{ fulilianNpmLib, ... }:
fulilianNpmLib.buildNpmPackage {
  dirs = [
    "web"

    # @fulilian/shared ships as a file: workspace dep of web, so its source
  ...
  buildPhase = ''
    ...
    # outDir in vite.config.ts points to ../fulilian_cli/web_dist for the
