...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      fulilian-agent = self'.packages.default;
      fulilianVenv = fulilian-agent.fulilianVenv;
      ...
      evalHomeModule =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              home = {
                username = "fulilian-check";
                homeDirectory = "/home/fulilian-check";
                stateVersion = "24.11";
              };
            }
            { services.fulilian-agent = settings; }
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.fulilian-agent = programs;
              services.fulilian-agent = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.fulilian-agent. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.fulilian-agent);
      ...
      configKeys = pkgs.runCommand "fulilian-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${fulilianVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "fulilian-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "fulilian-build-package" { } ''
          echo "PASS: package built at ${fulilian-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "fulilian-build-devshell" { } ''
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.FULILIAN_TEST = "1";
                environmentFiles = [ "/run/secrets/fulilian-env" ];
                fulilianHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "fulilian" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "fulilian" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.fulilianAgentSetup.data;

            failures =
              lib.optional (names != [
                "fulilian-agent"
                "fulilian-backend"
              ]) "expected fulilian-agent + fulilian-backend processes, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/fulilian gateway" (argvOf "fulilian-agent")
              ) "gateway process does not run `fulilian gateway`: ${argvOf "fulilian-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/fulilian serve" (argvOf "fulilian-backend")
              ) "backend process does not run `fulilian serve`: ${argvOf "fulilian-backend"}"
              ++ lib.optional (
                !lib.hasInfix "--no-open" (argvOf "fulilian-backend")
              ) "backend must pass --no-open so a service never opens a browser"
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/fulilian-check/.fulilian" (envOf n)) names
              ) "gateway and backend must share one FULILIAN_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.FULILIAN_HOME or null != "/home/fulilian-check/.fulilian"
              ) "programs.fulilian-agent.enable must export FULILIAN_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "fulilian-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/fulilian-check/.fulilian/SOUL.md" activation
              ) "fulilianHomeFiles must install into FULILIAN_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads FULILIAN_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "FULILIAN_MANAGED=home-manager" (envOf n)) names
              ) "processes must report FULILIAN_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "fulilian-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "fulilian-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            # the priority.
            sameAsDefault = "/home/fulilian-check";
            ...
            cases = [
              ...
              {
                name = "fulilianHomeFiles needs no directory";
                ok = accepts { fulilianHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "fulilian-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one FULILIAN_HOME ───────────────
        # `programs.enable` exports FULILIAN_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.fulilian
        # while the services use the FULILIAN_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/fulilian-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                fulilianHome = "/home/fulilian-check/.fulilian-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "fulilian-desktop") cfg.home.packages;
            ...
            # second Fulilian runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "fulilian-agent") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.fulilian-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.fulilian-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one fulilian-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "FULILIAN_HOME" != "/home/fulilian-check/.fulilian-work"
              ) "the launcher must carry FULILIAN_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "FULILIAN_HOME")})"
              ++ lib.optional (
                setValue "FULILIAN_MANAGED" != "home-manager"
              ) "the launcher must report FULILIAN_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "FULILIAN_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "FULILIAN_DESKTOP_FULILIAN" != "${lib.head agentPackages}/bin/fulilian"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "FULILIAN_DESKTOP_FULILIAN")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "FULILIAN_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "FULILIAN_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "FULILIAN_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "FULILIAN_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "FULILIAN_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "fulilian-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares FULILIAN_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set FULILIAN_HOME" wrapper
              ) "the launcher must carry FULILIAN_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "FULILIAN_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "FULILIAN_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? fulilian-backend || cfg.launchd.agents ? fulilian-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "fulilian-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `fulilian`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.fulilian-agent.enable = true;";
              }
              {
                value = false;
                expect = "programs.fulilian-agent.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "fulilian-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "fulilian-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "fulilian" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."fulilian-agent-setup".text;

            failures =
              lib.optional (names != [
                "fulilian-agent"
                "fulilian-backend"
              ]) "expected fulilian-agent + fulilian-backend units, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/fulilian gateway" (execOf "fulilian-agent")
              ) "gateway unit does not run `fulilian gateway`: ${execOf "fulilian-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/fulilian dashboard" (execOf "fulilian-backend")
              ) "backend unit does not run `fulilian dashboard`: ${execOf "fulilian-backend"}"
              ++ lib.optional (
                units.fulilian-agent.environment.FULILIAN_HOME != units.fulilian-backend.environment.FULILIAN_HOME
              ) "gateway and backend must share one FULILIAN_HOME"
              ++ lib.optional (
                !lib.hasInfix "/var/lib/fulilian/.fulilian/SOUL.md" activation
              ) "fulilianHomeFiles must install into FULILIAN_HOME";
          ...
          in
          pkgs.runCommand "fulilian-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs fulilian, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.fulilian-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              lib.optional (!lib.hasInfix "bin/fulilian serve --host 127.0.0.1" direct)
                "without waitFor the backend must exec fulilian directly, got: ${direct}"
              ++ lib.optional (lib.hasInfix "fulilian-backend-launch" direct)
                "without waitFor the backend must not use the launcher"

              # The hostname mode polls the resolver, then binds the name.
              ++ lib.optional (!lib.hasInfix "fulilian-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec fulilian, so that it keeps the MainPID"
              ...
              ++ lib.optional (!lib.hasInfix "bin/fulilian dashboard" interfaceScript)
          ...
          in
          pkgs.runCommand "fulilian-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        # $FULILIAN_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          let
            envScript = (import ./moduleCommon.nix { inherit lib; }).mkEnvScript {
              inherit pkgs;
              environment = {
                FULILIAN_PUBLIC = "visible";
              };
            };
          in
          pkgs.runCommand "fulilian-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'FULILIAN_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--fulilian-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "fulilian-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "fulilian-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${fulilian-agent}/bin/fulilian || (echo "FAIL: fulilian binary missing"; exit 1)
          test -x ${fulilian-agent}/bin/fulilian-agent || (echo "FAIL: fulilian-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${fulilian-agent}/bin/fulilian --version 2>&1 | grep -qi "fulilian" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "fulilian-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in fulilian fulilian-agent fulilian-acp; do
            test -x ${fulilian-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "fulilian-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking fulilian --help ==="
          ${fulilian-agent}/bin/fulilian --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${fulilian-agent}/bin/fulilian --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "fulilian-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${fulilian-agent}/share/fulilian-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${fulilian-agent}/share/fulilian-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "FULILIAN_BUNDLED_SKILLS" ${fulilian-agent}/bin/fulilian || \
            (echo "FAIL: FULILIAN_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: FULILIAN_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${fulilian-agent}/share/fulilian-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${fulilian-agent}/share/fulilian-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "FULILIAN_OPTIONAL_SKILLS" ${fulilian-agent}/bin/fulilian || \
            (echo "FAIL: FULILIAN_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, FULILIAN_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "fulilian-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${fulilian-agent}/share/fulilian-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${fulilian-agent}/share/fulilian-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "FULILIAN_BUNDLED_PLUGINS" ${fulilian-agent}/bin/fulilian || \
            (echo "FAIL: FULILIAN_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: FULILIAN_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "fulilian-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${fulilian-agent}/share/fulilian-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${fulilian-agent}/share/fulilian-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${fulilian-agent}/share/fulilian-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "FULILIAN_BUNDLED_LOCALES" ${fulilian-agent}/bin/fulilian || \
            (echo "FAIL: FULILIAN_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: FULILIAN_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + FULILIAN_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && FULILIAN_BUNDLED_LOCALES=${fulilian-agent}/share/fulilian-agent/locales \
            ${fulilianVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with FULILIAN_BUNDLED_LOCALES set"; exit 1)
        ...
        # FULILIAN_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "fulilian-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${fulilian-agent}/share/fulilian-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${fulilian-agent}/share/fulilian-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "FULILIAN_OPTIONAL_MCPS" ${fulilian-agent}/bin/fulilian || \
            (echo "FAIL: FULILIAN_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: FULILIAN_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${fulilian-agent}/bin/fulilian mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: fulilian mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "fulilian-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${fulilian-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${fulilian-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "FULILIAN_TUI_DIR" ${fulilian-agent}/bin/fulilian || \
            (echo "FAIL: FULILIAN_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: FULILIAN_TUI_DIR set in wrapper"
        ...
        # Verify FULILIAN_NODE is set in wrapper and points to Node 26+
        # (Fulilian pins its toolchain to Node 26 everywhere)
        fulilian-node = pkgs.runCommand "fulilian-node-version" { } ''
          set -e
          echo "=== Checking FULILIAN_NODE in wrapper ==="
          grep -q "FULILIAN_NODE" ${fulilian-agent}/bin/fulilian || \
            (echo "FAIL: FULILIAN_NODE not set in wrapper"; exit 1)
          echo "PASS: FULILIAN_NODE present in wrapper"

          FULILIAN_NODE=$(sed -n "s/^export FULILIAN_NODE='\(.*\)'/\1/p" ${fulilian-agent}/bin/fulilian)
          test -x "$FULILIAN_NODE" || (echo "FAIL: FULILIAN_NODE=$FULILIAN_NODE not executable"; exit 1)
          echo "PASS: FULILIAN_NODE executable at $FULILIAN_NODE"

          NODE_MAJOR=$("$FULILIAN_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, Fulilian requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All FULILIAN_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify FULILIAN_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "fulilian-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(FULILIAN_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking FULILIAN_MANAGED guards ==="
          check_blocked "config set" ${fulilian-agent}/bin/fulilian config set model foo
          check_blocked "config edit" ${fulilian-agent}/bin/fulilian config edit
        ...
        extra-python-packages = let
          ...
          fulilianWithExtra = fulilian-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "fulilian-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${fulilianWithExtra}/bin/fulilian || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${fulilianWithExtra}/bin/fulilian || \
          ...
          if grep -q "PYTHONPATH" ${fulilian-agent}/bin/fulilian; then
        ...
        extra-dependency-groups = let
          fulilianWithGroups = fulilian-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "fulilian-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${fulilianWithGroups}"
          echo "venv: ${fulilianWithGroups.fulilianVenv}"
        ...
        messaging-variant = pkgs.runCommand "fulilian-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.fulilianVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "fulilian-config-roundtrip" {
          ...
          merge_and_load() {
            local fulilian_home="$1"
            export FULILIAN_HOME="$fulilian_home"
            ${configMergeScript} ${nixSettings} "$fulilian_home/config.yaml"
            ${fulilianVenv}/bin/python3 -c '
import json, sys
