...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Fulilian derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            fulilian-agent = prev.fulilian-agent.overrideAttrs (_old: {
              FULILIAN_NIX_BUILD = "1";
            });
          })
        ]
      );
  ...
  # $FULILIAN_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$FULILIAN_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        fulilian-agent = prev.fulilian-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "fulilian-agent-env" {
    fulilian-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "fulilian-agent-editable-env" {
    fulilian-agent = dependency-groups;
  };
}
