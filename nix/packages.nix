# nix/packages.nix — Fulilian Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      ...
      minimal = pkgs.callPackage ./fulilian-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.fulilianTui;
        web = full.fulilianWeb;
        desktop = full.fulilianDesktop;

        update-npm-lockfile = full.fulilianNpmLib.updateNpmLockfile;
      };
    };
}
