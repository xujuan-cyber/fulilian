...
_FULILIAN_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (FULILIAN_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `fulilian tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "video_gen": {
        "description": (
            ...
            "``fulilian tools`` → Video Generation."
        ),
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (FULILIAN_KANBAN_TASK env "
    ...
    # Coding posture (base Fulilian — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in fulilian_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Fulilian toolsets (CLI + messaging platforms)
    #
    ...
    # the `fulilian send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "fulilian-cli": {
        ...
        "tools": _FULILIAN_CORE_TOOLS,
        "includes": []
    },

    "fulilian-cron": {
        # Mirrors fulilian-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `fulilian tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as fulilian-cli; gated by `fulilian tools`",
        "tools": _FULILIAN_CORE_TOOLS,
        "includes": []
    },
    ...
    "fulilian-bluebubbles": {
        "description": "BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server",
        "tools": _FULILIAN_CORE_TOOLS,
        "includes": []
    },

    "fulilian-homeassistant": {
    ...
    "fulilian-email": {
        "description": "Email bot toolset - interact with Fulilian via email (IMAP/SMTP)",
        "tools": _FULILIAN_CORE_TOOLS,
        "includes": []
    },
    ...
    "fulilian-sms": {
        "description": "SMS bot toolset - interact with Fulilian via SMS (Twilio)",
        "tools": _FULILIAN_CORE_TOOLS,
        "includes": []
    },
    ...
    "fulilian-webhook": {
        ...
        "tools": _FULILIAN_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "fulilian-gateway": {
        ...
        "tools": [],
        "includes": ["fulilian-telegram", "fulilian-discord", "fulilian-whatsapp", "fulilian-slack", "fulilian-signal", "fulilian-bluebubbles", "fulilian-homeassistant", "fulilian-email", "fulilian-sms", "fulilian-mattermost", "fulilian-matrix", "fulilian-dingtalk", "fulilian-feishu", "fulilian-wecom", "fulilian-wecom-callback", "fulilian-weixin", "fulilian-qqbot", "fulilian-webhook", "fulilian-yuanbao"]
    },

    # ── FuLiLian CTF solver toolset ─────────────────────────────────
    # 这里曾经挂着 "memory" 与 "skill_manage"，理由是回合后自省
    # （agent/background_review.py）要求这两个名字出现在 valid_tool_names 里
    # 才会触发记忆 / skill 审查 fork。但 CTF 路径已用
    # skip_background_review=True 关掉该 fork（run_agent.py:9229，A3），
    # 触发条件无人消费 —— 实测 3 题 43 次工具调用里这两个工具调用 0 次，
    # 而它们的 schema 是 5,631 字符（14 个工具共 22,942 字符的 24.5%），
    # 随每次 API 调用重发 ≈ 1,400 tokens，占全部 input token 的 8%。
    #
    # ⚠️ 耦合：若将来重新开启 background_review，必须把这两个名字加回来，
    # 否则自省 fork 永不触发且**静默** —— 不会报错，只是经验不再沉淀。
    # CTF 经验沉淀应在批次结束后统一做一次，不是每题一次。
    "ctf_solve": {
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``fulilian-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_FULILIAN_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``fulilian-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (fulilian-<name>).
        # Gives them _FULILIAN_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("fulilian-"):
            platform_name = name[len("fulilian-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_FULILIAN_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
