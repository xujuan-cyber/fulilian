...
# Fulilian WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: FULILIAN_TINI_SHIM_TARGET overrides the `/init` path (and
# FULILIAN_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${FULILIAN_TINI_SHIM_TARGET:-/init}"
WRAPPER="${FULILIAN_TINI_SHIM_WRAPPER:-/opt/fulilian/docker/main-wrapper.sh}"
