...
#
# Wired into the image as /etc/cont-init.d/01-fulilian-setup by the
...
FULILIAN_HOME="${FULILIAN_HOME:-/opt/data}"
INSTALL_DIR="/opt/fulilian"

# Drop to fulilian via s6-setuidgid, but skip it when already non-root.
as_fulilian() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid fulilian "$@"; }
...
# starts non-root. The baked install tree under /opt/fulilian is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $FULILIAN_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass FULILIAN_UID/FULILIAN_GID — or the PUID/PGID aliases — which the
...
# `id -u fulilian` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# fulilian UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u fulilian)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-fulilian UID).
...
and the baked /opt/fulilian install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e FULILIAN_UID=\$(id -u) -e FULILIAN_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the fulilian user to that UID/GID at boot and chowns the data
...
# --- Bootstrap FULILIAN_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid fulilian mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom FULILIAN_HOME paths whose parents only
# root can create (e.g. `FULILIAN_HOME=/home/fulilian/.fulilian` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for FULILIAN_UID/FULILIAN_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  FULILIAN_UID/
# FULILIAN_GID still win when both are set.  See #15290, salvages #25872.
FULILIAN_UID="${FULILIAN_UID:-${PUID:-}}"
FULILIAN_GID="${FULILIAN_GID:-${PGID:-}}"

if [ -n "${FULILIAN_UID:-}" ] && validate_uid_gid "$FULILIAN_UID" && [ "$FULILIAN_UID" != "$(id -u fulilian)" ]; then
    echo "[stage2] Changing fulilian UID to $FULILIAN_UID"
    usermod -u "$FULILIAN_UID" fulilian
fi
if [ -n "${FULILIAN_GID:-}" ] && validate_uid_gid "$FULILIAN_GID" && [ "$FULILIAN_GID" != "$(id -g fulilian)" ]; then
...
# host's `docker` group (or root). The supervised fulilian user (UID 10000)
...
# fulilian process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes fulilian. Idempotent across container
...
#   - fulilian is already a member of the right group (idempotent restart)
...
# When FULILIAN_UID is remapped or the top-level $FULILIAN_HOME isn't owned by
# the runtime fulilian UID, restore ownership to fulilian — but ONLY for the
# directories fulilian actually writes to. The full $FULILIAN_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of fulilian-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_fulilian_uid=$(id -u fulilian)

path_has_symlink_component() {
    path="$1"
    root="${2:-$FULILIAN_HOME}"
...
chown_fulilian_tree() {
    ...
    chown -R fulilian:fulilian "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_fulilian_owner() {
    target="$1"
    find "$target" \( ! -user fulilian -o ! -group fulilian \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$FULILIAN_HOME" 2>/dev/null)" != "$actual_fulilian_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $FULILIAN_HOME (targeted) to fulilian ($actual_fulilian_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $FULILIAN_HOME: chown the directory itself (not its contents)
    # so fulilian can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$FULILIAN_HOME"; then
        :
    else
        chown fulilian:fulilian "$FULILIAN_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $FULILIAN_HOME failed (rootless container?) — continuing"
    fi
    # Fulilian-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by fulilian (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$FULILIAN_HOME/$sub" ] && tree_has_non_fulilian_owner "$FULILIAN_HOME/$sub"; then
            chown_fulilian_tree "$FULILIAN_HOME/$sub"
        fi
    done
fi
...
# fulilian user. Hosted/container instances keep mutable state under
# $FULILIAN_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# FULILIAN_DISABLE_LAZY_INSTALLS=1. Keeping /opt/fulilian root-owned and
...
# $FULILIAN_HOME/lazy-packages dir on the data volume (Dockerfile sets
# FULILIAN_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to fulilian
...
# Always reset ownership of $FULILIAN_HOME/profiles to fulilian on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> fulilian …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as fulilian and walks
...
if [ -d "$FULILIAN_HOME/profiles" ] && tree_has_non_fulilian_owner "$FULILIAN_HOME/profiles"; then
    chown_fulilian_tree "$FULILIAN_HOME/profiles"
fi

# Always reset ownership of $FULILIAN_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged fulilian runtime even
...
# gate as profiles/).
if [ -d "$FULILIAN_HOME/cron" ] && tree_has_non_fulilian_owner "$FULILIAN_HOME/cron"; then
    chown_fulilian_tree "$FULILIAN_HOME/cron"
fi

# Always ensure logs/gateways is fulilian-owned (#45258). Formerly healed by
...
# top-level $FULILIAN_HOME is mis-owned, so a warm volume with fulilian-owned
# FULILIAN_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid fulilian mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as fulilian.
if [ -d "$FULILIAN_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$FULILIAN_HOME/logs/gateways"; then
        :
    else
        chown fulilian:fulilian "$FULILIAN_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# fulilian pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged fulilian gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $FULILIAN_HOME is
...
# and cron/.
if [ -d "$FULILIAN_HOME/platforms/pairing" ] && tree_has_non_fulilian_owner "$FULILIAN_HOME/platforms/pairing"; then
    chown_fulilian_tree "$FULILIAN_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$FULILIAN_HOME/pairing" ] && tree_has_non_fulilian_owner "$FULILIAN_HOME/pairing"; then
    chown_fulilian_tree "$FULILIAN_HOME/pairing"
fi

# Reset ownership of fulilian-owned top-level state files on every boot.
# The targeted data-volume chown above only covers fulilian-owned
# *subdirectories*; loose state files living directly under $FULILIAN_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> fulilian …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged fulilian runtime then hits
...
# sweep so host-owned files in a bind-mounted $FULILIAN_HOME are never
...
# entries of fulilian_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$FULILIAN_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$FULILIAN_HOME/$f"; then
            :
        else
            chown fulilian:fulilian "$FULILIAN_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the fulilian runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$FULILIAN_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$FULILIAN_HOME/config.yaml"; then
        :
    else
        chown fulilian:fulilian "$FULILIAN_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$FULILIAN_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as fulilian user ---
# Run as fulilian via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $FULILIAN_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_fulilian mkdir -p \
    "$FULILIAN_HOME/backups" \
    "$FULILIAN_HOME/cron" \
    "$FULILIAN_HOME/sessions" \
    "$FULILIAN_HOME/logs" \
    "$FULILIAN_HOME/logs/gateways" \
    "$FULILIAN_HOME/hooks" \
    "$FULILIAN_HOME/memories" \
    "$FULILIAN_HOME/skills" \
    "$FULILIAN_HOME/skins" \
    "$FULILIAN_HOME/plans" \
    "$FULILIAN_HOME/workspace" \
    "$FULILIAN_HOME/home" \
    "$FULILIAN_HOME/pairing" \
    "$FULILIAN_HOME/platforms/pairing" \
    "$FULILIAN_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/fulilian/.install_method (see Dockerfile), NOT written here into
# $FULILIAN_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $FULILIAN_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.fulilian:/opt/data) and sometimes shared with a
...
# 'fulilian update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $FULILIAN_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/fulilian stamp, so deleting the data-dir copy is safe).
if [ -f "$FULILIAN_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$FULILIAN_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$FULILIAN_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$FULILIAN_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$FULILIAN_HOME/$dest"; then
            :
        else
            as_fulilian cp "$INSTALL_DIR/$src" "$FULILIAN_HOME/$dest"
        fi
    fi
}
...
# the first-boot seed above depends on /opt/fulilian/.env.example being present
...
# not generate one. Fulilian loads $FULILIAN_HOME/.env with override=True, so
...
if [ -n "${API_SERVER_KEY:-}" ]; then
    if [ -f "$FULILIAN_HOME/.env" ] && grep -q '^API_SERVER_KEY=..*' "$FULILIAN_HOME/.env" 2>/dev/null; then
        echo "[stage2] Warning: API_SERVER_KEY is set in both the container environment and $FULILIAN_HOME/.env — the .env value wins at runtime (loaded with override=True)"
    else
        ...
        if [ -f "$FULILIAN_HOME/.env" ] && ! refuse_symlinked_path "clean" "$FULILIAN_HOME/.env"; then
            sed -i '/^API_SERVER_KEY=$/d' "$FULILIAN_HOME/.env" 2>/dev/null || true
        fi
        echo "[stage2] API_SERVER_KEY provided via container environment — skipping generation"
    fi
elif ! grep -q '^API_SERVER_KEY=..*' "$FULILIAN_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$FULILIAN_HOME/.env"; then
        :
    else
        if [ ! -f "$FULILIAN_HOME/.env" ]; then
            ...
            (umask 077 && as_fulilian touch "$FULILIAN_HOME/.env") 2>/dev/null || true
        fi
        if [ -f "$FULILIAN_HOME/.env" ]; then
            ...
            if [ -n "$_gen_key" ]; then
                ...
                sed -i '/^API_SERVER_KEY=$/d' "$FULILIAN_HOME/.env" 2>/dev/null || true
                if printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$FULILIAN_HOME/.env" 2>/dev/null; then
                    echo "[stage2] Generated API_SERVER_KEY for the loopback gateway api_server"
                else
                    echo "[stage2] Warning: could not write API_SERVER_KEY to $FULILIAN_HOME/.env (read-only volume?) — gateway api_server (cron fires) will be unavailable"
                fi
            fi
            unset _gen_key
        else
            echo "[stage2] Warning: could not create $FULILIAN_HOME/.env — gateway api_server (cron fires) will be unavailable"
        fi
    fi
fi
...
if [ -f "$FULILIAN_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$FULILIAN_HOME/.env"; then
        :
    else
        chown fulilian:fulilian "$FULILIAN_HOME/.env" 2>/dev/null || true
        chmod 600 "$FULILIAN_HOME/.env" 2>/dev/null || true
    fi
fi
...
# as the unprivileged `fulilian` user — without this it gets EACCES on every
...
# fulilian group (any group member could e.g. stop/suspend this machine), not
...
if [ -S /.fly/api ]; then
    if refuse_symlinked_path "chgrp/chmod" /.fly/api; then
        :
    elif chgrp fulilian /.fly/api 2>/dev/null && chmod g+w /.fly/api 2>/dev/null; then
        echo "[stage2] Granted fulilian group access to the Fly Machines API socket"
    else
...
# $FULILIAN_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `fulilian update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set FULILIAN_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$FULILIAN_HOME/config.yaml" ]; then
    s6-setuidgid fulilian "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$FULILIAN_HOME/auth.json" ] && [ -n "${FULILIAN_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$FULILIAN_HOME/auth.json"; then
        :
    else
        printf '%s' "$FULILIAN_AUTH_JSON_BOOTSTRAP" > "$FULILIAN_HOME/auth.json"
        chown fulilian:fulilian "$FULILIAN_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$FULILIAN_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via FULILIAN_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$FULILIAN_HOME/auth.json" ] && [ -n "${FULILIAN_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$FULILIAN_HOME/auth.json"; then
        :
    else
        s6-setuidgid fulilian "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$FULILIAN_HOME/auth.json" \
...
# can set FULILIAN_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# FULILIAN_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$FULILIAN_HOME/gateway_state.json" ] && \
        [ "${FULILIAN_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$FULILIAN_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$FULILIAN_HOME/gateway_state.json"
        chown fulilian:fulilian "$FULILIAN_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$FULILIAN_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_fulilian "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/fulilian/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Fulilian spawns for the
...
# main-wrapper.sh propagates it into the supervised ``fulilian`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-fulilian, dashboard, etc.).
