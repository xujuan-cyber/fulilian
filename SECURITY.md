# FuLiLian Security Policy

This document describes FuLiLian's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/xujuan-cyber/fulilian/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **FuLiLian does not operate a bug
bounty program.**
...
- Environment details (`fulilian --version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted FuLiLian access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in FuLiLian's documentation
...
FuLiLian supports two OS-level isolation postures. They address
...
FuLiLian supports this in two ways:

- **FuLiLian's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, FuLiLian's in-process heuristics
...
FuLiLian filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in FuLiLian
itself. Bugs in FuLiLian's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   FuLiLian does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, FuLiLian's own documentation, or reasonable
  operator expectations would predict — including cases where
  FuLiLian has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  fulilian-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not FuLiLian's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in FuLiLian. Bugs in Fulilian
...
- FuLiLian includes supply-chain guards for MCP server
