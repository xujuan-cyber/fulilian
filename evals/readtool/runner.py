"""Run the read-tool eval through the REAL Fulilian AIAgent.

For each task: fresh temp FULILIAN_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    fulilian_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".fulilian"
    ...
    try:
        ...
        shutil.rmtree(fulilian_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.fulilian/.env; set +a  — then relaunch."
        )
