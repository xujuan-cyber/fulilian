...
class AIAgent:
    ...
    def _ensure_lmstudio_runtime_loaded(
        ...
        from fulilian_cli.models import ensure_lmstudio_model_loaded
    ...
    def _invoke_api_request_error_hook(
        ...
        *,
        ...
        try:
            from fulilian_cli import lifecycle as _lifecycle
    ...
    def _try_refresh_env_client_credentials(self) -> bool:
        ...
        if (
            ...
            if self.provider == "kimi-coding":
                from fulilian_cli.auth import _resolve_kimi_base_url

                base_url = _resolve_kimi_base_url(
                    api_key, pconfig.inference_base_url, env_url
                ).rstrip("/")
            elif self.provider == "zai":
                from fulilian_cli.auth import _resolve_zai_base_url
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if (
            ...
            try:
                from fulilian_cli.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from fulilian_cli.models import lmstudio_model_reasoning_options
    ...
    def _ollama_supports_thinking_cached(self) -> bool:
        ...
        try:
            from fulilian_cli.models import ollama_model_supports_thinking
    ...
    def run_conversation(
        ...
        from fulilian_cli.observability.relay_shared_metrics import (
            finish_task_run,
            start_task_run,
        )
...
def _build_ctf_system_prompt() -> str:
    ...
    Injects the 4-stage cognitive loop (RECON/PLAN/EXECUTE/REFLECT),
    ...
    return (
        ...
        "\n"
        "### PHASE 1 — RECON（侦察）\n"
        "反隧道视野：至少列出 3 种不同的可能解法方向。\n"
        "不要过早锁定单一方向。\n"
        "- 检查题目类型（web/crypto/reverse/pwn/forensics/misc）\n"
        "- 列出所有可能的攻击面\n"
        "- 从多角度评估题目\n"
        "\n"
        "### PHASE 2 — PLAN（规划）\n"
        "写出具体执行步骤 + ABANDON IF 子句。\n"
        "\n"
        "ABANDON IF 子句声明：\n"
        "- 同一攻击类 3 次变体失败 → 强制切换攻击类\n"
        "- 连续 5 轮无新 Fact（CONFIRMED/REFUTED 不变）→ 切换方向或终止\n"
        "- 明确到达不可达目标（INFRA_BLOCKED）→ 标记 ABANDONED\n"
        "\n"
        "### PHASE 3 — EXECUTE（执行）\n"
        "状态账本：每步更新 CONFIRMED/REFUTED/OPEN/NEXT 状态。\n"
        "- CONFIRMED：已确认的发现（如端口开放、版本号、漏洞存在）\n"
        "- REFUTED：已推翻的假设（如\"XX 漏洞不存在 — 404 返回\"）\n"
        "- OPEN：待探索的方向（如\"尝试 XX 攻击\"）\n"
        "- NEXT：下一步计划\n"
        "- 硬规则：同一攻击类 3 次变体失败 → 强制切换攻击类\n"
        "\n"
        "### PHASE 4 — REFLECT（反思）\n"
        "每次工具调用循环后反思：\n"
        "- 这次学到了什么？\n"
...
def _run_solver_turn(
    ...
    """Create an AIAgent, run a conversation turn, and print summary.

    Args:
        phase_label: Optional label like "[PLAN]" / "[EXECUTE]" for Architect mode.
    Returns:
        dict with keys ``result`` (run_conversation result) and ``agent``
        (the AIAgent instance; None on init failure).
    """
...
def main(
    ...
    Args:
        ...
        mode (str): Run mode — "" (default) or "ctf" (injects CTF cognitive architecture prompt).
        architect_model (str): Expensive model used for planning in Architect mode (CTF only).
        executor_model (str): Cheap model used for execution in Architect mode (CTF only).
    ...
    # F0-005/F4 衔接：CTF 模式默认启用 ctf_solve 工具集
    # （verify_flag / submit_flag / checkpoint 等），除非显式指定了工具集
    if mode == "ctf" and not enabled_toolsets_list:
        enabled_toolsets_list = ["ctf_solve"]
        print("🧩 CTF mode: enabled toolset ctf_solve")
    ...
    # Build CTF ephemeral prompt if mode is "ctf"
    ctf_prompt = _build_ctf_system_prompt() if mode == "ctf" else None

    # Resolve max_turns default: 10 for normal, 30 for CTF
    if max_turns is None:
        max_turns = 30 if mode == "ctf" else 10
    ...
    if mode == "ctf":
        print(f"🧩 CTF Mode: Cognitive architecture injected (max_turns={max_turns})")
    print("\n" + "=" * 50)

    # ── Architect mode (贵模型规划 + 便宜模型执行) ──
    if mode == "ctf" and architect_model:
        plan_turns = min(3, max_turns)
        print(f"\n🏗️  Architect mode: planning with {architect_model} ({plan_turns} turns), "
              f"executing with {executor_model or model} ({max_turns} turns)")

        plan_out = _run_solver_turn(
            query=user_query + "\n\n只做规划，输出 PLAN 步骤，不要执行任何操作。",
            model=architect_model,
            max_turns=plan_turns,
            base_url=base_url, api_key=api_key,
            enabled_toolsets_list=enabled_toolsets_list,
            disabled_toolsets_list=disabled_toolsets_list,
            save_trajectories=save_trajectories,
            verbose=verbose, log_prefix_chars=log_prefix_chars,
            ctf_prompt=ctf_prompt, phase_label="[PLAN]",
        )
