...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated FULILIAN_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="fulilian_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".fulilian"))
    prev_home = os.environ.get("FULILIAN_HOME")
    os.environ["FULILIAN_HOME"] = os.path.join(test_home, ".fulilian")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("fulilian_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["FULILIAN_HOME"] = prev_home
