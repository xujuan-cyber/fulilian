...
class TestHasKnownPricing:
    ...
    def test_unknown_custom_model(self):
        assert _has_known_pricing("FP16_Fulilian_4.5") is False
...
class TestTerminalFormatting:
    def test_terminal_format_has_sections(self, populated_db):
        ...
        assert "Fulilian Insights" in text
...
class TestEdgeCases:
    ...
    def test_custom_model_shows_zero_cost(self, db):
        ...
        db.create_session(session_id="s1", source="cli", model="FP16_Fulilian_4.5")
        ...
        assert "FP16_Fulilian_4.5" in report["overview"]["models_without_pricing"]

        models = report["models"]
        custom = next(m for m in models if m["model"] == "FP16_Fulilian_4.5")
