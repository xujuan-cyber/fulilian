...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate FULILIAN_HOME under tmp_path."""
    monkeypatch.setenv("FULILIAN_HOME", str(tmp_path / ".fulilian"))
    (tmp_path / ".fulilian").mkdir()

    # Force the constants/image cache helpers to re-read FULILIAN_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("fulilian_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_fulilian_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under FULILIAN_HOME — gateway cleanup
