...
``fulilian_subprocess_env`` / ``_sanitize_subprocess_env``, which the factory
...
``FULILIAN_HOME`` or secret-scrub propagation.
...
SCAN_DIRS = ("agent", "fulilian_cli", "tools", "gateway", "cron", "tui_gateway")
SCAN_ROOT_FILES = ("cli.py", "fulilian_constants.py")
...
ALLOWED_RAW_SPAWN_ENV_FILES = {
    # THE owner module: _sanitize_subprocess_env / fulilian_subprocess_env /
