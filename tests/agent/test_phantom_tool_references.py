...
Covers:
  * FULILIAN_AGENT_HELP_GUIDANCE degrades to the docs-only variant when the
...
class TestFulilianAgentHelpGuidance:
    def test_skill_variant_used_when_skill_view_present(self):
        from agent.prompt_builder import FULILIAN_AGENT_HELP_GUIDANCE
        assert "skill_view(name='fulilian-agent')" in FULILIAN_AGENT_HELP_GUIDANCE
...
class TestEssentialSkillsUndisableable:
    def test_agent_side_reader_strips_essential(self, monkeypatch, tmp_path):
        ...
        assert "fulilian-agent" not in disabled
    ...
    def test_skill_manage_delete_refused(self):
        ...
        msg = _pinned_guard("fulilian-agent")
...
class TestEssentialOnlySync:
    def test_opted_out_sync_seeds_only_essential(self, monkeypatch, tmp_path):
        """A profile with .no-bundled-skills still gets the fulilian-agent skill."""
        ...
        for cat, name in [
            ("autonomous-ai-agents", "fulilian-agent"),
            ("media", "gif-search"),
        ]:
            d = bundled / cat / name
        ...
        monkeypatch.setattr(ss, "_fulilian_home", lambda: home)
        ...
        assert result["copied"] == ["fulilian-agent"]
        assert (home / "skills" / "autonomous-ai-agents" / "fulilian-agent" / "SKILL.md").exists()
