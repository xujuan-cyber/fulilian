...
class TestTruncateContent:
    ...
    def _reset_truncation_state(self, monkeypatch):
        ...
        monkeypatch.setattr("fulilian_cli.config.load_config", default_load_config)
        monkeypatch.setattr("fulilian_cli.config.load_config_readonly", default_load_config)
    ...
    def test_truncation_warning_points_to_config_key(self, monkeypatch):
        ...
        monkeypatch.setattr("fulilian_cli.config.load_config", fake_load_config)
        monkeypatch.setattr("fulilian_cli.config.load_config_readonly", fake_load_config)
...
class TestDynamicContextFileCap:
    ...
    def _no_explicit_config(self, monkeypatch):
        ...
        monkeypatch.setattr("fulilian_cli.config.load_config_readonly", lambda: {})
...
class TestBuildContextFilesPrompt:
    ...
    def test_fulilian_md_still_wins_over_agents_override(self, tmp_path):
        (tmp_path / ".fulilian.md").write_text("Fulilian-first context.")
        (tmp_path / "AGENTS.override.md").write_text("Override context.")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert "Fulilian-first context" in result
    ...
    def test_empty_soul_md_adds_nothing(self, tmp_path, monkeypatch):
        monkeypatch.setenv("FULILIAN_HOME", str(tmp_path / "fulilian_home"))
        fulilian_home = tmp_path / "fulilian_home"
        fulilian_home.mkdir()
        (fulilian_home / "SOUL.md").write_text("\n\n", encoding="utf-8")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert result == ""
    ...
    # --- .fulilian.md / FULILIAN.md discovery ---
...
# =========================================================================
# .fulilian.md helper functions
# =========================================================================


class TestFindFulilianMd:
    def test_finds_in_cwd(self, tmp_path):
        (tmp_path / ".fulilian.md").write_text("rules")
        assert _find_fulilian_md(tmp_path) == tmp_path / ".fulilian.md"



    def test_walks_to_git_root(self, tmp_path):
        (tmp_path / ".git").mkdir()
        (tmp_path / ".fulilian.md").write_text("root rules")
        sub = tmp_path / "a" / "b"
        sub.mkdir(parents=True)
        assert _find_fulilian_md(sub) == tmp_path / ".fulilian.md"
    ...
    def test_no_git_root_checks_cwd_only(self, tmp_path):
        ...
        the way to / and pick up a .fulilian.md planted in /tmp, /home, or /
...
class TestEnvironmentHints:
    ...
    def test_environment_hint_from_env_var_is_appended(self, monkeypatch):
        """FULILIAN_ENVIRONMENT_HINT lets an embedder describe the runtime env."""
        import agent.prompt_builder as _pb
        monkeypatch.setattr(_pb, "is_wsl", lambda: False)
        monkeypatch.delenv("TERMINAL_ENV", raising=False)
        monkeypatch.setenv("FULILIAN_ENVIRONMENT_HINT", "Running inside an OpenShell sandbox.")
        _pb._clear_backend_probe_cache()
        result = _pb.build_environment_hints()
