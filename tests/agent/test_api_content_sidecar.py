...
class TestPrologueStamping:
    ...
    def test_no_stamp_without_injections(self):
        agent = _FakeAgent()
        with patch("fulilian_cli.plugins.invoke_hook", return_value=[]):
            ctx = _build(agent)
...
@pytest.fixture()
def wire_env():
    """Mock provider + isolated FULILIAN_HOME + a shared SessionDB.
    ...
    test_home = tempfile.mkdtemp(prefix="fulilian_api_content_")
