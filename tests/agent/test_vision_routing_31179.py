...
def isolated_home(monkeypatch):
    """Temp FULILIAN_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="fulilian_test_31179_")
    fulilian_home = os.path.join(test_home, ".fulilian")
    os.makedirs(fulilian_home)
...
def _fresh_modules():
    """Drop cached fulilian modules so each test reloads against current env."""
    _drop_reload_targets()
