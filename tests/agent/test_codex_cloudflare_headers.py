...
The official Codex endpoint must receive Fulilian' own harness identity, rather
...
These tests pin:
- the required Fulilian originator
- the versioned Fulilian User-Agent
...
class TestCodexCloudflareHeaders:
    ...
    def test_user_agent_advertises_fulilian_version(self):
...
class TestPrimaryClientWiring:
    ...
    def test_apply_client_headers_on_base_url_change(self):
        ...
        with patch("run_agent.OpenAI") as mock_openai:
            ...
            assert headers.get("originator") == "fulilian-agent"
            assert headers.get("ChatGPT-Account-ID") == "acct-rotation"
            assert headers.get("User-Agent") == f"FulilianAgent/{__version__}"
