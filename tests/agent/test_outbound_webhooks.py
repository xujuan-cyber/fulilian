...
class TestDelivery:
    def test_delivery_with_hmac_signature(self, http_server):
        ...
        assert req["headers"]["X-Fulilian-Event"] == "on_session_end"
        assert req["headers"]["X-Fulilian-Delivery"]
        expected = hmac.new(
            secret.encode(), req["body"], hashlib.sha256
        ).hexdigest()
        assert req["headers"]["X-Fulilian-Signature-256"] == f"sha256={expected}"
    ...
    def test_unsigned_delivery_has_no_signature_header(self, http_server):
        ...
        assert len(http_server.captured) == 1
        assert "X-Fulilian-Signature-256" not in http_server.captured[0]["headers"]
    ...
    def test_delivery_id_matches_header_and_body(self, http_server):
        """The X-Fulilian-Delivery header and the signed body's delivery_id
        ...
        req = http_server.captured[0]
        payload = json.loads(req["body"])
        assert payload["delivery_id"] == req["headers"]["X-Fulilian-Delivery"]
    ...
    def test_events_enqueued_at_exit_still_delivered(self, http_server, tmp_path):
        """A short-lived process (`fulilian chat -q`, cron) exits right after
