...
from fulilian_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(fulilian_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <fulilian_home>/plugins/<name> and opt it in."""
    plugin_dir = fulilian_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("FULILIAN_HOME", str(tmp_path / "fulilian_empty"))
