...
def test_root_session_no_compression(db):
    ...
    assert prov["currentFulilianSessionId"] == "root1"
    assert prov["rootFulilianSessionId"] == "root1"
    assert prov["parentFulilianSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_fulilian_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentFulilianSessionId"] == "old"
    assert prov["rootFulilianSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousFulilianSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootFulilianSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"fulilian"}
    assert "sessionProvenance" in meta["fulilian"]
    assert meta["fulilian"]["sessionProvenance"]["currentFulilianSessionId"] == "root1"
