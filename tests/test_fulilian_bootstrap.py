"""Tests for fulilian_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Fulilian entry point
(fulilian, fulilian-agent, fulilian-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Fulilian entry point imports fulilian_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported fulilian_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("fulilian_bootstrap", None)
    import fulilian_bootstrap  # noqa: WPS433
    return fulilian_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Fulilian CLI
...
class TestEntryPointsImportBootstrap:
    """Every Fulilian entry point must import fulilian_bootstrap as its
    ...
    # Entry points that invoke Fulilian as a process.  Each one must
    # import fulilian_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "fulilian_cli/main.py",   # fulilian CLI (console_script)
        "run_agent.py",          # fulilian-agent (console_script)
        "acp_adapter/entry.py",  # fulilian-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import fulilian_bootstrap' and that
        ...
        half-finished ``fulilian update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``fulilian_bootstrap`` as a top-level module) leaves fulilian
        ...
        """
        # Resolve relative to the fulilian-agent repo root.  Tests live
        # at tests/test_fulilian_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # fulilian start even when fulilian_bootstrap hasn't been
        ...
        assert first_import_name == "fulilian_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'fulilian_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import fulilian_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Fulilian's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("FULILIAN_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["FULILIAN_PYTHON_SRC_ROOT"] = env
            elif "FULILIAN_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["FULILIAN_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/fulilian")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("FULILIAN_PYTHON_SRC_ROOT", None)
            else:
                os.environ["FULILIAN_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/fulilian", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Fulilian root.  The guard must relocate Fulilian to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/fulilian"])
        assert result[0] == "/opt/fulilian"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Fulilian root.
        assert result.index("/opt/fulilian") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["FULILIAN_PYTHON_SRC_ROOT"] = "/env/fulilian"
            hb.harden_import_path()
            assert sys.path[0] == "/env/fulilian"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("FULILIAN_PYTHON_SRC_ROOT", None)
