...
Uses a throwaway FULILIAN_HOME so it never touches ~/.fulilian. No real names.
"""
...
def _fresh_home():
    """Point FULILIAN_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_fulilian_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["FULILIAN_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER FULILIAN_HOME is set.
...
# a different get_fulilian_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"FULILIAN_HOME (throwaway): {HOME}")
