"""Unit tests for ``fulilian_cli.proxy_cli`` command handlers.
...
from fulilian_cli import proxy_cli


@pytest.fixture
def fulilian_home(tmp_path, monkeypatch):
    """Point FULILIAN_HOME at a temp dir so the wizard doesn't touch the
...
def test_cmd_install_success_returns_0(fulilian_home, monkeypatch):
    monkeypatch.setattr(ip, "install_iron_proxy", lambda **kw: fulilian_home / "iron-proxy")
    monkeypatch.setattr(ip, "iron_proxy_version", lambda b: "v0.39.0-test")
    rc = proxy_cli.cmd_install(_args())
    assert rc == 0


def test_cmd_install_failure_returns_1(fulilian_home, monkeypatch):
    def boom(**kw):
...
def test_cmd_setup_from_bitwarden_refuses_on_empty_vault(fulilian_home, monkeypatch):
    ...
    monkeypatch.setattr(ip, "find_iron_proxy", lambda **kw: fulilian_home / "iron-proxy")
    monkeypatch.setattr(ip, "iron_proxy_version", lambda b: "test")
    monkeypatch.setattr(
        ip, "ensure_ca_cert",
        lambda **kw: (fulilian_home / "ca.crt", fulilian_home / "ca.key"),
    )
...
def test_cmd_restart_propagates_start_failure(fulilian_home, monkeypatch):
...
# ---------------------------------------------------------------------------
# _load_env_file_into_environ — setup discovers keys kept only in ~/.fulilian/.env
# ---------------------------------------------------------------------------
...
def test_cmd_status_returns_0(fulilian_home, monkeypatch):
...
def test_cmd_config_returns_0_when_present(fulilian_home, monkeypatch):
    fake = ip.ProxyStatus()
    fake.config_path = fulilian_home / "proxy.yaml"
...
def test_register_cli_uses_egress_command_dest():
    ...
    the inbound OAuth 'fulilian proxy' subparser (dest='proxy_command').
    A future grep-and-refactor on proxy_command should not hit this
    subparser by accident."""

    parser = argparse.ArgumentParser(prog="fulilian egress")
...
def test_cmd_start_refuses_when_bitwarden_mode_but_disabled(fulilian_home, monkeypatch):
...
def test_cmd_setup_audit_log_failure_is_warning_not_abort(fulilian_home, monkeypatch):
    ...
    monkeypatch.setattr(ip, "discover_provider_mappings", lambda **kw: [
        ip.TokenMapping(
            proxy_token="fulilian-proxy-deadbeef",
