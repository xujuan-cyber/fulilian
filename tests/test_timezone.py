"""
Tests for timezone support (fulilian_time module + integration points).
...
import fulilian_time


def _reset_fulilian_time_cache():
    """Reset the fulilian_time module cache (replacement for removed reset_cache)."""
    fulilian_time._cached_tz = None
    fulilian_time._cached_tz_name = None
    fulilian_time._cache_resolved = False


# =========================================================================
# fulilian_time.now() — core helper
# =========================================================================

class TestFulilianTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_fulilian_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["FULILIAN_TIMEZONE"] = "Asia/Kolkata"
        result = fulilian_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["FULILIAN_TIMEZONE"] = "UTC"
        result = fulilian_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["FULILIAN_TIMEZONE"] = "Europe/London"
        tz = fulilian_time.get_timezone()
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When FULILIAN_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) FULILIAN_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("FULILIAN_TIMEZONE=" + os.environ.get("FULILIAN_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "FULILIAN_TIMEZONE=NOT_SET" in result["output"], (
            "FULILIAN_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When FULILIAN_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("FULILIAN_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=fulilian_tz) which shifted
        absolute time when system-local tz != Fulilian tz.  The fix interprets
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Fulilian tz
        ...
        # Use a Fulilian timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _fulilian_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["FULILIAN_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_fulilian_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Fulilian timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["FULILIAN_TIMEZONE"] = "US/Eastern"
        _reset_fulilian_time_cache()
