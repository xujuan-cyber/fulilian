...
def cron_store(tmp_path, monkeypatch):
    ...
    (fulilian_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("FULILIAN_HOME", str(fulilian_home))
    monkeypatch.setattr(J, "FULILIAN_DIR", fulilian_home)
    monkeypatch.setattr(J, "CRON_DIR", fulilian_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", fulilian_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", fulilian_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_fulilian_now", lambda: now):
            due = J.get_due_jobs()
