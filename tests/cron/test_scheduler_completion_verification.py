...
def _run_booked_job(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(fulilian_state, "SessionDB", _RecordingSessionDB)
    monkeypatch.setattr(run_agent, "AIAgent", _FakeCronAgent)
    monkeypatch.setattr(
        "fulilian_constants.resolve_reasoning_config", lambda *_a, **_k: None
    )
    ...
    monkeypatch.setenv("FULILIAN_TEST_RUNTIME_KEY", "unused-placeholder")

    def _fake_runtime(**_kwargs):
        return {
            "api_key": os.environ.get("FULILIAN_TEST_RUNTIME_KEY", ""),
            "base_url": None,
    ...
    monkeypatch.setattr(
        "fulilian_cli.runtime_provider.resolve_runtime_provider", _fake_runtime
    )
