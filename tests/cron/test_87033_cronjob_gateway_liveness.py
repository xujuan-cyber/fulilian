...
(``fulilian cron list`` / ``fulilian cron status``); the agent path did not.
...
def fulilian_env(tmp_path, monkeypatch):
    """Isolate FULILIAN_HOME for each test so jobs don't leak."""
...
class TestCreateSurfacesGatewayLiveness:
    def test_create_with_gateway_running_has_no_warning(self, fulilian_env):
    ...
    def test_create_without_gateway_warns_not_scheduled(self, fulilian_env):
        with (
    ...
    def test_non_builtin_provider_is_exempt(self, fulilian_env):
    ...
    def test_failed_probe_stays_neutral(self, fulilian_env):
...
class TestListSurfacesGatewayLiveness:
    ...
    def test_list_with_gateway_running_has_no_warning(self, fulilian_env):
    ...
    def test_list_without_gateway_warns_jobs_inert(self, fulilian_env):
        _create_job()
    ...
    def test_list_empty_without_gateway_stays_quiet(self, fulilian_env):
    ...
    def test_list_non_builtin_provider_is_exempt(self, fulilian_env):
        _create_job()
        with patch_liveness(provider="chronos", pids=[]):
            result = self._list_jobs()
...
class _LivenessPatches:
    ...
    def __enter__(self):
        ...
        self._stack.enter_context(
            patch(
                "fulilian_cli.cron._active_cron_provider_name",
...
class TestRuntimeLockFirstLiveness:
    ...
    def test_lock_active_reports_alive_despite_empty_pid_scan(self, fulilian_env):
    ...
    def test_lock_inactive_falls_back_to_pid_scan(self):
        ...
        import fulilian_cli.cron as cron_cli

        with (
            patch("fulilian_cli.cron._active_cron_provider_name", return_value="builtin"),
            patch("gateway.status.is_gateway_runtime_lock_active", return_value=False),
            patch("fulilian_cli.gateway.find_gateway_pids", return_value=[424242]),
        ):
    ...
    def test_no_lock_no_pids_is_false(self):
        ...
        with (
            patch("fulilian_cli.cron._active_cron_provider_name", return_value="builtin"),
            patch("gateway.status.is_gateway_runtime_lock_active", return_value=False),
            patch("fulilian_cli.gateway.find_gateway_pids", return_value=[]),
        ):
...
class TestCronStatusLockFirst:
    """`fulilian cron status` shares the lock-first false-alarm fix (#95947).
    ...
    def _run_status(self, *, pids, lock_active, lock_pid=None):
        ...
        with (
            patch("fulilian_cli.cron._active_cron_provider_name", return_value="builtin"),
            patch("fulilian_cli.gateway.find_gateway_pids", return_value=list(pids)),
            patch(
                "gateway.status.is_gateway_runtime_lock_active",
    ...
    def test_lock_active_suppresses_not_running_false_alarm(self, fulilian_env):
    ...
    def test_no_lock_no_pids_still_warns(self, fulilian_env):
