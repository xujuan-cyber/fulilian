...
profile's FULILIAN_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.fulilian/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active FULILIAN_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever FULILIAN_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    FULILIAN_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import fulilian_constants

    monkeypatch.setattr(
        fulilian_constants, "_get_platform_default_fulilian_home", lambda: root
    )
    monkeypatch.setenv("FULILIAN_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile FULILIAN_HOME (<root>/profiles/<name>), the cron store
    ...
    assert fulilian_constants.get_fulilian_home().resolve() == profile_home.resolve()
    assert fulilian_constants.get_default_fulilian_root().resolve() == root.resolve()

    # cron/jobs.py computes FULILIAN_DIR from get_fulilian_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.FULILIAN_DIR.resolve() == profile_home.resolve()
        assert (
