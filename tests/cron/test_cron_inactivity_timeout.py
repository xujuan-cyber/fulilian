...
- Unlimited timeout (FULILIAN_CRON_TIMEOUT=0)
- Backward compat: FULILIAN_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """FULILIAN_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """FULILIAN_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("FULILIAN_CRON_TIMEOUT", "1200")
...
class TestSysPathOrdering:
    ...
    def test_fulilian_time_importable(self):
        """fulilian_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _fulilian_now
        assert callable(_fulilian_now)
