...
class TestGoogleWorkspaceCredentialFiles:
    ...
    def test_entries_are_registered_when_files_exist(self, tmp_path):
        ...
        fulilian_home.mkdir()
        (fulilian_home / "google_token.json").write_text("{}")
        (fulilian_home / "google_client_secret.json").write_text("{}")
        ...
        try:
            ...
            assert "/root/.fulilian/google_token.json" in container_paths
            assert "/root/.fulilian/google_client_secret.json" in container_paths
        finally:
            clear_credential_files()
