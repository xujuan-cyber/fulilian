...
def test_frontmatter_required_fields():
    ...
    fulilian = fm["metadata"]["fulilian"]
    assert fulilian["tags"]
    assert "related_skills" in fulilian
...
def test_author_credits_human_first():
    fm, _ = _frontmatter_and_body()
    assert not fm["author"].startswith("FuLiLian"), "human contributor must be credited first"
