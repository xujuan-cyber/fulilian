...
class TestApprovalModeParsing:
    ...
    def test_config_bool_false_maps_to_off(self):
        with mock_patch("fulilian_cli.config.load_config_readonly", return_value={"approvals": {"mode": False}}):
...
class TestSmartApproval:
    ...
    def test_smart_approval_does_not_allowlist_the_pattern_for_session(self, monkeypatch):
        ...
        monkeypatch.setenv("FULILIAN_SESSION_KEY", session_key)
...
class TestDetectDangerousRm:
    ...
    def test_nonrecursive_verification_artifact_cleanup_is_not_dangerous(self):
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
            for prefix in ("fulilian-verify-", "fulilian-ad-hoc-"):
    ...
    def test_symlinked_temp_dir_only_exempts_canonical_target(self, tmp_path):
        ...
        basename = "fulilian-verify-example.py"
    ...
    def test_verification_cleanup_exemption_rejects_broader_deletions(self):
        commands = (
            "rm -rf /tmp/fulilian-verify-example.py",
            "rm -f /tmp/fulilian-verify-example.py /tmp/other.py",
            "rm -f /tmp/nested/../fulilian-verify-example.py",
            "rm -f /tmp/a/../../tmp/fulilian-verify-example.py",
            "rm -f /var/tmp/fulilian-verify-example.py",
            "rm -f /tmp/fulilian-verify-*",
            "rm -f /tmp/fulilian-verify-$(touch>/tmp/pwned).py",
            "rm -f /tmp/fulilian-ad-hoc-`touch>/tmp/pwned`.py",
            "rm -f /tmp/fulilian-verify-example.py; touch /tmp/pwned",
        )
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
...
class TestWindowsShellDestructiveCommands:
    def test_windows_destructive_requires_approval(self):
        cases = [
            (r"cmd /c del /f /q C:\tmp\fulilian-victim\file.txt", "Windows cmd destructive delete"),
            (r"cmd.exe /k rmdir /s /q C:\tmp\fulilian-victim", "Windows cmd destructive delete"),
            ...
            (r"powershell Remove-Item -Recurse -Force C:\tmp\fulilian-victim",
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"FULILIAN_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_to_sensitive_target(self):
        for command in (
            ...
            "echo x | tee /dev/sda",
            "echo x | tee ~/.fulilian/.env",
            "echo x | tee $FULILIAN_HOME/.env",
            'echo x | tee "$FULILIAN_HOME/.env"',
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
...
class TestFulilianConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.fulilian/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_write_idioms_against_config(self):
        for command in (
            "echo 'approvals:' > ~/.fulilian/config.yaml",
            "echo '  mode: off' >> ~/.fulilian/config.yaml",
            "echo x | tee ~/.fulilian/config.yaml",
            "echo x | tee $FULILIAN_HOME/config.yaml",
            "cp /tmp/evil.yaml ~/.fulilian/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
            assert key is not None, command
    ...
    def test_reads_and_unrelated_writes_are_safe(self):
        # Reading config is not a write; a non-Fulilian absolute config.yaml is
        # handled by the project patterns, not the Fulilian-home rule.
        for cmd in (
            "cat ~/.fulilian/config.yaml",
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.fulilian/config.yaml/.env must require approval — the
...
class TestWindowsAbsolutePathFolding:
    """Windows absolute home / Fulilian-home prefixes must fold to ~/ and
    ~/.fulilian/ in dangerous-command detection.
    ...
    SSH, and Fulilian config/env files returned "safe" without an approval prompt.
    The OS-specific ``Path.home()`` / ``get_fulilian_home()`` tests above only
    exercise this branch on a Windows host; these monkeypatch a Windows-style
    HOME/FULILIAN_HOME so the fold is verified on the POSIX CI runner too."""
...
class TestSmartDeniedPrompt:
    ...
    def test_smart_deny_uses_locale_specific_once_deny_choices(self, monkeypatch, capsys):
        monkeypatch.setenv("FULILIAN_LANGUAGE", "tr")
...
class TestGatewayProtection:
    ...
    def test_gateway_run_backgrounded_detected(self):
        cmd = "kill 1605 && cd ~/.fulilian/fulilian-agent && source venv/bin/activate && python -m fulilian_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc
        for variant in (
            "python -m fulilian_cli.main gateway run --replace &",
            "nohup python -m fulilian_cli.main gateway run --replace",
        ):
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart fulilian-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_forms_still_flagged_dangerous(self):
        for cmd in (
            ...
            # In-place edit of the Fulilian security config via IFS.
            "sed${IFS}-i ~/.fulilian/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(cmd)
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep fulilian) bypasses the pkill/killall name-matching
    ...
    def test_kill_pgrep_expansion_detected(self):
        for cmd in (
            'kill -9 $(pgrep -f "fulilian.*gateway")',
            "kill -9 `pgrep fulilian`",
            "kill $(pgrep gateway)",
        ):
            dangerous, _, desc = detect_dangerous_command(cmd)
            assert dangerous is True, cmd
    ...
    def test_kill_pidof_expansion_detected(self):
        """`kill $(pidof fulilian)` is the BSD/Linux equivalent of the
        pgrep expansion and bypasses the pkill/killall name pattern
        in the same way. See issue #33071."""
        dangerous, _, desc = detect_dangerous_command("kill -TERM $(pidof fulilian_cli.main)")
        assert dangerous is True
        assert "pidof" in desc.lower() or "pgrep" in desc.lower()
        assert detect_dangerous_command("kill -9 `pidof fulilian`")[0] is True
...
class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the Fulilian service
    label achieves the same effect as `fulilian gateway stop|restart` and
    must require the same approval. See issue #33071.
    """

    def test_launchctl_against_fulilian_label_detected(self):
        for cmd in (
            "launchctl stop ai.fulilian.gateway",
            "launchctl kickstart -k system/ai.fulilian.gateway",
            "launchctl bootout system/ai.fulilian.gateway",
            "launchctl unload ~/Library/LaunchAgents/ai.fulilian.gateway.plist",
        ):
            dangerous, _, desc = detect_dangerous_command(cmd)
            assert dangerous is True, cmd

    def test_unrelated_labels_not_flagged(self):
        """Read-only inspection, and lifecycle ops on non-Fulilian labels, are
    ...
    def test_spliced_detection_does_not_flag_prose_or_other_services(self):
        ...
        a fulilian-gateway identifier, so quoted prose and non-gateway fulilian
    ...
    def test_label_built_before_verb_detected(self):
        ...
        "fulilian"/"ai.fulilian" to appear AFTER the verb and missed this
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("FULILIAN_GATEWAY_SESSION", "FULILIAN_CRON_SESSION",
                      "FULILIAN_YOLO_MODE",
                      "FULILIAN_SESSION_KEY", "FULILIAN_INTERACTIVE")
        }
        os.environ.pop("FULILIAN_YOLO_MODE", None)
        os.environ.pop("FULILIAN_INTERACTIVE", None)
        # FULILIAN_CRON_SESSION takes priority over FULILIAN_GATEWAY_SESSION in
        ...
        os.environ.pop("FULILIAN_CRON_SESSION", None)
        os.environ["FULILIAN_GATEWAY_SESSION"] = "1"
        os.environ["FULILIAN_SESSION_KEY"] = self.SESSION_KEY
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_import_error_allows_when_fail_open_or_disabled(self, enabled, fail_open):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("fulilian_cli.config.load_config_readonly", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"FULILIAN_INTERACTIVE": "1"}, clear=False):
...
GATEWAY_LIFECYCLE_LAUNCHCTL = (
    ...
    "launchctl unload ~/Library/LaunchAgents/ai.fulilian.gateway.plist",
    "launchctl load ~/Library/LaunchAgents/ai.fulilian.gateway.plist",
    "launchctl stop ai.fulilian.gateway",
    "launchctl restart ai.fulilian.gateway",
    "launchctl bootout gui/501/ai.fulilian.gateway",
    "launchctl remove ai.fulilian.gateway",
    "launchctl disable gui/501/ai.fulilian.gateway",
)
...
class TestLifecycleGuardLaunchctlParity:
    ...
    under ``_FULILIAN_GATEWAY == "1"``, the ``cron.lifecycle_guard`` block is
    ...
    def test_unrelated_labels_are_not_blocked(self):
        ...
        services, including other Fulilian ones, stay runnable."""
        from cron.lifecycle_guard import contains_gateway_lifecycle_command

        for cmd in (
            "launchctl bootout gui/501/com.example.unrelated",
            "launchctl remove ai.fulilian.update-checker",
