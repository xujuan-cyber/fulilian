...
def test_helper_defaults_false(monkeypatch):
...
def test_helper_reads_config_true(monkeypatch):
    import fulilian_cli.config as cfgmod

    monkeypatch.setattr(cfgmod, "load_config_readonly", lambda: _cfg(True))
    assert approval._dangerous_command_fail_closed() is True


def test_helper_tolerates_config_failure(monkeypatch):
    import fulilian_cli.config as cfgmod
