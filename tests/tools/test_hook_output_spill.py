...
class GetSpillConfigTests(unittest.TestCase):
    ...
    def test_load_config_exception_is_swallowed(self):
        with patch("fulilian_cli.config.load_config", side_effect=RuntimeError("bad")):
            cfg = hos.get_spill_config()
        self.assertEqual(cfg["max_chars"], hos.DEFAULT_MAX_CHARS)
        self.assertTrue(cfg["enabled"])
...
class SpillIfOversizedTests(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="fulilian-spill-test-")
    ...
    def test_default_directory_uses_fulilian_home(self):
        """When no directory override, spill under FULILIAN_HOME/hook_outputs."""
        test_home = tempfile.mkdtemp(prefix="fulilian-home-")
        try:
            with patch.dict(os.environ, {"FULILIAN_HOME": test_home}):
                # Also patch get_fulilian_home to the env var to mirror production.
            ...
            # ~/.fulilian/hook_outputs depending on get_fulilian_home behaviour.
            candidates = [
                Path(test_home) / "hook_outputs" / "sess",
                Path(os.path.expanduser("~/.fulilian/hook_outputs/sess")),
            ]
