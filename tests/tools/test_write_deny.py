...
class TestWriteDenyExactPaths:
    ...
    def test_fulilian_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.fulilian/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "fulilian_root"
        ...
        # Sanity check: FULILIAN_HOME does point to the profile dir, not the root.
        from fulilian_constants import get_fulilian_home, get_default_fulilian_root
...
class TestWriteAllowed:
    ...
    def test_fulilian_control_files_requested_writable(self):
