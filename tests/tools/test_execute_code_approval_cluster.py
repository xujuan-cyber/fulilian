...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad FULILIAN_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: FULILIAN_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_fulilian_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "FULILIAN_HOME": "/h", "FULILIAN_PROFILE": "p",
        "FULILIAN_CONFIG": "/c.yaml", "FULILIAN_ENV": "/e",
        "FULILIAN_DELEGATED_CHILD_CONTEXT": "1",
        # other FULILIAN_* → dropped (broad prefix removed)
        "FULILIAN_BASE_URL": "https://x", "FULILIAN_INTERACTIVE": "1",
        "FULILIAN_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "FULILIAN_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "FULILIAN_BASE_URL", "FULILIAN_INTERACTIVE", "FULILIAN_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped FULILIAN_* vars."""
    import logging
