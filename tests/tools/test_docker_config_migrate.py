...
def _run_migration(fulilian_home: Path, **env_overrides: str) -> subprocess.CompletedProcess[str]:
    env = os.environ.copy()
    env.update(
        {
            "FULILIAN_HOME": str(fulilian_home),
            "FULILIAN_SKIP_CHMOD": "1",
...
def test_docker_config_migrate_does_not_rewrite_invalid_yaml(tmp_path: Path) -> None:
    ...
    assert proc.returncode == 0, proc.stderr
    assert "Migrating config schema" not in proc.stdout
    assert "fulilian config:" in proc.stderr
    assert config_path.read_text(encoding="utf-8") == original
    assert not list(tmp_path.glob("*.bak-*"))
...
def test_docker_config_migrate_skip_env_leaves_config_unchanged(tmp_path: Path) -> None:
    ...
    proc = _run_migration(tmp_path, FULILIAN_SKIP_CONFIG_MIGRATION="1")
...
def test_docker_config_migrate_second_boot_preserves_env_byte_for_byte(tmp_path: Path) -> None:
    ...
    ``$FULILIAN_HOME/.env``. The first boot migrates the stale config and bumps
