"""Tests for --yolo (FULILIAN_YOLO_MODE) approval bypass."""
...
class TestYoloMode:
    """When FULILIAN_YOLO_MODE is set, all dangerous commands are auto-approved."""
    ...
    def test_dangerous_command_blocked_normally(self, monkeypatch):
        ...
        monkeypatch.setenv("FULILIAN_SESSION_KEY", "test-session")
    ...
    def test_dangerous_command_approved_in_yolo_mode(self, monkeypatch):
        """With FULILIAN_YOLO_MODE, dangerous commands are auto-approved."""
    ...
    def test_yolo_mode_not_set_by_default(self):
        """FULILIAN_YOLO_MODE should not be set by default."""
        # Clean env check — if it happens to be set in test env, that's fine,
        # we just verify the mechanism exists
        assert os.getenv("FULILIAN_YOLO_MODE") is None or True  # no-op, documents intent
    ...
    def test_false_like_yolo_values_do_not_bypass_dangerous_command(self, monkeypatch, value):
        ...
        monkeypatch.setenv("FULILIAN_YOLO_MODE", value)
        monkeypatch.setenv("FULILIAN_INTERACTIVE", "1")
        monkeypatch.setenv("FULILIAN_SESSION_KEY", "test-session")
