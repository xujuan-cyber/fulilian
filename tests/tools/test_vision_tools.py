        ...
        async def resolve(config=None, env_model=None):
            with ExitStack() as st:
                ...
                if config is not None:
                    st.enter_context(patch(
                        "fulilian_cli.config.load_config", return_value=config,
                    ))
...
class TestVisionConfig:
    ...
    async def test_temperature_and_timeout_come_from_config_with_defaults(self, tmp_path):
        ...
        async def call_with(config):
            ...
            with (
                patch("fulilian_cli.config.load_config", return_value=config),
                patch(
                    "tools.vision_tools._image_to_base64_data_url",
    ...
    def test_worker_count_tracks_host_cpus_with_env_override(self):
        ...
        def resolve(host_cpus, env_value=None):
            with (
                ...
                patch("fulilian_cli.config.load_config", side_effect=Exception),
            ):
                if env_value is None:
                    os.environ.pop("FULILIAN_VISION_MAX_CONCURRENCY", None)
                else:
                    os.environ["FULILIAN_VISION_MAX_CONCURRENCY"] = env_value
