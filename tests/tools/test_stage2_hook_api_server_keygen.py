...
was gated on ``[ -f "$FULILIAN_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        "set -eu\n"
        f"{env_setup}"
        f'FULILIAN_HOME="{home}"\n'
        # In tests we run unprivileged; as_fulilian is a passthrough then.
        'as_fulilian() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    Fulilian loads $FULILIAN_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/fulilian/.env.example -> $FULILIAN_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $FULILIAN_HOME/.env from it on first boot"
    )
