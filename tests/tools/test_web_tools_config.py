...
class TestBackendSelection:
    ...
    ``fulilian tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
...
class TestFirecrawlEnvResolution:
    """Verify Firecrawl reads env values from fulilian_cli.config.get_env_value,
    not just os.getenv.  This catches the regression reported in #40190 where
    values stored in ~/.fulilian/.env were invisible to the provider."""
...
class TestSiblingProvidersEnvResolution:
    ...
    ~/.fulilian/.env), not bare os.getenv. Parametrized over the four
    ...
    def test_tavily_request_reads_key_via_get_env_value(self, monkeypatch):
        ...
        with patch(
            ...
            assert headers["X-Client-Name"] == "fulilian-agent"
