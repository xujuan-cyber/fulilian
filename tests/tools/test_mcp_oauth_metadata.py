...
Covers:
- :class:`FulilianTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.FulilianMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _FULILIAN_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# FulilianTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = FulilianTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("FULILIAN_HOME", str(tmp_path))
        storage = FulilianTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (FulilianMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: FulilianTokenStorage, **context_attrs):
    ...
    provider = _FULILIAN_PROVIDER_CLS.__new__(_FULILIAN_PROVIDER_CLS)
    provider._fulilian_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("FULILIAN_HOME", str(tmp_path))
        storage = FulilianTokenStorage("mgr-srv")
        ...
        with patch.object(
            _FULILIAN_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("FULILIAN_HOME", str(tmp_path))
        storage = FulilianTokenStorage("flow-srv")
        ...
        with patch.object(
            _FULILIAN_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",
