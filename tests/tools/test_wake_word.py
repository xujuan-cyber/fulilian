"""Tests for tools.wake_word — the "Hey Fulilian" hotword detector.
...
def test_config_defaults_and_clamping():
    ...
    assert ww.wake_phrase({"phrase": "hey fulilian"}) == "hey fulilian"
    assert ww.wake_phrase({}) == "hey fulilian"
...
def test_looks_like_path():
    assert ww._looks_like_path("models/hey_fulilian.onnx")
...
def test_load_wake_word_config_guards_non_dict(monkeypatch):
    monkeypatch.setattr(
        "fulilian_cli.config.load_config", lambda: {"wake_word": "oops"}
    )
...
def test_requirements_openwakeword_available(monkeypatch):
    ...
    r = ww.check_wake_word_requirements(
        {"provider": "openwakeword", "phrase": "hey fulilian"}
    )
    assert r["available"] is True
    assert r["provider"] == "openwakeword"
    assert r["phrase"] == "hey fulilian"
...
def _install_fake_openwakeword(monkeypatch):
    ...
    class _FakeModel:
        def __init__(self, wakeword_models, inference_framework="onnx"):
            ...
            self.models = {"hey_fulilian": object()}

        def predict(self, frame):
            return {"hey_fulilian": 0.0}
...
def test_openwakeword_ensures_base_models_for_custom_path(monkeypatch):
    ...
    eng = ww._OpenWakeWordEngine(
        {"provider": "openwakeword", "openwakeword": {"model": "/models/hey_fulilian.onnx"}}
    )
    assert calls["download"] == [["/models/hey_fulilian.onnx"]]
    assert eng._labels == ["hey_fulilian"]


def test_bundled_hey_fulilian_model_ships_on_disk():
    # The "hey fulilian" wake word works out of the box only if the model is
