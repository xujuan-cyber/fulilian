...
and the shared slash-command handlers (fulilian_cli/write_approval_commands.py).
...
@pytest.fixture
def fulilian_home(monkeypatch):
    d = tempfile.mkdtemp(prefix="fulilian_wa_test_")
    home = os.path.join(d, ".fulilian")
    os.makedirs(home)
...
def _set_approval(subsystem, enabled):
    import fulilian_cli.config as cfg
    c = cfg.load_config()
...
def test_default_gate_is_off(fulilian_home):
...
def test_invalid_subsystem_is_off(fulilian_home):
    from tools import write_approval as wa
...
def test_memory_gate_off_allows_write(fulilian_home):
...
def test_cli_memory_approve_without_live_agent_uses_fresh_store(fulilian_home, capsys):
...
def test_load_on_disk_store_honors_configured_limits_and_permissions(fulilian_home, monkeypatch):
...
def test_handle_approve_all(fulilian_home):
...
def test_handle_approval_on(fulilian_home):
    from fulilian_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
...
def test_handle_approval_off(fulilian_home):
    from fulilian_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
    out = handle_pending_subcommand(
...
def test_memory_inline_approve_writes(fulilian_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_deny_blocks(fulilian_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_invalid_params_rejected_before_staging(fulilian_home):
