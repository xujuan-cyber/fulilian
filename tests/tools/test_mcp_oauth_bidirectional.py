"""Regression test for the ``FulilianMCPOAuthProvider.async_auth_flow`` bidirectional
generator bridge.
...
@pytest.mark.asyncio
async def test_fulilian_provider_forwards_asend_values(tmp_path, monkeypatch):
    ...
    assert _FULILIAN_PROVIDER_CLS is not None, "SDK OAuth types must be available"
...
@pytest.mark.asyncio
async def test_fulilian_provider_forwards_401_triggers_refresh(tmp_path, monkeypatch):
