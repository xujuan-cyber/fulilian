...
A Bot Mode agent invoked as a short-lived ``fulilian -p <bot> chat -Q
...
``fulilian_cli.oneshot``) a bounded linger over every tracked
...
def test_config_reader_falls_back_when_config_unreadable(monkeypatch):
    ...
    monkeypatch.setattr(
        "fulilian_cli.config.read_raw_config", _boom, raising=False
    )
...
def test_oneshot_module_lingers_before_agent_close():
    """fulilian_cli/oneshot.py must wait for pending completions before
    agent.close() (which kill_all()s the task's processes)."""
    src = (REPO_ROOT / "fulilian_cli" / "oneshot.py").read_text(encoding="utf-8")
...
def _run_e2e_parent(tmp_path, *, linger: bool) -> Path:
    ...
    env = dict(os.environ)
    env.setdefault("FULILIAN_HOME", str(tmp_path / "fulilian_home"))
    proc = subprocess.run(
