...
Keys added via ``fulilian auth add <provider>`` live in the credential pool /
auth store and used to be invisible to the voice tools, which only read
``os.environ`` + ``~/.fulilian/.env`` via ``get_env_value``. The shared
...
def _no_dotenv(monkeypatch):
    """Keep the developer's real ~/.fulilian/.env out of these tests."""
