...
NOT from FULILIAN_SESSION_ID: constructing a child agent calls
set_current_session_id(child.session_id), clobbering the FULILIAN_SESSION_ID
...
def _patch_delegate(monkeypatch):
    ...
    def clobbering_build_child(**kw):
        ...
        # FULILIAN_SESSION_ID ContextVar + os.environ, clobbering the spawner's
...
def test_apiserver_session_with_id_dispatches_background(monkeypatch):
    """async_delivery=False + a raw session id (FULILIAN_SESSION_ID) →
    ...
    monkeypatch.setenv("FULILIAN_SESSION_ID", "raw-sid-7")
    set_session_vars(
        platform="api_server",
        chat_id="raw-sid-7",
    ...
    # FULILIAN_SESSION_ID with (see clobbering_build_child).
