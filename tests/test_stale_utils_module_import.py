...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.fulilian/fulilian-agent/utils.py)
...
2. ``fulilian update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``fulilian_cli/main.py`` (the ``fulilian update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``fulilian_constants`` after the
