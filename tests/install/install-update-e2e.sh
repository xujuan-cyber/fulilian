...
#
# Installs a real, earlier Fulilian the way a user does, applies ONE update route,
# and requires the checkout to land on this commit with a working `fulilian`.
#
...
# Keep sandbox state out of the default .fulilian-sandbox so a run never clobbers
...
SANDBOX_DIR_NAME=".fulilian-sandbox-e2e-$ROUTE"
export FULILIAN_DEV_SANDBOX_DIR="$SANDBOX_DIR_NAME"
...
LOG_DIR="${FULILIAN_E2E_LOG_DIR:-$(mktemp -d -t fulilian-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED fulilian accept FLAG on `fulilian update`?
#
...
# that silently rots. `fulilian update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "fulilian update --help 2>&1" | grep -qF -- "$flag"
}
...
require_fulilian_works() {
  local when="$1" out
  out="$(in_sandbox "fulilian --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "fulilian --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "fulilian runs $when"
}

# ── install the earlier Fulilian ─────────────────────────────────────────────
...
require_fulilian_works 'after install'

# ── apply exactly one update route ─────────────────────────────────────────
case "$ROUTE" in
  update)
    step 'ROUTE: fulilian update'
    ...
    # fulilian which it accepts; older ones read the prompt from stdin, so close it.
    if update_supports --yes; then
      update_cmd="fulilian update --yes"
    else
      update_cmd="fulilian update </dev/null"
    fi
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      collect_sandbox_logs update
      fail "fulilian update failed ($update_cmd)"
    fi
    require_landed_on_target 'fulilian update'
    require_fulilian_works 'after fulilian update'
    ;;
  installer)
    ...
    require_fulilian_works 'after installer re-run'
    ;;
esac
