"""Tests for IRC gateway configuration via `fulilian setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Fulilian install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Fulilian install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#fulilian")
            monkeypatch.setenv("IRC_NICKNAME", "fulilian-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `fulilian setup gateway` experience with IRC present."""
