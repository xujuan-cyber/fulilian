...
class TestGmiAliases:
    ...
    def test_providers_normalize_provider(self):
        from fulilian_cli.providers import normalize_provider as normalize_provider_in_providers
...
class TestGmiConfigRegistry:
    def test_optional_env_vars_include_gmi(self):
        ...
        # fulilian model, not via upgrade prompts.
...
class TestGmiProvidersModule:
    def test_overlay_exists(self):
        ...
        assert "gmi" in FULILIAN_OVERLAYS
        overlay = FULILIAN_OVERLAYS["gmi"]
...
class TestGmiDoctor:
    def test_provider_env_hints_include_gmi(self):
        from fulilian_cli.doctor import _PROVIDER_ENV_HINTS
...
class TestGmiAuxiliary:
    def test_resolve_provider_client_uses_gmi_aux_default(self, monkeypatch):
        ...
        # GMI profile declares default_headers with a FulilianAgent User-Agent
        ...
        assert headers.get("User-Agent", "").startswith("FulilianAgent/")

    def test_gmi_profile_declares_fulilian_user_agent(self):
        """The GMI plugin sets a FulilianAgent/<ver> User-Agent on its profile."""
        from providers import get_provider_profile

        profile = get_provider_profile("gmi")
        assert profile is not None
        ua = profile.default_headers.get("User-Agent", "")
        assert ua.startswith("FulilianAgent/"), (
            f"expected GMI profile User-Agent to start with 'FulilianAgent/', got {ua!r}"
        )
...
class TestGmiMainFlow:
    def test_chat_parser_accepts_gmi_provider(self, monkeypatch):
        ...
        monkeypatch.setattr("fulilian_cli.config.get_container_exec_info", lambda: None)
        monkeypatch.setattr(
            "fulilian_cli.main.cmd_chat",
            lambda args: recorded.setdefault("provider", args.provider),
        )
        monkeypatch.setattr(sys, "argv", ["fulilian", "chat", "--provider", "gmi"])

        from fulilian_cli.main import main
    ...
    def test_select_provider_and_model_routes_gmi_to_generic_flow(self, monkeypatch):
        ...
        monkeypatch.setattr("fulilian_cli.auth.resolve_provider", lambda *args, **kwargs: None)
        ...
        monkeypatch.setattr("fulilian_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
        monkeypatch.setattr("fulilian_cli.main._model_flow_api_key_provider", fake_model_flow_api_key_provider)
    ...
    def test_model_flow_api_key_provider_persists_gmi_selection(self, monkeypatch):
        ...
        with patch(
            ...
            from fulilian_cli.main import _model_flow_api_key_provider
        ...
        config = yaml.safe_load((get_fulilian_home() / "config.yaml").read_text()) or {}
