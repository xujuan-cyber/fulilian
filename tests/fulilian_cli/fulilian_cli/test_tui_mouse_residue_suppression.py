...
The Python launcher (`fulilian --tui …`) has a ~100–300ms cold-start window
where stdin is still in cooked + echo mode. If a previous Fulilian session
...
from fulilian_cli.main import _suppress_mouse_residue_early
...
class TestEarlyMouseDisable:
    ...
    def test_respects_diagnostic_escape_hatch(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["fulilian", "--tui"])
        monkeypatch.delenv("FULILIAN_TUI", raising=False)
        monkeypatch.setenv("FULILIAN_TUI_NO_EARLY_DISABLE", "1")
    ...
    def test_oserror_is_swallowed(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["fulilian", "--tui"])
        monkeypatch.delenv("FULILIAN_TUI", raising=False)
        monkeypatch.delenv("FULILIAN_TUI_NO_EARLY_DISABLE", raising=False)
