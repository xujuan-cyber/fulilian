...
fulilian_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `fulilian config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_fulilian_home = get_fulilian_home()`` at import time (module
    singleton), so monkeypatching FULILIAN_HOME after import doesn't move it.
    ...
    this only matters for tests that swap FULILIAN_HOME.)
    """
    import cli

    cli._fulilian_home = home
