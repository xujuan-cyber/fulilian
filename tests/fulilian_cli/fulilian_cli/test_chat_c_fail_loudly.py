...
Regression for #86794: a background/quiet `fulilian chat -c "<title>" -q "..."`
...
def isolated_home(tmp_path, monkeypatch):
    """Point FULILIAN_HOME at a temp dir so session creation stays isolated."""
...
def _create_titled_session(title):
    from fulilian_cli.main import _create_titled_session as fn
...
class TestChatCFailLoudlyOnStderr:
    ...
    def test_missing_session_fails_on_stderr(self, isolated_home, monkeypatch):
        ...
        assert not stderr_lines[0].startswith("Use 'fulilian sessions list'")
