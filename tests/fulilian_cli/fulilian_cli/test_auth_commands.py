...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    from fulilian_cli.auth_commands import auth_add_command
    ...
    payload = json.loads((tmp_path / "fulilian" / "auth.json").read_text())
...
def test_auth_add_configured_provider_uses_canonical_pool_key(tmp_path, monkeypatch):
    ...
    payload = json.loads((fulilian_home / "auth.json").read_text(encoding="utf-8"))
...
def test_auth_add_migrates_display_name_derived_legacy_pool_key(
    ...
    with patch("fulilian_cli.models.clear_provider_models_cache") as clear_cache:
        auth_add_command(_Args())
...
def test_auth_list_includes_non_registry_configured_provider(
    ...
    from fulilian_cli.auth_commands import auth_list_command
...
def test_interactive_auth_add_accepts_non_registry_configured_provider(
    ...
    from fulilian_cli import auth_commands
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "fulilian_cli.auth._nous_device_code_login",
        lambda **kwargs: {
    ...
    # `fulilian auth add nous` must also populate providers.nous so the
    ...
    # raises "Fulilian is not logged into Nous Portal" and the agent dies.
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`fulilian auth add nous --type oauth --label <name>` must preserve the
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``fulilian auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``fulilian auth list`` showed two labels sharing
    ...
    monkeypatch.setattr("fulilian_cli.auth._codex_device_code_login", lambda: next(logins))
...
def test_codex_auth_status_reports_pool_only_credential(tmp_path, monkeypatch):
    ...
    from fulilian_cli.auth import get_codex_auth_status
...
def test_codex_runtime_pool_only_rate_limit_is_not_missing_auth(tmp_path, monkeypatch):
    ...
    from fulilian_cli.auth import AuthError, CODEX_RATE_LIMITED_CODE, resolve_codex_runtime_credentials
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """fulilian auth add xai-oauth must set active_provider and write a pool entry.
...
def test_auth_add_xai_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``fulilian auth add xai-oauth`` runs must produce independent pool entries.
...
def test_auth_remove_codex_migrates_legacy_dict_suppression(tmp_path, monkeypatch):
    ...
    payload = json.loads((tmp_path / "fulilian" / "auth.json").read_text(encoding="utf-8"))
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`fulilian logout --provider openai-codex` must still clear model.provider.
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from fulilian_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
# =============================================================================
# Unified credential-source stickiness — every source Fulilian reads from has a
...
def test_seed_from_singletons_respects_fulilian_pkce_suppression(tmp_path, monkeypatch):
    """anthropic fulilian_pkce must not re-seed from ~/.fulilian/.anthropic_oauth.json when suppressed."""
    fulilian_home = tmp_path / "fulilian"
    fulilian_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("FULILIAN_HOME", str(fulilian_home))

    import yaml
    (fulilian_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (fulilian_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["fulilian_pkce"]},
    }))

    # Stub the readers so only fulilian_pkce is "available"; claude_code returns None
    import agent.anthropic_adapter as aa
    monkeypatch.setattr(aa, "read_fulilian_oauth_credentials", lambda: {
    ...
    # fulilian_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "fulilian_pkce" not in active
