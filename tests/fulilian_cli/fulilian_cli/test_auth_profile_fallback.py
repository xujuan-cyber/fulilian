...
When ``FULILIAN_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.fulilian/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.fulilian            (has its own auth.json fixture)
    * Profile     -> tmp_path/.fulilian/profiles/coder   (active, FULILIAN_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Fulilian is not
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (FULILIAN_HOME != ~/.fulilian, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    fulilian_home = tmp_path / "classic"
    fulilian_home.mkdir()
