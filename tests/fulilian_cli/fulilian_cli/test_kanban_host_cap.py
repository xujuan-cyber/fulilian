...
1. The standalone daemon path (``fulilian kanban daemon --force`` /
   :func:`fulilian_cli.kanban_db.run_daemon`) never resolved
...
def test_run_daemon_resolves_and_passes_max_in_progress(
    ...
    ``fulilian kanban dispatch`` paths both resolved the memory-derived
    default.
    """
    captured: dict = {}
...
def test_nonspawnable_review_does_not_tax_ready_budget(
    ...
    import fulilian_cli.config as cfgmod
    import fulilian_cli.profiles as profmod
