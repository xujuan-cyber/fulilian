...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "fulilian_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # FULILIAN_HOME.  For a standard install (FULILIAN_HOME unset) that root is
        # the platform-native default (~/.fulilian), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from fulilian_constants import get_default_fulilian_root
        assert env.get("FULILIAN_HOME") == str(get_default_fulilian_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (FULILIAN_DESKTOP=1) must NOT
