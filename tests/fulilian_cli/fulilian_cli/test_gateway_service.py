...
import fulilian_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    ...
    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`fulilian gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `fulilian gateway restart` in the background; when a
        ...
        assert "fulilian gateway status" in output
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``FULILIAN_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../fulilian_test`` FULILIAN_HOME — silently breaking
        ...
        carry those markers.
        """
        unit_path = tmp_path / "fulilian-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")

        monkeypatch.setattr(
            gateway_cli, "get_systemd_unit_path", lambda system=False: unit_path
        )
        # Realistic generated unit referencing a pytest tmpdir FULILIAN_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="FULILIAN_HOME=/tmp/pytest-of-alice/pytest-42/'
            'popen-gw0/test_x/fulilian_test"\n'
        )
        monkeypatch.setattr(
            gateway_cli,
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_tmp_home_in_systemd_unit(self):
        unit = '[Service]\nEnvironment="FULILIAN_HOME=/tmp/fulilian-e2e-41264"\n'
        assert (
            gateway_cli._temp_home_in_service_definition(unit)
            == "/tmp/fulilian-e2e-41264"
        )
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="FULILIAN_HOME={tmp_path}/fulilian-home"\n'
...
class TestGetCronDrainTimeout:
    def test_missing_config_falls_back_to_default(self, monkeypatch):
        monkeypatch.delenv("FULILIAN_CRON_DRAIN_TIMEOUT", raising=False)
    ...
    def test_env_overrides_config(self, monkeypatch):
        monkeypatch.setenv("FULILIAN_CRON_DRAIN_TIMEOUT", "45")
        monkeypatch.setattr(
            gateway_cli,
            "read_raw_config",
...
class TestGeneratedSystemdUnits:
    ...
    def test_timeout_stop_sec_covers_default_cron_drain_floor(self, monkeypatch):
        ...
        monkeypatch.delenv("FULILIAN_RESTART_DRAIN_TIMEOUT", raising=False)
        monkeypatch.delenv("FULILIAN_CRON_DRAIN_TIMEOUT", raising=False)
    ...
    def test_user_unit_does_not_leak_profile_node_symlink_target(self, tmp_path, monkeypatch):
        ...
        profile_node_bin = tmp_path / ".fulilian" / "profiles" / "jarvis" / "node" / "bin"
    ...
    def test_launchd_plist_persists_configured_nofile_soft_limit(self, monkeypatch):
        ...
        plist rewrite by `fulilian gateway start` cannot strip the FD floor and
        reintroduce EMFILE crashes (launchd default soft limit is 256)."""
        import fulilian_cli.resource_limits as resource_limits
...
class TestLaunchdServiceRecovery:
    ...
    def test_refresh_defers_reload_when_running_inside_gateway_tree(self, tmp_path, monkeypatch):
        ...
        plist_path = tmp_path / "ai.fulilian.gateway.plist"
        ...
        monkeypatch.setattr(
            ...
            lambda: (
                ...
                "<string>/Users/alice/.fulilian</string></plist>"
            ),
        )
    ...
    def test_launchd_status_reports_fallback_when_unsupported_and_pid_running(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "get_fulilian_home", lambda: tmp_path)
...
class TestGatewaySystemServiceRouting:
    ...
    def test_launchd_restart_uses_sigusr1_and_exit_wait_budget(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "get_launchd_label", lambda: "ai.fulilian.gateway")
        ...
        assert ("observe", "ai.fulilian.gateway", 654, "gui/501") in calls
        out = capsys.readouterr().out
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.fulilian.gateway"])
            ),
        )
...
class TestSystemUnitFulilianHome:
    """FULILIAN_HOME in system units must reference the target user, not root."""

    def test_empty_managed_node_dir_uses_only_ambient_fallback(
        self, monkeypatch, tmp_path
    ):
        managed_bin = tmp_path / ".fulilian" / "node" / "bin"
        ...
        gateway_cli._append_node_dir_for_service(entries, tmp_path / ".fulilian")
    ...
    def test_managed_node_makes_system_unit_independent_of_callers_path(
        ...
        target_fulilian = target_home / ".fulilian"
        root_home = tmp_path / "root"
        root_fulilian = root_home / ".fulilian"
        managed_bin = target_fulilian / "node" / "bin"
        managed_bin.mkdir(parents=True)
        node = managed_bin / "node"
        node.write_text("#!/bin/sh\n")
        node.chmod(0o755)
        root_fulilian.mkdir(parents=True)

        monkeypatch.setattr(Path, "home", staticmethod(lambda: root_home))
        monkeypatch.setenv("FULILIAN_HOME", str(root_fulilian))
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", str(target_home)),
        )
        monkeypatch.setattr(gateway_cli, "get_fulilian_home", lambda: root_fulilian)
    ...
    def test_node_path_lookup_remains_fallback_without_managed_node(
        ...
        monkeypatch.setattr(
            "fulilian_constants.iter_fulilian_node_dirs", lambda root=None: []
        )
        monkeypatch.setattr(
            "fulilian_constants.fulilian_managed_node_tree_present",
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'FULILIAN_HOME=/home/alice/.fulilian' in unit
        assert '/root/.fulilian' not in unit


    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's FULILIAN_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        fulilian_home = str(gateway_cli.get_fulilian_home().resolve())
        assert f'FULILIAN_HOME={fulilian_home}' in unit


class TestSystemUnitRefreshSyncsFulilianHome:
    """sudo system refresh must not flip TimeoutStopSec via /root/.fulilian."""

    def test_refresh_adopts_unit_fulilian_home_before_rewriting(self, tmp_path, monkeypatch):
        root_home = tmp_path / "root"
        alice_home = tmp_path / "alice"
        root_fulilian = root_home / ".fulilian"
        alice_fulilian = alice_home / ".fulilian"
        root_fulilian.mkdir(parents=True)
        alice_fulilian.mkdir(parents=True)
        (root_fulilian / "config.yaml").write_text(
            "agent:\n  restart_drain_timeout: 60\n", encoding="utf-8"
        )
        (alice_fulilian / "config.yaml").write_text(
        ...
        # Correct installed unit (operator's FULILIAN_HOME + drain timeout).
        monkeypatch.setenv("FULILIAN_HOME", str(alice_fulilian))
        ...
        # Simulate sudo without inherited FULILIAN_HOME (falls back to root).
        monkeypatch.setenv("FULILIAN_HOME", str(root_fulilian))
        ...
        assert os.environ["FULILIAN_HOME"] == str(alice_fulilian)
    ...
    def test_is_current_syncs_before_reading_unit(self, tmp_path, monkeypatch):
        ...
        unit's pinned FULILIAN_HOME *before* it reads/compares the unit.
        ...
        monkeypatch.setattr(gateway_cli, "_sync_fulilian_home_from_systemd_unit", tracking_sync)
    ...
    def test_start_and_restart_delegate_sync_to_chokepoint(self, monkeypatch):
        ...
        for entry in ("systemd_start", "systemd_restart"):
            ...
            monkeypatch.setattr(
                gateway_cli,
                "_sync_fulilian_home_from_systemd_unit",
...
class TestFulilianHomeForTargetUser:
    """Unit tests for _fulilian_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("FULILIAN_HOME", raising=False)

        result = gateway_cli._fulilian_home_for_target_user("/home/alice")
        assert result == "/home/alice/.fulilian"
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "fulilian gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        root_home = tmp_path / "root"
        target_home = tmp_path / "home" / "alice"
        root_profile = root_home / ".fulilian" / "profiles" / "mybot"
        root_profile.mkdir(parents=True)

        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("FULILIAN_HOME", str(root_profile))
        monkeypatch.setattr(gateway_cli, "get_fulilian_home", lambda: root_profile)
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", str(target_home)),
        ...
        assert f'FULILIAN_HOME={target_home / ".fulilian" / "profiles" / "mybot"}' in unit

    def test_launchd_plist_wraps_gateway_stderr_with_timestamps(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".fulilian" / "profiles" / "mybot"
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("FULILIAN_HOME", str(profile_dir))
        monkeypatch.setattr(gateway_cli, "get_fulilian_home", lambda: profile_dir)
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".fulilian" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.fulilian.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".fulilian" / "fulilian-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".fulilian" / "fulilian-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        root_home = tmp_path / "root"
        root_home.mkdir()
        project = root_home / ".fulilian" / "fulilian-agent"
        ...
        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("FULILIAN_HOME", str(root_home / ".fulilian"))
        monkeypatch.setattr(gateway_cli, "get_fulilian_home", lambda: root_home / ".fulilian")
        ...
        # WorkingDirectory is anchored at the target user's FULILIAN_HOME (stable,
        ...
        assert "WorkingDirectory=/home/alice/.fulilian" in unit
        assert "WorkingDirectory=/home/alice/.fulilian/fulilian-agent" not in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "fulilian-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "fulilian-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'fulilian gateway install' inside Docker exits 0 with container guidance."""
        import pytest
...
class TestLegacyFulilianUnitDetection:
    """Tests for _find_legacy_fulilian_units / has_legacy_fulilian_units.

    These guard against the scenario that tripped Luis in April 2026: an
    older install left a ``fulilian.service`` unit behind when the service was
    renamed to ``fulilian-gateway.service``. After PR #5646 (signal recovery
    via systemd), the two services began SIGTERM-flapping over the same
    Telegram bot token in a 30-second cycle.

    The detector must flag ``fulilian.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``fulilian-gateway-<profile>.service``) or unrelated third-party services.
    """

    # Minimal ExecStart that looks like our gateway
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=Fulilian Gateway\n[Service]\n"
        "ExecStart=/usr/bin/python -m fulilian_cli.main gateway run --replace\n"
    )
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        (user_dir / "fulilian.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        (system_dir / "fulilian.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        results = gateway_cli._find_legacy_fulilian_units()
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m fulilian_cli.main gateway run
          - python path/to/fulilian_cli/main.py gateway run
          - fulilian gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m fulilian_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/fulilian/fulilian_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/fulilian gateway run --replace",
            "ExecStart=/venv/bin/python /opt/fulilian/gateway/run.py",
        ]
        for i, execstart in enumerate(variants):
            name = "fulilian.service" if i == 0 else "fulilian.service"  # same name
            # Test each variant fresh
            (user_dir / "fulilian.service").write_text(
                f"[Unit]\nDescription=Old Fulilian\n[Service]\n{execstart}\n",
                encoding="utf-8",
            )
            results = gateway_cli._find_legacy_fulilian_units()
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "fulilian.service" in out
        assert "fulilian gateway migrate-legacy" in out



class TestRemoveLegacyFulilianUnits:
    """Tests for remove_legacy_fulilian_units (the migration action)."""
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        legacy = user_dir / "fulilian.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        removed, remaining = gateway_cli.remove_legacy_fulilian_units(interactive=False)
        ...
        assert any("--user stop fulilian.service" in c for c in cmds_joined)
        assert any("--user disable fulilian.service" in c for c in cmds_joined)
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (fulilian-gateway-coder.service)
        ...
        profile_unit = user_dir / "fulilian-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "fulilian-gateway.service"
...
class TestMigrateLegacyCommand:
    """Tests for the `fulilian gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        # public, just confirm that `fulilian gateway --help` shows it.
        import subprocess
        import sys

        project_root = cli_main.PROJECT_ROOT if hasattr(cli_main, "PROJECT_ROOT") else None
        if project_root is None:
            import fulilian_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "fulilian_cli.main", "gateway", "--help"],
    ...
    def test_gateway_command_migrate_legacy_dispatches(
        ...
        monkeypatch.setattr(gateway_cli, "remove_legacy_fulilian_units", fake_remove)
        monkeypatch.setattr(gateway_cli, "supports_systemd_services", lambda: True)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: False)
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "fulilian_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemdInstallOffersLegacyRemoval:
    ...
    def test_install_offers_removal_when_legacy_detected(
        ...
        # has_legacy_fulilian_units must return True
        monkeypatch.setattr(gateway_cli, "has_legacy_fulilian_units", lambda: True)
        monkeypatch.setattr(gateway_cli, "remove_legacy_fulilian_units", fake_remove)
    ...
    def test_install_skips_legacy_check_when_none_present(
        ...
        monkeypatch.setattr(gateway_cli, "has_legacy_fulilian_units", lambda: False)
        monkeypatch.setattr(gateway_cli, "remove_legacy_fulilian_units", fake_remove)
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "fulilian-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "fulilian-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "fulilian-gateway.service",
        )
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``fulilian gateway start --system``).
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "fulilian-gateway")
        ...
        assert "sudo systemctl start fulilian-gateway" in out
        assert "sudo fulilian gateway uninstall --system" in out
        assert "fulilian gateway install" in out


class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``fulilian gateway start --system`` etc.) must
...
class TestServiceWorkingDirIsStable:
    ...
    (FULILIAN_HOME), never the source checkout / worktree, so a relocated or
    deleted checkout can't crash-loop the unit on CHDIR (status=200).
    """



    def test_user_unit_workingdirectory_is_fulilian_home_not_checkout(self, tmp_path, monkeypatch):
        home = tmp_path / ".fulilian"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_fulilian_home", lambda: home)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_workingdirectory_is_fulilian_home(self, tmp_path, monkeypatch):
        import re
...
class TestLaunchctlBootstrapEioRetry:
    ...
    PLIST = "/tmp/ai.fulilian.gateway.plist"
    DOMAIN = "gui/501"
    LABEL = "ai.fulilian.gateway"
...
class TestTimeoutStopSecCoversCronFloor:
    ...
    def _unit_with_config(self, tmp_path, monkeypatch, config_yaml, env=None):
        fulilian = tmp_path / "home" / ".fulilian"
        fulilian.mkdir(parents=True)
        (fulilian / "config.yaml").write_text(config_yaml, encoding="utf-8")
        monkeypatch.setenv("FULILIAN_HOME", str(fulilian))
        monkeypatch.delenv("FULILIAN_RESTART_DRAIN_TIMEOUT", raising=False)
        monkeypatch.delenv("FULILIAN_CRON_DRAIN_TIMEOUT", raising=False)
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
        monkeypatch.setattr(
    ...
    def test_env_override_extends_the_leash(self, tmp_path, monkeypatch):
        unit = self._unit_with_config(
            ...
            monkeypatch,
            "agent:\n  restart_drain_timeout: 0\n",
            env={"FULILIAN_CRON_DRAIN_TIMEOUT": "200"},
        )
