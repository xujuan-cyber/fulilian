"""Tests for the per-profile Projects store (fulilian_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="FuLiLian", folders=["/tmp/fulilian"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "fulilian-agent"
    assert proj.name == "FuLiLian"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/fulilian"
    assert [f.path for f in proj.folders] == ["/tmp/fulilian"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "fulilian-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' FULILIAN_HOME.
