"""Tests for the Phase 4 s6 dispatch helper in fulilian_cli.gateway.

`_dispatch_via_service_manager_if_s6` decides whether a
`fulilian gateway start/stop/restart` invocation should be routed to
...
def test_dispatch_all_handles_partial_failure(
    ...
    monkeypatch.setattr(
        "fulilian_cli.service_manager.detect_service_manager", lambda: "s6",
    )
    monkeypatch.setattr(
        "fulilian_cli.service_manager.get_service_manager", lambda: rec,
    )
...
def _stub_s6(monkeypatch: pytest.MonkeyPatch, *, on_s6: bool) -> _CallRecorder:
    ...
    rec = _CallRecorder()
    monkeypatch.setattr(
        "fulilian_cli.service_manager.detect_service_manager",
...
def test_redirect_falls_back_when_sleep_missing(
    ...
    monkeypatch.setattr("fulilian_cli.gateway._profile_suffix", lambda: "")

    def missing_sleep(file: str, args: list[str]) -> None:
        raise FileNotFoundError(2, "No such file or directory", file)

    monkeypatch.setattr("fulilian_cli.gateway.os.execvp", missing_sleep)
    block_calls: list[bool] = []
    monkeypatch.setattr(
        "fulilian_cli.gateway._block_until_terminated",
        lambda: block_calls.append(True),
    )
    monkeypatch.delenv("FULILIAN_S6_SUPERVISED_CHILD", raising=False)
    monkeypatch.delenv("FULILIAN_GATEWAY_NO_SUPERVISE", raising=False)
