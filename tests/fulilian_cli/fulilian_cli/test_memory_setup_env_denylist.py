...
``fulilian_cli.memory_setup._write_env_vars`` persists provider plugin
credentials to ``~/.fulilian/.env``. It previously called ``Path.write_text``
...
other subprocess-influencing or Fulilian-runtime-location name) could
otherwise plant a value into ``.env`` via the interactive memory-setup
wizard. The next Fulilian process would load it through the
...
from fulilian_cli.config import ensure_fulilian_home, get_env_path, load_env
from fulilian_cli.memory_setup import _write_env_vars


def _env_file_keys() -> set[str]:
    """Parse ``~/.fulilian/.env`` directly and return the set of keys present.
...
def _fulilian_home(tmp_path, monkeypatch):
...
def test_denylisted_key_does_not_block_other_writes(capsys):
    ...
    _write_env_vars({
        ...
        "FULILIAN_LANGFUSE_PUBLIC_KEY": "pk-test-123",
        "OPENROUTER_API_KEY": "sk-or-test-456",
    })

    assert "LD_PRELOAD" not in _env_file_keys()
    env = load_env()
    assert env["FULILIAN_LANGFUSE_PUBLIC_KEY"] == "pk-test-123"
    assert env["OPENROUTER_API_KEY"] == "sk-or-test-456"


def test_legitimate_fulilian_integration_key_still_writable():
    """``FULILIAN_*`` overall is NOT blocked — only the four runtime
    location names (HOME/PROFILE/CONFIG/ENV). Integration credentials
    following the ``FULILIAN_*`` convention (FULILIAN_LANGFUSE_*,
    FULILIAN_SPOTIFY_*, FULILIAN_QWEN_BASE_URL, ...) must keep working or
    the memory-setup wizard regresses for every plugin that follows
    the convention."""
    _write_env_vars({
        "FULILIAN_LANGFUSE_PUBLIC_KEY": "pk-lf-789",
        "FULILIAN_QWEN_BASE_URL": "https://example.com/v1",
    })

    env = load_env()
    assert env["FULILIAN_LANGFUSE_PUBLIC_KEY"] == "pk-lf-789"
    assert env["FULILIAN_QWEN_BASE_URL"] == "https://example.com/v1"
...
def test_explicit_fulilian_home_writes_to_that_env_file(tmp_path):
    """Plugin ``post_setup`` hooks (e.g. Supermemory) pass an explicit
    Fulilian home; keep that target while still routing through
    ``save_env_value`` validation instead of writing directly."""
    home = tmp_path / "plugin-home"

    _write_env_vars({"MEM0_API_KEY": "m0-test-key-abc"}, fulilian_home=home)
