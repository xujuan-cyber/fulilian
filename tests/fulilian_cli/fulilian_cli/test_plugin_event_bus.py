...
Covers:
  ...
  - Namespace spoofing (fulilian:, foreign, already-colon'd) is rejected
  ...
  - `fulilian plugins show` output includes emits/listens
"""
...
@pytest.mark.parametrize(
    "bad_event",
    [
        "fulilian:x",   # reserved core prefix
...
def test_emit_rejects_namespaced_names(bad_event):
    ...
    for name in (bad_event, f"b:{bad_event}", "fulilian:x", "a:x", "b:x"):
...
def test_subscribe_is_unrestricted():
    """Any plugin may subscribe to any event, including fulilian: and foreign."""
    manager = _fresh_manager()
    ctx_a = _make_ctx(manager, "plugin_a", key="a")
    got = []
    # None of these raise — only emit is namespace-gated.
    ctx_a.subscribe("fulilian:core_event", lambda **p: got.append("fulilian"))
    ctx_a.subscribe("b:ping", lambda **p: got.append("b"))
    assert "fulilian:core_event" in manager._subscriptions
    assert "b:ping" in manager._subscriptions
