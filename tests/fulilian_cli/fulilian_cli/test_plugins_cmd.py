"""Tests for fulilian_cli.plugins_cmd — the ``fulilian plugins`` CLI subcommand."""
...
class TestResolveGitExecutable:
    ...
    def test_prefers_shutil_which(self):
        import fulilian_cli.plugins_cmd as pc
...
class TestGitPullPluginDirAutostash:
    ...
    def test_conflicting_local_edit_is_preserved_in_stash(self, tmp_path):
        ...
        assert "fulilian-plugin-update-autostash" in stash_list
...
class TestReadManifest:
    ...
    def test_invalid_yaml_returns_empty_and_logs(self, tmp_path, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="fulilian_cli.plugins_cmd"):
            result = _read_manifest(tmp_path)
        assert result == {}
...
class TestCmdInstall:
    ...
    def test_install_requires_identifier(self):
        from fulilian_cli.plugins_cmd import cmd_install

        with pytest.raises(SystemExit):
            cmd_install("")

    @patch("fulilian_cli.plugins_cmd._resolve_git_url")
    ...
    @patch("fulilian_cli.plugins_cmd._display_after_install")
    @patch("fulilian_cli.plugins_cmd.shutil.move")
    @patch("fulilian_cli.plugins_cmd.shutil.rmtree")
    @patch("fulilian_cli.plugins_cmd._plugins_dir")
    @patch("fulilian_cli.plugins_cmd._read_manifest")
    @patch("fulilian_cli.plugins_cmd.subprocess.run")
...
class TestCmdUpdate:
    ...
    @patch("fulilian_cli.plugins_cmd._sanitize_plugin_name")
    @patch("fulilian_cli.plugins_cmd._plugins_dir")
    @patch("fulilian_cli.plugins_cmd.subprocess.run")
    def test_update_git_pull_success(self, mock_run, mock_plugins_dir, mock_sanitize):
        from fulilian_cli.plugins_cmd import cmd_update
...
class TestCmdRemove:
    ...
    def test_remove_deletes_plugin(self, mock_rmtree, mock_plugins_dir, mock_sanitize):
        from fulilian_cli.plugins_cmd import cmd_remove
...
class TestCmdList:
    ...
    def test_list_empty_plugins_dir(self, mock_plugins_dir):
        from fulilian_cli.plugins_cmd import cmd_list
...
class TestCopyExampleFiles:
    ...
    def test_handles_copy_error_gracefully(self, tmp_path):
        ...
        with patch(
            "fulilian_cli.plugins_cmd.shutil.copy2",
...
class TestPromptPluginEnvVars:
    ...
    def test_prompts_for_missing_var_rich_format(self):
        from fulilian_cli.plugins_cmd import _prompt_plugin_env_vars
        from unittest.mock import MagicMock, patch
        ...
        with patch("fulilian_cli.config.get_env_value", return_value=None), \
             patch("builtins.input", return_value="pk-lf-123"), \
             patch("fulilian_cli.config.save_env_value") as mock_save:
    ...
    def test_secret_uses_masked_prompt(self):
        ...
        with patch("fulilian_cli.config.get_env_value", return_value=None), \
             patch("fulilian_cli.plugins_cmd.masked_secret_prompt", return_value="s3cret") as mock_prompt, \
             patch("fulilian_cli.config.save_env_value"):
            _prompt_plugin_env_vars(manifest, console)
...
class TestProviderDiscovery:
    ...
    def test_save_context_engine(self, tmp_path, monkeypatch):
        ...
        from fulilian_cli.plugins_cmd import _save_context_engine
    ...
    def test_discover_context_engines_empty(self):
        ...
        with patch("plugins.context_engine.discover_context_engines",
            ...
            from fulilian_cli.plugins_cmd import _discover_context_engines
...
def test_portable_manifest_is_visible_to_plugin_cli(tmp_path):
    ...
    from fulilian_cli.agent_plugins import PLUGIN_SCHEMA_V1
    from fulilian_cli.plugins_cmd import _read_manifest_info
