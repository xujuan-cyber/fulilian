...
`FULILIAN_REDACT_SECRETS` at module-import time. `fulilian_cli/main.py` at
...
in .env), the toggle was silently ignored in both `fulilian chat` and
`fulilian gateway run`.

Fix: bridge `security.redact_secrets` from config.yaml → `FULILIAN_REDACT_SECRETS`
env var in `fulilian_cli/main.py` BEFORE the `setup_logging()` call.
"""
import os
import subprocess
import sys
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports fulilian_cli.main and checks
    # _REDACT_ENABLED. Must be a subprocess — we need a clean module state.
    probe = textwrap.dedent(
        """\
        import sys, os
        # Make absolutely sure the env var is not pre-set
        os.environ.pop("FULILIAN_REDACT_SECRETS", None)
        sys.path.insert(0, %r)
        import fulilian_cli.main  # triggers the bridge + setup_logging
        import agent.redact
        print(f"REDACT_ENABLED={agent.redact._REDACT_ENABLED}")
        print(f"ENV_VAR={os.environ.get('FULILIAN_REDACT_SECRETS', '<unset>')}")
        """
    ) % str(REPO_ROOT)

    env = dict(os.environ)
    env["FULILIAN_HOME"] = str(fulilian_home)
    env.pop("FULILIAN_REDACT_SECRETS", None)
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    """.env FULILIAN_REDACT_SECRETS takes precedence over config.yaml."""
    fulilian_home = tmp_path / ".fulilian"
    fulilian_home.mkdir()
    (fulilian_home / "config.yaml").write_text(
        textwrap.dedent(
    ...
    (fulilian_home / ".env").write_text("FULILIAN_REDACT_SECRETS=true\n")
