...
_GATEWAY_CMD = "python -m fulilian_cli.main gateway run"
...
class TestProcFallback:
    ...
    def test_detects_gateway_pid_via_proc(self):
        my_pid = os.getpid()
        entries = {
            my_pid: "python -m fulilian_cli.main",   # own process — excluded
            12345: _GATEWAY_CMD,
            99999: _OTHER_CMD,
        }
        ...
        with (
            ...
            patch("fulilian_cli.gateway._get_ancestor_pids", return_value=set()),
...
class TestPsFallbackBsdCompat:
    ...
    def test_ps_flags_are_bsd_compatible(self):
        ...
        with (
            patch("fulilian_cli.gateway.is_windows", return_value=False),
...
class TestGetServicePidsAllProfiles:
    ...
    def test_default_scope_uses_current_profile_label(self):
        ...
        with (
            patch("fulilian_cli.gateway.is_macos", return_value=True),
            patch("fulilian_cli.gateway.supports_systemd_services", return_value=False),
            patch(
                "fulilian_cli.gateway.get_launchd_label",
                return_value="ai.fulilian.gateway.myprofile",
            ),
            patch(
                "fulilian_cli.gateway._locate_launchd_gateway_service",
        ...
        assert located == ["ai.fulilian.gateway.myprofile"]
        launchctl_calls = [
            c[0][0]
    ...
    def test_all_profiles_enumerates_all_gateway_labels(self):
        ...
        widens the EXCLUDE set with unmapped ai.fulilian.gateway* agents
        (#74075 belt-and-suspenders)."""
        located = []
        label_pids = {
            "ai.fulilian.gateway": 123,
            "ai.fulilian.gateway-profile-b": 456,
        }
        ...
        with (
            ...
            patch(
                "fulilian_cli.gateway.get_launchd_label",
                return_value="ai.fulilian.gateway",
            ),
            patch(
                "fulilian_cli.gateway.launchd_gateway_labels_for_install",
                return_value=["ai.fulilian.gateway", "ai.fulilian.gateway-profile-b"],
            ),
            patch(
                "fulilian_cli.gateway._locate_launchd_gateway_service",
                side_effect=_fake_locate,
            ...
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=(
                    "999\t0\tai.fulilian.gateway-unmapped\n"
    ...
    def test_all_profiles_empty_when_no_gateway_labels(self):
        """When no ai.fulilian.gateway* labels exist, all_profiles returns empty."""
        with (
            patch("fulilian_cli.gateway.is_macos", return_value=True),
            patch("fulilian_cli.gateway.supports_systemd_services", return_value=False),
            patch("subprocess.run") as mock_run,
    ...
    def test_all_profiles_handles_broken_pid_column(self):
        ...
        with (
            ...
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=(
                    "-\t0\tai.fulilian.gateway.broken\n"
                    "  123  \t0\tai.fulilian.gateway.ok\n"
                ),
                stderr="",
            )
            pids = gateway_mod._get_service_pids(all_profiles=True)
    ...
    def test_all_profiles_preserves_systemd_behavior(self):
        ...
        lists every fulilian-gateway* unit unconditionally."""
        with (
            patch("fulilian_cli.gateway.is_macos", return_value=False),
            patch("fulilian_cli.gateway.supports_systemd_services", return_value=True),
            patch("subprocess.run") as mock_run,
        ):
            def _run_side_effect(args, **kwargs):
                ...
                if "list-units" in cmd_str:
                    return MagicMock(
                        returncode=0,
                        stdout="fulilian-gateway-jarvis.service loaded active running\n",
                        stderr="",
                    )
