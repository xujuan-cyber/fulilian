"""Tests for the Command Installation check in fulilian doctor."""
...
import fulilian_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal FULILIAN_HOME + PROJECT_ROOT for doctor tests."""
    ...
    fulilian_bin = venv_bin_dir / "fulilian"
    fulilian_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    ...
    return home, project, fulilian_bin
...
class TestDoctorCommandInstallation:
    ...
    def test_fix_repairs_wrong_symlink(self, monkeypatch, tmp_path):
        home, project, fulilian_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create a symlink pointing to wrong target
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "fulilian"
        wrong_target = tmp_path / "wrong_fulilian"
        ...
        assert cmd_link.resolve() == fulilian_bin.resolve()
