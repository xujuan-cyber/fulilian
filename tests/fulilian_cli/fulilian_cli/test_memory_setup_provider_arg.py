"""Tests for `fulilian memory setup [provider]` routing.
...
``fulilian memory setup honcho``) without the interactive picker — which matters
because the per-provider ``fulilian <provider>`` subcommand is only registered
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "fulilian memory setup" in out
...
class TestInstallDependenciesRunner:
    ...
    def _run_with_missing_dep(self, tmp_path, which_side_effect, run_behavior=None):
        ...
        # The hermetic conftest sets FULILIAN_DISABLE_LAZY_INSTALLS=1 so no test
        ...
        with _patch.dict(os.environ, {"FULILIAN_DISABLE_LAZY_INSTALLS": "0"}), \
             patch("plugins.memory.find_provider_dir", return_value=tmp_path), \
             patch("fulilian_cli.tools_config.shutil.which", side_effect=which_side_effect), \
             patch("fulilian_cli.tools_config.subprocess.run", fake_run):
