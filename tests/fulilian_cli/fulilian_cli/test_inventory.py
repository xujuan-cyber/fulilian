"""Behavior tests for fulilian_cli.inventory.
...
def test_explicit_only_keeps_anthropic_row_with_oauth_credentials():
    ...
    Claude Code (~/.claude/.credentials.json) and Fulilian' own device flow
    ...
    with (
        ...
        patch(
            "fulilian_cli.inventory._anthropic_oauth_credentials_present",
            return_value=True,
        ),
    ):
        payload = build_models_payload(ctx, explicit_only=True)
...
def test_anthropic_oauth_presence_accepts_pool_only_oauth_entry():
    ...
    """
    from fulilian_cli.inventory import _anthropic_oauth_credentials_present
    ...
    with (
        ...
        patch(
            "fulilian_cli.auth.read_credential_pool",
            return_value=[
...
def test_build_models_payload_forwards_refresh_flag():
    ...
    with patch("fulilian_cli.model_switch.list_authenticated_providers", side_effect=_capture):
...
def test_list_authenticated_providers_refresh_busts_cache():
    ...
    with patch("fulilian_cli.models.clear_provider_models_cache") as clear:
