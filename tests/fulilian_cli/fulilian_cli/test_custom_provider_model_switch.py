"""Tests that `fulilian model` always shows the model selection menu for custom
...
def config_home(tmp_path, monkeypatch):
    """Isolated FULILIAN_HOME with a minimal config."""
...
class TestCustomProviderModelSwitch:
    ...
    def test_custom_endpoint_switch_prunes_stale_model_config_pool_entry(
        ...
        from fulilian_cli.main import _model_flow_custom
        ...
        with patch(
            ...
            return_value={
             ...
             patch("fulilian_cli.secret_prompt.masked_secret_prompt", return_value="sk-new"), \
             patch("fulilian_cli.main._prompt_custom_api_mode_selection", return_value=""), \
             patch(
                 "builtins.input",
                 side_effect=[
    ...
    def test_env_template_api_key_is_preserved_in_model_config(self, config_home, monkeypatch):
        ...
        with patch("fulilian_cli.models.fetch_api_models", return_value=["qwen3.6-35b-fast"]) as mock_fetch, \
             patch("fulilian_cli.curses_ui.curses_radiolist", side_effect=ImportError), \
    ...
    def test_key_env_custom_provider_persists_reference_not_secret(self, config_home, monkeypatch):
        ...
        with patch("fulilian_cli.models.fetch_api_models", return_value=["qwen3.6-35b-fast"]), \
             patch("fulilian_cli.curses_ui.curses_radiolist", side_effect=ImportError), \
             patch("builtins.input", return_value="1"), \
             patch("builtins.print"):
            _model_flow_named_custom({}, provider_info)
    ...
    def test_env_ref_base_url_preserves_api_key_ref_through_picker(
        ...
        with patch("fulilian_cli.main._prompt_provider_choice",
                   side_effect=_pick_neuralwatt), \
             patch("fulilian_cli.models.fetch_api_models",
    ...
    def test_key_env_providers_dict_entry_does_not_add_api_key(
        ...
        config_path.write_text(
            ...
            "    key_env: FULILIAN_CRS_HENKEE_KEY\n"
            "    transport: anthropic_messages\n"
            "    model: claude-opus-4-7\n"
            "    default_model: claude-opus-4-7\n"
            "custom_providers: []\n"
        )
        monkeypatch.setenv("FULILIAN_CRS_HENKEE_KEY", "cr_live_secret_xyz")
        ...
        assert entry["key_env"] == "FULILIAN_CRS_HENKEE_KEY"
        ...
        assert "${FULILIAN_CRS_HENKEE_KEY}" not in saved_text
    ...
    def test_picker_recognizes_current_provider_alias_when_name_differs(
        ...
        with patch(
            "fulilian_cli.main._prompt_provider_choice",
    ...
    def test_key_env_providers_dict_preserves_existing_api_key(
        ...
        config_path.write_text(
            ...
            "    name: CRS Henkee\n"
            "    base_url: http://127.0.0.1:3000/api/v1\n"
            "    api_key: ${FULILIAN_CRS_HENKEE_KEY}\n"
            "    key_env: FULILIAN_CRS_HENKEE_KEY\n"
            "    transport: anthropic_messages\n"
            "    model: claude-opus-4-7\n"
            "    default_model: claude-opus-4-7\n"
        ...
        provider_info = {
            ...
            "api_mode": "anthropic_messages",
            "provider_key": "crs-henkee",
            "api_key_ref": "${FULILIAN_CRS_HENKEE_KEY}",  # raw template preserved
        }
        ...
        assert entry["api_key"] == "${FULILIAN_CRS_HENKEE_KEY}"
        assert "cr_live_secret_xyz" not in saved_text


class TestCustomProviderDiscoverModels:
    """#18726: honor ``discover_models: false`` in the terminal ``fulilian model``
    ...
    def test_discover_false_with_only_singular_model_skips_probe(self, config_home):
        ...
        with patch("fulilian_cli.models.fetch_api_models") as mock_fetch, \
             patch("fulilian_cli.models.fetch_ollama_local_models") as mock_ollama, \
             patch("fulilian_cli.curses_ui.curses_radiolist", side_effect=ImportError), \
             patch("builtins.input", return_value="1"), \
             patch("builtins.print"):
            _model_flow_named_custom({}, provider_info)
    ...
    def test_probe_empty_falls_back_to_configured_list(self, config_home):
        ...
        with patch("fulilian_cli.models.fetch_api_models", return_value=[]), \
             patch("fulilian_cli.curses_ui.curses_radiolist", side_effect=ImportError), \
             patch("builtins.input", return_value="2"), \
             patch("builtins.print"):
            _model_flow_named_custom({}, provider_info)
