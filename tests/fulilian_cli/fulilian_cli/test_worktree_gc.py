"""Behavior contracts for fulilian_cli.worktree_gc (attended reclaim).
...
def _add_worktree(repo_path, name, branch=None):
    ...
    branch = branch or f"fulilian/{name}"
...
class TestAuditVerdicts:
    ...
    def test_unique_unpushed_commits_keep(self, repo):
        tree, _ = _add_worktree(repo, "fulilian-work")
        (tree / "new.py").write_text("x = 1\n")
        _git(["add", "."], tree)
        _git(["commit", "-m", "unique work"], tree)
        records = worktree_gc.audit_worktrees(str(repo), with_sizes=False)
        record = _verdict(records, "fulilian-work")
        assert record.verdict == "keep"
    ...
    def test_patch_equivalent_commits_reap(self, repo):
        ...
        tree, _ = _add_worktree(repo, "fulilian-merged")
        ...
        records = worktree_gc.audit_worktrees(str(repo), with_sizes=False)
        assert _verdict(records, "fulilian-merged").verdict == "reap"

    def test_live_locked_tree_keeps(self, repo):
        tree, _ = _add_worktree(repo, "fulilian-live")
        _git(["worktree", "lock", str(tree),
              "--reason", f"fulilian pid={os.getpid()}"], repo)
        records = worktree_gc.audit_worktrees(str(repo), with_sizes=False)
        record = _verdict(records, "fulilian-live")
        assert record.verdict == "keep"
...
class TestReclaim:
    def test_reap_removes_tree_and_branch(self, repo):
        tree, branch = _add_worktree(repo, "fulilian-clean")
        records = worktree_gc.audit_worktrees(str(repo), with_sizes=False)
        actions = worktree_gc.reclaim_worktrees(str(repo), records=records)
        assert any("removed fulilian-clean" in a for a in actions)
    ...
    def test_untracked_files_archived_before_removal(self, repo):
        ...
        assert not tree.exists()
        archive_root = Path.home() / ".fulilian" / "archive" / "worktree-prune"
    ...
    def test_dry_run_changes_nothing(self, repo):
        tree, _ = _add_worktree(repo, "fulilian-clean")
        records = worktree_gc.audit_worktrees(str(repo), with_sizes=False)
    ...
    def test_frozen_list_ignores_trees_created_after_audit(self, repo):
        ...
        _add_worktree(repo, "fulilian-old")
        records = worktree_gc.audit_worktrees(str(repo), with_sizes=False)
        late_tree, _ = _add_worktree(repo, "fulilian-late")
        worktree_gc.reclaim_worktrees(str(repo), records=records)
    ...
    def test_dead_locked_tree_is_unlocked_and_reaped(self, repo):
        tree, _ = _add_worktree(repo, "fulilian-zombie")
        _git(["worktree", "lock", str(tree),
              "--reason", "fulilian pid=999999999"], repo)
        records = worktree_gc.audit_worktrees(str(repo), with_sizes=False)
        assert _verdict(records, "fulilian-zombie").verdict == "reap"
        worktree_gc.reclaim_worktrees(str(repo), records=records)
        assert not tree.exists()
...
class TestBranchGC:
    ...
    def test_checked_out_and_protected_kept(self, repo):
        _tree, branch = _add_worktree(repo, "fulilian-active")
        records = worktree_gc.audit_branches(str(repo))
        by_name = {record.name: record for record in records}
