...
* Be supported-platform-only — no-op silently elsewhere so ``fulilian update``
...
class TestInstallCuaDriverUpgrade:
    ...
    def test_install_target_writability_is_probed_for_real_on_macos(self):
        ...
        ``macos_only``: ``/Applications`` is the only install target Fulilian
    ...
    def test_missing_explicit_override_does_not_install_standard_driver(
        ...
        monkeypatch.setenv("FULILIAN_CUA_DRIVER_CMD", "/missing/custom/cua-driver")
        with patch.object(
                 tools_config,
                 "_resolved_cua_driver_cmd",
                 return_value=None,
...
class TestRequireConfirmedUpdate:
    """`fulilian update` passes require_confirmed_update=True: the full
    ...
    Explicit `fulilian computer-use install --upgrade` keeps the old
    ...
    def test_explicit_upgrade_still_falls_through_on_indeterminate(self):
        # `fulilian computer-use install --upgrade` (default flag): the old
    ...
    def test_incompatible_driver_repairs_on_posix_despite_indeterminate_check(self):
        """Fulilian' own version floor is the confirmation. When the installed
        driver fails the runtime contract, the `fulilian update` refresh must
...
class TestUpdateCheckTimeoutDefaults:
    ...
    full reinstall fall-through during `fulilian update`.
    """
...
class TestInstallerTimeoutDrainIsBounded:
    ...
    def test_drain_that_times_out_still_surfaces_the_run_timeout(self):
        ...
        so the existing manual re-run hint prints and ``fulilian update``
...
class TestCuaVersionSummary:
    """`fulilian computer-use status` prints one line, whatever the binary says.

    A binary chosen by FULILIAN_CUA_DRIVER_CMD is under no obligation to answer
