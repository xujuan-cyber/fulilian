...
def fulilian_home(tmp_path, monkeypatch):
    ...
    # /etc/fulilian on the dev/CI box can't influence the test.
    monkeypatch.setenv("FULILIAN_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(fulilian_home, monkeypatch):
    from fulilian_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(fulilian_home, monkeypatch):
    from fulilian_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_fulilian_dotenv(fulilian_home=str(home))
