"""Tests for fulilian_cli configuration management."""
    ...
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            if sys.platform == "win32":
                # Windows default is %LOCALAPPDATA%\fulilian — see
                # fulilian_constants._get_platform_default_fulilian_home.
                ...
                assert home == base / "fulilian"
            else:
                assert home == Path.home() / ".fulilian"


class TestEnsureFulilianHome:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from fulilian_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``fulilian logs`` surfaces it)
    ...
    def test_corrupt_config_is_backed_up(self, tmp_path, capsys):
        ...
        """
        from fulilian_cli import config as cfg_mod
...
class TestEmptyConfigSections:
    ...
    def test_null_override_of_non_dict_default_still_applies(self, tmp_path):
        ...
        from fulilian_cli.config import _deep_merge
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_quotes_values_containing_hash(self, tmp_path):
        ...
        with patch.dict(os.environ, {"FULILIAN_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"FULILIAN_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"FULILIAN_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"FULILIAN_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestSanitizeEnvLines:
    ...
    def test_migrate_reports_normalized_line_formatting(self, capsys):
        ...
        with (
            patch("fulilian_cli.config.sanitize_env_file", return_value=2),
            patch(
                "fulilian_cli.config.check_config_version",
                return_value=(latest_version, latest_version),
            ),
            patch("fulilian_cli.config.read_raw_config", return_value={}),
            patch("fulilian_cli.config.get_missing_env_vars", return_value=[]),
            patch("fulilian_cli.config.get_missing_config_fields", return_value=[]),
            patch("fulilian_cli.config.get_missing_skill_config_vars", return_value=[]),
        ):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from fulilian_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """FULILIAN_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `fulilian setup`) lets a
        ...
        via config.yaml; FULILIAN_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from fulilian_cli.config import OPTIONAL_ENV_VARS
        assert "FULILIAN_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestMemoryProviderEnvVarsRegistry:
    ...
    and `fulilian setup` surface it (previously only Honcho was listed, leaving
...
class TestConfigSupportFloor:
    ...
    def test_v11_config_is_refused_and_untouched(self, tmp_path, capsys):
        ...
        assert "run `fulilian setup` to regenerate" in captured.out
    ...
    def test_floor_message_uses_display_fulilian_home(self):
        from fulilian_cli.config_migrations import support_floor_message
        from fulilian_constants import display_fulilian_home

        msg = support_floor_message()
        assert f"{display_fulilian_home()}/config.yaml" in msg

    def test_registry_has_no_targets_below_floor(self):
        from fulilian_cli.config_migrations import (
            MIGRATIONS,
...
class TestCustomProviderCompatibility:
    ...
    def _run_ladder(current_ver: int):
        from fulilian_cli.config_migrations import run_migrations
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or selected Fulilian runtime/security controls.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Fulilian
    process on next startup via the dotenv → ``os.environ`` chain in
    ``fulilian_cli/env_loader.py``.
    ...
    def _fulilian_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "FULILIAN_SPOTIFY_CLIENT_ID",
            "FULILIAN_QWEN_BASE_URL",
            "FULILIAN_MAX_ITERATIONS",
        ],
    )
    def test_fulilian_integration_keys_still_writable(self, allowed_key):
        """``FULILIAN_*`` overall is NOT blocked.
    ...
    def test_preexisting_optional_mcps_override_still_loads(self, tmp_path):
        ...
        (tmp_path / ".env").write_text(
            f"FULILIAN_OPTIONAL_MCPS={catalog}\n",
            encoding="utf-8",
        )
        invalidate_env_cache()

        assert load_env()["FULILIAN_OPTIONAL_MCPS"] == str(catalog)

    @pytest.mark.parametrize(
        ("key", "expected"),
        [
            ("Path", "PATH"),
            ("Fulilian_Yolo_Mode", "FULILIAN_YOLO_MODE"),
            ("Fulilian_Optional_Mcps", "FULILIAN_OPTIONAL_MCPS"),
            ("Fulilian_Copilot_Acp_Command", "FULILIAN_COPILOT_ACP_COMMAND"),
            ("Fulilian_Copilot_Acp_Args", "FULILIAN_COPILOT_ACP_ARGS"),
        ],
    )
    def test_windows_policy_names_are_case_insensitive(self, key, expected):
        from fulilian_cli.config import _env_var_policy_name
    ...
    def test_windows_env_assignment_matching_is_case_insensitive(self, prefix):
        from fulilian_cli.config import _env_line_defines_key
...
"""
    ...
    def test_persist_migration_writes_full_read_raw_config(self, tmp_path):
        from fulilian_cli.config import _persist_migration, read_raw_config
    ...
    def test_preserves_existing_codex_app_server_auto_value(self, tmp_path):
        with patch.dict(os.environ, {"FULILIAN_HOME": str(tmp_path)}):
            self._write(
                ...
                "_config_version: 31\n"
                "compression:\n"
                "  codex_app_server_auto: fulilian\n",
            )

            migrate_config(interactive=False, quiet=True)

            raw = yaml.safe_load((tmp_path / "config.yaml").read_text())
            assert raw["compression"]["codex_app_server_auto"] == "fulilian"
...
class TestConfigCommandFailClosedSurface:
    """`fulilian config set/unset` must exit cleanly (no traceback) when the
