"""Tests for fulilian_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import fulilian_cli.gui_uninstall as gu


def _make_agent(fulilian_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = fulilian_home / "fulilian-agent"
    (agent_root / "fulilian_cli").mkdir(parents=True)
    (agent_root / "fulilian_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(fulilian_home: Path) -> None:
    """Create the source-built GUI artifacts a `fulilian desktop` run produces."""
    desktop = fulilian_home / "fulilian-agent" / "apps" / "desktop"
    ...
    (fulilian_home / "fulilian-agent" / "node_modules").mkdir(parents=True)
    (fulilian_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(fulilian_home: Path) -> None:
    (fulilian_home / "config.yaml").write_text("x: 1\n")
    (fulilian_home / ".env").write_text("KEY=secret\n")
    (fulilian_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(fulilian_home)
    _make_gui_build(fulilian_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(fulilian_home)
    ...
    assert summary["fulilian_home"] == str(fulilian_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `fulilian desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    fulilian_home = tmp_path / ".fulilian"
    _make_agent(fulilian_home)
    icon = lde.icon_path(fulilian_home / "fulilian-agent")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(fulilian_home)
    ...
    assert lde.icon_path(fulilian_home / "fulilian-agent").exists()
    # The agent itself survives a GUI uninstall.
    assert (fulilian_home / "fulilian-agent" / "fulilian_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".fulilian")
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import fulilian_cli.uninstall as uninstall
