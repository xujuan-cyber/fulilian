"""Comprehensive tests for fulilian_cli.profiles module.
...
from fulilian_cli import profiles
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and FULILIAN_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.fulilian/profiles)
    * FULILIAN_HOME  -> tmp_path/.fulilian  (so get_fulilian_home() agrees)
    * Creates the bare-minimum ~/.fulilian directory.
    """
...
class TestGetProfileDir:
    ...
    def test_default_returns_fulilian_home(self, profile_env):
        tmp_path = profile_env
        result = get_profile_dir("default")
        assert result == tmp_path / ".fulilian"
...
class TestNoSkillsOptOut:
    """Tests for `fulilian profile create --no-skills` and the opt-out marker."""
    ...
    def test_delete_marker_re_enables_seeding(self, profile_env, monkeypatch):
        ...
        called = []
        stdout_by_call = [
            '{"copied": ["fulilian-agent"], "skipped_opt_out": true}',
            '{"copied": []}',
        ]
        monkeypatch.setattr(
            "subprocess.run",
        ...
        assert r1.get("copied") == ["fulilian-agent"]
        assert len(called) == 1
...
class TestBackfillProfileEnvs:
    """Tests for backfill_profile_envs() — the `fulilian update` pass that
    ...
    def test_copies_default_env_into_envless_profiles(self, profile_env):
        import stat
        tmp_path = profile_env
        (tmp_path / ".fulilian" / ".env").write_text("OPENROUTER_API_KEY=root-key\n")
...
class TestDeleteProfile:
    ...
    def test_rmtree_failure_raises(self, profile_env):
        ...
        with patch("fulilian_cli.profiles._cleanup_gateway_service"), \
             patch("fulilian_cli.profiles.time.sleep"), \
             patch("fulilian_cli.profiles.shutil.rmtree", side_effect=PermissionError("locked")):
    ...
    def test_backend_scan_only_matches_this_profile(self, profile_env, monkeypatch):
        ...
        procs = [
            ...
            FakeProc(101, ["python", "-m", "fulilian_cli.main", "--profile", "coder", "serve"]),
            # Interactive chat for coder → NOT a backend subcommand, skipped.
            FakeProc(102, ["python", "-m", "fulilian_cli.main", "--profile", "coder", "chat"]),
            # Backend for a different profile → skipped.
            FakeProc(103, ["python", "-m", "fulilian_cli.main", "--profile", "other", "serve"]),
            # This very process → skipped even if it matched.
            FakeProc(self_pid, ["python", "-m", "fulilian_cli.main", "--profile", "coder", "serve"]),
        ]
    ...
    def test_backend_scan_matches_shebang_exec_of_fulilian_shim(self, profile_env, monkeypatch):
        """A `fulilian` console-script shim spawned directly (e.g. Electron's
        findOnPath('fulilian') resolution) reports argv[0] as the interpreter
        (python3) and argv[1] as the shim's path -- not "fulilian" -- because
        ...
        procs = [
            ...
            # being the python interpreter, not "fulilian".
            FakeProc(201, ["/usr/bin/python3", "/Users/x/.local/bin/fulilian", "--profile", "coder", "serve",
                            "--host", "127.0.0.1", "--port", "0"]),
            # Same shape but a different profile → skipped.
            FakeProc(202, ["/usr/bin/python3", "/Users/x/.local/bin/fulilian", "--profile", "other", "serve"]),
            # Non-fulilian script run by python3 → skipped.
    ...
    def test_backend_scan_rejects_unrelated_fulilian_prefixed_script(self, profile_env, monkeypatch):
        """A user's own script that happens to start with "fulilian" (e.g.
        fulilian-notes.py, fulilian-unrelated-tool) must NOT be misidentified as
        the console-script shim just because argv[0] is a python interpreter
        and argv[1]'s basename starts with "fulilian" -- only the actual known
        console-script entry points (fulilian, fulilian-agent, fulilian-acp) count.
        """
        create_profile("coder", no_alias=True)
        profile_dir = get_profile_dir("coder")
        ...
        procs = [
            ...
            FakeProc(301, ["/usr/bin/python3", "/Users/x/scripts/fulilian-notes.py",
                            "--profile", "coder", "serve"]),
            FakeProc(302, ["/usr/bin/python3", "/Users/x/scripts/fulilian-unrelated-tool",
                            "--profile", "coder", "serve"]),
        ]
    ...
    def test_backend_scan_matches_all_known_console_script_shims(self, profile_env, monkeypatch):
        """The other two real console-script entry points (fulilian-agent,
        fulilian-acp -- see pyproject.toml [project.scripts]) must also be
        recognized via the shebang-exec path, not just the primary "fulilian"
        shim.
        """
        create_profile("coder", no_alias=True)
        profile_dir = get_profile_dir("coder")
        ...
        self_pid = os.getpid()
        procs = [
            FakeProc(401, ["/usr/bin/python3", "/Users/x/.local/bin/fulilian-agent",
                            "--profile", "coder", "serve"]),
            FakeProc(402, ["/usr/bin/python3", "/Users/x/.local/bin/fulilian-acp",
                            "--profile", "coder", "serve"]),
        ]
...
class TestActiveProfile:
    ...
    def test_set_to_default_removes_file(self, profile_env):
        ...
        create_profile("coder", no_alias=True)
        set_active_profile("coder")
        active_path = tmp_path / ".fulilian" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom FULILIAN_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("FULILIAN_HOME", str(custom))
        # With Docker-aware roots, a custom FULILIAN_HOME is the default —
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        monkeypatch.setattr("fulilian_cli.profiles.shutil.which", lambda name: "/opt/fulilian/bin/fulilian")
        from fulilian_cli.profiles import create_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
        assert wrapper.name == "mybot"
        content = wrapper.read_text()
        assert content.startswith("#!/bin/sh")
        assert "exec /opt/fulilian/bin/fulilian -p mybot" in content


    @pytest.mark.windows_only
    def test_remove_finds_bat_on_windows(self, profile_env):
        from fulilian_cli.profiles import create_wrapper_script, remove_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
...
class TestFindAliasForProfile:
    ...
    def test_profile_named_alias(self, profile_env):
        from fulilian_cli.profiles import create_wrapper_script, find_alias_for_profile
    ...
    def test_ignores_unrelated_files(self, profile_env):
        ...
        from fulilian_cli.profiles import _get_wrapper_dir, find_alias_for_profile
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        old_dir = tmp_path / ".fulilian" / "profiles" / "oldname"
        assert old_dir.is_dir()

        # Mock alias collision to avoid subprocess calls
        with patch("fulilian_cli.profiles.check_alias_collision", return_value="skip"):
            new_dir = rename_profile("oldname", "newname")

        assert not old_dir.is_dir()
        assert new_dir.is_dir()
        assert new_dir == tmp_path / ".fulilian" / "profiles" / "newname"

    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        tmp_path = profile_env
        create_profile("ssi_health", no_alias=True)
        honcho_path = tmp_path / ".fulilian" / "honcho.json"
        ...
        assert "fulilian.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["fulilian_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["fulilian_heimdall"]["peerName"] == "user-peer"
...
# ===================================================================
# TestGetProfilesRoot / TestGetDefaultFulilianHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_fulilian_home()."""



    def test_default_fulilian_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_fulilian_home() returns FULILIAN_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("FULILIAN_HOME", str(docker_home))
        home = _get_default_fulilian_home()
        assert home == docker_home



    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under FULILIAN_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("FULILIAN_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_gateway_running_check_falls_back_to_runtime_state(self, profile_env):
        ...
        from fulilian_cli.profiles import _check_gateway_running
        ...
        with patch("gateway.status.get_running_pid", return_value=None), patch(
            ...
            return_value="fulilian gateway run --replace",
        ):
...
class TestProfilesToServe:
    ...
    def test_off_returns_only_active_named(self, profile_env, monkeypatch):
        # A named profile's gateway runs with FULILIAN_HOME pointing at the
        # profile dir; get_active_profile_name() infers the name from there.
        create_profile("coder", no_alias=True)
        monkeypatch.setenv("FULILIAN_HOME", str(get_profile_dir("coder")))
    ...
    def test_on_returns_default_plus_all_named(self, profile_env):
        ...
        assert serve["default"] == _get_default_fulilian_home()
...
class TestResolveProfileEnvSpelling:
    """resolve_profile_env() keeps the configured FULILIAN_HOME spelling as
    ...
    def test_resolution_matrix_preserves_configured_spelling(self, monkeypatch, tmp_path):
        ...
        custom = tmp_path / "custom-fulilian"
        ...
        for env_home, profile, expected in cases:
            monkeypatch.setenv("FULILIAN_HOME", str(env_home))
    ...
    def test_unset_env_falls_back_to_default_root(self, monkeypatch):
        # No FULILIAN_HOME: the platform default root applies (existing contract).
        monkeypatch.delenv("FULILIAN_HOME", raising=False)
        assert Path(resolve_profile_env("default")) == _get_default_fulilian_home()
