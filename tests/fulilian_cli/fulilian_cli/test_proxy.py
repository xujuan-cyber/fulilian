"""Tests for the `fulilian proxy` subcommand and its upstream adapters."""
...
def _write_xai_pool_entry(
    ...
    *,
    ...
    """Write an xai-oauth pool entry into a hermetic FULILIAN_HOME."""
    auth_path = fulilian_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {},
...
def test_xai_adapter_retry_rotates_pool_entry_on_429(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("fulilian_cli.auth.refresh_xai_oauth_pure", _refresh_must_not_run)
...
from fulilian_cli.proxy.server import create_app  # noqa: E402
