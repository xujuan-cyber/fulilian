...
def test_config_get_unset_subcommands_parse():
    """`fulilian config get/unset` parse key args (and --json for get)."""
    parser = argparse.ArgumentParser(prog="fulilian")
...
# ── deprecated `fulilian login` fails gracefully, not with argparse error ────
#
# `fulilian login` is a removed command; its handler (`login_command` in
# `fulilian_cli/auth.py`) prints a deprecation notice pointing at `fulilian auth` /
# `fulilian model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `fulilian --help`.
