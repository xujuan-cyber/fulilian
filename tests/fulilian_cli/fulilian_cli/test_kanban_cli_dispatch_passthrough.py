...
def isolated_kanban_home(monkeypatch):
    """Spin up a fresh FULILIAN_HOME with a clean kanban DB."""
    test_home = tempfile.mkdtemp(prefix="kanban_cli_passthrough_")
    os.makedirs(os.path.join(test_home, "profiles", "default"), exist_ok=True)
    monkeypatch.setenv("FULILIAN_HOME", test_home)
    for mod in list(sys.modules.keys()):
        if mod.startswith("fulilian_cli") or mod.startswith("fulilian_state") or mod == "fulilian_constants":
            del sys.modules[mod]
    yield test_home


def test_cli_dispatch_passes_max_in_progress_from_config(isolated_kanban_home, monkeypatch):
    """#33488: fulilian kanban dispatch must pass kanban.max_in_progress from
    ...
    monkeypatch.setattr(
        "fulilian_cli.config.load_config", lambda: fake_config
    )
...
def test_cli_max_flag_overrides_config_max_spawn(isolated_kanban_home, monkeypatch):
    ...
    monkeypatch.setattr("fulilian_cli.config.load_config", lambda: fake_config)
