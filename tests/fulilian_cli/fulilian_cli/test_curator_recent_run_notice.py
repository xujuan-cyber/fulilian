...
The notice prints the most recent curator run summary on `fulilian update`,
...
session start) so users normally never see the rename map. `fulilian update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    importlib.reload(fulilian_main)
