"""Tests for fulilian_cli.agent_import — ``fulilian import-agent``.

Covers: source detection, Claude Code and Codex parsing, mapping into the
real Fulilian stores (memories/MEMORY.md, config.yaml command_allowlist /
...
Uses the profile_env fixture pattern from tests/fulilian_cli/test_profiles.py:
Path.home() and FULILIAN_HOME are redirected to tmp_path so nothing touches
the real ~/.fulilian.
"""
...
from fulilian_cli.agent_import import (
    ENTRY_DELIMITER,
    AgentImporter,
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and FULILIAN_HOME (profile_env pattern)
# ---------------------------------------------------------------------------

@pytest.fixture()
def profile_env(tmp_path, monkeypatch):
    """Isolated environment: Path.home() -> tmp_path, FULILIAN_HOME -> tmp/.fulilian."""
...
@pytest.fixture()
def fulilian_home(profile_env):
    return profile_env / ".fulilian"
...
def run_import(agent, source, fulilian_home, execute, overwrite=False):
    return AgentImporter(
        agent=agent,
        source_root=source,
        target_root=fulilian_home,
        execute=execute,
        overwrite=overwrite,
    ).run()
...
class TestDetection:
    ...
    def test_unsupported_agent_raises(self, fulilian_home, tmp_path):
        with pytest.raises(ValueError):
            AgentImporter("cursor", tmp_path, fulilian_home)
...
class TestDryRun:
    def test_claude_dry_run_writes_nothing(self, claude_tree, fulilian_home):
        before = snapshot_tree(fulilian_home)
        report = run_import("claude-code", claude_tree, fulilian_home, execute=False)
        assert snapshot_tree(fulilian_home) == before
        assert report["dry_run"] is True
        assert report["summary"]["imported"] > 0


    def test_dry_run_and_real_run_plan_same_items(self, claude_tree, fulilian_home):
        preview = run_import("claude-code", claude_tree, fulilian_home, execute=False)
        applied = run_import("claude-code", claude_tree, fulilian_home, execute=True)
...
class TestClaudeCodeImport:
    @pytest.fixture()
    def report(self, claude_tree, fulilian_home):
        return run_import("claude-code", claude_tree, fulilian_home, execute=True)


    def test_allowlist_lands_in_config_yaml(self, report, fulilian_home):
        config = yaml.safe_load((fulilian_home / "config.yaml").read_text())
...
class TestCodexImport:
    @pytest.fixture()
    def report(self, codex_tree, fulilian_home):
        return run_import("codex", codex_tree, fulilian_home, execute=True)


    def test_mcp_servers_from_config_toml(self, report, fulilian_home):
        config = yaml.safe_load((fulilian_home / "config.yaml").read_text())
    ...
    def test_skill_copied(self, report, fulilian_home):
        assert (fulilian_home / "skills" / "codex-imports" / "db-migrate" / "SKILL.md").exists()
...
class TestSecretsNeverImported:
    def test_no_secret_values_anywhere_claude(self, claude_tree, fulilian_home):
        run_import("claude-code", claude_tree, fulilian_home, execute=True)
        blob = "".join(
            p.read_text(encoding="utf-8", errors="replace")
            for p in fulilian_home.rglob("*") if p.is_file()
        )
        assert "ghp_SECRET123" not in blob
        assert "Bearer abc123" not in blob
        assert "sk-ant-SUPERSECRET" not in blob

    def test_no_secret_values_anywhere_codex(self, codex_tree, fulilian_home):
        run_import("codex", codex_tree, fulilian_home, execute=True)
        blob = "".join(
            p.read_text(encoding="utf-8", errors="replace")
            for p in fulilian_home.rglob("*") if p.is_file()
        )
    ...
    def test_stripped_secrets_reported(self, claude_tree, fulilian_home):
        report = run_import("claude-code", claude_tree, fulilian_home, execute=True)
    ...
    def test_non_secret_header_kept(self, claude_tree, fulilian_home):
        run_import("claude-code", claude_tree, fulilian_home, execute=True)
        config = yaml.safe_load((fulilian_home / "config.yaml").read_text())
...
class TestMalformedInputs:
    def test_bad_settings_json_reports_error(self, profile_env, fulilian_home):
        root = profile_env / ".claude"
        root.mkdir()
        (root / "settings.json").write_text("{not json!!", encoding="utf-8")
        (root / "CLAUDE.md").write_text("- still importable\n", encoding="utf-8")
        report = run_import("claude-code", root, fulilian_home, execute=True)
        ...
        assert "still importable" in (
            fulilian_home / "memories" / "MEMORY.md").read_text()


    def test_empty_tree_all_skipped(self, profile_env, fulilian_home):
        root = profile_env / ".codex"
        root.mkdir()
        report = run_import("codex", root, fulilian_home, execute=True)
        assert report["summary"]["imported"] == 0
...
class TestMergeSemantics:
    ...
    def test_existing_mcp_server_conflicts_without_overwrite(
            self, claude_tree, fulilian_home):
    ...
    def test_existing_skill_conflicts_without_overwrite(
            self, claude_tree, fulilian_home):
        dest = fulilian_home / "skills" / "claude-code-imports" / "deploy-helper"
    ...
    def test_reimport_is_idempotent_for_memory(self, claude_tree, fulilian_home):
        run_import("claude-code", claude_tree, fulilian_home, execute=True)
        first = (fulilian_home / "memories" / "MEMORY.md").read_text()
        report = run_import("claude-code", claude_tree, fulilian_home, execute=True)
        assert (fulilian_home / "memories" / "MEMORY.md").read_text() == first
...
class TestExistingMemoryStorePreserved:
    ...
    @pytest.fixture()
    def seeded_home(self, fulilian_home):
        memory = fulilian_home / "memories" / "MEMORY.md"
...
class TestExistingConfigPreserved:
    ...
    @pytest.fixture()
    def config_path(self, fulilian_home):
        return fulilian_home / "config.yaml"
    ...
    def test_malformed_config_is_left_byte_identical(
            self, claude_tree, fulilian_home, config_path):
    ...
    def test_malformed_config_records_an_error_not_an_import(
        ...
        for kind in CONFIG_WRITING_KINDS:
            ...
            assert "fulilian config edit" in reason
    ...
    def test_unreadable_config_is_left_byte_identical(
        ...
        try:
            report = run_import("claude-code", claude_tree, fulilian_home,
                                execute=True)
            statuses = {
                i["status"]
                for kind in CONFIG_WRITING_KINDS
    ...
    def test_config_write_is_atomic_and_leaves_no_temp_files(
        ...
        leftovers = [p.name for p in fulilian_home.glob(".tmp*")]
        assert leftovers == []

    def test_symlinked_config_stays_a_symlink(
            self, claude_tree, fulilian_home, tmp_path, config_path):
    ...
    def test_failed_write_does_not_truncate_the_existing_config(
            self, fulilian_home, config_path, monkeypatch):
        """An interrupted dump leaves the previous file complete."""
        from fulilian_cli import agent_import
...
class TestCliWiring:
    ...
    def test_command_dry_run_via_cli_writes_nothing(
            self, claude_tree, fulilian_home, capsys):
        ...
        assert not (fulilian_home / "memories" / "MEMORY.md").exists()
        assert not (fulilian_home / "skills" / "claude-code-imports").exists()
        config_text = (fulilian_home / "config.yaml").read_text(encoding="utf-8") \
            if (fulilian_home / "config.yaml").exists() else ""
