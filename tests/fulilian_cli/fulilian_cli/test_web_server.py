"""Tests for fulilian_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_fulilian_home):
    """Drop the example-dashboard fixture into the per-test FULILIAN_HOME
    ...
    isolated ``FULILIAN_HOME``.

    The user-plugin source is preferred over a transient
    ``FULILIAN_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, fulilian-achievements, model providers
    ...
    # plugin in exactly as a real operator would with `fulilian plugins
    # enable example`.
    ...
    #      and start serving requests against a torn-down FULILIAN_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Fulilian vars not present in .env."""
        env_file = tmp_path / ".env"
...
class TestSessionTokenInjection:
    """The desktop shell mints FULILIAN_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        import fulilian_cli.web_server as ws

        original_app = ws.app
        original_token = ws._SESSION_TOKEN
        monkeypatch.setenv("FULILIAN_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
    ...
    def test_falls_back_to_random_token(self, monkeypatch):
        ...
        monkeypatch.delenv("FULILIAN_DASHBOARD_SESSION_TOKEN", raising=False)
        with patch.object(
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_fulilian_home):
        """Create a TestClient and isolate the state DB under the test FULILIAN_HOME."""
        try:
            from starlette.testclient import TestClient
        except ImportError:
        ...
        from fulilian_constants import get_fulilian_home
        from fulilian_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(fulilian_state, "DEFAULT_DB_PATH", get_fulilian_home() / "state.db")
    ...
    def test_profiles_sidebar_heals_stale_schema_store(self):
        ...
        array — the desktop rendered "No sessions yet" after `fulilian update`
    ...
    def test_startup_eager_reconcile_heals_stale_store(self):
        ...
        #79531/#80037: after `fulilian update` an old-schema state.db used to
    ...
    def test_startup_eager_reconcile_never_raises(self, monkeypatch):
        ...
        monkeypatch.setattr(fulilian_state, "SessionDB", boom)
    ...
    def test_heal_gives_up_when_reconcile_cannot_fix_the_store(self, monkeypatch):
        ...
        original_init = fulilian_state.SessionDB.__init__
        ...
        # the class on fulilian_state covers every open the helper makes.
        monkeypatch.setattr(fulilian_state.SessionDB, "__init__", counting_init)
    ...
    def test_generic_corruption_does_not_trigger_writable_heal(
        ...
        monkeypatch.setattr(fulilian_state, "SessionDB", corrupt_open)
    ...
    def test_messaging_platforms_profile_scopes_gateway_reads(self, monkeypatch):
        ...
        FULILIAN_HOME contextvar override (#56986), so /api/messaging/platforms
    ...
    def test_declared_surface_put_writes_config_and_secret(self):
        from fulilian_constants import get_fulilian_home
        from fulilian_cli.config import load_env
        ...
        config_path = get_fulilian_home() / "hindsight" / "config.json"
    ...
    def test_put_memory_provider_config_writes_config_and_secret(self):
        from fulilian_constants import get_fulilian_home
        from fulilian_cli.config import load_config, load_env
    ...
    def _isolate_honcho_config(self):
        # Honcho tests write the suite-wide FULILIAN_HOME honcho.json; snapshot and
        # restore it so provider status/config state never leaks across tests.
        from fulilian_constants import get_fulilian_home

        path = get_fulilian_home() / "honcho.json"
    ...
    def test_put_honcho_writes_host_block_root_and_secret(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_fulilian_home() / "honcho.json").read_text(encoding="utf-8"))
        # baseUrl is root-scoped; the rest live in the active host block.
        assert cfg["baseUrl"] == "https://honcho.example.dev"
        assert cfg["hosts"]["fulilian"]["workspace"] == "myws"
        assert cfg["hosts"]["fulilian"]["peerName"] == "eri"
        assert cfg["hosts"]["fulilian"]["environment"] == "local"
        assert cfg["hosts"]["fulilian"]["sessionStrategy"] == "per-repo"
        # The key lands where the client reads first; GET keeps it write-only.
        assert cfg["hosts"]["fulilian"]["apiKey"] == "hch-test-key"
    ...
    def test_get_media_requires_auth(self):
        from fulilian_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_update_fulilian_returns_docker_guidance_without_spawning(self, monkeypatch):
        import fulilian_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn fulilian update")
        ...
        # method through fulilian_cli.config directly.
        monkeypatch.setattr(
            "fulilian_cli.config.detect_install_method", lambda *_a, **_k: "docker"
        )
        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(web_server, "_spawn_fulilian_action", fail_spawn)
        web_server._ACTION_PROCS.pop("fulilian-update", None)
        web_server._ACTION_RESULTS.pop("fulilian-update", None)

        resp = self.client.post("/api/fulilian/update")
    ...
    def test_update_fulilian_returns_apt_guidance_without_spawning(self, monkeypatch):
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("APT-managed update guard should not spawn fulilian update")

        monkeypatch.setattr(web_server, "_dashboard_local_update_managed_externally", lambda: False)
        # The shared admission gate (#91277 Phase 3) resolves the install
        # method through fulilian_cli.config directly, so patch it there (the
        # web_server module alias only feeds the /update/check endpoint).
        monkeypatch.setattr(
            "fulilian_cli.config.detect_install_method", lambda *_a, **_k: "apt"
        )
        ...
        assert data["update_command"] == "pkg upgrade fulilian-agent"
        assert spawned is False

        check = self.client.get("/api/fulilian/update/check")
        assert check.status_code == 200
        check_data = check.json()
        assert check_data["install_method"] == "apt"
        assert check_data["can_apply"] is False
        assert check_data["update_command"] == "pkg upgrade fulilian-agent"
    ...
    def test_update_status_recovers_completed_result_after_dashboard_restart(self, monkeypatch, tmp_path):
        ...
        action_id = "c" * 32
        (tmp_path / "fulilian-update.log").write_text(
            "=== fulilian-update started 2026-08-17 11:19:34 ===\n"
            "pulling updates...\n",
            encoding="utf-8",
        )
        (tmp_path / "update.log").write_text(
            "=== fulilian update started 2026-08-17T11:19:35 ===\n"
            "✓ Update complete!\n"
            f"=== fulilian-update completed {action_id} ===\n",
            encoding="utf-8",
        )
        ...
        status = self.client.get("/api/actions/fulilian-update/status?lines=2000")
        ...
        assert f"=== fulilian-update completed {action_id} ===" in data["lines"]

    def test_update_fulilian_spawns_with_action_id(self, monkeypatch):
        import fulilian_cli.web_server as web_server
        ...
        monkeypatch.setattr(web_server, "_spawn_fulilian_action", fake_spawn)
        web_server._ACTION_PROCS.pop("fulilian-update", None)
        web_server._ACTION_RESULTS.pop("fulilian-update", None)
        ...
        assert calls == [
            (["update"], "fulilian-update", {"FULILIAN_ACTION_ID": "a" * 32})
        ]

    def test_update_fulilian_reuses_running_action(self, monkeypatch):
        import fulilian_cli.web_server as web_server
        ...
        web_server._ACTION_PROCS["fulilian-update"] = Proc()
        web_server._ACTION_IDS["fulilian-update"] = "b" * 32

        try:
            resp = self.client.post("/api/fulilian/update")
        finally:
            web_server._ACTION_PROCS.pop("fulilian-update", None)
            web_server._ACTION_IDS.pop("fulilian-update", None)
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        ...
        tests/fulilian_cli/test_setup_hidden_env.py.
        """
        from fulilian_cli.web_server import (
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a Fulilian provider — keep the user's aggregator instead of writing a
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from fulilian_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from fulilian_cli.config import save_env_value
        from fulilian_cli.web_server import _SESSION_TOKEN
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "suggested_username": "fulilian_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/FulilianSetupBot/fulilian_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/FulilianSetupBot/fulilian_pair_restart_fails_bot",
            ...
            return {
                "status": "ready",
                "bot_username": "fulilian_pair_restart_fails_bot",
        ...
        monkeypatch.setattr(ws, "_spawn_fulilian_action", fail_spawn_action)
    ...
    def test_unauthenticated_api_blocked(self):
        ...
        from starlette.testclient import TestClient
        from fulilian_cli.web_server import app
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from fulilian_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``fulilian model`` custom flow. Regression for the desktop loop where a
    ...
    def test_deleting_the_active_custom_endpoint_clears_its_model_mirror(self):
        ...
        """
        from fulilian_cli.config import custom_endpoint_key_env, get_env_value, load_config
    ...
    def test_numeric_yaml_provider_key_can_be_activated_and_deleted(self):
        ...
        """
        from fulilian_cli.config import get_config_path, load_config
    ...
    def test_custom_endpoint_save_scopes_to_the_requested_profile(self):
        ...
        """
        from fulilian_cli import profiles as profiles_mod
        from fulilian_cli.config import custom_endpoint_key_env
        from fulilian_constants import get_fulilian_home

        default_home = get_fulilian_home()
        worker_home = profiles_mod.get_profile_dir("worker")
        worker_home.mkdir(parents=True)
    ...
    def test_custom_endpoint_save_leaves_a_hand_written_env_ref_alone(self, monkeypatch):
        ...
        from fulilian_cli.config import custom_endpoint_key_env, get_config_path, get_env_value
    ...
    def test_two_endpoints_on_one_host_keep_separate_credentials(self):
        ...
        the first's key.
        """
        from fulilian_cli.config import custom_endpoint_key_env, get_env_value
...
class TestBuildSchemaFromConfig:
    ...
    def test_overrides_applied(self):
        from fulilian_cli.web_server import CONFIG_SCHEMA
    ...
    def test_timezone_field_is_searchable_select(self):
        ...
        """
        from fulilian_cli.web_server import CONFIG_SCHEMA, _timezone_options
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from fulilian_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_fulilian_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_profiles_create_builder_mcp_auth_is_profile_scoped(
        ...
        from fulilian_constants import get_fulilian_home
        import fulilian_cli.profiles as profiles_mod
    ...
    def test_toolsets_resolve_subscription_features_once(self, monkeypatch):
        import fulilian_cli.tools_config as tools_config
        from fulilian_cli.nous_subscription import NousSubscriptionFeatures
    ...
    def test_get_toolset_config_reports_truthful_provider_status(self, monkeypatch):
        ...
        """
        import fulilian_cli.tools_config as tools_config
        from fulilian_cli.nous_account import NousPortalAccountInfo

        # Logged out of Nous Portal → managed subscription rows need sign-in.
        monkeypatch.setattr(
            "fulilian_cli.nous_subscription.get_nous_portal_account_info",
    ...
    def test_terminal_ssh_probe_ready_when_configured(self, monkeypatch):
        ...
        config["terminal"]["ssh_user"] = "fulilian"
        save_config(config)
        ...
        assert "fulilian@devbox.example.com" in ssh["detail"]
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from fulilian_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from fulilian_cli.web_server import _denormalize_config_from_web
...
class TestStatusInstallId:
    ...
    Fulilian home (not the profile home), survives a fresh process-cache read,
    ...
    def test_status_reports_persistent_install_id(self, monkeypatch):
        ...
        id_file = get_default_fulilian_root() / "install_id"
    ...
    def test_all_profiles_of_one_install_share_the_id(self, monkeypatch, tmp_path):
        """FULILIAN_HOME=<root> and FULILIAN_HOME=<root>/profiles/<name> resolve to
        the same id file — profiles share one physical install identity."""
        import fulilian_cli.web_server as ws

        root = tmp_path / "fulilian-root"
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_non_dict(self):
        from fulilian_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.fulilian/dashboard-themes/."""
    ...
    def test_ignores_transient_profile_override(self, tmp_path, monkeypatch):
        ...
        token = set_fulilian_home_override(str(other))
        try:
            results = web_server._discover_user_themes()
        finally:
...
class TestThemeBootstrapCSS:
    ...
    @staticmethod
    def _write_theme(fulilian_home, name="ocean"):
        themes_dir = fulilian_home / "dashboard-themes"
    ...
    def test_user_theme_renders_bundle_vars(self, tmp_path, monkeypatch):
        ...
        assert css.startswith('<style id="fulilian-theme-bootstrap">')
...
class TestDeleteSessionEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_fulilian_home):
        ...
        monkeypatch.setattr(
            fulilian_state, "DEFAULT_DB_PATH", get_fulilian_home() / "state.db"
        )
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``fulilian_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in fulilian_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_fulilian_home):
        ...
        # Pin the SessionDB to the isolated FULILIAN_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_fulilian_home, _install_example_plugin):
        ...
        installs it into the per-test ``FULILIAN_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into FULILIAN_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, fulilian-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (fulilian-achievements is loaded by default).
        resp = self.client.get("/api/plugins/fulilian-achievements/overview")
        assert resp.status_code == 401
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_ignore_profile_home_override(self, tmp_path, monkeypatch):
        ...
        FULILIAN_HOME override scopes a request to another profile."""
        from fulilian_constants import (
            reset_fulilian_home_override,
            set_fulilian_home_override,
        )
        ...
        monkeypatch.setenv("FULILIAN_HOME", str(launch_home))
        from fulilian_cli import web_server
        token = set_fulilian_home_override(str(other))
        try:
    ...
    def test_user_plugins_found_under_profile_scoped_process(self, tmp_path, monkeypatch):
        ...
        sets FULILIAN_HOME=<root>/profiles/<name>) must still discover user
        plugins installed in the fulilian root's plugins/ directory."""
        root = tmp_path / "fulilian-root"
...
# or ``sh -c 'printf …'``) instead of the real ``fulilian --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_tui_python_command_uses_child_path(self, tmp_path):
        ...
        command = f"fulilian-review-python{Path(sys.executable).suffix}"
        ...
        env = {
            "FULILIAN_CWD": str(tmp_path),
            "FULILIAN_PYTHON": command,
            "PATH": str(bin_dir),
        }

        main_mod._apply_tui_python_env(env)

        assert env["FULILIAN_PYTHON"] == command
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from fulilian_cli.pty_bridge import PtyUnavailableError
        ...
        import fulilian_cli.web_server as ws_mod
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from fulilian_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    import fulilian_cli.main as cli_main
    import fulilian_cli.web_server as ws
    ...
    gateway_url = env.get("FULILIAN_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_fulilian_home, _install_example_plugin):
        ...
        the per-test ``FULILIAN_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_fulilian_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.setenv("FULILIAN_DESKTOP", "1")

        with self._client():
            assert called.wait(3.0), "expected cron tick under FULILIAN_DESKTOP=1"
...
class TestServeIndexMissingIndex:
    ...
    def _client_with_dist(tmp_path, monkeypatch, *, write_index: bool):
        ...
        monkeypatch.setattr(ws, "WEB_DIST", dist)
        monkeypatch.delenv("FULILIAN_SERVE_HEADLESS", raising=False)
        spa_app = FastAPI()
        ws.mount_spa(spa_app)
...
class TestHeadlessServeTokenPage:
    """Headless `fulilian serve` must serve the Desktop token handshake page
    at `/` when the dashboard auth gate is off (#94227).

    The Electron renderer boots by fetching `/` and extracting
    ``window.__FULILIAN_SESSION_TOKEN__`` for WebSocket auth. Headless serve
    ...
    def _headless_client(monkeypatch, *, gated: bool):
        ...
        monkeypatch.setenv("FULILIAN_SERVE_HEADLESS", "1")
        spa_app = FastAPI()
    ...
    def test_root_serves_token_page_when_not_gated(self, monkeypatch):
        ...
        match = re.search(
            r'window\.__FULILIAN_SESSION_TOKEN__\s*=\s*("(?:\\.|[^"\\])*")',
            resp.text,
        )
        ...
        assert "window.__FULILIAN_AUTH_REQUIRED__=false" in resp.text
...
class TestHashedAssetCacheHeaders:
    ...
    def test_serve_css_is_immutable_and_keeps_prefix_rewrites(
        ...
        prefixed = client.get(
            ...
            headers={"X-Forwarded-Prefix": "/fulilian"},
        )
        assert prefixed.status_code == 200
        assert prefixed.headers["cache-control"] == self._IMMUTABLE
        assert "url(/fulilian/ds-assets/bg.png)" in prefixed.text
        assert "url(/fulilian/fonts-terminal/x.woff2)" in prefixed.text
