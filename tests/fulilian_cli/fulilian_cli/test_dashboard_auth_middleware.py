...
def _complete_stub_login(client) -> None:
    ...
    client's cookie jar holds ``fulilian_session_at`` / ``fulilian_session_rt``
...
class _UnreachableProvider(StubAuthProvider):
    ...
    def verify_session(self, *, access_token: str):
        from fulilian_cli.dashboard_auth.base import ProviderError
...
def _mint_stub_at(stub: StubAuthProvider) -> str:
    ...
    state = dict(
        seg.split("=", 1)
        for seg in ls.cookie_payload["fulilian_session_pkce"].split(";")
        if "=" in seg
    )["state"]
    verifier = dict(
        seg.split("=", 1)
