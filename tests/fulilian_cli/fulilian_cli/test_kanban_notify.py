...
def kanban_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("FULILIAN_MEDIA_ALLOW_DIRS", str(tmp_path))
    kb.init_db()
    return home
...
async def test_notifier_artifact_delivery_skips_missing_files(kanban_home, tmp_path, monkeypatch):
    ...
    import os
    os.environ["FULILIAN_KANBAN_TASK"] = tid
    try:
        kt._handle_complete({
        ...
        os.environ.pop("FULILIAN_KANBAN_TASK", None)
...
def test_migration_backfills_legacy_gateway_subs_to_notify_wake(kanban_home):
    from fulilian_cli.kanban_db import _migrate_add_optional_columns
