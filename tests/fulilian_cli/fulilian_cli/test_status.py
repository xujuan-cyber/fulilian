...
def test_show_status_termux_gateway_section_skips_systemctl(monkeypatch, capsys, tmp_path):
    from fulilian_cli import status as status_mod
    ...
    monkeypatch.setattr(status_mod, "get_fulilian_home", lambda: tmp_path, raising=False)
    ...
    assert "Start with:   fulilian gateway" in output
...
class TestShowStatusXaiOAuth:
    """xAI OAuth row in fulilian status."""
    ...
    def test_logged_in_shows_auth_store(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(auth_mod, "get_xai_oauth_auth_status",
                            lambda: {"logged_in": True, "auth_store": "/home/u/.fulilian/auth.json"},
                            raising=False)

        status_mod.show_status(SimpleNamespace(all=False, deep=False))
        out = capsys.readouterr().out

        assert "Auth file:  /home/u/.fulilian/auth.json" in out
    ...
    def test_status_function_returns_none_does_not_crash(self, monkeypatch, capsys, tmp_path):
        ...
        assert "not logged in (run: fulilian auth add xai-oauth)" in out


def test_show_status_reports_gateway_session_last_activity(monkeypatch, capsys, tmp_path):
    """fulilian status should surface freshest gateway last_active (#72016)."""
    from fulilian_cli import status as status_mod
    import fulilian_cli.auth as auth_mod
    import fulilian_cli.gateway as gateway_mod
    ...
    monkeypatch.setattr(fulilian_state, "SessionDB", _FakeDB)
