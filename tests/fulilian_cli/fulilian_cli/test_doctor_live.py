"""Tests for ``fulilian doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from fulilian_cli import doctor_live
from fulilian_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `fulilian doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("fulilian_cli.doctor.FULILIAN_HOME", tmp_path / "home")
        monkeypatch.setattr("fulilian_cli.doctor.PROJECT_ROOT", tmp_path / "root")
