"""Regression tests for #60328: --yolo must set FULILIAN_YOLO_MODE in
...
def _run_main_and_capture_yolo_at_startup(monkeypatch, argv):
    """Run main() with *argv*, capturing FULILIAN_YOLO_MODE at the
    ...
    def spy_prepare_startup(args):
        yolo_at_startup["value"] = os.environ.get("FULILIAN_YOLO_MODE")

    monkeypatch.setattr(
        "fulilian_cli.main._prepare_agent_startup", spy_prepare_startup
    )
    # Stub cmd_chat so main() returns cleanly without entering chat.
    monkeypatch.setattr("fulilian_cli.main.cmd_chat", lambda args: None)
    monkeypatch.delenv("FULILIAN_YOLO_MODE", raising=False)
    monkeypatch.setattr(sys, "argv", argv)

    from fulilian_cli.main import main as cli_main
...
def test_top_level_yolo_flag_sets_env_before_startup(monkeypatch):
    """fulilian --yolo must set FULILIAN_YOLO_MODE before
    _prepare_agent_startup imports tools.approval."""
    result = _run_main_and_capture_yolo_at_startup(
        monkeypatch, ["fulilian", "--yolo"]
    )
    assert result == "1", (
        "FULILIAN_YOLO_MODE was not '1' when _prepare_agent_startup was "
