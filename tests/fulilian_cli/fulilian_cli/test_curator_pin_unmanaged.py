...
`fulilian curator pin <name>` on an unmanaged skill (curation-eligible but no
...
becomes meaningful after `fulilian curator adopt`) and makes the message say
...
def test_pin_unmanaged_records_flag_and_prints_adopt_hint(monkeypatch, capsys):
    import fulilian_cli.curator as curator_cli
    ...
    assert "fulilian curator adopt legacy-skill" in out
