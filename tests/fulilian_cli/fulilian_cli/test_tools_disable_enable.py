"""Tests for fulilian tools disable/enable/list command (backend)."""
...
class TestToolsDisableBuiltin:
    ...
    def test_disable_removes_toolset_from_platform(self):
        ...
        with patch("fulilian_cli.tools_config.load_config", return_value=config), \
             patch("fulilian_cli.tools_config.save_config") as mock_save:
...
class TestToolsDisableMcp:
    ...
    def test_disable_unknown_server_prints_error(self, capsys):
        config = {"mcp_servers": {}}
        with patch("fulilian_cli.tools_config.load_config", return_value=config), \
             patch("fulilian_cli.tools_config.save_config"):
    ...
    try:
        with patch(
            ...
            side_effect=discover_deferred_platform,
             ...
             patch("fulilian_cli.tools_config.load_config", return_value=config), \
             patch("fulilian_cli.tools_config.save_config") as save:
