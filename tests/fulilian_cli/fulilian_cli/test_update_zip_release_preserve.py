...
(release/win-unpacked/Fulilian.exe) exists only in the live tree. Swapping
...
def test_staged_apps_swap_preserves_live_release_dir(tmp_path, monkeypatch):
    ...
    # live tree: apps/desktop/release/win-unpacked/Fulilian.exe + old source
    root = tmp_path / "install"
    live_apps = root / "apps" / "desktop"
    (live_apps / "release" / "win-unpacked").mkdir(parents=True)
    (live_apps / "release" / "win-unpacked" / "Fulilian.exe").write_bytes(b"MZbuilt")
    ...
    monkeypatch.setattr(fulilian_main, "PROJECT_ROOT", root)
    ...
    exe = root / "apps" / "desktop" / "release" / "win-unpacked" / "Fulilian.exe"
