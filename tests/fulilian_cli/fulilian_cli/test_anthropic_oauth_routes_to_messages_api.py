...
* ``fulilian_cli.providers.determine_api_mode`` correctly mapped
  ``api.anthropic.com`` to ``anthropic_messages``.
* ``fulilian_cli.runtime_provider._detect_api_mode_for_url`` did NOT, so
...
class TestExplicitRuntimeForAnthropic:
    ...
    Exercised whenever the user (or a Fulilian subcommand) passes an
...
class TestPoolEntryForAnthropic:
    ...
    has added an OAuth credential via ``fulilian auth add anthropic
