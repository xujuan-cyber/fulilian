"""Tests for xAI Grok OAuth — tokens stored in Fulilian auth store (~/.fulilian/auth.json)."""
...
def _setup_fulilian_auth(
    ...
    *,
    ...
    """Write xAI OAuth tokens into the Fulilian auth store at the given root."""
    ...
    auth_file = fulilian_home / "auth.json"
...
def _patch_httpx_client(monkeypatch, response):
    ...
    monkeypatch.setattr("fulilian_cli.auth.httpx.Client", _factory)
    return holder
...
def test_save_and_read_xai_oauth_tokens_roundtrip(tmp_path, monkeypatch):
    ...
    fulilian_home.mkdir(parents=True, exist_ok=True)
    (fulilian_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
...
def test_refresh_xai_oauth_tokens_preserves_active_provider(tmp_path, monkeypatch):
    ...
    _setup_fulilian_auth(fulilian_home, access_token=near, refresh_token="rt-old")
    monkeypatch.setenv("FULILIAN_HOME", str(fulilian_home))
    ...
    monkeypatch.setattr("fulilian_cli.auth.refresh_xai_oauth_pure", _fake_pure)
...
def test_resolve_xai_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("fulilian_cli.auth._refresh_xai_oauth_tokens", _fake_refresh)
...
# ``XAI_BASE_URL`` / ``FULILIAN_XAI_BASE_URL`` are a credential-leak vector
...
def _seed_xai_oauth_state(
    fulilian_home: Path, state: dict, *, active_provider: str = "xai-oauth"
) -> None:
    fulilian_home.mkdir(parents=True, exist_ok=True)
    auth_store = {
        "version": 1,
    ...
    (fulilian_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    """
    fulilian_home = tmp_path / "fulilian"
    _seed_xai_oauth_state(fulilian_home, dict(_STALE_XAI_OAUTH_STATE), active_provider="nous")
    monkeypatch.setenv("FULILIAN_HOME", str(fulilian_home))
    ...
    monkeypatch.setattr("fulilian_cli.auth._refresh_xai_oauth_tokens", _terminal_refresh)
    ...
    raw = json.loads((fulilian_home / "auth.json").read_text())
...
def test_refresh_xai_oauth_pure_403_marked_tier_denied_not_relogin(monkeypatch):
    ...
    being active. Re-running ``fulilian model`` won't help in that
...
def test_format_auth_error_tier_denied_does_not_suggest_relogin():
    ...
    Regression for #26847: telling a tier-gated user to ``fulilian model``
    ...
    assert "fulilian model" not in rendered.lower()
...
def test_xai_oauth_discovery_raises_typed_error_on_malformed_json(monkeypatch):
    ...
    from fulilian_cli.auth import _xai_oauth_discovery
...
def test_refresh_xai_oauth_pure_rejects_non_https_token_endpoint(monkeypatch):
    ...
    Fulilian that didn't validate) must not be silently honored on the refresh
...
def test_xai_oauth_discovery_validates_endpoints(monkeypatch):
    ...
    monkeypatch.setattr("fulilian_cli.auth.httpx.get", _fake_get)
    with pytest.raises(AuthError) as exc:
        _xai_oauth_discovery()
    assert exc.value.code == "xai_discovery_invalid"
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    """After `fulilian model` -> xai-oauth, the singleton holds tokens.  load_pool
    must surface that as a pool entry so `fulilian auth list` reflects truth and
    ...
    _setup_fulilian_auth(fulilian_home, access_token=fresh, refresh_token="rt-1")
    monkeypatch.setenv("FULILIAN_HOME", str(fulilian_home))
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    """End-to-end regression: ``fulilian auth remove xai-oauth 1`` for a
    ...
    from agent.credential_pool import load_pool
    from fulilian_cli.auth_commands import auth_remove_command
    ...
    # Act: the user runs `fulilian auth remove xai-oauth 1`.
    ...
    # nothing to import.
    raw_after = json.loads((fulilian_home / "auth.json").read_text())
    ...
    assert not pool_after.has_credentials(), (
        ...
        "and rebuilds the entry on every Fulilian invocation."
    )


def test_login_xai_oauth_relogin_clears_suppression_and_reseeds(tmp_path, monkeypatch):
    """remove -> ``fulilian model`` re-login (``_login_xai_oauth``) must clear the
    ...
    only ``auth_add_command`` cleared it — the ``fulilian model`` re-login path did
    not. So after remove -> re-login the seed kept skipping and ``fulilian auth
    ...
    new_access = _jwt_with_exp(int(time.time()) + 2 * 60 * 60)
    monkeypatch.setattr(
        "fulilian_cli.auth._xai_oauth_device_code_login",
        lambda **kwargs: {
            "tokens": {
                "access_token": new_access,
    ...
    monkeypatch.setattr(
        "fulilian_cli.auth._update_config_for_provider",
...
def test_pool_sync_back_writes_to_singleton(tmp_path, monkeypatch):
    ...
    _setup_fulilian_auth(fulilian_home, access_token=expired, refresh_token="rt-old")
    monkeypatch.setenv("FULILIAN_HOME", str(fulilian_home))
    ...
    monkeypatch.setattr("fulilian_cli.auth.refresh_xai_oauth_pure", _fake_refresh)
...
def test_runtime_provider_uses_pool_entry_for_xai_oauth(tmp_path, monkeypatch):
    ...
    fulilian_home = tmp_path / "fulilian"
    fresh = _jwt_with_exp(int(time.time()) + 2 * 60 * 60)
    _setup_fulilian_auth(fulilian_home, access_token=fresh)
    monkeypatch.setenv("FULILIAN_HOME", str(fulilian_home))
    monkeypatch.delenv("FULILIAN_XAI_BASE_URL", raising=False)
    monkeypatch.delenv("XAI_BASE_URL", raising=False)
...
def test_pool_refresh_recovers_when_other_process_already_refreshed(tmp_path, monkeypatch):
    ...
    _setup_fulilian_auth(fulilian_home, access_token=in_memory_at, refresh_token="rt-shared")
    monkeypatch.setenv("FULILIAN_HOME", str(fulilian_home))
    ...
    def _fake_refresh(access_token, refresh_token, **kwargs):
        ...
        (fulilian_home / "auth.json").write_text(json.dumps(raw))
        raise AuthError(
...
def test_pool_manual_entry_does_not_sync_back_to_singleton(tmp_path, monkeypatch):
    """`fulilian auth add xai-oauth` entries (source='manual:xai_pkce') are
    ...
    _setup_fulilian_auth(fulilian_home, access_token=singleton_at, refresh_token="rt-singleton")
    monkeypatch.setenv("FULILIAN_HOME", str(fulilian_home))
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    provider (visible to ``fulilian auth status``, ``fulilian setup``, and the
    ``fulilian`` no-arg dispatcher).  Pin the ``set_active=False`` contract so
    no future refactor regresses to the legacy semantic."""
    from agent.credential_pool import load_pool

    fulilian_home = tmp_path / "fulilian"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_fulilian_auth(fulilian_home, access_token=near_expiry, refresh_token="rt-xai")
    monkeypatch.setenv("FULILIAN_HOME", str(fulilian_home))
