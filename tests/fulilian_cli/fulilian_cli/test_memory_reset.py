"""Tests for the `fulilian memory reset` CLI command.
...
- Profile-scoped reset (uses FULILIAN_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake FULILIAN_HOME with memory files."""
    fulilian_home = tmp_path / ".fulilian"
    memories = fulilian_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("FULILIAN_HOME", str(fulilian_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nFulilian repo is at ~/.fulilian/fulilian-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `fulilian memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        fulilian_home = tmp_path / ".fulilian"
        (fulilian_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("FULILIAN_HOME", str(fulilian_home))
