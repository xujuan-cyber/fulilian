...
Regression guard for the bug where `fulilian model` saved `provider: upstage`
correctly but, on re-entry, showed a different provider as active. Root cause:
`fulilian_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageOverlay:
    def test_overlay_exists(self):
        ...
        assert "upstage" in FULILIAN_OVERLAYS
        overlay = FULILIAN_OVERLAYS["upstage"]
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `fulilian_cli/main.py` (cmd_model): read
