"""Tests for fulilian_cli/completion.py — shell completion script generation."""
...
from fulilian_cli.completion import _walk, generate_bash, generate_zsh, generate_fish
...
def _make_parser() -> argparse.ArgumentParser:
    """Build a minimal parser that mirrors the real fulilian structure."""
    p = argparse.ArgumentParser(prog="fulilian")
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "_fulilian_completion()" in out
        assert "complete -F _fulilian_completion fulilian" in out
...
class TestGenerateZsh:
    ...
    def test_preserves_valid_zsh_arguments_alias_syntax(self):
        ...
        assert "'(-)'{-p,--profile}'[Profile name]:profile:_fulilian_profiles'" in out
...
class TestProfileCompletion:
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        """After 'fulilian profile use', complete with profile names."""
        out = generate_bash(_make_parser())
        # The profile case should have _fulilian_profiles for name-taking actions
        lines = out.split("\n")
        ...
        for line in lines:
            ...
            if in_profile_case and "_fulilian_profiles" in line:
                has_profiles_in_action = True
                break
        assert has_profiles_in_action, "profile actions should complete with _fulilian_profiles"
    ...
    def test_fish_profile_actions_complete_names(self):
        ...
        assert "__fulilian_profiles" in out
        count = out.count("(__fulilian_profiles)")
