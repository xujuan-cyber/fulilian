"""``fulilian`` must survive git operations on the checkout (launcher layout).

The Windows ``fulilian`` command is a launcher derived from the venv console
script. Its canonical home is the managed binary dir ``FULILIAN_HOME\\bin`` —
OUTSIDE the git checkout — because the earlier in-checkout home
(``fulilian-agent\\bin``) was swept by ``fulilian update``'s autostash
(``git stash push --include-untracked``) and, with the desktop updater's
``--keep-stash``, never restored: ``fulilian`` stopped resolving in every new
...
existing install's PATH to the canonical layout from the ``fulilian update``
tail. Platform verdict, PATH values, and registry I/O are injected
parameters (same pattern as ``fulilian_constants.venv_bin_dir``), so these
...
from fulilian_cli._install_repair import (
...
def _make_managed(tmp_path, monkeypatch, *, relocatable: bool = False):
    """Fake managed layout: FULILIAN_HOME/fulilian-agent/venv/Scripts + launchers."""
    home = tmp_path / "fulilian"
    root = home / "fulilian-agent"
...
def test_source_checkout_untouched(tmp_path, monkeypatch):
    """A checkout NOT under FULILIAN_HOME gains nothing anywhere."""
    home = tmp_path / "fulilian-home"
    home.mkdir()
    monkeypatch.setenv("FULILIAN_HOME", str(home))
    root = tmp_path / "src" / "fulilian-agent"
    scripts = root / "venv" / "Scripts"
    scripts.mkdir(parents=True)
    for name in _WINDOWS_BIN_LAUNCHERS:
...
def test_profile_session_still_heals_the_shared_bin(tmp_path, monkeypatch):
    """Under ``fulilian -p <name>`` FULILIAN_HOME points inside profiles/<name>;
...
# ---------------------------------------------------------------------------
# migrate_windows_bin_path — the `fulilian update` tail migration
# ---------------------------------------------------------------------------
...
def test_migration_moves_path_to_home_bin_and_strips_legacy(managed_install):
    ...
    (root / "bin").mkdir()
    (root / "bin" / "fulilian.exe").write_bytes(b"legacy copy")
    ...
    assert (root / "bin" / "fulilian.exe").read_bytes() == b"legacy copy"
...
def test_normalize_windows_path_equivalences():
    assert (
        _normalize_windows_path(r"C:\Users\Me\AppData\Local\fulilian\bin")
        == _normalize_windows_path("c:/users/me/appdata/local/FULILIAN/BIN/")
    )
...
def test_repo_gitignores_the_legacy_bin_dir():
    ...
    launchers at ``<checkout>/bin``. ``fulilian update`` autostashes with
    ...
    result = subprocess.run(
        ["git", "-C", str(repo_root), "check-ignore", "-q", "bin/fulilian.exe"],
        capture_output=True,
    )
    assert result.returncode == 0, (
        "bin/fulilian.exe is not gitignored — fulilian update's autostash "
