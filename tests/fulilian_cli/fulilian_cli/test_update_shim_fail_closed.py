...
The bug class: on Windows, when `fulilian.exe`/sibling shims could not be
...
import fulilian_cli._install_repair as ir
import fulilian_cli.update_cmd as update_cmd


def _make_shims(scripts_dir: Path, names=("fulilian", "fulilian-gateway")) -> list[Path]:
...
def test_strict_quarantine_refuses_before_install(windows, tmp_path, monkeypatch):
    ...
    # fulilian.exe cannot be renamed; fulilian-gateway.exe can
    real_rename = Path.rename

    def deny_fulilian(self, target):
        if self.name == "fulilian.exe":
            raise PermissionError(13, "held open")
        return real_rename(self, target)

    monkeypatch.setattr(Path, "rename", deny_fulilian)
    monkeypatch.setattr(cli_main, "_fulilian_exe_shims", lambda d: shims)
    ...
    assert install_ran == []
    assert "fulilian.exe" in exc_info.value.failed_shims
    # The successful rename (fulilian-gateway.exe) was rolled back.
    assert (scripts / "fulilian-gateway.exe").exists()
    assert not list(scripts.glob("fulilian-gateway.exe.old.*"))


def test_non_strict_keeps_warn_and_try(windows, tmp_path, monkeypatch):
    scripts = tmp_path / "venv" / "Scripts"
    shims = _make_shims(scripts, names=("fulilian",))
    monkeypatch.setattr(
        Path, "rename",
...
def test_recovery_install_cmd_fail_closed(windows, tmp_path, monkeypatch):
    ...
    _make_shims(scripts, names=("fulilian",))
...
def test_refusal_writes_marker_and_exits_2(monkeypatch, capsys):
    ...
    exc = cli_main.ShimQuarantineError(["fulilian.exe"])
    with pytest.raises(SystemExit) as exit_info:
        update_cmd._refuse_update_for_contended_shims(exc)
    assert exit_info.value.code == 2
    assert wrote == [1]
    out = capsys.readouterr().out
    assert "fulilian.exe" in out
