"""Tests for the top-level `./fulilian` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./fulilian` should use `fulilian_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "fulilian"
    called = []

    fake_main_module = types.ModuleType("fulilian_cli.main")

    def fake_main():
        called.append("fulilian_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "fulilian_cli.main", fake_main_module)
    ...
    assert called == ["fulilian_cli.main"]
