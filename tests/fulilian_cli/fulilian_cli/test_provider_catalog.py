"""Tests for the unified provider catalog (fulilian_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``fulilian model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from fulilian_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
