"""Tests for fulilian_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $FULILIAN_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Fulilian-managed node dir.
"""
...
def _make_fulilian_node(fulilian_home: Path) -> Path:
    """Create a fake $FULILIAN_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = fulilian_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    fulilian_home = fake_home / ".fulilian"
    _make_fulilian_node(fulilian_home)
    ...
    removed = uninstall.remove_node_symlinks(fulilian_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    fulilian_home = fake_home / ".fulilian"
    node_bin = _make_fulilian_node(fulilian_home)
