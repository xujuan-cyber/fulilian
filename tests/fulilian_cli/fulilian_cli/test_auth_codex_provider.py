"""Tests for Codex auth — tokens stored in Fulilian auth store (~/.fulilian/auth.json)."""
...
def _setup_fulilian_auth(fulilian_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Fulilian auth store."""
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    ...
    _setup_fulilian_auth(fulilian_home, access_token="")
...
def test_resolve_codex_runtime_credentials_falls_back_to_pool_when_singleton_empty(tmp_path, monkeypatch):
    ...
    (fulilian_home / "auth.json").write_text(json.dumps(auth_store))
    monkeypatch.setenv("FULILIAN_HOME", str(fulilian_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((fulilian_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``fulilian auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``fulilian auth add openai-codex``.
    ...
    (fulilian_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `fulilian auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``fulilian auth add openai-codex`` for accounts B and C
    ...
    (fulilian_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``fulilian auth add openai-codex`` (account B and account C).
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Fulilian auth store, not ~/.codex/."""
    fulilian_home = tmp_path / "fulilian"
    ...
    _save_codex_tokens({"access_token": "fulilian-at", "refresh_token": "fulilian-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Fulilian store
    assert not (codex_home / "auth.json").exists()

    # Fulilian auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "fulilian-at"


def test_resolve_returns_fulilian_auth_store_source(tmp_path, monkeypatch):
    fulilian_home = tmp_path / "fulilian"
    _setup_fulilian_auth(fulilian_home)
    monkeypatch.setenv("FULILIAN_HOME", str(fulilian_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "fulilian-auth-store"
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run fulilian auth".
    """
    from fulilian_cli.auth import (
    ...
    assert "fulilian auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from fulilian_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from fulilian_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def _patch_httpx_post(monkeypatch, responses):
    """Patch fulilian_cli.auth.httpx.Client so .post() returns queued responses."""
    seq = iter(responses)
    ...
    monkeypatch.setattr("fulilian_cli.auth.httpx.Client", lambda *a, **k: _FakeClient())
