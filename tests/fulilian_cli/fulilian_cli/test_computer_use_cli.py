...
def _run(*args: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [sys.executable, "-m", "fulilian_cli.main", "computer-use", *args],
        capture_output=True,
        text=True,
        timeout=30,
    )
...
def _invoke(monkeypatch: pytest.MonkeyPatch, *args: str) -> int:
    ...
    cli_main = import_module("fulilian_cli.main")
    monkeypatch.setattr(sys, "argv", ["fulilian", "computer-use", *args])
...
def test_computer_use_status_returns_nonzero_for_incompatible_standard_driver(
    ...
    assert "Run: fulilian computer-use install" in output
...
def test_computer_use_status_returns_nonzero_for_incompatible_custom_driver(
    ...
    monkeypatch.setenv("FULILIAN_CUA_DRIVER_CMD", driver)
    monkeypatch.setattr(cua_backend, "resolve_cua_driver_cmd", lambda: driver)
    monkeypatch.setattr(
        tools_config,
        "_cua_driver_contract_status",
    ...
    assert _invoke(monkeypatch, "status") == 1
    output = capsys.readouterr().out
    assert "custom binary from FULILIAN_CUA_DRIVER_CMD" in output
