"""`fulilian skin set` — deterministic single-color tweak of the active skin.
...
from fulilian_cli import skin_cmd
from fulilian_constants import get_fulilian_home


def _skins():
    d = get_fulilian_home() / "skins"
    d.mkdir(parents=True, exist_ok=True)
    return d


def _activate(name: str) -> None:
    (get_fulilian_home() / "config.yaml").write_text(f"display:\n  skin: {name}\n", encoding="utf-8")
...
def test_set_forks_a_builtin_without_inventing_a_background():
    ...
    assert (get_fulilian_home() / "config.yaml").read_text().find("default-custom") != -1
