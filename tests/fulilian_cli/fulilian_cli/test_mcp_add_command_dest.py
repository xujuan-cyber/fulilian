"""Regression test: ``fulilian mcp add --command`` must not clobber the
top-level ``args.command`` subparser dest.

The top-level argparse parser uses ``dest="command"`` for its subparsers
(``fulilian_cli/_parser.py``).  The dispatcher in ``fulilian_cli/main.py``
...
result, ``fulilian mcp add foo --url ...`` silently launches chat instead
...
def _build_parser():
    """Minimal replica of the slice of the fulilian parser that exhibits
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`fulilian mcp add foo --url ...` must keep args.command == "mcp".
