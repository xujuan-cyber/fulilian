"""Tests for the multi-board kanban layer (``fulilian kanban boards …``).
...
* Current-board persistence via ``<root>/kanban/current`` and
  ``FULILIAN_KANBAN_BOARD`` env var.
* ``connect(board=)`` isolation — writes on one board don't leak.
* ``create_board`` / ``list_boards`` / ``remove_board`` round trip.
* CLI surface: ``fulilian kanban boards list/create/switch/rm``.
* ``_default_spawn`` injects ``FULILIAN_KANBAN_BOARD`` into worker env.
"""
...
def fresh_home(tmp_path, monkeypatch):
    """Isolated FULILIAN_HOME with no prior kanban state.

    The autouse hermetic conftest already nukes credentials + TZ; this
    fixture layers a per-test FULILIAN_HOME plus a path-init cache reset
    ...
    # Also reset fulilian_constants cache so get_default_fulilian_root() re-reads.
    try:
...
class TestSlugValidation:
    @pytest.mark.parametrize("good", [
        "default", "atm10-server", "fulilian-agent", "proj_1", "a",
...
class TestPathResolution:
    ...
    def test_env_var_db_override_still_wins(self, fresh_home, tmp_path, monkeypatch):
        """``FULILIAN_KANBAN_DB`` pins the file regardless of board= arg."""
        forced = tmp_path / "custom.db"
        monkeypatch.setenv("FULILIAN_KANBAN_DB", str(forced))
...
class TestConnectionIsolation:
    ...
    def test_connect_env_var_overrides_current(self, fresh_home, monkeypatch):
        ...
        monkeypatch.setenv("FULILIAN_KANBAN_BOARD", "envwin")
        with kb.connect() as conn:
...
class TestWorkerSpawnEnv:
    """Ensure the dispatcher pins ``FULILIAN_KANBAN_BOARD`` / DB / workspaces on spawn.
    ...
    def test_default_spawn_sets_env_vars(self, fresh_home, monkeypatch):
        ...
        env = captured["env"]
        assert env["FULILIAN_KANBAN_BOARD"] == "spawntest"
        assert env["FULILIAN_KANBAN_TASK"] == "t_abc"
        # DB path should match the per-board DB, not the legacy default.
        expected_db = fresh_home / "kanban" / "boards" / "spawntest" / "kanban.db"
        assert env["FULILIAN_KANBAN_DB"] == str(expected_db)
        expected_ws = fresh_home / "kanban" / "boards" / "spawntest" / "workspaces"
        assert env["FULILIAN_KANBAN_WORKSPACES_ROOT"] == str(expected_ws)
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    """Run ``fulilian kanban …`` with PYTHONPATH pinned to the worktree."""
    env = dict(os.environ)
    env["PYTHONPATH"] = str(_WORKTREE)
    if env_extra:
        env.update(env_extra)
    return subprocess.run(
        [sys.executable, "-m", "fulilian_cli.main", "kanban"] + args,
        env=env,
        capture_output=True,
        text=True,
        cwd=str(_WORKTREE),
...
class TestCLI:
    def test_boards_list_default_only(self, tmp_path):
        env = {"FULILIAN_HOME": str(tmp_path)}
