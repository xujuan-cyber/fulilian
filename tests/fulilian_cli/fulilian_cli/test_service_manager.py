"""Tests for fulilian_cli.service_manager — the abstract ServiceManager
...
def test_windows_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # against the `fulilian_cli` package succeeds — gateway_windows is
    # imported lazily inside the wrapper and may not yet be loaded.
    import fulilian_cli.gateway_windows  # noqa: F401
    ...
    monkeypatch.setattr("fulilian_cli.gateway_windows", _FakeWindowsModule)
    monkeypatch.setattr(
...
# unprivileged fulilian user from every lifecycle op. These tests run
...
def test_seed_supervise_skeleton_creates_expected_layout(tmp_path) -> None:
    ...
    from fulilian_cli.service_manager import _seed_supervise_skeleton
...
def test_render_run_script_uses_replace_to_take_over_stale_holder() -> None:
    ...
    ``fulilian gateway run``, an agent action, the Open WebUI helper) holds
    the per-FULILIAN_HOME PID lock; the supervised slot then execs a bare
    ...
    Covers both the default (root FULILIAN_HOME, no ``-p``) and named-profile
    render paths.
    """
    default_text = S6ServiceManager._render_run_script("default", {})
    # Root profile: bare `fulilian gateway run --replace` (no -p flag).
    assert "fulilian gateway run --replace" in default_text
    assert "fulilian -p default" not in default_text
...
#
# `fulilian gateway stop` inside a container dispatches through
...
def test_s6_log_run_creates_leaf_as_fulilian_without_chown(
    s6_scandir, fake_subprocess_run,
) -> None:
    """log/run must not root-chown/unlink volume paths; create leaf as fulilian.

    #45258 parent ownership is stage2's job (``logs/gateways`` seeded as
    fulilian). Restartable log/run must not pathname-chown or pathname-rm a
    fulilian-writable tree from root — that is a symlink TOCTOU hole.
    """
    ...
    assert not any(line.lstrip().startswith("chown ") for line in log_text.splitlines()), (
        "restartable log/run must not invoke chown on fulilian-writable paths; "
        f"saw: {log_text!r}"
    )
    assert 's6-setuidgid fulilian mkdir -p "$log_dir"' in log_text
    assert 's6-setuidgid fulilian rm -f "$log_dir/lock"' in log_text
    assert 'else\n  mkdir -p "$log_dir"\n  rm -f "$log_dir/lock"\nfi\n' in log_text
    ...
    mkdir_as_fulilian_idx = log_text.index('s6-setuidgid fulilian mkdir -p "$log_dir"')
    rm_as_fulilian_idx = log_text.index('s6-setuidgid fulilian rm -f "$log_dir/lock"')
    exec_idx = log_text.index("s6-log 1 ")
    assert mkdir_as_fulilian_idx < rm_as_fulilian_idx < exec_idx
...
def test_s6_log_run_never_invokes_chown_with_symlinked_log_dir(tmp_path) -> None:
    ...
    gateways = fulilian_home / "logs" / "gateways"
    ...
    # Mark the drop so fake rm can refuse unlink outside FULILIAN_HOME the way
    # a real fulilian uid cannot delete a foreign root-owned lock.
    ...
    (bin_dir / "s6-setuidgid").write_text(
        "#!/bin/sh\n"
        "shift\n"
        'FULILIAN_TEST_DROPPED=1 exec "$@"\n',
        encoding="utf-8",
    )
    real_rm = "/bin/rm"
    (bin_dir / "rm").write_text(
        "#!/bin/sh\n"
        # Privilege-dropped: no-op. Models that fulilian cannot unlink a foreign
        ...
        'if [ -n "$FULILIAN_TEST_DROPPED" ]; then\n'
        "  exit 0\n"
        "fi\n"
        f'exec {real_rm} "$@"\n',
        encoding="utf-8",
    ...
    env = os.environ.copy()
    env["FULILIAN_HOME"] = str(fulilian_home)
