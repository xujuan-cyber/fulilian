...
the SAME provider universe as ``fulilian model`` (the CLI/TUI picker).
...
def test_every_fulilian_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `fulilian model` universe."""
    ...
    assert not missing, (
        "providers shown in `fulilian model` but not configurable in the desktop "
