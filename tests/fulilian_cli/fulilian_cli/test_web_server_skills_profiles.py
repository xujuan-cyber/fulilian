...
profile's FULILIAN_HOME, and the dashboard's own profile stays untouched.
"""
import pytest
import yaml
...
class TestProfileScopedHubActions:
    def test_hub_install_spawns_with_profile_flag(
        ...
        """Hub installs must go through a fresh ``fulilian -p <profile>``
        ...
        monkeypatch.setattr(web_server, "_spawn_fulilian_action", _fake_spawn)
        resp = client.post(
