"""``fulilian config set`` must parse list/mapping literals, not store them as strings.

Before this fix, ``fulilian config set platform_toolsets.discord '["file","web"]'``
...
def user_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("FULILIAN_MANAGED_DIR", raising=False)
...
def test_list_literal_is_parsed_to_list(user_home):
    from fulilian_cli.config import set_config_value, read_raw_config
...
def test_round_trip_through_load_config(user_home):
    ...
    from fulilian_cli.config import set_config_value, load_config
