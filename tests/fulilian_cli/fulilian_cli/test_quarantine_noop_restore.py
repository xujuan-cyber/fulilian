...
``fulilian*.exe`` shims aside (``fulilian.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``fulilian`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("fulilian", "fulilian-agent", "fulilian-acp", "fulilian-gateway"):
...
# ---------------------------------------------------------------------------
# fulilian_cli.main._run_quarantined_install
# ---------------------------------------------------------------------------
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "fulilian.exe" in names, "fulilian.exe must be restored after a no-op install"
    assert "fulilian-acp.exe" in names
    assert "fulilian-gateway.exe" in names
    assert (scripts / "fulilian.exe").read_bytes() == b"MZ-old-fulilian"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "fulilian.exe").read_bytes() == b"MZ-new-fulilian"
...
# ---------------------------------------------------------------------------
# fulilian_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "fulilian.exe" in names, "fulilian.exe must be restored after a no-op recovery install"
    assert (scripts / "fulilian.exe").read_bytes() == b"MZ-old-fulilian"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "fulilian.exe").write_bytes(b"MZ-new-fulilian")
