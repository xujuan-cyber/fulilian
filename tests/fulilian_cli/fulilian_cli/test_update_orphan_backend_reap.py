...
fires SIGTERM + app.quit() and spawns fulilian-setup, but its Python backend
(``python.exe -m fulilian_cli.main serve``) survives the teardown race. The
Desktop is gone — nothing will respawn that backend — yet the venv-holder
guard refused on it and the update dead-ended with "Fulilian is still running"
...
reap (with its full child tree — the managed .fulilian-runtime interpreter
...
_SERVE_ARGV = [
    "C:\\fulilian\\venv\\Scripts\\python.exe",
    "-m",
    "fulilian_cli.main",
    "serve",
    "--host",
...
def _holders(pid=200, cmdline="python.exe -m fulilian_cli.main serve"):
...
def test_backend_with_live_parent_keeps_refusal():
    parent = _proc(50, ["Fulilian.exe"], create_time=10.0)
...
def test_non_backend_holder_keeps_refusal():
    repl = _proc(300, ["python.exe", "-m", "fulilian_cli.main", "chat"], ppid=999)
    fake = _fake_psutil({300: repl})
    with patch.dict(sys.modules, {"psutil": fake}):
        holders = _holders(pid=300, cmdline="python.exe -m fulilian_cli.main chat")
...
def test_orphan_root_plus_managed_runtime_descendant_qualifies():
    ...
    # serve root and its .fulilian-runtime interpreter child. The child's live
    # parent IS the orphan root, so the set is safe — only the root is
    # returned (taskkill /T reaps the descendant with it).
    backend = _proc(200, _SERVE_ARGV, ppid=999)
    child_argv = [
        "C:\\fulilian\\.fulilian-runtime\\python\\generation-1\\python.exe",
        "worker.py",
    ]
