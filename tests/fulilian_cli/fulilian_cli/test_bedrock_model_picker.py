...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (FULILIAN_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via FULILIAN_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in FULILIAN_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from fulilian_cli.providers import FULILIAN_OVERLAYS
        assert "bedrock" in FULILIAN_OVERLAYS
