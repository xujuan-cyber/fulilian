...
option absent from both sets, so every ``fulilian --reasoning high <cmd> …``
invocation mis-classified ``high`` as the first positional and forced eager
plugin CLI discovery (~500-650ms startup tax).

Both sites now share ``fulilian_cli._parser.top_level_value_flag_sets()``,
...
from fulilian_cli.main import _first_positional_argv, _plugin_cli_discovery_needed
...
def test_first_positional_skips_reasoning_value(monkeypatch):
    monkeypatch.setattr("sys.argv", ["fulilian", "--reasoning", "high", "chat", "hello"])
    assert _first_positional_argv() == "chat"
    assert _plugin_cli_discovery_needed() is False


@pytest.mark.parametrize("argv", [
    ["fulilian", "--reasoning", "ultra", "--provider", "openai", "-z", "prompt here"],
    ["fulilian", "-m", "x", "--reasoning=low", "chat"],
])
