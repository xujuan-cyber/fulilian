...
Covers:
- Defense 1: gateway stop/restart refuse when _FULILIAN_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "fulilian gateway uninstall",
        "fulilian  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "FULILIAN GATEWAY RESTART",           # uppercase
    ])
    def test_fulilian_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.fulilian.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.fulilian/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l fulilian-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.fulilian.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.fulilian.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.fulilian.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.fulilian/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.fulilian.gateway-apollo:/a.plist' "
            "'ai.fulilian.gateway-cronus:/c.plist' 'ai.fulilian.gateway-plutus:/p.plist' "
            "'ai.fulilian.gateway:/Users/botuser/Library/LaunchAgents/ai.fulilian.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated fulilian note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.fulilian.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.fulilian.gateway',
        "launchctl kick'start' -k gui/501/ai.fulilian.gateway",
        "launchctl kick\\start -k gui/501/ai.fulilian.gateway",
        'launchctl "kickstart" -k gui/501/ai.fulilian.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.fulilian.gateway',
        "launchctl dis\\able gui/501/ai.fulilian.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.fulilian."gateway"',
        # Same class on the systemctl and fulilian-CLI branches.
        'systemctl re"start" fulilian-gateway',
        'fulilian gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.fulilian.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.fulilian.update-checker',
        'systemctl re"start" fulilian-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "fulilian cron list",
        "fulilian update",
        "fulilian config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `fulilian gateway start` is benign — starting a gateway from inside a
        ...
        "fulilian gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway fulilian
        # services must not be falsely blocked (the old `.*fulilian` matched any
        # fulilian token).
        "launchctl unload ai.fulilian.update-checker.plist",
        "launchctl restart ai.fulilian.daemon",
        ...
        "systemctl restart fulilian-meta.service",
        "systemctl restart fulilian-cron-helper",
        ...
        "echo after the fulilian gateway restarted cleanly",
        "the fulilian gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "fulilian skill view gateway-notes && echo fulilian gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `fulilian` is a path component
        # there, not a command.
        "cat '/docs/fulilian gateway restart-notes.md'",
        "less /home/user/notes/fulilian gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "fulilian gateway restart",
        "fulilian gateway restart; echo done",
        "fulilian gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;fulilian gateway restart",
        "true && fulilian gateway stop",
        "echo $(fulilian gateway restart)",
        "echo `fulilian gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.fulilian.gateway"])',
        'subprocess.run(["fulilian", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.fulilian.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["fulilian", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: fulilian gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `fulilian -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry FULILIAN_HOME/FULILIAN_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("FULILIAN_PROFILE", "zeus")
        monkeypatch.delenv("FULILIAN_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "fulilian -p zeus gateway stop",
        "fulilian -p zeus gateway restart",
        "fulilian --profile zeus gateway restart",
        "fulilian --profile zeus gateway stop",
        "fulilian --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "fulilian -v -p zeus gateway restart",
        "fulilian -p zeus -v gateway restart",
        "fulilian --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "fulilian -p venus gateway stop",
        "fulilian -p venus gateway restart",
        "fulilian --profile venus gateway restart",
        "fulilian --profile=venus gateway stop",
        "fulilian -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "fulilian -p zeus gateway start",
        "fulilian -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("fulilian gateway restart")
        assert _contains_gateway_lifecycle_command("fulilian gateway stop")

    def test_fulilian_home_derived_profile(self, monkeypatch):
        # Without FULILIAN_PROFILE the guard falls back to the FULILIAN_HOME-
        # derived profile identity (get_active_profile_name) — the signal the
        # gateway process itself carries.
        monkeypatch.delenv("FULILIAN_PROFILE", raising=False)
        monkeypatch.delenv("FULILIAN_PROFILE_NAME", raising=False)
        import fulilian_cli.profiles as profiles_mod

        monkeypatch.setattr(profiles_mod, "get_active_profile_name", lambda: "zeus")
        assert _contains_gateway_lifecycle_command("fulilian -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("fulilian -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("fulilian -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("fulilian gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_fulilian_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade fulilian then run fulilian gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_fulilian_gateway_once.sh). The script must live under
        # FULILIAN_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("FULILIAN_HOME", str(tmp_path / ".fulilian"))
        scripts_dir = tmp_path / ".fulilian" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_FULILIAN_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _FULILIAN_GATEWAY=1.

    Issue #37453: systemctl --user restart fulilian-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart fulilian-gateway",
        "systemctl --user restart fulilian-gateway",
        "systemctl stop fulilian-gateway.service",
        "fulilian gateway restart",
        "fulilian gateway uninstall",
        "launchctl kickstart gui/501/ai.fulilian.gateway",
        "launchctl bootout gui/501/ai.fulilian.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.fulilian.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.fulilian/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.fulilian.gateway.restart-once.plist",
        "pkill -f fulilian.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart fulilian-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.fulilian.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-fulilian label: label-independent detection is the point
        # (#62891 second reproduction used `ai.fulilian.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.fulilian.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _FULILIAN_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _FULILIAN_GATEWAY=1 is in the
        ...
        result = json.loads(tt.terminal_tool(command="fulilian gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["fulilian gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.fulilian.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.fulilian.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-fulilian systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run fulilian gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("fulilian gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfefulilian gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under FULILIAN_HOME/scripts (the
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("fulilian gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/fulilian\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 fulilian gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_fulilian_home()` falls back to Path.home(), which raises
        RuntimeError when neither FULILIAN_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart fulilian-gateway' .",
        "grep -rn 'fulilian gateway restart' ./logs",
        "rg 'fulilian gateway restart' ../archive",
        "grep -c 'systemctl stop fulilian-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM fulilian_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell fulilian gateway restart"',
        'sqlite3 ./db ".system systemctl restart fulilian-gateway"',
        'psql ./x -c "\\! systemctl restart fulilian-gateway"',
        "grep -r 'fulilian gateway restart' . | sh",
        "grep -r 'fulilian gateway restart' ./logs | bash",
        "grep -r 'fulilian gateway restart' . | sudo sh",
        "grep -r 'x' . ; fulilian gateway restart",
        "grep -r 'x' . && systemctl restart fulilian-gateway",
        'grep -r "$(fulilian gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart fulilian-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell fulilian gateway restart"',
        'sqlite3 db ".system systemctl restart fulilian-gateway"',
        'psql -c "\\! systemctl restart fulilian-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    fulilian_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run fulilian gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("FULILIAN_HOME", str(tmp_path / ".fulilian"))
        (tmp_path / ".fulilian").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run fulilian gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell fulilian gateway restart"',
        'psql -c "\\! systemctl restart fulilian-gateway"',
        "grep 'systemctl restart fulilian-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart fulilian-gateway",
        'grep "$(systemctl restart fulilian-gateway)" f',
        "grep 'restart' log; systemctl restart fulilian-gateway",
        'sqlite3 db "SELECT 1"; fulilian gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "fulilian gateway restart",
        "sudo systemctl stop fulilian-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart fulilian-gateway%'\""
        )
