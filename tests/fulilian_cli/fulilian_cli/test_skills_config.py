"""Tests for fulilian_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("fulilian_cli.skills_config.save_config")
...
class TestIsSkillDisabled:
    ...
    @patch("fulilian_cli.config.load_config")
    ...
    @patch("fulilian_cli.config.load_config")
    @patch.dict("os.environ", {"FULILIAN_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """FULILIAN_SESSION_PLATFORM should be used when FULILIAN_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_fulilian_platform_takes_precedence(self, tmp_path, monkeypatch):
        """FULILIAN_PLATFORM should win over FULILIAN_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from fulilian_cli.skills_config import _get_categories
        skills = [
