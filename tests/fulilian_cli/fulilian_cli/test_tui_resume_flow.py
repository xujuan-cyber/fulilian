...
def main_mod(monkeypatch):
    import fulilian_cli.main as mod
...
def test_termux_skips_bundled_skill_sync_when_stamp_fresh(monkeypatch, tmp_path, main_mod):
    ...
    monkeypatch.setattr(main_mod, "get_fulilian_home", lambda: tmp_path)
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        """
        import fulilian_cli.oneshot as oneshot
        from fulilian_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """fulilian -z bypasses FulilianCLI, but recall still needs SessionDB."""
    from fulilian_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "fulilian_state", mod("fulilian_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "fulilian_cli.config",
        mod("fulilian_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "fulilian_cli.models",
        mod("fulilian_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    monkeypatch.setitem(
        sys.modules,
        "fulilian_cli.tools_config",
        mod("fulilian_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["FULILIAN_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nous/fulilian-test", provider="nous", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["FULILIAN_MODEL"] == "nous/fulilian-test"
    assert env["FULILIAN_INFERENCE_MODEL"] == "nous/fulilian-test"
    assert env["FULILIAN_TUI_PROVIDER"] == "nous"
    assert env["FULILIAN_INFERENCE_PROVIDER"] == "nous"
    assert env["FULILIAN_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["FULILIAN_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("fulilian-tui-active-session-")
...
def test_make_tui_argv_dev_prebuilds_fulilian_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("FULILIAN_TUI_DIR", raising=False)
