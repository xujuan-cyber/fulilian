"""Regression tests for #34107 — Docker UID/GID handling in ensure_fulilian_home.

When Fulilian runs in Docker with ``FULILIAN_UID=1000`` / ``FULILIAN_GID=911``,
the entrypoint chowns the top-level ``FULILIAN_HOME`` once at startup. But
...
The fix is a ``_chown_to_fulilian_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_fulilian_uid_gid
# ---------------------------------------------------------------------------


class TestResolveFulilianUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("FULILIAN_UID", "1000")
        monkeypatch.setenv("FULILIAN_GID", "911")
        from fulilian_cli.config import _resolve_fulilian_uid_gid
...
# ---------------------------------------------------------------------------
# _chown_to_fulilian_uid
# ---------------------------------------------------------------------------


class TestChownToFulilianUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_fulilian_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("FULILIAN_UID", raising=False)
        monkeypatch.delenv("FULILIAN_GID", raising=False)
        from fulilian_cli import config as cfg
