...
locally, one reached over SSH, one behind a plain URL+token, or Fulilian Cloud —
and only the first two run with ``FULILIAN_DESKTOP=1`` in their environment.
...
model "You are chatting inside the Fulilian desktop app".
...
class TestDesktopUiToolset:
    ...
    def test_stays_off_the_core_tool_list(self):
        ...
        assert GUI_TOOLS.isdisjoint(_FULILIAN_CORE_TOOLS)
...
class TestSurfaceResolution:
    ...
    def test_desktop_env_alone_does_not_grant_them(self, no_desktop_env):
        ...
        The embedded terminal pane runs `fulilian --tui` against this same
        backend; env-keyed gating handed it GUI tools it cannot answer.
        """
        no_desktop_env.setenv("FULILIAN_DESKTOP", "1")
        assert "desktop_ui" not in server._gui_surface_toolsets("tui")
...
class TestResolverPlumbing:
    ...
    def test_explicit_env_pin_still_wins(self, no_desktop_env):
        """FULILIAN_TUI_TOOLSETS is an operator override; surface can't re-add."""
        no_desktop_env.setenv("FULILIAN_TUI_TOOLSETS", "web,memory")
