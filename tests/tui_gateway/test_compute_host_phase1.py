...
def test_compute_host_workers_inherit_tui_pool_env_or_8(monkeypatch):
    monkeypatch.delenv("FULILIAN_TUI_RPC_POOL_WORKERS", raising=False)
    monkeypatch.delenv("FULILIAN_COMPUTE_HOST_WORKERS", raising=False)
    assert _default_workers() == 8

    monkeypatch.setenv("FULILIAN_TUI_RPC_POOL_WORKERS", "11")
    assert _default_workers() == 11

    # Dead-RC tombstone: malformed env falls back to 8, not the old except-branch 4.
    monkeypatch.setenv("FULILIAN_TUI_RPC_POOL_WORKERS", "not-an-int")
    assert _default_workers() == 8
