...
binary download are exercised manually by `fulilian secrets bitwarden
...
def fulilian_home(tmp_path, monkeypatch):
    """Point Fulilian at an isolated home directory."""
    home = tmp_path / ".fulilian"
    home.mkdir()
    monkeypatch.setenv("FULILIAN_HOME", str(home))
    # Some modules cache get_fulilian_home; clear if needed.
    import fulilian_constants
    if hasattr(fulilian_constants, "_FULILIAN_HOME_CACHE"):
        fulilian_constants._FULILIAN_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(fulilian_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from fulilian_cli.env_loader import _apply_external_secret_sources
