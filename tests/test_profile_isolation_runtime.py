...
``_FULILIAN_HOME_OVERRIDE`` ContextVar, not the process environment.  State that
...
with real temp FULILIAN_HOME directories (no mocks) and assert the *active*
...
def two_profiles(tmp_path):
    """Two distinct profile FULILIAN_HOME dirs with the dir skeleton created."""
...
class TestRichSentStorePathResolution:
    ...
    def test_store_path_follows_override(self, two_profiles, monkeypatch):
        prof_a, prof_b = two_profiles
        # Ensure no ambient FULILIAN_HOME env masks the test.
...
class TestThreadContextPropagation:
    ...
    def test_run_async_worker_preserves_override(self, two_profiles):
        ...
        leaks, every async tool that resolves get_fulilian_home() leaks.
        """
        import asyncio
