...
an isolated daemon separately, then point this script at its socket:
    ...
    cua-driver serve --embedded --socket /tmp/fulilian-cua-0-9-live.sock \
        --no-permissions-gate --no-overlay
    CUA_DRIVER_LIVE_SOCKET=/tmp/fulilian-cua-0-9-live.sock \
...
async def run_smoke(socket_path: str) -> dict[str, dict[str, Any]]:
    session_id = f"fulilian-cua-live-{uuid.uuid4().hex[:8]}"
    ...
    file_descriptor, temporary_name = tempfile.mkstemp(
        prefix="fulilian-cua-live-", suffix=".txt"
    )
                ...
                try:
                    ...
                    if (
                        ...
                        if not isinstance(editor, dict):
                            ...
                            marker = "fulilian foreground smoke"
                            type_args = {
                                "pid": launched_pid,
                                "window_id": window_id,
