...
Override the image with ``FULILIAN_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("FULILIAN_TEST_IMAGE", "fulilian-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``FULILIAN_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("FULILIAN_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged fulilian user
# ---------------------------------------------------------------------------
#
# Background: every Fulilian runtime path inside the container drops to UID
# 10000 (the ``fulilian`` user) via ``s6-setuidgid fulilian``. ``docker exec``
...
# fails (PermissionError swallowed) for fulilian, so a test that ran as root
...
# helpers below run every probe under ``-u fulilian`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: fulilian).
