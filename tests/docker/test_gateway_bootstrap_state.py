...
Build the real image and verify the actual runtime behavior:
  ...
  1. FULILIAN_GATEWAY_BOOTSTRAP_STATE=running on a fresh volume seeds
...
def test_seeds_running_state_on_blank_volume(
    ...
    """FULILIAN_GATEWAY_BOOTSTRAP_STATE=running on a fresh volume must
    seed gateway_state.json with a valid running state."""
    _start_container(
        built_image, container_name,
        "FULILIAN_GATEWAY_BOOTSTRAP_STATE=running",
    )
...
def test_no_seed_when_env_unset(
    ...
    """No FULILIAN_GATEWAY_BOOTSTRAP_STATE = no seed file written."""
...
def test_non_running_value_ignored(
    ...
    for bogus in ("stopped", "Running", "1", "true", "starting"):
        ...
        _start_container(
            built_image, name,
            f"FULILIAN_GATEWAY_BOOTSTRAP_STATE={bogus}",
        )
        r = docker_exec_sh(
            name,
            "test -f /opt/data/gateway_state.json && "
...
def _cleanup_bind_mount(built_image: str, container_name: str, host_dir: Path) -> None:
    """Remove root/fulilian-owned files left in a bind-mounted host dir.

    The stage2 hook chowns /opt/data (and its contents) to UID 10000
    (fulilian), which the host test user cannot delete. We run a throwaway
