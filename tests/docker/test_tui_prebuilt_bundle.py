...
package-lock.json. Without FULILIAN_TUI_DIR set, ``_make_tui_argv`` falls
...
The fix is ``ENV FULILIAN_TUI_DIR=/opt/fulilian/ui-tui`` in the Dockerfile, which
...
def _exec_py(image: str, py: str) -> str:
    """Run a Python snippet inside the image as the fulilian user, return stdout."""
    inner = (
        "source /opt/fulilian/.venv/bin/activate && "
        "cd /opt/fulilian && "
        f"python3 -c {shlex.quote(py)}"
    )
    # Drop to the fulilian user (UID 10000) so we exercise the same path the
    # dashboard PTY child runs as — not root.
    cmd = [
        "docker", "run", "--rm", "--entrypoint", "su", image,
        "fulilian", "-s", "/bin/bash", "-c", inner,
    ]
...
def test_fulilian_tui_dir_env_is_set(built_image: str) -> None:
    """FULILIAN_TUI_DIR must point at the prebuilt bundle dir in the image."""
    r = subprocess.run(
        ["docker", "run", "--rm", "--entrypoint", "sh", built_image,
         "-c", 'printf "%s" "$FULILIAN_TUI_DIR"'],
        capture_output=True, text=True, timeout=60,
    )
    assert r.returncode == 0, r.stderr[-2000:]
    assert r.stdout.strip() == "/opt/fulilian/ui-tui", (
        f"FULILIAN_TUI_DIR={r.stdout.strip()!r} (expected /opt/fulilian/ui-tui)"
    )
...
def test_prebuilt_bundle_present_and_no_runtime_install(built_image: str) -> None:
    ...
    py = (
        ...
        "from fulilian_cli.main import _tui_need_npm_install, _find_bundled_tui, _make_tui_argv\n"
        "ui = Path('/opt/fulilian/ui-tui')\n"
    ...
    # With FULILIAN_TUI_DIR set, _make_tui_argv returns the prebuilt path BEFORE
