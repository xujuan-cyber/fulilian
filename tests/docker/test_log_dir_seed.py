"""Runtime smoke test for Docker $FULILIAN_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the fulilian user after container boot.
...
registered later runs its log service as the dropped fulilian user and
...
def test_logs_gateways_seeded_and_fulilian_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by fulilian after boot."""
    ...
    # Both must be owned by fulilian
    r = docker_exec_sh(
        container_name,
    ...
    assert "logs=fulilian" in r.stdout, (
        f"logs/ not owned by fulilian: {r.stdout}"
    )
    assert "gateways=fulilian" in r.stdout, (
        f"logs/gateways/ not owned by fulilian: {r.stdout}"
    )
