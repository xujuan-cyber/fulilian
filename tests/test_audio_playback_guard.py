...
leaked shell variable, so ``scripts/run_tests.sh``'s ``env -i`` was no help:
  ...
  1. A test drives the ``voice.toggle`` RPC with ``action="tts"``. The
     ...
     ``os.environ["FULILIAN_VOICE_TTS"] = "1"``.
  ...
  3. Any later test that drives a turn to completion hits the TTS dispatch in
     ``prompt.submit``, which calls ``fulilian_cli.voice.speak_text`` on a
...
Two independent defences in ``tests/conftest.py``, one test each below:
  ...
  * ``_FULILIAN_BEHAVIORAL_VARS`` now blanks ``FULILIAN_VOICE`` /
    ``FULILIAN_VOICE_TTS`` at every test setup, so step 2 cannot cross a test
    boundary.
...
def test_voice_toggle_still_leaks_the_env_var_but_speech_is_stubbed(monkeypatch):
    ...
    assert os.environ.get("FULILIAN_VOICE_TTS") == "1"
    ...
    # calls it (a late `from fulilian_cli.voice import speak_text`), with the
...
def test_voice_env_does_not_leak_into_the_next_test():
    ...
    one above, in the same process, which left ``FULILIAN_VOICE_TTS=1`` set.
    Before ``FULILIAN_VOICE``/``FULILIAN_VOICE_TTS`` were added to
    ``_FULILIAN_BEHAVIORAL_VARS``, this assertion failed.
    """
    assert "FULILIAN_VOICE_TTS" not in os.environ
    assert "FULILIAN_VOICE" not in os.environ
...
def test_guard_can_be_opted_out_of_explicitly():
    ...
    import fulilian_cli.voice as voice
