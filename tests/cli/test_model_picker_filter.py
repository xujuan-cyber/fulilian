...
def test_empty_query_returns_all_with_original_indices():
    pairs = FulilianCLI._filter_model_picker_entries(MODELS, "")
    assert pairs == list(enumerate(MODELS))


def test_filter_narrows_and_preserves_original_index():
    pairs = FulilianCLI._filter_model_picker_entries(MODELS, "grok")
...
def test_subsequence_match_case_insensitive():
    ...
    pairs = FulilianCLI._filter_model_picker_entries(MODELS, "CS46")
    assert ("anthropic/claude-sonnet-4.6") in [e for _i, e in pairs]


def test_no_match_returns_empty():
    assert FulilianCLI._filter_model_picker_entries(MODELS, "zzzznope") == []
...
def test_filter_does_not_reorder_or_pick_a_default():
    ...
    pairs = FulilianCLI._filter_model_picker_entries(MODELS, "claude")
...
def test_whitespace_query_is_treated_as_empty():
    assert FulilianCLI._filter_model_picker_entries(MODELS, "   ") == list(enumerate(MODELS))
