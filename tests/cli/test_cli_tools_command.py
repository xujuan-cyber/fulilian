...
def _make_cli(enabled_toolsets=None):
    """Build a minimal FulilianCLI stub without running __init__."""
...
class TestToolsSlashList:
    ...
    def test_list_calls_backend(self, capsys):
        cli_obj = _make_cli()
        with patch("fulilian_cli.tools_config.load_config",
...
class TestToolsSlashDisableWithReset:
    ...
    def test_disable_applies_directly_and_resets_session(self):
        ...
        with patch("fulilian_cli.tools_config.load_config",
             ...
             patch("fulilian_cli.tools_config.save_config"), \
             patch("fulilian_cli.tools_config._get_platform_tools", return_value={"memory"}), \
...
class TestToolsSlashEnableWithReset:
    ...
    def test_enable_applies_directly_and_resets_session(self):
        ...
        with patch("fulilian_cli.tools_config.load_config",
             ...
             patch("fulilian_cli.tools_config.save_config"), \
             patch("fulilian_cli.tools_config._get_platform_tools", return_value={"memory", "web"}), \
             patch("fulilian_cli.config.load_config", return_value={}), \
             patch.object(cli_obj, "new_session") as mock_reset:
