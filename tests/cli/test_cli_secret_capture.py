...
from fulilian_cli.callbacks import prompt_for_secret
...
def test_secret_capture_callback_can_be_completed_from_cli_state_machine():
    ...
    with patch("fulilian_cli.callbacks.save_env_value_secure") as save_secret:
...
def test_secret_capture_uses_masked_prompt_without_tui():
    ...
    with patch("fulilian_cli.callbacks.masked_secret_prompt", return_value="secret-value"), patch(
        "fulilian_cli.callbacks.save_env_value_secure"
    ) as save_secret:
        save_secret.return_value = {
            "success": True,
            "stored_as": "TENOR_API_KEY",
...
def test_secret_capture_timeout_clears_hidden_input_buffer():
    ...
    with patch("fulilian_cli.callbacks.queue.Queue.get", side_effect=queue.Empty), patch(
        "fulilian_cli.callbacks._time.monotonic",
        side_effect=[0, 121],
    ):
        result = prompt_for_secret(cli, "TENOR_API_KEY", "Tenor API key")
...
def test_cli_chat_registers_secret_capture_callback():
    ...
    with patch("cli.get_tool_definitions", return_value=[]), patch.dict(
        "os.environ", {"LLM_MODEL": "", "FULILIAN_MAX_ITERATIONS": ""}, clear=False
