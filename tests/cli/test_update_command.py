...
Verifies that ``FulilianCLI._handle_update_command`` correctly:
...
Also verifies that ``fulilian_cli.main._launch_tui`` correctly handles exit
...
def _make_self(modal_response):
    ...
    so we don't need a full ``FulilianCLI`` construction.
...
def _call(self_):
    ...
    return FulilianCLI._handle_update_command(self_)
...
def test_managed_install_refuses_and_does_not_set_pending_relaunch(capsys):
    ...
    with (
        patch("fulilian_cli.config.is_managed", return_value=True),
        patch(
            "fulilian_cli.config.format_managed_message",
...
def test_affirmative_answer_sets_pending_relaunch_and_returns_true(answer, capsys):
    ...
    with patch("fulilian_cli.config.is_managed", return_value=False):
        result = _call(self_)
