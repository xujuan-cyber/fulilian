...
def _isolate(tmp_path, monkeypatch):
    """Isolate FULILIAN_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal FulilianCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = FulilianCLI.__new__(FulilianCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["fulilian-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Fulilian' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Fulilian' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Fulilian' minimum context length."""
