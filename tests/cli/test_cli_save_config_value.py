...
class TestSaveConfigValueAtomic:
    ...
    def config_env(self, tmp_path, monkeypatch):
        ...
        # save_config_value resolves the target live via get_fulilian_home(), so
        # point FULILIAN_HOME at the temp dir (the _fulilian_home import-time
        # constant is no longer consulted).
        monkeypatch.setenv("FULILIAN_HOME", str(fulilian_home))
        monkeypatch.setattr("cli._fulilian_home", fulilian_home)
        return config_path
    ...
    def test_model_write_runs_shared_cron_drift_warning(self, config_env, monkeypatch):
        warning = MagicMock()
        monkeypatch.setattr(
            "fulilian_cli.config.warn_unpinned_cron_jobs_after_model_config_change",
            warning,
        )
...
class TestSaveConfigValueTargetsUserConfig:
    """Regression: persisted runtime settings must land in FULILIAN_HOME/config.yaml
    ...
    install whose FULILIAN_HOME/config.yaml did not exist yet, save_config_value
    fell back to the checked-in cli-config.yaml. The toggle reported success, but
    startup read FULILIAN_HOME/config.yaml and never saw the setting."""

    def test_creates_user_config_when_absent(self, tmp_path, monkeypatch):
        # Fresh FULILIAN_HOME with NO config.yaml (managed/desktop first launch).
        fulilian_home = tmp_path / ".fulilian"
        fulilian_home.mkdir()
        monkeypatch.setenv("FULILIAN_HOME", str(fulilian_home))
    ...
    def test_does_not_write_repo_cli_config(self, tmp_path, monkeypatch):
        ...
        result = yaml.safe_load((fulilian_home / "config.yaml").read_text())
        assert result["wake_word"]["enabled"] is True
