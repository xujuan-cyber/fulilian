...
Run from the PR worktree:
    ...
    cd ~/.fulilian/fulilian-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.fulilian/fulilian-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.fulilian/fulilian-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a fulilian-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a fulilian-agent checkout"
)
...
# Isolated FULILIAN_HOME for the config write.
home = tempfile.mkdtemp()
os.environ["FULILIAN_HOME"] = home
