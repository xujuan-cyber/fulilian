...
class TestIsBotMentioned:
    ...
    def test_full_user_id_in_body(self):
        assert self.adapter._is_bot_mentioned("hey @fulilian:example.org help")

    def test_localpart_in_body(self):
        assert self.adapter._is_bot_mentioned("fulilian can you help?")


    def test_matrix_pill_in_formatted_body(self):
        html = '<a href="https://matrix.to/#/@fulilian:example.org">Fulilian</a> help'
        assert self.adapter._is_bot_mentioned("Fulilian help", html)
    ...
    def test_m_mentions_user_ids_authoritative(self):
        ...
        assert self.adapter._is_bot_mentioned(
            "please reply",  # no @fulilian anywhere in body
            mention_user_ids=["@fulilian:example.org"],
        )
...
class TestStripMention:
    ...
    def test_strip_full_user_id(self):
        result = self.adapter._strip_mention("@fulilian:example.org help me")
        assert result == "help me"

    def test_localpart_preserved(self):
        """Bare localpart (no @) is preserved — avoids false positives in paths."""
        result = self.adapter._strip_mention("fulilian help me")
        assert result == "fulilian help me"
...
async def test_require_mention_default_processes_mentioned(monkeypatch):
    ...
    event = _make_event("@fulilian:example.org help me")
...
async def test_bare_mention_passes_empty_string(monkeypatch):
    ...
    adapter = _make_adapter()
    event = _make_event("@fulilian:example.org")
...
async def test_dm_mention_thread_creates_thread(monkeypatch):
    ...
    adapter = _make_adapter()
    _set_dm(adapter)
    event = _make_event("@fulilian:example.org help me", event_id="$dm1")
