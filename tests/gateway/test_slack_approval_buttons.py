...
class TestSlackExecApproval:
    ...
    async def test_sends_blocks_with_buttons(self):
        ...
        assert "fulilian_approve_once" in action_ids
        assert "fulilian_approve_session" in action_ids
        assert "fulilian_approve_always" in action_ids
        assert "fulilian_deny" in action_ids
    ...
    async def test_smart_deny_owner_override_hides_persistent_buttons(self):
        ...
        assert [element["action_id"] for element in elements] == [
            "fulilian_approve_once", "fulilian_deny",
        ]
...
class TestSlackApprovalAction:
    ...
    async def test_truncates_inflated_original_text(self):
        ...
        action = {"action_id": "fulilian_approve_once", "value": "session-key"}
...
class TestSlackReactionForwarding:
    ...
    async def test_reaction_synthesizes_message_in_thread(self):
        ...
        assert synth["_fulilian_reaction"]["name"] == "thumbsup"
        assert synth["_fulilian_reaction"]["action"] == "added"
        assert synth["_fulilian_reaction"]["reacted_to_ts"] == "2000.0"
        # Pre-authorized as addressed-to-the-bot (skips mention gate only).
        assert synth["_fulilian_force_process"] is True
    ...
    async def test_target_channel_handoff_routes_top_level(self):
        ...
        assert synth["_fulilian_no_thread_response"] is True
        assert synth["_fulilian_reaction_source_channel"] == "C1"
    ...
    async def test_target_channel_with_thread_routes_to_thread(self):
        ...
        assert "_fulilian_no_thread_response" not in forwarded[0]
