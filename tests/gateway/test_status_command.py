from fulilian_state import AsyncSessionDB, SessionDB
...
async def test_first_run_slack_home_channel_onboarding_uses_parent_command(monkeypatch):
    ...
    assert "/fulilian sethome" in onboarding
...
async def test_status_command_bypasses_active_session_guard():
    ...
    async def fake_handler(event):
        ...
        return "📊 **Fulilian Gateway Status**\n**Agent Running:** Yes ⚡"
...
async def test_profile_command_reports_source_stamped_profile(monkeypatch, tmp_path):
    ...
    profile_home = fulilian_home / "profiles" / "milo"
...
async def test_context_all_appends_expanded_listings():
    ...
    fake_details = {
        "skills": [
            {"name": "fulilian-agent", "index_tokens": 30, "skill_md_tokens": 2_500},
        ],
        "toolsets": [
    ...
    assert "fulilian-agent" in result
