...
class TestTelegramYamlConfigLoading:
    ...
    def test_extra_reply_to_mode_off(self, tmp_path, monkeypatch):
        ...
        fulilian_home = self._write_config(
            tmp_path, "telegram:\n  extra:\n    reply_to_mode: \"off\"\n"
        )
...
class TestDMTopicFallbackReplyToMode:
    ...
    reply_to_mode 'off' was ignored when sending via Fulilian-created DM topic
...
class TestDMTopicSyntheticSendRouting:
    ...
    response in a different chat lane than the Fulilian topic the session runs
    in. The no-anchor branch must prefer the Fulilian topic's
    ``message_thread_id`` whenever it resolves.
    """

    def test_no_anchor_prefers_fulilian_topic_thread_id(self):
