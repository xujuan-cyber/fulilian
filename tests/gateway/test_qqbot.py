...
class TestDefaultInteractionDispatch:
    ...
    async def test_update_prompt_click_writes_response_file(self, tmp_path, monkeypatch):
        """update_prompt:y click writes 'y' to ~/.fulilian/.update_response."""
        adapter = self._make_adapter()
        fulilian_home = tmp_path / "fulilian_home"
        fulilian_home.mkdir()
        monkeypatch.setattr(
            "fulilian_constants.get_fulilian_home",
            lambda: fulilian_home,
        )
        ...
        response = fulilian_home / ".update_response"
