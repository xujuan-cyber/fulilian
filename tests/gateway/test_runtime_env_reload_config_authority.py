...
FULILIAN_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env FULILIAN_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``fulilian config get terminal.backend`` still says local.
    """
    fulilian_home = tmp_path / ".fulilian"
    fulilian_home.mkdir()
    (fulilian_home / "config.yaml").write_text(
    ...
    (fulilian_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
