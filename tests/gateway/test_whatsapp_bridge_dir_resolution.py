...
(/opt/fulilian/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable FULILIAN_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_fulilian_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable FULILIAN_HOME."""
    ...
    monkeypatch.setattr(
        "fulilian_constants.get_fulilian_home", lambda: fulilian_home
    )
    ...
    expected = fulilian_home / "scripts" / "whatsapp-bridge"
