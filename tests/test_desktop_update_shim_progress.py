...
def progress(tmp_path):
    ...
    status = tmp_path / "fulilian-update-status"
...
def test_every_state_carries_a_clock(progress):
    ...
    for state, message in [
        ...
        ("done", ""),
        ("manual", "Reopen Fulilian to finish."),
        ("error", "Update failed."),
    ]:
...
# Stands in for `fulilian update`, and reports the stage that was on screen
...
n="$(cat "$FULILIAN_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$FULILIAN_TEST_CALLS"
for f in "$TMPDIR"/fulilian-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$FULILIAN_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$FULILIAN_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "fulilian-agent"
    (install_root / "venv" / "bin").mkdir(parents=True)
    fulilian = install_root / "venv" / "bin" / "fulilian"
    fulilian.write_text(FAKE_FULILIAN)
    fulilian.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "FULILIAN_TEST_CAPTURE": str(capture),
        "FULILIAN_TEST_CALLS": str(calls),
        "FULILIAN_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".fulilian-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`fulilian update` is the longest wait in the hand-off and the one the
