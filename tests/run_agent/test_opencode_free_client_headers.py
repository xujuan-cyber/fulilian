...
@patch("run_agent.OpenAI")
def test_opencode_free_sends_fulilian_attribution(mock_openai):
    """Keyless requests still identify as Fulilian (attribution headers match
    ...
    headers = _zen_call_headers(mock_openai)
    assert headers.get("X-Title") == "FuLiLian"
    assert str(headers.get("User-Agent", "")).startswith("FulilianAgent/")
