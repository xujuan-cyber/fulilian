"""Persistent slash-command worker — one FulilianCLI per TUI session.

Protocol: reads JSON lines from stdin {id, command}, writes {id, ok, output|error} to stdout.
"""

# Stop a ``utils/`` (or ``proxy/``, ``ui/``) package in the launch directory
# from shadowing Fulilian's own top-level modules.  This worker is spawned as
...
# and crash the child in a retry loop (issue #51286).  ``fulilian_bootstrap``
...
def _env_float(name: str, default: float) -> float:
    ...
    import time on a typo (e.g. ``FULILIAN_SLASH_WATCHDOG_POLL_S=2s``) and kill
...
_WATCHDOG_POLL_S = max(0.05, _env_float("FULILIAN_SLASH_WATCHDOG_POLL_S", 2.0))
_ORPHAN_GRACE_S = max(0.0, _env_float("FULILIAN_SLASH_WATCHDOG_GRACE_S", 5.0))
...
def _prepare_slash_worker_runtime() -> None:
    """Start bounded MCP discovery before FulilianCLI snapshots tools.

    Each slash_worker child is its own process — the parent ``fulilian serve``
    discovery thread does not populate this registry (issue #61891).
    """
    import logging

    from fulilian_cli.mcp_startup import (
...
def _run(cli: FulilianCLI, command: str) -> str:
...
def main():
    ...
    os.environ["FULILIAN_SESSION_KEY"] = args.session_key
    os.environ["FULILIAN_INTERACTIVE"] = "1"

    # Start before the (hundreds-of-ms) FulilianCLI build — that window is itself
    ...
    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = FulilianCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
