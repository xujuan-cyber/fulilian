...
El instalador se encarga de todo: uv, Python 3.11, Node.js, ripgrep, ffmpeg, **y un Git Bash portátil** (MinGit, descomprimido en `%LOCALAPPDATA%\fulilian\git` — no requiere administrador, completamente aislado de cualquier instalación de Git del sistema). Fulilian usa este Git Bash incluido para ejecutar comandos de shell.
...
> **Android / Termux:** La ruta manual probada está documentada en la [guía de Termux](https://github.com/xujuan-cyber/fulilian). En Termux, Fulilian instala el extra `.[termux]` curado porque el extra completo `.[all]` actualmente incluye dependencias de voz incompatibles con Android.
>
> **Windows:** Windows nativo es totalmente compatible — el comando de PowerShell de arriba instala todo. Si prefieres usar WSL2, el comando de Linux también funciona allí. La instalación nativa de Windows se encuentra en `%LOCALAPPDATA%\fulilian`; WSL2 instala en `~/.fulilian` como en Linux.

Después de la instalación:

```bash
source ~/.bashrc    # recargar shell (o: source ~/.zshrc)
fulilian              # ¡empieza a chatear!
```
...
```bash
fulilian              # CLI interactiva — inicia una conversación
fulilian model        # Elige tu proveedor y modelo LLM
fulilian tools        # Configura qué herramientas están habilitadas
fulilian config set   # Establece valores de configuración individuales
fulilian gateway      # Inicia el gateway de mensajería (Telegram, Discord, etc.)
fulilian setup        # Ejecuta el asistente de configuración completo
fulilian claw migrate # Migra desde OpenClaw (si vienes de OpenClaw)
fulilian update       # Actualiza a la última versión
fulilian doctor       # Diagnostica cualquier problema
```

📖 **[Documentación completa →](https://github.com/xujuan-cyber/fulilian)**

---

## Evita la colección de claves API — Nous Portal

Fulilian funciona con cualquier proveedor que quieras — eso no cambiará. Pero si prefieres no recopilar cinco claves API separadas para el modelo, búsqueda web, generación de imágenes, TTS y un navegador en la nube, **[Nous Portal](https://portal.nousresearch.com)** las cubre todas bajo una sola suscripción:
...
Esto te autentica vía OAuth, establece Nous como tu proveedor y activa el Tool Gateway. Comprueba qué está conectado en cualquier momento con `fulilian portal info`. Detalles completos en la [página de documentación del Tool Gateway](https://github.com/xujuan-cyber/fulilian).
...
Fulilian tiene dos puntos de entrada: inicia la interfaz de terminal con `fulilian`, o ejecuta el gateway y habla con él desde Telegram, Discord, Slack, WhatsApp, Signal o Email. Una vez en una conversación, muchos comandos de barra son compartidos entre ambas interfaces.

| Acción                              | CLI                                           | Plataformas de mensajería                                                         |
| ----------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------- |
| Empezar a chatear                   | `fulilian`                                      | Ejecuta `fulilian gateway setup` + `fulilian gateway start`, luego envía un mensaje al bot |
...
Para las listas de comandos completas, consulta la [guía de CLI](https://github.com/xujuan-cyber/fulilian) y la [guía del Gateway de Mensajería](https://github.com/xujuan-cyber/fulilian).

---

## Documentación

Toda la documentación está en **[github.com/xujuan-cyber/fulilian](https://github.com/xujuan-cyber/fulilian)**:

| Sección                                                                                             | Contenido                                                    |
| --------------------------------------------------------------------------------------------------- | ------------------------------------------------------------ |
| [Inicio rápido](https://github.com/xujuan-cyber/fulilian)              | Instalar → configurar → primera conversación en 2 minutos   |
| [Uso de CLI](https://github.com/xujuan-cyber/fulilian)                             | Comandos, atajos de teclado, personalidades, sesiones        |
| [Configuración](https://github.com/xujuan-cyber/fulilian)               | Archivo de configuración, proveedores, modelos, todas las opciones |
| [Gateway de Mensajería](https://github.com/xujuan-cyber/fulilian)           | Telegram, Discord, Slack, WhatsApp, Signal, Home Assistant   |
| [Seguridad](https://github.com/xujuan-cyber/fulilian)                        | Aprobación de comandos, emparejamiento por DM, aislamiento en contenedor |
| [Herramientas y Toolsets](https://github.com/xujuan-cyber/fulilian)   | Más de 40 herramientas, sistema de toolsets, backends de terminal |
| [Sistema de Habilidades](https://github.com/xujuan-cyber/fulilian)   | Memoria procedimental, Skills Hub, creación de habilidades   |
| [Memoria](https://github.com/xujuan-cyber/fulilian)                   | Memoria persistente, perfiles de usuario, mejores prácticas  |
| [Integración MCP](https://github.com/xujuan-cyber/fulilian)              | Conecta cualquier servidor MCP para capacidades extendidas   |
| [Programación Cron](https://github.com/xujuan-cyber/fulilian)           | Tareas programadas con entrega a plataforma                  |
| [Archivos de Contexto](https://github.com/xujuan-cyber/fulilian) | Contexto de proyecto que da forma a cada conversación      |
| [Arquitectura](https://github.com/xujuan-cyber/fulilian)            | Estructura del proyecto, bucle del agente, clases principales |
| [Contribuir](https://github.com/xujuan-cyber/fulilian)              | Configuración de desarrollo, proceso de PR, estilo de código |
| [Referencia de CLI](https://github.com/xujuan-cyber/fulilian)             | Todos los comandos y flags                                   |
| [Variables de Entorno](https://github.com/xujuan-cyber/fulilian) | Referencia completa de variables de entorno                  |

---

## Migración desde OpenClaw

Si vienes de OpenClaw, Fulilian puede importar automáticamente tu configuración, memorias, habilidades y claves API.

**Durante la configuración inicial:** El asistente de configuración (`fulilian setup`) detecta automáticamente `~/.openclaw` y ofrece migrar antes de que comience la configuración.

**En cualquier momento después de instalar:**

```bash
fulilian claw migrate              # Migración interactiva (preset completo)
fulilian claw migrate --dry-run    # Vista previa de qué se migraría
fulilian claw migrate --preset user-data   # Migrar sin secretos
fulilian claw migrate --overwrite  # Sobreescribir conflictos existentes
```
...
- **Habilidades** — habilidades creadas por el usuario → `~/.fulilian/skills/openclaw-imports/`
...
Consulta `fulilian claw migrate --help` para todas las opciones, o usa la habilidad `openclaw-migration` para una migración guiada interactiva por el agente con vistas previas de dry-run.
...
Inicio rápido para colaboradores — clona y comienza con `setup-fulilian.sh`:

```bash
git clone https://github.com/xujuan-cyber/fulilian.git
cd fulilian-agent
./setup-fulilian.sh     # instala uv, crea venv, instala .[all], enlaza ~/.local/bin/fulilian
./fulilian              # detecta automáticamente el venv, no necesitas hacer `source` primero
```
