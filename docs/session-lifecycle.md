...
The canonical transcript store is SQLite via `SessionDB` (from `fulilian_state`). The
...
Counts consecutive restarts via a JSON file (`{FULILIAN_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `fulilian update`, `fulilian gateway restart`,
or `/restart`.
