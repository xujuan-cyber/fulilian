...
When running Fulilian inside Docker, the default `network_mode: host` gives the
...
The Fulilian [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
│                                             │
│   ┌──────────────┐   ┌──────────────────┐   │
│   │ fulilian-agent │   │ fulilian-dashboard │   │
│   └──────┬───────┘   └────────┬─────────┘   │
│          │                    │              │
│          ▼                    │              │
│   ┌──────────────┐            │              │
...
#
# Usage:
#   FULILIAN_UID=$(id -u) FULILIAN_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=fulilian,fulilian-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://fulilian-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Fulilian trust model and vulnerability reporting
