...
has to be thrown away or summarized. Fulilian has always done this in one batch:
...
Fulilian folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Fulilian's normal session flush is
...
With `micro_compact` unset or `false` Fulilian behaves exactly as it always has:
...
Defaults to `$FULILIAN_HOME/logs/agent.log`. It reports passes, outcome mix, net
