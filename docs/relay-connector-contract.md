...
This document is the formal interface between the **Fulilian gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `fulilian gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`fulilian gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `fulilian` CLI.
