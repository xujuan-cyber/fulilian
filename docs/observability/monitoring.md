...
Fulilian gateway daemon, exported over OTLP/HTTP to an operator-configured
...
capture is a separate plane served by Fulilian's native NeMo Relay SDK
...
| --- | --- | --- |
| Gateway gauges | `/v1/metrics` | `fulilian.gateway.up/state/busy/drainable/active_agents/background_work/background_delegations/restart_requested`, `fulilian.platform.up/degraded` with bounded `error_code` attributes |
...
`fulilian.gateway.active_agents`, `fulilian.gateway.background_work`, and
`fulilian.gateway.background_delegations` are complementary. `active_agents`
...
```bash
fulilian monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install 'fulilian-agent[otlp]'`),
...
a plain `fulilian gateway run`: the exporter lives in the gateway process, so
...
`fulilian.gateway.up`, `fulilian.platform.up`, and `fulilian.platform.degraded`.
...
absent_over_time(fulilian_gateway_up[5m])

# Locally owned bridge is explicitly down.
fulilian_platform_up == 0

# Scheduler thread is stale even though the gateway may still be alive.
fulilian_cron_scheduler_heartbeat_age_seconds > 180

# Ticker loops but has not completed a successful tick recently.
fulilian_cron_scheduler_last_success_age_seconds > 300

# One or more jobs are beyond their existing scheduler grace window.
fulilian_cron_jobs_overdue > 0

# Catch-up counter increased, proving at least one stale occurrence was
# collapsed and run once after a delay.
increase(fulilian_cron_scheduler_catch_up_occurrences[15m]) > 0
```

Cron execution lifecycle records arrive as `fulilian.cron_execution` spans.
Alert or derive events from bounded attributes such as:

```text
fulilian.status = failed|unknown
fulilian.delivery_outcome = failed|not_configured
fulilian.error_class = auth_failed|rate_limited|timeout|network_error|
...
3. a cron lifecycle feed keyed only by opaque `fulilian.job_key`;
...
FuLiLian-owned Relay transport health remains in scope. A separate gateway
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log /tmp/fulilian_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log /tmp/fulilian_otel_capture.jsonl --wait 8
...
`fulilian.error_class = ...` list in this file's alert section and the enum's unit
...
```bash
fulilian monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
...
The `fulilian monitoring` CLI intentionally exposes `status` only. This first
release covers only FuLiLian-owned service-health and operational-diagnostic
signals, including FuLiLian-owned Relay transport health. Team Gateway's
