...
Fulilian includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Fulilian and does not require a Fulilian observability plugin. Fulilian remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
Fulilian execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `fulilian-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

> [!WARNING]
> This removes the Fulilian `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `FULILIAN_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `FULILIAN_NEMO_RELAY_ATOF_*` and `FULILIAN_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, Fulilian does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

Fulilian requires NeMo Relay 0.7.1 or later within the 0.7 release line. That
...
Fulilian installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Fulilian process so configured interceptors can
...
Collection remains off unless Fulilian policy enables it:
...
opt-in. Set `FULILIAN_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, Fulilian does not invoke Relay's plugin initializer, so Relay
...
from the selected file only. If the selected file cannot be loaded, Fulilian
...
retain the same `session_id` and add `fulilian.session.segment` plus
`fulilian.session.segment_reason` (`compaction` or `max_turns`).

## Process-Wide Plugin Policy and Profile Isolation

Relay plugin configuration is a process-level deployment choice, not a Fulilian
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that Fulilian process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, Fulilian logs:

```text
Relay plugins are active process-wide and apply to all profiles hosted by this Fulilian process.
```
...
profile's Relay scope stack. Native dynamic plugins are loaded into the Fulilian
process and share the same policy boundary.

Run profiles in separate Fulilian processes when they require different trust
...
Fulilian core owns one Relay host and one isolated Relay session scope per Fulilian
session. Core lifecycle producers use
`fulilian_cli.observability.relay_runtime` to obtain the shared session handle or
run Relay scope, LLM, tool, and mark APIs in that session context. New product
marks do not require Fulilian plugin registration. Shared-metrics marks must
...
```text
Fulilian turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Fulilian shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Fulilian sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Fulilian used for the logical call, such as
...
New calls use `fulilian.model_route.count`. The previous
`fulilian.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `fulilian.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; Fulilian
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Fulilian processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `fulilian.task_run`, parented to
the owning Fulilian session. The start counter contains only bounded execution
...
provider attempts for the same Fulilian API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Fulilian tool-call
...
cancellations. Active task ownership follows the task ID if Fulilian rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`fulilian.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. Fulilian
...
tool or plugin names. Fulilian does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `fulilian.tool_approval` marks and
...
Successful skill mutations emit `fulilian.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `fulilian.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. Fulilian derives reuse and patch-generation
...
```text
$FULILIAN_HOME/telemetry/shared_metrics/metrics.sqlite3
$FULILIAN_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Fulilian version,
...
schema field name, its current scope is one `FULILIAN_HOME`, so it is more
...
Deleting `$FULILIAN_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `FULILIAN_HOME`. To reset it, stop Fulilian
processes and remove `$FULILIAN_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Fulilian CLI turn against the deterministic local model server:
