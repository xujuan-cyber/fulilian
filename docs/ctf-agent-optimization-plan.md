...
| 基准 | **难题 holdout（3 题 / 3 个维度）+ 跑批器** | `benchmarks/fixtures-hard/`、`manifest-ctf-hard.yaml`、`ctf_hard_run.sh` |
...
→ **holdout 已可用作对照地形**，但见第 4 条：三题 3/3 一次通过，要用它们做
...
4. ~~**没有难题 holdout**~~ —— **已解除**（`9f1caf1`）。三题已建成并跑通，
   A10 遵守率已借此证明（3/3）。**但暴露了新问题：这三题当前就 3/3 一次通过，
   存在天花板效应** —— 用它验收 P0.1 等优化会测不出提升。可用的替代信号是
   `api_calls / solve_reference_steps` = 2.4–4.5×（解得出，但绕远路）。
   → 后续要么加更难的题，要么把"路径经济性"而非"解出率"作为主指标。
...
| 2b | **提高 holdout 难度，或改以"路径经济性"为主指标** | 现三题 3/3 一次通过 → 天花板效应，验收不了 P0.1 类优化；可用信号是 api_calls 2.4–4.5× 参考解 | **新增（由 2 派生）** |
