...
| 3 | **A3 对照跑** | ⚠️ **按原设计跑不出东西**：A9 之后两条审查路径都硬门控在 `memory`/`skill_manage` 上，翻 `skip_background_review` 测出的 0 是设计产物。**须与第 6 项合并为一次跑**：同时回退 A3+A9，见 §9「A9 之后 A3 已成死开关」 | **设计已修正** |
...
| 6 | **重跑 P0.5.4 对照（n≥3）** | 首屏 −3,186/次是确定的，但 api_calls 48→31 是 n=1，不能归因。**与第 3 项合并为一次对照跑** | **待做（下一次跑批）** |
