...
it is automatically downloaded from GitHub releases to $FULILIAN_HOME/bin/tirith.
...
def _get_fulilian_home() -> str:
    """Return the Fulilian home directory, respecting FULILIAN_HOME env var."""
    return str(get_fulilian_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_fulilian_home(), ".tirith-install-failed")
...
def _fulilian_bin_dir() -> str:
    """Return $FULILIAN_HOME/bin, creating it if needed."""
    d = os.path.join(_get_fulilian_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $FULILIAN_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_fulilian_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $FULILIAN_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $FULILIAN_HOME/bin/tirith
    ...
    fulilian_bin = os.path.join(_fulilian_bin_dir(), "tirith")
    if os.path.isfile(fulilian_bin) and os.access(fulilian_bin, os.X_OK):
        _resolved_path = fulilian_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return fulilian_bin
