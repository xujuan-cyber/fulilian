...
provider** — turn on a backend by enabling a plugin (``fulilian plugins
enable video_gen/<name>``) and selecting it in ``fulilian tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/fulilian-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `fulilian tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"installed video gen backends, or `fulilian tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `fulilian tools` → "
...
# `fulilian tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`fulilian tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if provider is None:
        parts.append(
            ...
            "until the user picks one via `fulilian tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
