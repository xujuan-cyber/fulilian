...
def _is_fulilian_provider_credential(name: str) -> bool:
    """True if ``name`` is a Fulilian-managed provider credential (API key,
    token, or similar) per ``_FULILIAN_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Fulilian API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    let a skill tunnel a Fulilian credential into the execute_code child.
    """
    try:
        ...
        return True
    # Dynamically-generated Fulilian-internal secrets (AUXILIARY_*_API_KEY /
    ...
    if _is_fulilian_internal_secret(name):
        return True
    return name in _FULILIAN_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Fulilian-managed provider credentials (from
    ``_FULILIAN_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Fulilian-managed
    ...
    Non-Fulilian third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_fulilian_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Fulilian provider "
                "credential %r (blocked by _FULILIAN_PROVIDER_ENV_BLOCKLIST). "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Fulilian-managed provider credentials must not be passed
                ...
                if _is_fulilian_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Fulilian "
                        "provider credential %r from config.yaml (blocked "
                        "by _FULILIAN_PROVIDER_ENV_BLOCKLIST). Operator "
