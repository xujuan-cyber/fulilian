...
LOCAL_STT_COMMAND_ENV = "FULILIAN_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "FULILIAN_LOCAL_STT_LANGUAGE"
...
# DeepInfra STT base URL now resolved via fulilian_cli.models.deepinfra_base_url (shared).
...
def _resolve_stt_language(
    ...
    *,
    ...
    Resolution order (first non-empty wins):
      ...
      3. ``FULILIAN_LOCAL_STT_LANGUAGE`` env var (legacy escape hatch)
...
def _try_lazy_install_stt() -> bool:
    ...
    try:
        ...
        logger.warning(
            ...
            "This is often a permission issue: the Fulilian process user cannot "
            "write to the virtual environment. Try running manually as the "
            "venv owner: `stat -c '%%u' '$(dirname $(dirname $(which python3)))'` "
            "then `su - <owner> -c 'VIRTUAL_ENV=/opt/fulilian/.venv "
...
#
# The single-env-var ``FULILIAN_LOCAL_STT_COMMAND`` escape hatch is preserved
...
def _is_local_stt_provider(provider: str, stt_config: Dict[str, Any]) -> bool:
    """Return whether *provider* is exempt from Fulilian's remote upload cap."""
...
def _render_command_stt_template(
    ...
    def replace_match(match: "re.Match[str]") -> str:
        ...
        token = f"__FULILIAN_STT_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _transcribe_command_stt(
    ...
    try:
        with tempfile.TemporaryDirectory(prefix=f"fulilian-cmd-stt-{provider_name}-") as tmpdir:
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set FULILIAN_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _unregistered_stt_provider_error(provider: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "error": (
            ...
            "provider registered that name. Run `fulilian plugins list` to see "
...
def _apply_pre_transcription_hook(
    *,
    ...
    """
    try:
        from fulilian_cli.plugins import has_hook, invoke_hook
...
def _prepare_audio_for_transcription(
    ...
    temp_dir = tempfile.mkdtemp(prefix="fulilian-silk-")
...
def _start_idle_unload_watcher(timeout_seconds: int) -> None:
    ...
    with _idle_unload_mgmt_lock:
        ...
        _idle_unload_thread = threading.Thread(
            target=_watch, name="fulilian-stt-idle-unload", daemon=True
        )
...
def _transcribe_local_command(
    ...
    *,
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="fulilian-local-stt-") as output_dir:
            ...
            # Scrub Fulilian secrets from the child env (sibling path to #56332 /
            # _run_command_stt — this local-whisper path previously inherited
            # the full process environment).
            from tools.environments.local import fulilian_subprocess_env

            child_env = fulilian_subprocess_env(inherit_credentials=False)
            subprocess.run(
                shlex.split(command),
                check=True,
                capture_output=True,
...
def _transcribe_groq(
    ...
    *,
    ...
    ``stt.language`` (config.yaml) > ``FULILIAN_LOCAL_STT_LANGUAGE`` (env).
...
def _transcribe_openai(
    ...
    *,
    ...
    try:
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="fulilian-stt-") as work_dir:
                try:
...
def _transcribe_deepinfra(
    ...
    *,
    ...
    ``fulilian_cli.models`` helpers so every DeepInfra surface resolves the
...
def _trim_silence_for_cloud_stt(
    ...
    work_dir = tempfile.mkdtemp(prefix="fulilian-stt-trim-")
...
def _dispatch_stt_provider(
    ...
    # unavailable). Surface it — with its `fulilian tools` remediation —
