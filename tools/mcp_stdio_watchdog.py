...
<url>``) is spawned as a direct child of the Fulilian process. Fulilian's own
...
Fulilian process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Fulilian startup's opt-in
