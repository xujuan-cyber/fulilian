...
Then set ``CAMOFOX_URL=http://localhost:9377`` in ``~/.fulilian/.env``.
...
from fulilian_cli.config import cfg_get, load_config, read_raw_config
...
def is_camofox_mode() -> bool:
    ...
    ``fulilian tools``). ``CAMOFOX_URL`` is the server ADDRESS only — its
...
def _managed_persistence_enabled() -> bool:
    """Return whether Fulilian-managed persistence is enabled for Camofox.
...
def _camofox_identity_override(task_id: Optional[str], camofox_cfg: Dict[str, Any]) -> Optional[Dict[str, str]]:
    ...
    so Fulilian operates in the same browser profile instead of creating a
...
def _adopt_existing_tab_enabled(camofox_cfg: Dict[str, Any]) -> bool:
    """Return whether Fulilian should recover an existing Camofox tab ID."""
...
def _loopback_rewrite_enabled(camofox_cfg: Dict[str, Any]) -> bool:
    ...
    ``http://127.0.0.1:9377``.  That is correct for Fulilian talking to the
...
def _adopt_existing_tab(session: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Some integrations own the visible Camofox tab outside Fulilian. Gateway
...
def _get_session(task_id: Optional[str]) -> Dict[str, Any]:
    ...
    derived from the Fulilian profile so the Camofox server can map it
...
def camofox_vision(question: str, annotate: bool = False,
    ...
    try:
        ...
        screenshots_dir = get_fulilian_home() / "browser_screenshots"
