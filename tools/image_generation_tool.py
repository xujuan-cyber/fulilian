...
selectable via ``fulilian tools`` → Image Generation; the active model is
...
def _resolve_managed_fal_gateway():
    """Resolve the FAL route from the stored `fulilian tools` selection.
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                ...
                f"  • Pick a different model via `fulilian tools` → Image Generation."
                f"{gateway_message}"
            ) from exc
        raise
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    if env is not None:
        ...
        if remote_home:
            return f"{str(remote_home).rstrip('/')}/.fulilian"
    ...
    # Fulilian cache roots can be translated without side effects. SSH can still
    ...
    try:
        ...
        if source_images and not edit_endpoint:
            raise ValueError(
                ...
                f"via `fulilian tools` → Image Generation."
            )
...
def check_fal_api_key() -> bool:
    """True if the FAL backend selected via `fulilian tools` (or, on a
...
def _build_no_backend_setup_message() -> str:
    ...
    if managed_nous_tools_enabled():
        lines.append(
            ...
            "gateway enabled (`fulilian setup`)"
        )
    lines.append(
        "  3. Configure a different image_gen provider via `fulilian tools` "
        "→ Image Generation (run `fulilian plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
    ...
    if provider is None:
        return json.dumps({
            ...
            "error": (
                ...
                f"registered that name. Run `fulilian plugins list` to see "
        ...
        if "image_url" in kwargs or "reference_image_urls" in kwargs:
            ...
            return json.dumps({
                ...
                "error": (
                    ...
                    f"supports editing via `fulilian tools` → Image Generation."
                ),
...
# model/provider via `fulilian tools` or `/skills`.
