...
``fulilian mcp install <name>`` in the terminal.
"""
...
def setup_mcp_tool(
    ...
    if callback is None:
        return tool_error(
            "setup_mcp is only available in the Fulilian desktop app. Use the "
            "terminal instead: `fulilian mcp install <name>` for catalog entries, "
            "`fulilian mcp login <name>` for OAuth."
        )
...
SETUP_MCP_SCHEMA = {
    ...
    "description": (
        ...
        "Fulilian desktop chat. The card lets them install a catalog entry, "
        ...
        "continue without the server. Catalog names: run `fulilian mcp catalog` "
