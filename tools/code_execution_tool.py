...
Lets the LLM write a Python script that calls Fulilian tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `fulilian_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `fulilian_tools.py` with file-based RPC stubs
...
# available on every platform Fulilian itself runs on.
...
# match a safe prefix, the operational FULILIAN_ allowlist, or (on Windows) an
# OS-essential name.  Delegate-task child context is also an exact-name
# operational marker: without it, a sandbox script that spawns/imports Fulilian
# code can lose the DB-layer Kanban mutation guard while still inheriting
# FULILIAN_HOME.
#
# NB: the broad "FULILIAN_" prefix was deliberately removed (#27303) — it leaked
# FULILIAN_*-named config that lacks a secret substring (e.g. FULILIAN_BASE_URL,
# FULILIAN_KANBAN_DB, FULILIAN_*_WEBHOOK).  The child only needs the few
# location/profile vars in _FULILIAN_CHILD_ALLOWED below; FULILIAN_RPC_SOCKET /
# FULILIAN_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational FULILIAN_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set fulilian_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_FULILIAN_CHILD_ALLOWED = frozenset({
    "FULILIAN_HOME",
    "FULILIAN_PROFILE",
    "FULILIAN_CONFIG",
    "FULILIAN_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational FULILIAN_* vars (_FULILIAN_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret FULILIAN_* vars dropped by the tightened allowlist (#27303). The
    # broad "FULILIAN_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like FULILIAN_KANBAN_DB / FULILIAN_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _FULILIAN_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("FULILIAN_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped FULILIAN_* var.
            _dropped_fulilian.append(k)
    if _dropped_fulilian:
        logger.debug(
            "execute_code: dropped %d non-allowlisted FULILIAN_* var(s) from the "
            ...
            len(_dropped_fulilian),
            ", ".join(sorted(_dropped_fulilian)),
        )
...
# ---------------------------------------------------------------------------
# fulilian_tools.py code generator
# ---------------------------------------------------------------------------
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Fulilian-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Fulilian-profile soft guard."""',
...
def _sandbox_failure_hint(stderr_text: str, enabled_tools=None) -> Optional[str]:
    ...
    fulilian_tools import misuse (importing tools that aren't in the sandbox,
    ...
    try:
        m = re.search(
            r"cannot import name '(\w+)' from 'fulilian_tools'", window
        )
        if m:
            missing = m.group(1)
...
def generate_fulilian_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the fulilian_tools.py stub module.
...
"""Auto-generated Fulilian tools RPC stubs."""
...
def _connect():
    ...
    FULILIAN_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["FULILIAN_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    # re-accepts (FULILIAN_RPC_PERSISTENT=1); retry once on a fresh socket.
    _attempts = 2 if os.environ.get("FULILIAN_RPC_PERSISTENT") == "1" else 1
    with _call_lock:
...
"""Auto-generated Fulilian tools RPC stubs (file-based transport)."""
...
def _execute_remote(
    ...
    The script and the generated fulilian_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/fulilian_exec_{sandbox_id}"
    ...
    try:
        ...
        _ship_file_to_remote(env, f"{sandbox_dir}/fulilian_tools.py", tools_src)
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    child_env["FULILIAN_RPC_SOCKET"] = rpc_endpoint
    child_env["FULILIAN_RPC_TOKEN"] = rpc_token
    ...
    # FULILIAN_TIMEZONE is an internal Fulilian setting and must not leak
    # into child processes.
    _tz_name = os.getenv("FULILIAN_TIMEZONE", "").strip()
    if _tz_name:
        child_env["TZ"] = _tz_name
    child_env.pop("FULILIAN_TIMEZONE", None)

    apply_subprocess_home_env(child_env)
    # ``fulilian_tools.py`` always lives in the staging directory, so that
    # directory must be importable even when project mode changes CWD.
    # Fulilian's own package root is useful too, but only when the child
    # uses the same Python environment. Project mode can select an
    # external venv; exposing Fulilian's site-packages to that interpreter
    # can mix incompatible compiled extensions (for example, Python 3.12
    # NumPy with a Python 3.9 project interpreter).
    #
    # Before re-injecting PYTHONPATH, strip Fulilian-owned entries that
    # leaked through _scrub_child_env (PYTHONPATH is in _SAFE_ENV_PREFIXES
    # so it passes the scrub).  They are redundant for same-Fulilian-
    ...
    _strip_fulilian_owned_pythonpath(child_env)
    _fulilian_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    _existing_pp = child_env.get("PYTHONPATH", "")
    _pp_parts = [tmpdir]
    if _uses_fulilian_python_environment(child_python):
        _pp_parts.append(_fulilian_root)
    elif child_python not in _external_env_logged:
        # Import behavior changes silently otherwise — surface it (once
        # per interpreter path) so "import fulilian_constants suddenly
        # fails" reports are diagnosable without log spam.
        _external_env_logged.add(child_python)
        logger.info(
            "execute_code: child interpreter %s is outside the Fulilian "
            "environment; fulilian root omitted from PYTHONPATH",
            child_python,
        )
    if _existing_pp:
...
def execute_code(
    ...
    # terminal() path refuses `launchctl bootout ai.fulilian.gateway`, but the
    ...
    # --- Set up temp directory with fulilian_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="fulilian_sandbox_")
    ...
    # selector from FULILIAN_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"fulilian_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated fulilian_tools module.
        ...
        with open(os.path.join(tmpdir, "fulilian_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  FULILIAN_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.fulilian/.env')).
...
def _load_config() -> dict:
    ...
    ``fulilian --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (fulilian-agent's python). Reproducible and the
...
# Interpreter paths already reported as outside the Fulilian environment —
...
def _python_environment_prefix(python_path: str) -> str:
    ...
    sticky empty result would silently drop the fulilian root from every
...
def _uses_fulilian_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Fulilian's active Python environment.
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``fulilian tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and fulilian-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.fulilian/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that calls Fulilian tools programmatically. "
        ...
        f"Available via `from fulilian_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from fulilian_tools import {import_str}` "
