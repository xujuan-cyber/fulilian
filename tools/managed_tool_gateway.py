...
def auth_json_path():
    """Return the Fulilian auth store path, respecting FULILIAN_HOME overrides."""
    return get_fulilian_home() / "auth.json"
...
def peek_nous_access_token() -> Optional[str]:
    ...
    Availability scans (`fulilian tools`, banner/status paint, provider
...
def read_nous_access_token() -> Optional[str]:
    ...
    try:
        from fulilian_cli.auth import resolve_nous_access_token
...
# managed vendor's gateway URL is pinned: adding one is a Fulilian release, and
