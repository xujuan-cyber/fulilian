...
Design contract (matches the Fulilian "no lazy-reading escape hatches / don't
...
def _check_metadata_block(frontmatter: Dict[str, Any]) -> List[LintFinding]:
    ...
    fulilian_meta = meta.get("fulilian") if isinstance(meta, dict) else None
    if not isinstance(fulilian_meta, dict):
        findings.append(
            LintFinding(
                WARNING,
                "missing-metadata",
                "frontmatter is missing metadata.fulilian.{tags, related_skills}.",
            )
        )
    else:
        if "tags" not in fulilian_meta:
            findings.append(
                LintFinding(
                    WARNING, "missing-metadata", "metadata.fulilian.tags is missing."
                )
            )
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("fulilian", "agent", "fulilian agent") and (
        author != "FuLiLian"
    ):
        findings.append(
            LintFinding(
                WARNING,
                "author-caps",
                f"author '{author}' should be 'FuLiLian' (proper caps) "
