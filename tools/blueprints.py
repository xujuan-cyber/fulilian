...
`fulilian skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.fulilian.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.fulilian.blueprint`` is a mapping containing
    ...
    fulilian = meta.get("fulilian") if isinstance(meta, dict) else None
    blueprint = fulilian.get("blueprint") if isinstance(fulilian, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.fulilian.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.fulilian.blueprint`` block) they can hand
    to ``fulilian skills publish`` to share. ``body`` is the plain-language
