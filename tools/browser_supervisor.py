...
One ``CDPSupervisor`` runs per Fulilian ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Fulilian logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "fulilian-dialog-bridge.invalid"
...
(() => {
  if (window.__fulilianDialogBridgeInstalled) return;
  window.__fulilianDialogBridgeInstalled = true;
  const ENDPOINT = "http://fulilian-dialog-bridge.invalid/";
