...
class SSHEnvironment(BaseEnvironment):
    ...
    def __init__(self, host: str, user: str, cwd: str = "~",
        ...
        self.control_dir = Path(tempfile.gettempdir()) / "fulilian-ssh"
        ...
        self._sync_manager = FileSyncManager(
            get_files_fn=lambda: iter_sync_files(f"{self._remote_home}/.fulilian"),
    ...
    def _ensure_remote_dirs(self) -> None:
        """Create base ~/.fulilian directory tree on remote in one SSH call."""
        base = f"{self._remote_home}/.fulilian"
    ...
    def _ssh_bulk_upload(self, files: list[tuple[str, str]]) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="fulilian-ssh-bulk-") as staging:
    ...
    def _ssh_bulk_download(self, dest: Path) -> None:
        """Download remote .fulilian/ as a tar archive."""
        # Tar from / with the full path so archive entries preserve absolute
        # paths (e.g. home/user/.fulilian/skills/f.py), matching _pushed_hashes keys.
        rel_base = f"{self._remote_home}/.fulilian".lstrip("/")
        ssh_cmd = self._build_ssh_command()
