...
def _block_message(operation: str, root: Path) -> str:
    ...
    return (
        f"Blocked: `{operation}` would rewrite Fulilian's live source checkout "
        ...
        "checkout, stop Fulilian, run the command externally, then restart "
        "Fulilian."
    )
...
def _scratch_dir_hint() -> str:
    ...
    base = Path(fulilian_home).expanduser() if fulilian_home else Path.home() / ".fulilian"
