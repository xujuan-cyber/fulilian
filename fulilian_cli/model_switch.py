...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from fulilian_cli.model_switch import
...
def resolve_display_context_length(
    ...
    if config_context_length is not None and (
        ...
        try:
            from fulilian_cli.route_identity import should_clear_context_pin
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if not new_model:
            if pdef.base_url:
                from fulilian_cli.runtime_provider import _auto_detect_local_model
    ...
    # =================================================================
        ...
        try:
            from fulilian_cli.config import load_config
            from fulilian_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from fulilian_cli.providers import resolve_user_provider as _ruser
    ...
    # kimi) the same way.
    from fulilian_cli.models import opencode_provider_family as _oc_family_fn
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from fulilian_cli.config import normalize_extra_headers
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    def _fetch_one(slug: str) -> None:
        try:
            ...
            if models:
                from fulilian_cli.models import update_provider_cache_entry
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from fulilian_cli/models.py (OPENROUTER_MODELS,
    ...
    from fulilian_cli.config import coerce_provider_id, stringify_provider_map
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from fulilian_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                from fulilian_cli.auth import get_provider_auth_state as _nous_state
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for grp in ep_groups.values():
            ...
            if _probe_live:
                ...
                try:
                    from fulilian_cli.models import cached_fetch_api_models
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from fulilian_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    """
    from fulilian_cli.models import fetch_openrouter_models
