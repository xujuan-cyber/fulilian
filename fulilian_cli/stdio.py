...
on Japanese locales, etc.).  Fulilian's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``fulilian_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Fulilian is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``FULILIAN_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("FULILIAN_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Fulilian-managed Git install directories so
    ...
    # already-running shells don't see the change — which means fulilian
    ...
    # install without Fulilian-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Fulilian's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Fulilian's
    ``fulilian config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Fulilian (User env var in Windows Settings, or
    export in a PowerShell profile) and Fulilian picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Fulilian-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\fulilian\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``fulilian`` from right after install) retain their old PATH.

    Any subprocess Fulilian spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Fulilian-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "fulilian", "git", "cmd"),
        os.path.join(local_appdata, "fulilian", "git", "bin"),
        os.path.join(local_appdata, "fulilian", "git", "usr", "bin"),
        # Fulilian venv Scripts directory — host of the fulilian.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes fulilian, but harmless to include.
        os.path.join(local_appdata, "fulilian", "fulilian-agent", "venv", "Scripts"),
