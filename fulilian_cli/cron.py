"""
Cron subcommand for fulilian CLI.
...
# commands at execution time when ``_FULILIAN_GATEWAY=1``.
...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Start it with: fulilian gateway install", Colors.DIM))
    print(color("                    sudo fulilian gateway install --system  # Linux servers", Colors.DIM))
    print(color("     Check status:  fulilian cron status", Colors.DIM))
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'fulilian cron create ...' or the /cron command in chat.", Colors.DIM))
        return
...
def cron_tick():
    ...
    try:
        ...
        print("  Check `fulilian cron status` and the gateway log for details.")
        return 1
    return 0
...
def cron_incidents(args) -> int:
    ...
    ``fulilian cron incidents [--state <s>]`` lists incidents (the stored error
    is redacted and truncated at write time, safe for terminal display);
    ``fulilian cron incidents ack <id>`` closes one so its failure ping stays
    ...
    if action == "ack":
        ...
        if not incident_id:
            print(
                color(
                    "✗ Incident ID required: fulilian cron incidents ack <incident_id>",
                    Colors.RED,
                )
            )
            return 1
    ...
    print(
        color(
            ...
            "fulilian cron incidents ack <id>",
            Colors.DIM,
        )
    )
    return 0
...
def cron_status():
    ...
    if pids or gateway_alive_via_lock:
        ...
        if hb_age is not None and hb_age > STALE_AFTER:
            ...
            print("  Cron jobs may NOT be firing. Restart: fulilian gateway restart")
            ...
            if last_error:
                ...
                if "Permission denied" in last_error:
                    print(color(
                        ...
                        "(e.g. rewritten by a root `docker exec fulilian "
                        "fulilian cron ...`). Fix ownership to match the "
        ...
        print("    fulilian gateway install    # Install as a user service")
        print("    sudo fulilian gateway install --system  # Linux servers: boot-time system service")
        print("    fulilian gateway            # Or run in foreground")
...
def _job_action(action: str, job_id: str, success_verb: str) -> int:
    ...
    if action == "run":
        ...
        # the CLI inherits a gateway/desktop session env (FULILIAN_SESSION_KEY);
...
def cron_resume(args) -> int:
    ...
    from cron.jobs import AmbiguousJobReference, _fulilian_now, rearm_oneshot

    run_at = _fulilian_now().isoformat() if args.run_now else args.run_at
    try:
...
def cron_notepad(args) -> int:
    """Handle ``fulilian cron notepad <job_id> [get|set|delete|list]``.
    ...
    try:
        if action == "set":
            if key is None or value is None:
                print(color("Usage: fulilian cron notepad <job_id> set <key> <value>", Colors.RED))
                return 1
        ...
        if action == "get":
            if key is None:
                print(color("Usage: fulilian cron notepad <job_id> get <key>", Colors.RED))
                return 1
        ...
        if action == "delete":
            if key is None:
                print(color("Usage: fulilian cron notepad <job_id> delete <key>", Colors.RED))
                return 1
...
def cron_command(args):
    ...
    print("Usage: fulilian cron [list|create|edit|pause|resume|run|remove|status|runs|tick]")
    sys.exit(1)
