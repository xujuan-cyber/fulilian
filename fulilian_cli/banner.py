...
_UPSTREAM_REPO_URL = "https://github.com/xujuan-cyber/fulilian.git"
_OFFICIAL_REPO_CANONICAL = "github.com/xujuan-cyber/fulilian"
...
def _github_compare_behind(current_rev: str, target_rev: str) -> Optional[int]:
    ...
    if not (_is_full_sha(current_rev) and _is_full_sha(target_rev)):
        return None
    url = (
        "https://api.github.com/repos/xujuan-cyber/fulilian/"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    canonical xujuan-cyber/fulilian repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"FuLiLian v{VERSION} ({RELEASE_DATE})"
...
def _format_update_notice(behind: int) -> str:
    ...
    from fulilian_cli.config import get_managed_update_command, recommended_update_command
    if behind > 0:
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    if not _has_native_mcp:
        try:
            from fulilian_cli.plugins import get_portable_mcp_server_names_nowait
    ...
    try:
        from fulilian_cli.codex_runtime_switch import get_current_runtime
        from fulilian_cli.config import load_config as _load_cfg
