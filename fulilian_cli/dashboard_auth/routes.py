...
def _redirect_uri(request: Request) -> str:
    ...
    Three resolution tiers:
      ...
      1. ``FULILIAN_DASHBOARD_PUBLIC_URL`` env var or
      ...
      2. ``X-Forwarded-Prefix: /fulilian`` (Mission Control deploys) — we
...
def _prefix(request: Request) -> str:
    ...
    ``fulilian_cli.dashboard_auth.prefix`` for the normalisation rules.
    """
    from fulilian_cli.dashboard_auth.prefix import prefix_from_request
...
async def auth_login(request: Request, provider: str, next: str = ""):
    ...
    pkce = ls.cookie_payload.get("fulilian_session_pkce", "")
...
async def api_auth_ws_ticket(request: Request):
    ...
    from fulilian_cli.dashboard_auth.ws_tickets import TTL_SECONDS, mint_ticket
...
async def auth_native_refresh(request: Request, body: _NativeRefreshBody):
    ...
    """
    from fulilian_cli.dashboard_auth import list_session_providers
    from fulilian_cli.dashboard_auth.base import RefreshExpiredError
