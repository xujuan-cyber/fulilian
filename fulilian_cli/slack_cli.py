"""``fulilian slack ...`` CLI subcommands.

Today only ``fulilian slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ fulilian slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Fulilian. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Fulilian deployment — users can tweak them in the Slack UI after
    pasting.

    By default, this keeps Fulilian on Slack's older Assistant messaging
    ...
    """
    from fulilian_cli.commands import slack_app_manifest
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with Fulilian in threads and DMs.",
        }
        ...
        features["agent_view"] = {
            "agent_description": "Chat with Fulilian in Slack Messages.",
        }
        bot_scopes.append("assistant:write")
    ...
    display_information = {
        "name": bot_name[:35],
        "description": (bot_description or "Your Fulilian agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``fulilian_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$FULILIAN_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Fulilian")
    ...
    """
    name = getattr(args, "name", None) or "Fulilian"
    description = getattr(args, "description", None) or "Your Fulilian agent on Slack"
    ...
    if getattr(args, "slashes_only", False) and (
        ...
        print(
            "fulilian slack manifest: long description options cannot be used "
            "with --slashes-only",
            file=sys.stderr,
        )
        return 2
    if long_description_file is not None:
        ...
        try:
            ...
            print(
                f"fulilian slack manifest: cannot read long description from "
                f"{source_arg}: {exc}",
                file=sys.stderr,
            )
            return 2
    if (
        ...
        print(
            "fulilian slack manifest: long description must be at least "
    ...
    if (
        ...
        print(
            "fulilian slack manifest: long description must be at most "
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Fulilian app\n"
