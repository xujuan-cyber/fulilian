"""Fulilian skin/theme engine — the theme SDK for every surface.

A data-driven skin system that lets users (and Fulilian itself) customize the
visual appearance across the CLI, the TUI, and the desktop GUI from a single
file. Skins are defined as YAML files in ~/.fulilian/skins/ or as built-in presets.
...
``resolve_skin`` / ``skin.changed``). A skin dropped in ~/.fulilian/skins/ therefore
...
.. code-block:: yaml
    ...
    branding:
      agent_name: "FuLiLian"          # Banner title, status display
      welcome: "Welcome message"          # Shown at CLI startup
      goodbye: "Goodbye! ⚕"              # Shown on exit
      response_label: " ⚕ Fulilian "       # Response box header label
...
.. code-block:: python
    ...
    set_active_skin("mytheme")            # Switch to user skin from ~/.fulilian/skins/
...
Drop a YAML file in ``~/.fulilian/skins/<name>.yaml`` following the schema above.
...
class SkinConfig:
    ...
    banner_logo: str = ""    # Rich-markup ASCII art logo (replaces FULILIAN_AGENT_LOGO)
    banner_hero: str = ""    # Rich-markup hero art (replaces FULILIAN_CADUCEUS)
...
_BUILTIN_SKINS: Dict[str, Dict[str, Any]] = {
    "default": {
        ...
        "branding": {
            "agent_name": "FuLiLian",
            "welcome": "Welcome to FuLiLian! Type your message or /help for commands.",
            "goodbye": "Goodbye! ⚕",
            "response_label": " ⚕ Fulilian ",
            "prompt_symbol": "❯",
...
def _skins_dir() -> Path:
    ...
    return get_fulilian_home() / "skins"
