"""``fulilian approvals test`` — dry-run approval verdict for a command.
...
def approvals_test_command(args) -> int:
    """Handle ``fulilian approvals test <command...>``. Returns the exit code."""
    ...
    if words and words[0] == "--":
        words = words[1:]
    if not words:
        print("usage: fulilian approvals test [--env-type TYPE] [--json] -- <command...>")
        return EXIT_USAGE
    command = " ".join(words)
