...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from fulilian_cli.banner import format_banner_version_label

        print(format_banner_version_label())
    except Exception:
        from fulilian_cli import __release_date__, __version__

        print(f"FuLiLian v{__version__} ({__release_date__})")
    ...
    try:
        from fulilian_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
