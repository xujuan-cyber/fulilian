...
Used by ``fulilian kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``fulilian_cli/goals.py`` — same aux
...
FULILIAN_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("FULILIAN_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the FuLiLian board.
...
def _profile_author() -> str:
    """Mirror of ``fulilian_cli.kanban._profile_author``. Kept local to
    ...
    *,
    ...
    try:
        ...
        resp = call_llm(
            ...
            temperature=0.3,
            max_tokens=FULILIAN_KANBAN_SPECIFY_MAX_TOKENS,
