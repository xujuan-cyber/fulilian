"""
fulilian fallback — manage the fallback provider chain.
...
Subcommands:
  fulilian fallback [list]   Show the current fallback chain (default when no subcommand)
  fulilian fallback add      Pick provider + model via the same picker as `fulilian model`,
                           then append the selection to the chain
  fulilian fallback remove   Pick an entry to delete from the chain
  fulilian fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.fulilian/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from fulilian_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `fulilian model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  fulilian fallback add")
        print()
        return
    ...
    print("  Docs: https://github.com/xujuan-cyber/fulilian/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `fulilian model`, then append the selection to the chain."""
    from fulilian_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `fulilian model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `fulilian fallback list` to view, or `fulilian fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``fulilian fallback [subcommand]``."""
