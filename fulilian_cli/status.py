"""
Status command for fulilian CLI.

Shows the status of all FuLiLian components.
"""
...
from fulilian_cli.auth import AuthError, resolve_provider
from fulilian_cli.colors import Colors, color
from fulilian_cli.config import get_env_path, get_env_value, get_fulilian_home, load_config
...
from fulilian_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``fulilian config``'s
...
def _estop_status_line():
    """One-line pause banner for `fulilian status`, or None when not paused.

    Cheap: a single stat on $FULILIAN_HOME/ESTOP via agent.estop.
    """
    try:
    ...
    return f"⏸️  PAUSED (global emergency stop{suffix}; `fulilian resume` to lift)"


def show_status(args):
    """Show status of all FuLiLian components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ FuLiLian Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        # Read-only display: use the refresh-free snapshot so `fulilian status`
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: fulilian portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: fulilian model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: fulilian auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: fulilian auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: fulilian model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from fulilian_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'fulilian-agent[vercel]')"
        ...
        # provider's doctor rows (fail-soft — never break `fulilian status`).
        try:
    ...
    try:
        from fulilian_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   fulilian gateway")
    ...
    jobs_file = get_fulilian_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    if _session_count is not None and _session_count > 0:
        ...
        sessions_file = get_fulilian_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'fulilian doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'fulilian setup' to configure", Colors.DIM))
    print()
