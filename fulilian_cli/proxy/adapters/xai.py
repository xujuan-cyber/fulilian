...
# xAI's public API is OpenAI-compatible for the endpoints Fulilian commonly
# uses. The Responses endpoint is included because Fulilian' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Fulilian-managed OAuth credentials."""

    auth_hint = "fulilian auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`fulilian auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`fulilian auth reset xai-oauth` or re-authenticate with "
                    "`fulilian auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `fulilian auth add xai-oauth --type oauth`."
            )
