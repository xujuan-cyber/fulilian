...
the existing module-level functions in fulilian_cli.gateway and
fulilian_cli.gateway_windows directly. This protocol is a thin facade
...
``fulilian gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``fulilian gateway start/stop/restart``).
    """
    ...
    # `fulilian gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged fulilian
    ...
    detection always fails. Since every Fulilian runtime call inside the
    container drops to fulilian via ``s6-setuidgid``, that silent failure
...
# in ``fulilian_cli.gateway`` (systemd/launchd) and ``fulilian_cli.gateway_windows``
...
# active (set via the ``fulilian -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in fulilian_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from fulilian_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from fulilian_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from fulilian_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from fulilian_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in fulilian_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from fulilian_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from fulilian_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from fulilian_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from fulilian_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``fulilian_cli.gateway_windows`` (Scheduled Task /
    ...
    def is_running(self, name: str) -> bool:
        from fulilian_cli import gateway_windows
        from fulilian_cli.gateway import find_gateway_pids
...
#
# Per-profile gateways are registered dynamically when `fulilian profile create`
# runs inside the container (Phase 4). Static services (main-fulilian, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's FULILIAN_HOME blindly;
    derive the shared profile root from the current FULILIAN_HOME and map the
    ...
    fulilian_home = Path(os.environ.get("FULILIAN_HOME", "/opt/data"))
    if fulilian_home.parent.name == "profiles":
        root = fulilian_home.parent.parent
    else:
        root = fulilian_home
...
# UID/GID of the in-image ``fulilian`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the fulilian user.
    ...
    (the ``fulilian`` user — UID 10000 — running every Fulilian runtime
    ...
    is dispatched as the fulilian user.
    ...
    with fulilian ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           fulilian:fulilian, 0755 (parent must already exist)
    ``svc_dir/event/``                     fulilian:fulilian, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 fulilian:fulilian, 0755
    ``svc_dir/supervise/event/``           fulilian:fulilian, 03730
    ``svc_dir/supervise/control``          fulilian:fulilian, 0660    (FIFO)
    ...
    slot's supervise/ was fulilian-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _FULILIAN_UID, _FULILIAN_GID)
        except PermissionError:
            # Running as the fulilian user already — directory is fulilian-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _FULILIAN_UID, _FULILIAN_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is fulilian-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _FULILIAN_UID, _FULILIAN_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``fulilian -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`fulilian profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-fulilian, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources FULILIAN_HOME (and any extra env) via with-contenv —
             so e.g. ``-e FULILIAN_HOME=/data/fulilian`` is honored at run
          ...
          4. Drops to the fulilian user and exec's
             ``fulilian -p <profile> gateway run`` (or just ``fulilian
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``fulilian gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root FULILIAN_HOME profile" (the implicit profile that exists at
        the top of $FULILIAN_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``fulilian_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$FULILIAN_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``FULILIAN_HOME``,
        ...
        allocator in ``fulilian_cli.profiles._allocate_gateway_port``).
        ...
        lines = [
            ...
            "cd /opt/data",
            ". /opt/fulilian/.venv/bin/activate",
        ]
        ...
        # guard.
        lines.append("export FULILIAN_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's FULILIAN_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``fulilian gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-FULILIAN_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The FULILIAN_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own FULILIAN_HOME and s6 guarantees a
        # single supervised instance per slot, so there is no legitimate
        # supervised sibling for ``--replace`` to clobber.
        if profile == "default":
            gateway_cmd = "fulilian gateway run --replace"
        else:
            gateway_cmd = f"fulilian -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid fulilian {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${FULILIAN_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the FULILIAN_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e FULILIAN_HOME=/data/fulilian``
        gets its logs under /data/fulilian/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``fulilian logs`` reads and what
        ...
        return (
            ...
            f': "${{FULILIAN_HOME:=/opt/data}}"\n'
            f'log_dir="$FULILIAN_HOME/logs/gateways/{prof}"\n'
            # Create the leaf and clear a stale s6-log lock as fulilian when
            # this script starts as root. Never chown or unlink fulilian-writable
            # volume paths from this restartable root-context script:
            # log/supervise/control is fulilian-owned, so an unprivileged user
            # can race a pathname op through a symlink swap (CWE-59 /
            # CWE-367). Parent logs/gateways is seeded fulilian-owned at stage2
            # boot (#45258; tests/docker/test_log_dir_seed.py).
            f'if [ "$(id -u)" = 0 ]; then\n'
            f'  s6-setuidgid fulilian mkdir -p "$log_dir"\n'
            f'  s6-setuidgid fulilian rm -f "$log_dir/lock"\n'
            f'else\n'
            f'  mkdir -p "$log_dir"\n'
            f'  rm -f "$log_dir/lock"\n'
            f'fi\n'
            ...
            f'exec s6-setuidgid fulilian s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``fulilian gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``fulilian -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with fulilian ownership
            ...
            # the fulilian user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is fulilian-owned (see ``_seed_supervise_skeleton``), and on
