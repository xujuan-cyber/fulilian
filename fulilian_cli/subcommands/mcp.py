"""``fulilian mcp`` subcommand parser.
...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Fulilian as an MCP server",
        description=(
            "Manage MCP server connections and run Fulilian as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'fulilian mcp add' to connect to a new server, or\n"
            "'fulilian mcp serve' to expose Fulilian conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Fulilian as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `fulilian mcp add ...` to fall through to
    # interactive chat.
    ...
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `fulilian mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `fulilian mcp install n8n`)",
    )
