"""``fulilian debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for FuLiLian. Use 'fulilian debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    fulilian debug share              Upload debug report (asks for confirmation)
    fulilian debug share --yes        Skip confirmation (for scripts/CI)
    fulilian debug share --lines 500  Include more log lines
    fulilian debug share --expire 30  Keep paste for 30 days
    fulilian debug share --local      Print report locally (no upload)
    fulilian debug share --no-redact  Disable upload-time secret redaction
    fulilian debug share --nous       Upload to Nous-internal storage (private)
    fulilian debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'fulilian debug share'",
    )
