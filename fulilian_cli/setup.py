"""
Interactive setup wizard for FuLiLian.
...
_DOCS_BASE = "https://github.com/xujuan-cyber/fulilians"
...
def _supports_same_provider_pool_setup(provider: str) -> bool:
    ...
    if provider == "openrouter":
        return True
    from fulilian_cli.auth import PROVIDER_REGISTRY
...
# display_fulilian_home imported lazily at call sites (stale-module safety during fulilian update)
...
from fulilian_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from fulilian_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Fulilian Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Fulilian using environment variables or config commands:")
    print_info("  fulilian config set model.provider custom")
    print_info("  fulilian config set model.base_url http://localhost:8080/v1")
    print_info("  fulilian config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'fulilian setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt(question: str, default: str = None, password: bool = False) -> str:
    ...
    try:
        if password:
            ...
            from fulilian_cli.cli_output import line_input
...
_SETUP_NAVIGATION: ContextVar[_SetupNavigationState | None] = ContextVar(
    "fulilian_setup_navigation", default=None
)
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from fulilian_cli.curses_ui import curses_radiolist
...
def is_noninteractive() -> bool:
    ...
    ``FULILIAN_NONINTERACTIVE=1`` (see ``fulilian_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("FULILIAN_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``FULILIAN_NONINTERACTIVE=1`` or a closed/redirected
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'fulilian setup')")


def _print_setup_summary(config: dict, fulilian_home):
    ...
    try:
        from fulilian_cli.auth import resolve_provider
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'fulilian setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from fulilian_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `fulilian tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from fulilian_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'fulilian setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'fulilian setup tts'"))
    else:
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nous:
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'fulilian tools' → Speech-to-Text")
            )
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'fulilian setup terminal'"))
    ...
    # Spotify (OAuth via fulilian auth spotify — check auth.json, not env vars)
    try:
        from fulilian_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'fulilian setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {fulilian_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('fulilian setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('fulilian setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('fulilian setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('fulilian setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('fulilian setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('fulilian config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('fulilian config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('fulilian config set <key> <value>', Colors.GREEN)}")
...
# between `fulilian tools` and `fulilian setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``fulilian model``)
    ...
    provider added to ``fulilian model`` is automatically available here.

    When *quick* is True, skips credential rotation, vision, and TTS
    configuration — used by the streamlined first-time quick setup.
    """
    from fulilian_cli.config import load_config, save_config
    ...
    # Delegate to the shared fulilian model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from fulilian_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: fulilian model")
    ...
    # on demand via `fulilian auth add`, `fulilian setup` vision, and
    # `fulilian setup tts`. This keeps both quick and full setup thin.
...
def _install_neutts_deps() -> bool:
    ...
    from fulilian_cli.tools_config import _pip_install
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``fulilian model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from fulilian_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.fulilian/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Fulilian auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via fulilian setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'fulilian setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Fulilian runs shell commands and code.")
    ...
    # under ~/.fulilian/plugins/). Fail-soft: a broken plugin must not take the
    # setup wizard down.
    ...
    if selected_backend == "local":
        ...
        # configurable later via `fulilian setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `fulilian setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `fulilian egress setup` then `fulilian egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `fulilian egress setup`."
            )
        ...
        print_info("Requires the optional SDK: pip install 'fulilian-agent[vercel]'")
        ...
        try:
            ...
            # Managed uv first: $FULILIAN_HOME/bin is never on PATH, so a bare
            # which() misses the uv Fulilian installed. Bootstrapping one is
            ...
            from fulilian_cli.managed_uv import ensure_uv
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'fulilian-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into FULILIAN_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("FULILIAN_MAX_ITERATIONS")
    ...
    print_info("  Run `fulilian setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # FULILIAN_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.fulilian/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from fulilian_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_fulilian_home(Path(get_fulilian_home()))
    except Exception:
        pass
...
def _profile_name_from_fulilian_home(fulilian_home) -> str | None:
    """Return the active profile name when FULILIAN_HOME is a profile dir."""
    if fulilian_home.parent.name == "profiles":
        return fulilian_home.name
    return None
...
def _setup_bluebubbles():
    ...
    print_info("Connects Fulilian to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from fulilian_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://github.com/xujuan-cyber/fulilians/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://github.com/xujuan-cyber/fulilians/user-guide/messaging/webhooks/#configuring-routes")
    print()
...
def setup_gateway(config: dict):
    ...
    from fulilian_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Fulilian from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'fulilian setup gateway' later to configure.")
    else:
        for idx in selected:
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     fulilian config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
    ...
    # via `fulilian import` or `fulilian setup gateway`). Gating this on
        ...
        if supports_systemd and _system_scope_wizard_would_need_root():
            ...
            try:
                if supports_systemd:
                    ...
                    from fulilian_cli import gateway_windows
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `fulilian setup tools` and `fulilian tools` use the same flow:
    ...
    """
    from fulilian_cli.tools_config import tools_command
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this Fulilian profile and are not uploaded.")
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``fulilian_cli.auth`` — lists every supported
    ...
    """
    try:
        from fulilian_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from fulilian_cli.config
    ...
    if section_key == "model":
        ...
        from fulilian_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_fulilian.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_fulilian migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_fulilian", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Fulilian.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Fulilian to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Fulilian at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Fulilian at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Fulilian at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Fulilian at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Fulilian equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Fulilian config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Fulilian.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Fulilian's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(fulilian_home: Path) -> bool:
    ...
    print_info("Fulilian can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: fulilian claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: fulilian claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Fulilian configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Fulilian config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Fulilian (use fulilian claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``fulilian setup --portal`` and ``fulilian portal``. This is the
    ...
    Fulilian session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``fulilian setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``fulilian model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``fulilian portal`` always offers a model picker,
    ...
    print(color("│     ⚕ Fulilian Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `fulilian portal` == quick setup's Nous step.
    try:
        from fulilian_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `fulilian portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `fulilian portal`: %s", exc)
        print()
    ...
    print_info("  Run `fulilian portal info` to inspect routing.")
    print_info("  Run `fulilian` to start chatting.")


@contextmanager
def _setup_navigation_scope():
    """Install and reliably restore the setup menu navigation context."""
    from fulilian_cli.curses_ui import (
...
def run_setup_action_with_navigation(
    ...
    *,
    ...
    Shared commands such as ``fulilian model`` use the same provider/model
...
def _run_setup_wizard_impl(args):
    ...
    Supports full, quick, and section-specific setup:
      fulilian setup           — full or quick (auto-detected)
      fulilian setup model     — just model/provider
      fulilian setup tts       — just text-to-speech
      fulilian setup terminal  — just terminal backend
      fulilian setup gateway   — just messaging platforms
      fulilian setup tools     — just tool configuration
      fulilian setup telemetry — just local shared metrics
      fulilian setup agent     — just agent settings
    """
    from fulilian_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Fulilian Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ FuLiLian Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your FuLiLian installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_setup_steps(
                [("Quick Setup", lambda: _run_quick_setup(config, fulilian_home))]
            )
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Fulilian configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'fulilian setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(fulilian_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Fulilian?",
            [
                ...
                [
                    (
                        ...
                        lambda: _run_first_time_quick_setup(
                            config, fulilian_home, is_existing
                        ),
                    )
                ]
            )
    ...
    print_info(f"Data folder:  {fulilian_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'fulilian config edit'")
    ...
    # Tune later with `fulilian setup agent`.
    if not is_existing:
    ...
    def _gateway_step() -> None:
        ...
        from fulilian_cli.gateway import ensure_gateway_service
    ...
    _print_setup_summary(config, fulilian_home)


def _run_first_time_quick_setup(config: dict, fulilian_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``fulilian setup <section>``
    or switch providers with ``fulilian model``.
    """
    from fulilian_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'fulilian setup gateway'",
        ],
        0,
    )
    ...
    if gateway_choice == 0:
        ...
        # (e.g. via `fulilian import` from another machine).
        from fulilian_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    ...
    print()
    print_info("  Configure all settings:    fulilian setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  fulilian setup gateway")
...
def _print_macos_fda_tip() -> None:
    ...
    print_info("  your terminal (and Fulilian.app if you use Desktop), or run:")
    print_info("    open \"x-apple.systempreferences:com.apple.preference"
               ".security?Privacy_AllFiles\"")
    print_info("  The grant is permanent — it survives every Fulilian update.")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    ``fulilian-agent`` skill (the agent's operating manual for driving,
    configuring, and troubleshooting Fulilian) is always seeded — without
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``fulilian tools`` (which
    ...
    try:
        ...
        from fulilian_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``fulilian setup agent`` /
    ``fulilian config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, fulilian_home, is_existing: bool):
    ...
    print_info("essential `fulilian-agent` skill is always kept so the agent")
    print_info("can help you drive and configure Fulilian itself.")
    print()
    ...
    if path == 0:
        ...
        # `fulilian update` runs don't re-inject them. Essential skills (the
        # `fulilian-agent` operating manual) are still seeded by the sync.
        try:
        ...
        print_info("  Enable tools:        fulilian tools")
        print_info("  Seed skills:         fulilian skills opt-in --sync")
        print_info("  Add MCP servers:     fulilian mcp add")
        print_info("  Enable plugins:      fulilian plugins")
        print_info("  Tune agent settings: fulilian setup agent")
        print()
        _print_setup_summary(config, fulilian_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, fulilian_home)


def _blank_slate_walkthrough(config: dict, fulilian_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            set_bundled_skills_opt_out(True)
            # Essential skills (the `fulilian-agent` operating manual) are
            # still seeded even for an opted-out profile.
            sync_skills(quiet=True)
            print_info("No skills seeded (except the essential `fulilian-agent`")
            print_info("skill). A .no-bundled-skills marker keeps future")
            print_info("`fulilian update` runs from re-injecting them. Opt back in any")
            print_info("time with `fulilian skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `fulilian tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `fulilian plugins list` / `fulilian plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `fulilian plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `fulilian mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `fulilian mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   fulilian tools")
    print_info("  Seed skills:         fulilian skills opt-in --sync")
    print_info("  Add MCP servers:     fulilian mcp add")
    print_info("  Tune agent settings: fulilian setup agent")
    print()
...
def _run_quick_setup(config: dict, fulilian_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'fulilian setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Fulilian to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'fulilian setup gateway'.")
