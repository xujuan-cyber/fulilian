...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    if sys.platform == "win32":
        try:
            from fulilian_cli.gateway_windows import (
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from fulilian_cli.service_manager import detect_service_manager, get_service_manager
...
def _gateway_list() -> None:
    ...
    """
    try:
        from fulilian_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
from fulilian_constants import is_container, is_termux, is_wsl
...
def _windows_gateway_breakaway_state() -> bool | None:
    ...
    if not is_windows():
        return None
    from fulilian_cli._subprocess_compat import _WINDOWS_GATEWAY_BREAKAWAY_ENV
...
def generate_launchd_plist() -> str:
    ...
    nofile_block = ""
    try:
        from fulilian_cli.resource_limits import configured_nofile_soft_limit
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    try:
        ...
        from fulilian_cli.profiles import normalize_profile_name
...
def _setup_standard_platform(platform: dict):
    ...
    from fulilian_cli.setup_hidden_env import is_setup_hidden_env as _is_setup_hidden_env
