...
def _custom_provider_ssl_context(base_url: str):
    ...
    if not base_url:
        return None
    try:
        from fulilian_cli.config import get_custom_provider_tls_settings
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from fulilian_cli.model_catalog import get_default_model_from_cache
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from fulilian_cli.model_catalog import get_curated_openrouter_models
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from fulilian_cli.model_catalog import get_curated_nous_models
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from fulilian_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from fulilian_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from fulilian_cli.model_search import model_alias_canonical
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from fulilian_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def opencode_zen_free_headers() -> dict:
    ...
    """
    try:
        from fulilian_cli import __version__ as _v
    except Exception:
        _v = "0"
    return {
        "Authorization": "",
