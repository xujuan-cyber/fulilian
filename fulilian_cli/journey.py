"""``fulilian journey`` — what Fulilian has learned, on a timeline.
...
def _primary_hex() -> str:
    ...
    try:
        ...
        return skin.get_color("ui_primary", "") or skin.get_color("banner_title", "#5DB8F5")
    except Exception:
        return "#5DB8F5"
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    if not payload.get("nodes"):
        console.print(
            "[grey62]No learning yet — use Fulilian a while and your learned skills and "
...
if __name__ == "__main__":
    _p = argparse.ArgumentParser(prog="fulilian journey")
    register_cli(_p)
    _a = _p.parse_args()
    sys.exit(_a.func(_a))
