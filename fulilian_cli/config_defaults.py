"""Default configuration data for FuLiLian.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from fulilian_cli/config.py. Must not import from fulilian_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    # SQLite journal mode used by every Fulilian database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running Fulilian server processes.
    ...
    "session": {
        # Per-terminal `fulilian -c`: each CLI session drops a breadcrumb file
        # under $FULILIAN_HOME/terminal-sessions/<terminal-id>, and a bare
    ...
    "agent": {
        ...
        # all normalized by fulilian_cli.config.resolve_turn_limit.
        "max_turns": None,
        ...
        # budget. CLI one-shot equivalent: `fulilian chat --run-budget N`.
        ...
        # an interactive `fulilian gateway restart` must never block for hours
        ...
        # the Fulilian-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Fulilian
        ...
        # identity slot (SOUL.md). Empty by default. The FULILIAN_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Fulilian adds a coding operating brief
        ...
        # `fulilian status` / fleet monitoring). Retries never stop — this is a
        ...
        # ``FULILIAN_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        # bot_relay from a short-lived `fulilian -p <bot> chat -Q` recipient
        ...
        #             FULILIAN_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force FULILIAN_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Fulilian auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Fulilian sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Fulilian
        ...
        # bundled Fulilian image, which drops to the `fulilian` user via
    ...
    "web": {
        ...
        # firecrawl, keenable). Set by the `fulilian tools` picker's
    ...
    "browser": {
        ...
        # browser tools) runs on a Fulilian-managed SNAPSHOT of the user's
        ...
        # fresh session launches — driven by Fulilian' packaged Chromium. Only
        ...
        # deletes the snapshot store (~/.fulilian/browser-profile/) so copied
        ...
        # approval does it run `fulilian browser close-profile` (terminates the
        ...
        "camofox": {
            # When true, Fulilian sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Fulilian should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``fulilian chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.fulilian/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: fulilian sweeps the checkpoint base at startup
        ...
        # ``fulilian checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .fulilian.md, or .cursorrules before Fulilian applies
    ...
    # Single-query (``fulilian -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, Fulilian truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Fulilian' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); fulilian =
                                      # Fulilian' compression threshold triggers
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. Fulilian' local
        ...
        "codex_responses_compact_threshold": 200000,  # Server-side compaction trigger
                                      ...
                                      # the server compacts before Fulilian does.
    ...
    "auxiliary": {
        ...
        # Invoked by ``fulilian kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``fulilian kanban decompose`` and the kanban
        ...
        # ``fulilian profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `fulilian model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `fulilian` (and `fulilian chat`) launches by default:
        ...
        # REPL and `--tui` (or FULILIAN_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `fulilian --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `fulilian -c` muscle memory.  Default off so existing
        # users aren't surprised.  FULILIAN_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `fulilian --tui` drops a one-time hint
        ...
        # model — see fulilian_cli/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to FULILIAN_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/fulilian`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `fulilian pets`. Disabled until
        ...
        "pet": {
            ...
            # get_fulilian_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # protocol ping (see fulilian_cli/web_server.py — an event-loop stall
        ...
        # FULILIAN_TUI_WS_ORPHAN_REAP_GRACE_S env var remains an internal
        ...
        # (FULILIAN_TUI_SESSION_TTL_S, default 6h) are closed with
        ...
        # ``FULILIAN_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``FULILIAN_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``FULILIAN_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``FULILIAN_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``FULILIAN_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/fulilian``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Fulilian appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.fulilian/cache/piper-voices/
    ...
    # "Hey Fulilian" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey fulilian",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_fulilian" (the bundled, works-out-of-the-box default) OR a
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.fulilian/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.fulilian/cache/delegation/
    ...
    # response. If not, Fulilian feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Fulilian auto-pauses the goal and
    ...
    "moa": {
        ...
        # <fulilian_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.fulilian/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Project-local skill discovery: when a session starts inside a git
        # checkout, ``<root>/.fulilian/skills/`` and ``<root>/.agents/skills/``
        ...
        # with ``fulilian skills trust`` (run from inside it). Set to false to
        ...
        # Managed by ``fulilian skills trust`` / ``fulilian skills untrust``.
        "trusted_project_dirs": [],
        # Substitute ${FULILIAN_SKILL_DIR} and ${FULILIAN_SESSION_ID} in SKILL.md
        ...
        # (`fulilian skills install`). Runs ALONGSIDE the built-in skills
        ...
        # ~/.fulilian/skills/.curator_ledger.jsonl with before/after file
        # hashes; file contents are stored content-addressed (deduped) under
        # ~/.fulilian/.curator_backups/blobs/. Enables `fulilian curator ledger`
        # and single-mutation `fulilian curator rollback <entry-id>`.
    ...
    # cron daemon.
    #
    # See `fulilian curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `fulilian curator run --consolidate` overrides
        ...
        # normally restored on every `fulilian update`, so pruning them only
        ...
        # skills are kept forever. When > 0, `fulilian curator purge` deletes
        ...
        # skipped), snapshot ~/.fulilian/skills/ into
        # ~/.fulilian/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `fulilian curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for fulilian-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *FuLiLian*" header.
    ...
    # dangerous command. -q runs export FULILIAN_INTERACTIVE=1 (for interactive
    ...
    "approvals": {
        ...
        # modal; FULILIAN_TUI_NO_CONFIRM=1 force-skips that modal regardless of
    ...
    # replace Fulilian' built-in platform hint for a single messaging platform
    ...
    # ``fulilian plugins enable|disable`` and intentionally omitted here so an
    ...
    # stored approval from ~/.fulilian/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or FULILIAN_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # .cursorrules, project-local .fulilian config) always require human
        ...
        # longer trigger the startup banner. Add via `fulilian doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``fulilian_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Fulilian to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $FULILIAN_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via FULILIAN_CRON_MAX_PARALLEL env var.
        ...
        # FULILIAN_CRON_SCRIPT_TIMEOUT. Keep this in sync with
        ...
        # FULILIAN_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
        ...
        # overridable via FULILIAN_CRON_MEDIA_SEND_TIMEOUT env var. Keep in
    ...
    # todos to ready, and fires `fulilian -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `fulilian` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `fulilian kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     fulilian-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Fulilian tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.fulilian/logs/.
    ...
    # update model picker lists without shipping a fulilian-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://github.com/xujuan-cyber/fulilian/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `fulilian model` invocation; network failures
    ...
    # #8731).
    #
    # Provider keys accept the Fulilian provider id (as used elsewhere in
    ...
    "gateway": {
        ...
        # internal FULILIAN_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.fulilian/sessions/sessions.json entirely.
        ...
        # the FULILIAN_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.fulilian.gateway` that defenses 1-2 don't
        ...
        # `fulilian update` hang because it can never drain the gateway.
        "restart_loop_guard": {
            "max_restarts": 3,
            "window_seconds": 60,
            "max_gap_seconds": 300,
        ...
        # ``FULILIAN_GATEWAY_MAX_STARTS`` / ``FULILIAN_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.fulilian/.env,
        ...
        # behavior: files must live under the Fulilian cache, under
        ...
        # user. Bridged to FULILIAN_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Fulilian
        # cache (~/.fulilian/cache/{documents,images,audio,video,screenshots})
        ...
        # to FULILIAN_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # FULILIAN_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to FULILIAN_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.fulilian/state.db.
    ...
    "sessions": {
        ...
        # the default of ``fulilian sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.fulilian/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `fulilian sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `fulilian update` prints a one-line notice with
        ...
        # ~/.fulilian/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to FULILIAN_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # FULILIAN_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
        ...
        # (non-streaming) export such as `fulilian sessions export`. Checked
    ...
    # ``fulilian doctor`` behaviour.
    "doctor": {
        # Per-probe timeout (seconds) for the opt-in `fulilian doctor --live`
        # real-call backend probes (Firecrawl/FAL/browser/MCP/TTS/STT).
        "live_probe_timeout": 10,
    },

    # ``fulilian update`` behaviour.
    "updates": {
        ...
        #     DBs) into <FULILIAN_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``fulilian backup``-style zip
        #     of FULILIAN_HOME into <FULILIAN_HOME>/backups/, restorable with
        #     ``fulilian import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `fulilian update` does with uncommitted local changes to the
        ...
        # When `fulilian update` finds the source checkout parked on a feature
        ...
        #                         merge. `fulilian update --switch-branch`
    ...
    "lsp": {
        ...
        #              ``<FULILIAN_HOME>/lsp/bin/`` on first use.
    ...
    # `fulilian tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.fulilian/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.fulilian/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.fulilian/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Fulilian may use this encrypted
            ...
            # ~/.fulilian/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.fulilian/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Fulilian disables it for our users unless they
        # explicitly opt in here. When false (default), Fulilian sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Fulilian also calls
        ...
        "no_overlay": None,
        # cua-driver permission mode for each Fulilian computer-use runtime.
        ...
        # manifest used when permission_mode is "bounded". Fulilian passes the
    ...
    #
    # Configure with `fulilian egress setup`.  Disabled by default — the rest of
    # Fulilian works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.fulilian/bin/ on
    ...
    # Fulilian Desktop (Electron app) launch options. These only affect
    # `fulilian desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the FULILIAN_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
        ...
        # Ignored on macOS/Windows. Bridged to the FULILIAN_DESKTOP_PASSWORD_STORE
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Fulilian mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'fulilian model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Fulilian auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Fulilian' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Fulilian, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @fulilian:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. FULILIAN_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #fulilian)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: fulilian-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'fulilian-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Fulilian API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Fulilian API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Fulilian API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # FULILIAN_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to FULILIAN_TOOL_PROGRESS_MODE for backward
    ...
    # FULILIAN_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor retired its only consumer (the v3→4 migration).
    "FULILIAN_PREFILL_MESSAGES_FILE": {
    ...
    "FULILIAN_EPHEMERAL_SYSTEM_PROMPT": {
