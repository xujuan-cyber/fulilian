"""MCP picker — interactive `fulilian mcp picker` (also the default `fulilian mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``fulilian mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `fulilian plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from fulilian_cli.cli_output import prompt_yes_no
from fulilian_cli.curses_ui import curses_single_select
from fulilian_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Fulilian session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from fulilian_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `fulilian mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: fulilian mcp install <name>    Picker: fulilian mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Fulilian is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Fulilian — run `fulilian update` "
...
def show_catalog() -> None:
    """`fulilian mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`fulilian mcp picker` (and default `fulilian mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`fulilian mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `fulilian mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
