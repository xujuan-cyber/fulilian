...
from fulilian_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``xujuan-cyber/fulilian`` image bakes a ``docker``
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if is_nix_install_method(method):
        return _NIX_UPDATE_MSG
    if method == "docker":
        return "docker pull xujuan-cyber/fulilian:latest"
    if method == "apt":
...
FuLiLian runs as a published image (xujuan-cyber/fulilian), not a
...
Verify the new version after restart:
  docker run --rm xujuan-cyber/fulilian:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/xujuan-cyber/fulilian/tags
...
from fulilian_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        from fulilian_cli.toolset_validation import validate_platform_toolsets
...
def render_personality_prompt(value: Any) -> str:
    ...
    from fulilian_cli.personality import render_personality_prompt as _render
...
def show_config():
    ...
    try:
        from fulilian_cli.personality import active_personality_name
