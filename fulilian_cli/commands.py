"""Slash command definitions and autocomplete for the Fulilian CLI.
...
from fulilian_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Names a key in ``fulilian_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``fulilian_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("workspace", "Show or switch the current terminal/file workspace", "Session",
               cli_only=True, args_hint="[path]", aliases=("cwd",)),
    CommandDef("attach", "Attach text files to the next prompt", "Session",
               cli_only=True, args_hint="<file> [file...]", aliases=("file",)),
    ...
    CommandDef("snapshot", "Create or restore state snapshots of Fulilian config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Fulilian works on across turns until achieved", "Session",
    ...
    CommandDef("wake", "Toggle the 'Hey Fulilian' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control what Enter does while Fulilian is working", "Configuration",
    ...
    CommandDef("reload-skills", "Re-scan ~/.fulilian/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update FuLiLian to the latest version", "Info",
               busy_policy="dispatch", desktop="terminal"),
    CommandDef("version", "Show FuLiLian version", "Info", aliases=("v",),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`fulilian_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/fulilian`` subcommand mapping, and
    ...
    """
    try:
        from fulilian_cli.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
# Telegram allows up to 100 BotCommands. Fulilian ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Fulilian
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``fulilian skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``fulilian skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``fulilian skills list`` and
...
# unrelated command landed. These claim their slots right after /fulilian,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /fulilian <command>).
...
# native slot, the alias spelling stays reachable via /fulilian reset).
...
# ``/fulilian <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/fulilian" decision, not a silent clamp.
#   - topup: the billing/balance surface; reached via /fulilian topup on Slack.
#     (the rehaul folded the old /credits + /billing surfaces into /topup.)
#   - moa: high-cost slash mode, available through /fulilian moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /fulilian debug on Slack.
#   - egress: Docker-only proxy status; reachable as /fulilian egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /fulilian init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /fulilian version on
...
#   - diff: git working-tree diff; reached via /fulilian diff on Slack so it
...
#     /fulilian update on Slack. Demoted to free the native slot /approvals now
...
#   - heartbeat: session heartbeat management; reached via /fulilian heartbeat
#     on Slack. Added at the 50-cap — a native slot would clamp /insights.
#   - refine: on-demand memory/skill review; reached via /fulilian refine on
#     Slack. Added at the 50-cap — a native slot would clamp an existing
#     native slash.
#   - pause: global emergency stop; reached via /fulilian pause [off] on
#     Slack. Added at the 50-cap — a native slot would clamp /platform.
#   - whoami: one-off identity lookup; reached via /fulilian whoami on Slack.
...
#     /fulilian platform on Slack. Demoted when /save became gateway-available
#     (session export is an interactive surface; platform is a rare
#     informational lookup) — without this entry /save tips the registry
#     past the 50-cap and silently clamps /platform, breaking parity.
_SLACK_VIA_FULILIAN_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update", "heartbeat", "refine", "review", "pause", "whoami", "platform"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/fulilian <verb>`` subcommand.
    ...
    can still reach them via ``/fulilian <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/fulilian`` is always reserved as the first entry so the
    legacy ``/fulilian <subcommand>`` form keeps working for anything that
    ...
    # Reserve /fulilian as the catch-all top-level command.
    entries.append(("fulilian", "Talk to Fulilian or run a subcommand", "[subcommand] [args]"))
    seen.add("fulilian")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_FULILIAN_ONLY:
            # Intentionally Slack-via-/fulilian only (see _SLACK_VIA_FULILIAN_ONLY).
            return
    ...
    # everything except /fulilian, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://fulilian-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /fulilian handler.

    Maps both canonical names and aliases so /fulilian bg do stuff works
    the same as /fulilian background do stuff.

    Plugin-registered slash commands are included so ``/fulilian <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from fulilian_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        """Yield completions for /personality via fulilian_cli.personality."""
        try:
