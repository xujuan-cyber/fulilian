"""`fulilian checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.fulilian/checkpoints/``.  Actions:

    fulilian checkpoints               # same as `status`
    fulilian checkpoints status        # total size, project count, breakdown
    fulilian checkpoints list          # per-project checkpoint counts + workdir
    fulilian checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    fulilian checkpoints clear [-f]    # nuke the entire base (asks first)
    fulilian checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    fulilian checkpoints
    fulilian checkpoints prune --retention-days 3 --max-size-mb 200
    fulilian checkpoints clear -f
...
from fulilian_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: fulilian checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``fulilian checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `fulilian checkpoints` → status
