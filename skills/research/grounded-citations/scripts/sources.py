...
Ledger path resolution (first wins):
  --ledger PATH
  $FULILIAN_CITATION_LEDGER
  $FULILIAN_HOME/cache/citations/ledger.json
"""
...
from _fulilian_home import get_fulilian_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("FULILIAN_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_fulilian_home() / "cache" / "citations" / "ledger.json"
