#!/usr/bin/env bash
# GitHub environment detection helper for FuLiLian skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _fulilian_env="${FULILIAN_HOME:-$HOME/.fulilian}/.env"; [ -f "$_fulilian_env" ] && grep -q "^GITHUB_TOKEN=" "$_fulilian_env" 2>/dev/null; then
