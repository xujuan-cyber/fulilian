...
Author widget apps for the Fulilian TUI (`fulilian --tui`): glanceable ambient
...
- The TUI must be in use (`fulilian --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.fulilian/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.fulilian/logs/tui_gateway_crash.log` if a widget never appears.
