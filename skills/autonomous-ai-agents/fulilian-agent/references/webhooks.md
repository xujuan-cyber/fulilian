...
Create dynamic webhook subscriptions so external services (GitHub, GitLab, Stripe, CI/CD, IoT sensors, monitoring tools) can trigger Fulilian agent runs by POSTing events to a URL.
...
Add to `${FULILIAN_HOME:-~/.fulilian}/.env`:
```bash
...
All management is via the `fulilian webhook` CLI command:

### Create a subscription
```bash
fulilian webhook subscribe <name> \
...
- **Route scripts** (`--script` on subscribe, or `script:` on a config route): a script under `~/.fulilian/scripts/` receives the payload as JSON on stdin. JSON stdout replaces the payload before prompt templating; empty stdout, `[SILENT]`, or a nonzero exit ignores the webhook. `.sh`/`.bash` run with bash, everything else with Python. Scripts cannot live outside `~/.fulilian/scripts/` (path traversal is blocked).

```bash
fulilian webhook subscribe todoist-fulilian \
  --prompt "Task changed: {payload.content}" \
  --script "todoist-fulilian-label.py" \
...
```bash
fulilian webhook remove <name>
```

### Test a subscription
```bash
fulilian webhook test <name>
fulilian webhook test <name> --payload '{"key": "value"}'
```
...
### GitHub: PR reviews
```bash
fulilian webhook subscribe github-prs \
  --events "pull_request" \
...
```bash
fulilian webhook subscribe ci-builds \
  --events "pipeline" \
...
```bash
fulilian webhook subscribe alerts \
...
- Subscriptions persist to `~/.fulilian/webhook_subscriptions.json`

## How It Works

1. `fulilian webhook subscribe` writes to `~/.fulilian/webhook_subscriptions.json`
...
1. **Is the gateway running?** Check with `systemctl --user status fulilian-gateway` or `ps aux | grep gateway`
2. **Is the webhook server listening?** `curl http://localhost:8644/health` should return `{"status": "ok"}`
3. **Check gateway logs:** `grep webhook ~/.fulilian/logs/gateway.log | tail -20`
4. **Signature mismatch?** Verify the secret in your service matches the one from `fulilian webhook list`. GitHub sends `X-Hub-Signature-256`, GitLab sends `X-Gitlab-Token`.
5. **Firewall/NAT?** The webhook URL must be reachable from the service. For local development, use a tunnel (ngrok, cloudflared).
6. **Wrong event type?** Check `--events` filter matches what the service sends. Use `fulilian webhook test <name>` to verify the route works.
