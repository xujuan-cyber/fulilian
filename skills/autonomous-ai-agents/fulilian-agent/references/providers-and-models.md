...
| nous | OAuth device code | `fulilian auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `fulilian auth add openai-codex` |
| qwen-oauth | OAuth | `fulilian auth add qwen-oauth` |
| minimax-oauth | OAuth | `fulilian auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`fulilian auth`).
Fallback chain when the primary fails: `fulilian fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`fulilian_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   fulilian config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
