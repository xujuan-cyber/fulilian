...
`fulilian config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `fulilian tools` (interactive) or `fulilian tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_FULILIAN_CORE_TOOLS` is the default bundle most platforms inherit).
