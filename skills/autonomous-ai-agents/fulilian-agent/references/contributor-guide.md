...
Config: `~/.fulilian/config.yaml` (settings), `~/.fulilian/.env` (API keys) — both under `$FULILIAN_HOME` when it is set.
...
`_FULILIAN_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_fulilian_home()` for paths,
never hardcode `~/.fulilian`. For custom/local-only tools, write a plugin in
`~/.fulilian/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `fulilian_cli/commands.py`
...
- Tests auto-redirect `FULILIAN_HOME` to temp dirs — never touch real `~/.fulilian/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/fulilian_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_fulilian_home()` from `fulilian_constants` for all paths (profile-safe)
