# Themes / Skins — Author a Fulilian Color Theme

Author a Fulilian **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`fulilian_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.fulilian/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Fulilian itself.
...
- Write access to the Fulilian home dir — `~/.fulilian` by default, or `$FULILIAN_HOME`
  / the active profile's dir. Skins live in `<fulilian-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `fulilian config set`).
...
3. `write_file` it to `<fulilian-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `fulilian config set display.skin default` (or they can `/skin default`).
...
```
fulilian skin set <key> <hex>      # e.g. fulilian skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `fulilian skin set background "#08201f"` changes
only the background; `fulilian skin use <name>` / `fulilian skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.fulilian`** when a profile is active — resolve the real home
  from `$FULILIAN_HOME` first, falling back to `~/.fulilian`.
...
- **Never hand-edit `config.yaml` to activate.** Use `fulilian config set
...
- **You apply it, not the user.** `fulilian config set display.skin <name>` is
...
- `read_file` the written `<fulilian-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `fulilian config get display.skin` and confirm it reports `<name>`.
