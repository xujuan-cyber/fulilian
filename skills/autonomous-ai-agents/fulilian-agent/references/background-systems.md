...
the `cronjob` tool, the `fulilian cron` CLI (`list`, `add`, `edit`,
...
- **CLI:** `fulilian curator <verb>` — `status`, `usage`, `run`, `pause`,
...
- **Cost:** the deterministic inactivity/prune sweep runs for free. The
  ...
  **off by default** — opt in with `curator.consolidate: true` or
  `fulilian curator run --consolidate`. Routine background curation costs
  zero tokens.
- **Telemetry:** sidecar at `~/.fulilian/skills/.usage.json` holds
...
Users drive it via `fulilian kanban <verb>`; dispatcher-spawned workers
see a focused `kanban_*` toolset gated by `FULILIAN_KANBAN_TASK`, and
