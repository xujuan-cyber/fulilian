...
Fulilian runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`fulilian config edit` writes correctly.

### `execute_code` / Sandbox

**WinError 10106** from the sandbox child process — it can't create an
`AF_INET` socket. Root cause is usually Fulilian's env scrubber dropping
...
Fulilian-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
every Fulilian tool and most Windows APIs. Prefer forward slashes in code
