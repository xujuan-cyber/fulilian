...
## Layer 1 — Is this thing a Fulilian plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Fulilian process**. Its LLM calls go through Fulilian's already-configured provider. | Already uses Portal if user's Fulilian is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Fulilian. That LLM call is already authenticated
via Fulilian's provider config. The plugin's own server auth (e.g.
...
What FuLiLian has that *feels* like OAuth — `fulilian login --provider nous`
opening a browser, user signs in, token lands in `~/.fulilian/auth.json` — is a
**Fulilian-specific browser flow**. Under the hood it produces a credential
Fulilian uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Fulilian's existing Portal credential out of `~/.fulilian/auth.json`.
...
rides on the credential Fulilian already holds.

Where this could live in Fulilian:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Fulilian itself.
...
1. First decide: Fulilian plugin (runs inside Fulilian) or separate app? If plugin,
   it already uses Portal via Fulilian's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Fulilian agent itself") is fully supported. Fulilian treats agent
...
pitch in the Tool Gateway announcement is specifically about FuLiLian's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Fulilian is configured to use Portal as its provider. It is
