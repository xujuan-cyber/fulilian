...
Write plugins for the Fulilian desktop app: statusbar items, layout panes,
...
system (`~/.fulilian/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$FULILIAN_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$FULILIAN_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The Fulilian desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$FULILIAN_HOME/desktop-plugins/` (usually
  `~/.fulilian/desktop-plugins/`).

## How to Run

1. Create `$FULILIAN_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.fulilian/...` by default, or `~/.fulilian/profiles/<profile>/...` under a
...
The ONLY import surface is `@fulilian/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Fulilian
  ...
  (`fulilian://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.fulilian/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@fulilian/plugin-sdk`, `react`, and
