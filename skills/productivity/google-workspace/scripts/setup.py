#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for FuLiLian.
...
# Ensure sibling modules (_fulilian_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _fulilian_home import display_fulilian_home, get_fulilian_home

FULILIAN_HOME = get_fulilian_home()
TOKEN_PATH = FULILIAN_HOME / "google_token.json"
CLIENT_SECRET_PATH = FULILIAN_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = FULILIAN_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Fulilian profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Fulilian Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the Fulilian Docker image's "
        "uv-managed venv), install the optional extra instead:"
    )
    print("  fulilian setup")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Fulilian home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_fulilian_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Fulilian")
