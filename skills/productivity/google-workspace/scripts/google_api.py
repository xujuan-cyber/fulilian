#!/usr/bin/env python3
"""Google Workspace API CLI for FuLiLian.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Fulilian-facing JSON contract and falls back to the Python client
...
from _fulilian_home import get_fulilian_home
...
def _gws_binary() -> str | None:
    override = os.getenv("FULILIAN_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for FuLiLian")
