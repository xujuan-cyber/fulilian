...
## 5. Fulilian Configuration

### Config location

`$FULILIAN_HOME/config.yaml` (defaults to `~/.fulilian/config.yaml` when `FULILIAN_HOME` is unset)
...
Restart the Fulilian session for changes to take effect. The MCP connection is
