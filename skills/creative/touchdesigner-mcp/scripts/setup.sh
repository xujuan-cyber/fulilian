...
FULILIAN_HOME_DIR="${FULILIAN_HOME:-$HOME/.fulilian}"
FULILIAN_CFG="${FULILIAN_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Fulilian config has twozero_td MCP entry ──
if [[ ! -f "$FULILIAN_CFG" ]]; then
    echo -e " ${FAIL} Fulilian config not found at ${FULILIAN_CFG}"
    manual_steps+=("Create ${FULILIAN_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$FULILIAN_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Fulilian config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Fulilian config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$FULILIAN_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${FULILIAN_CFG} manually"); }
    manual_steps+=("Restart Fulilian session to pick up config change")
fi
