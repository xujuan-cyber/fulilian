"""ACP agent server — exposes FuLiLian via the Agent Client Protocol."""
...
from tools.approval import (
    reset_fulilian_interactive_context,
    set_fulilian_interactive_context,
)
...
def _named_custom_provider_catalogs() -> list[tuple[str, str, list[tuple[str, str]]]]:
    ...
    try:
        ...
        from fulilian_cli.models import should_use_ollama_native_catalog
        from fulilian_cli.providers import custom_provider_slug
    except ImportError:
        return []
...
# already uses (`fulilian_cli/moa_cmd.py`). This bounds each provider's row, not
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Fulilian running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Fulilian cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Fulilian/OpenAI-compatible user content payload."""
...
class FulilianACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Fulilian AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Fulilian version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Fulilian maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        The shared Fulilian inventory is also used by ``fulilian model``, the TUI,
        ...
        try:
            ...
            from fulilian_cli.models import normalize_provider, provider_label
            ...
            def choice_provider(model_id: str) -> str:
                ...
                if parts[:1] == ["custom"] and len(parts) > 1:
                    from fulilian_cli.models import _configured_custom_provider_ids
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from fulilian_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Fulilian estimates ``used`` from the same
    ...
    def _provenance_meta(
        self,
        acp_session_id: str,
        current_fulilian_session_id: str,
        previous_fulilian_session_id: Optional[str] = None,
    ) -> Optional[dict]:
        """Best-effort ``_meta.fulilian.sessionProvenance`` for an ACP session."""
        try:
            return session_provenance_meta(
                self.session_manager._get_db(),
                acp_session_id,
                current_fulilian_session_id,
                previous_fulilian_session_id=previous_fulilian_session_id,
            )
        except Exception:
            logger.debug(
    ...
    async def _send_session_info_update(
        ...
        session_id: str,
        *,
        current_fulilian_session_id: Optional[str] = None,
        previous_fulilian_session_id: Optional[str] = None,
    ) -> None:
        """Send ACP native session metadata after Fulilian changes it.

        When the internal Fulilian head rotated (e.g. compression-driven session
        split during a turn), pass ``previous_fulilian_session_id`` so the
        attached ``_meta.fulilian.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
        ...
        # fulilian_state.py schema — only started_at/ended_at). Use "now" as
        ...
        meta = self._provenance_meta(
            session_id,
            current_fulilian_session_id or session_id,
            previous_fulilian_session_id,
        )
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["fulilian-acp"],
    ...
    def _schedule_mcp_late_refresh(self, state: SessionState) -> None:
        ...
        """
        try:
            from fulilian_cli.mcp_startup import mcp_discovery_in_flight
        except Exception:
            return
        ...
        def _wait_then_refresh() -> None:
            try:
                from fulilian_cli.mcp_startup import join_mcp_discovery
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="fulilian-agent", version=FULILIAN_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Fulilian' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Fulilian setup/model selection out-of-band.
    ...
    def _history_summary_meta(message: dict[str, Any], text: str) -> dict[str, Any] | None:
        ...
        Two distinct keys under ``_meta.fulilian`` (ACP's extensibility
        ...
        * ``containsCompactionSummary: true`` — a merged-tail message: real
        ...
        if kind is None and message.get(COMPRESSED_SUMMARY_METADATA_KEY):
            ...
            kind = "standalone"
        if kind == "standalone":
            return {"fulilian": {"compactionSummary": True}}
        if kind == "merged":
            return {"fulilian": {"containsCompactionSummary": True}}
        return None
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Fulilian
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Fulilian on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Fulilian' local kawaii waiting/status
        ...
        # tools.approval (set_fulilian_interactive_context) rather than
        # os.environ["FULILIAN_INTERACTIVE"], so concurrent executor workers can't
        ...
        # callback shape — not the gateway-queue FULILIAN_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind FULILIAN_SESSION_KEY for this session so per-session caches
            ...
            try:
                ...
                # Without it the prompt advertises the global Fulilian workspace
                # while the tools are rooted at the client's project, so the
                # model emits absolute paths under ~/.fulilian/workspace and the
                # edit silently lands outside the editor's workspace.
                # cron_session="" explicitly marks this as a non-cron context,
                # masking any leaked process-global FULILIAN_CRON_SESSION (#37968).
            ...
            interactive_token = set_fulilian_interactive_context(True)
            ...
            previous_session_id = os.environ.get("FULILIAN_SESSION_ID")
            ...
            try:
                ...
                if interactive_token is not None:
                    reset_fulilian_interactive_context(interactive_token)
                # Restore FULILIAN_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("FULILIAN_SESSION_ID", None)
                else:
                    os.environ["FULILIAN_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            # Snapshot the internal Fulilian DB session id before the turn so we
            ...
            pre_turn_fulilian_id = getattr(state.agent, "session_id", None)
            # Wrap the executor call in a fresh copy of the current context so
            # concurrent ACP sessions on the shared ThreadPoolExecutor don't
            # stomp on each other's ContextVar writes (FULILIAN_SESSION_KEY in
        ...
        # _meta.fulilian.sessionProvenance so ACP clients can render the boundary
        # and keep old/new ids in lineage. The ACP session_id is unchanged.
        post_turn_fulilian_id = getattr(state.agent, "session_id", None)
        if (
            conn
            and post_turn_fulilian_id
            and pre_turn_fulilian_id
            and post_turn_fulilian_id != pre_turn_fulilian_id
        ):
            try:
                await self._send_session_info_update(
                    session_id,
                    current_fulilian_session_id=post_turn_fulilian_id,
                    previous_fulilian_session_id=pre_turn_fulilian_id,
                )
            except Exception:
                logger.debug(
        ...
        # Fulilian' local "waiting for model response" interrupt status is metadata,
    ...
    def _handle_slash_command(self, text: str, state: SessionState) -> str | None:
        ...
        # an unpinned handler bakes the Fulilian install tree into the session's
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"FuLiLian v{FULILIAN_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Fulilian has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
