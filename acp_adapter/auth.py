"""ACP auth helpers — detect and advertise Fulilian authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "fulilian-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Fulilian runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Fulilian can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Fulilian.
    ...
    not have Fulilian provider credentials configured yet, so Fulilian always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Fulilian using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Fulilian provider",
            description=(
                "Open Fulilian' interactive model/provider setup in a terminal. "
                "Use this when Fulilian has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
