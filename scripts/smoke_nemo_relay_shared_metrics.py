"""Run a real Fulilian CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_fulilian_executable(fulilian_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "fulilian",
        Path(".venv") / "Scripts" / "fulilian.exe",
    ):
        candidate = fulilian_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("fulilian")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Fulilian executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Fulilian source checkout containing .venv/bin/fulilian",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated FULILIAN_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["fulilian.client.active"] != [
        {
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['fulilian.client.active']}"
        )
    [model] = by_name["fulilian.model_route.count"]
    expected_model = {
        "name": "fulilian.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['fulilian.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["fulilian.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['fulilian.task_run.started']}"
        )
    [terminal] = by_name["fulilian.task_run.finished"]
    ...
    [tool] = by_name["fulilian.tool_call.count"]
    ...
    lifecycle = by_name["fulilian.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["fulilian.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Fulilian development environment requires jsonschema"
        ) from exc
    ...
    if metrics["fulilian.client.active"] != [
        {
            "name": "fulilian.client.active",
            "type": "counter",
            "dimensions": {},
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['fulilian.client.active']}"
        )
    [model] = metrics["fulilian.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['fulilian.model_route.count']}"
        )
    [terminal] = metrics["fulilian.task_run.finished"]
    ...
    [tool] = metrics["fulilian.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["fulilian.skill.lifecycle.count"]
    ...
    loads = metrics["fulilian.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    fulilian_repo = args.fulilian_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    fulilian = _resolve_fulilian_executable(fulilian_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="fulilian-relay-shared-metrics-"))
    home = root / "fulilian-home"
    ...
    try:
        ...
        python_paths = [str(fulilian_repo)]
    ...
    (root / "fulilian.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "fulilian.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Fulilian exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Fulilian model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Fulilian did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Fulilian did not print the mock model response")
    ...
    skill_result = subprocess.run(
        [
            ...
            "-c",
            "\n".join([
                "from fulilian_cli.observability import relay_shared_metrics",
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "fulilian.shared_metrics.v2.schema.json",
    )

    print("Fulilian -> NeMo Relay shared-metrics smoke test passed")
