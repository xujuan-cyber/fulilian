#!/usr/bin/env python3
"""Live test harness for FuLiLian's Tool Search feature.
...
# Force-isolate the test environment BEFORE any fulilian imports.
ORIGINAL_HOME = os.environ.get("FULILIAN_HOME")
ORIGINAL_AUTH = Path.home() / ".fulilian" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.fulilian/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.fulilian/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="fulilian_ts_live_"))
    fulilian_home = home_dir / ".fulilian"
    fulilian_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, fulilian_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".fulilian" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, fulilian_home / ".env")
        ...
        load_fulilian_dotenv(fulilian_home=str(Path.home() / ".fulilian"))
    ...
    (fulilian_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
...
def reset_module_state():
    """Drop cached modules so the new FULILIAN_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "fulilian_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["FULILIAN_HOME"] = str(home)
...
def main():
    ...
    # Restore original FULILIAN_HOME
    if ORIGINAL_HOME is not None:
        os.environ["FULILIAN_HOME"] = ORIGINAL_HOME
    else:
