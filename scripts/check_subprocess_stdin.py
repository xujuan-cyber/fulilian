...
When Fulilian runs in TUI mode, the gateway child process communicates with
...
# ``get_fulilian_home() / "plugins"`` (user) and ``./.fulilian/plugins/`` (project,
# gated behind ``FULILIAN_ENABLE_PROJECT_PLUGINS``) — see
# ``fulilian_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``fulilian_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import fulilian_constants (this script
    ...
    # plugins in get_fulilian_home()/plugins/ and project plugins in
    # ./.fulilian/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_fulilian_home() / "plugins"]
    if os.environ.get("FULILIAN_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".fulilian" / "plugins")
