#!/usr/bin/env bash
# Canonical test runner for fulilian-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.fulilian/...)
#
# Usage:
...
# (FULILIAN_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.fulilian/fulilian-agent/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.fulilian/fulilian-agent/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${FULILIAN_PYTHON:-}" ] && [ -x "$FULILIAN_PYTHON" ] \
    && "$FULILIAN_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: FULILIAN_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `fulilian` binary rather
  # than the devShell hook.
  PYTHON="$FULILIAN_PYTHON"
  echo "▶ no local venv — using Nix dev venv via FULILIAN_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and FULILIAN_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.fulilian/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.fulilian"
...
#
#   * FULILIAN_TEST_WORKERS / PATHS / FILE_TIMEOUT / FILE_RETRIES / SLICE are
#     read by run_tests_parallel.py at argparse-default time — inside the
#     stripped environment.
#   * FULILIAN_TEST_IMAGE is read by tests/docker/conftest.py to skip its
...
#     FULILIAN_GIT_SHA build-arg the workflow bakes in).
#
# These are test-infrastructure knobs, not credentials — same class as the
# FULILIAN_RUN_SLOW_PET_TESTS / FULILIAN_E2E_BROWSER opt-ins already forwarded.
# Keep this an explicit allowlist (no FULILIAN_TEST_* glob) so the "no
# credential can leak" property stays auditable at a glance.
TEST_ENV=()
for _test_var in FULILIAN_TEST_IMAGE FULILIAN_TEST_WORKERS FULILIAN_TEST_PATHS \
  FULILIAN_TEST_FILE_TIMEOUT FULILIAN_TEST_FILE_RETRIES FULILIAN_TEST_SLICE; do
  if [ -n "${!_test_var:-}" ]; then
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${FULILIAN_RUN_SLOW_PET_TESTS:+FULILIAN_RUN_SLOW_PET_TESTS="$FULILIAN_RUN_SLOW_PET_TESTS"} \
  ${FULILIAN_E2E_BROWSER:+FULILIAN_E2E_BROWSER="$FULILIAN_E2E_BROWSER"} \
