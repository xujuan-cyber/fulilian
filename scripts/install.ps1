# ============================================================================
# Fulilian Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$FulilianHome = $(if ($env:FULILIAN_HOME) { $env:FULILIAN_HOME } else { "$env:LOCALAPPDATA\fulilian" }),
    [string]$InstallDir = $(if ($env:FULILIAN_HOME) { "$env:FULILIAN_HOME\fulilian-agent" } else { "$env:LOCALAPPDATA\fulilian\fulilian-agent" }),
    ...
    # builds apps/desktop into a launchable Fulilian.exe.
    #
    # Why opt-in:
    #   * Fulilian-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Fulilian.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Fulilian.exe
    ...
    #     `fulilian desktop` already builds on demand.
...
# FULILIAN_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[fulilian] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'FulilianInstall.LongPath').Type) {
            Add-Type -Namespace 'FulilianInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [FulilianInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -FulilianHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('FulilianHome')) {
    $FulilianHome = ConvertTo-LongPath $FulilianHome
} else {
    $FulilianHome = ConvertTo-LongPath $(
        if ($env:FULILIAN_HOME) { $env:FULILIAN_HOME } else { "$env:LOCALAPPDATA\fulilian" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:FULILIAN_HOME) { "$env:FULILIAN_HOME\fulilian-agent" } else { "$env:LOCALAPPDATA\fulilian\fulilian-agent" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: FulilianHome=$FulilianHome InstallDir=$InstallDir"
}
...
$RepoUrlSsh = "git@github.com:xujuan-cyber/fulilian.git"
$RepoUrlHttps = "https://github.com/xujuan-cyber/fulilian.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * Fulilian Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $FulilianHome)) {
        New-Item -ItemType Directory -Force -Path $FulilianHome | Out-Null
    }
    $envFile = Join-Path $FulilianHome ".env"
...
function Install-AgentBrowser {
    ...
    # which every consumer (tools/browser_tool.py, `fulilian update`'s npx
    ...
    $prefixDir = Join-Path $FulilianHome "node"
...
function Install-Uv {
    # Fulilian owns its own uv at $FulilianHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (fulilian_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `fulilian update` stay in sync.
    $managedUv = Join-Path $FulilianHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $FulilianHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $FulilianHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $FulilianHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $FulilianHome "bin"
        ...
        # (fulilian_cli/managed_uv.py looks only at $FulilianHome\bin\uv.exe).
...
# Put the Fulilian-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone fulilian-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Fulilian-managed Node tree's bundled npm into $NpmRange.
#
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "Fulilian-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("fulilian-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and FULILIAN_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Fulilian-Setup.exe
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Fulilian setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Fulilian uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\fulilian\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Fulilian needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Fulilian copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\fulilian\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``FULILIAN_GIT_BASH_PATH`` (User scope) so Fulilian can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Fulilian-managed PortableGit install instead..."
    }

    # Download PortableGit into $FulilianHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $FulilianHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Fulilian features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Fulilian needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``FULILIAN_GIT_BASH_PATH`` (User env scope) so Fulilian can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $FulilianHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $FulilianHome\git\usr\bin\bash.exe
    $candidates += "$FulilianHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$FulilianHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("FULILIAN_GIT_BASH_PATH", $candidate, "User")
            $env:FULILIAN_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set FULILIAN_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Fulilian may not find Git Bash."
    Write-Info "If needed, set FULILIAN_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    if (Get-Command node -ErrorAction SilentlyContinue) {
        ...
        Write-Warn "Node.js $version is too old (Fulilian requires Node >=26)"
    }

    # Prefer a Fulilian-managed Node from a previous run over a too-old system one.
    $managedNode = "$FulilianHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$FulilianHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$FulilianHome\node"
        Write-Success "Node.js $version found (Fulilian-managed)"
        ...
        Update-ManagedNpm "$FulilianHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Fulilian-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $FulilianHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $FulilianHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\fulilian-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$FulilianHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$FulilianHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$FulilianHome\node") {
                    try {
                        Rename-Item "$FulilianHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "Fulilian-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$FulilianHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $FulilianHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\fulilian-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `fulilian
                    # update` path (#4735).
                    ...
                    $stashName = "fulilian-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. fulilian-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``fulilian update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Fulilian behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running fulilian processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                $extractPath = "$env:TEMP\fulilian-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `fulilian update` (see the notes at the shared clone-path
    ...
    # next `fulilian update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Fulilian-Setup.exe
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running fulilian process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running fulilian processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Fulilian_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (fulilian.exe) plus its child tree.
            & taskkill /F /T /IM fulilian.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM fulilian.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m fulilian_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not fulilian.exe.
            ...
            # Fulilian children re-exec through .fulilian-runtime, so killing only
            ...
            # user (see fulilian_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-Fulilian " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `fulilian update` / the
...
function Restore-VenvBackup {
    ...
    # venv so Fulilian (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `fulilian.exe` exists
...
try:
    ...
    out = []
    for s in specs:
        m = re.search(r'fulilian-agent\[([\w-]+)\]', s)
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install fulilian-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and fulilian.exe broken with
    ...
    # here, not 30 seconds later when the user runs `fulilian`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close Fulilian processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close Fulilian processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves Fulilian and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register fulilian.exe in dist-info/RECORD but fail to
        ...
        # `fulilian` command while fulilian-agent.exe / fulilian-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m fulilian_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `fulilian dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\fulilian_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `fulilian dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `fulilian dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: fulilian_cli/web_server.py"
        }
    }
...
function Install-FulilianCommandLaunchers {
    param(
        [Parameter(Mandatory=$true)] [string]$Root,
        [Parameter(Mandatory=$true)] [string]$Destination
    )

    # Expose ONLY the fulilian launchers on PATH -- never the whole
    # venv\Scripts directory, which contains python.exe / pip.exe and
    # silently hijacks the `python` command in every terminal (#83797).
    # Requiring fulilian.exe before creating the destination keeps the PATH
    # stage from reporting success with an unusable command (PR #92092).
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "fulilian.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the fulilian command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # fulilian_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    $requiredExe = Join-Path $Destination "fulilian.exe"
    $requiredCmd = Join-Path $Destination "fulilian.cmd"
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
              (Test-Path -LiteralPath $requiredCmd -PathType Leaf))) {
        throw "Cannot set up the fulilian command: launcher was not installed: $requiredExe"
    }
    return $Destination
}

function Set-PathVariable {
    Write-Info "Setting up fulilian command..."
    
    if ($NoVenv) {
        $fulilianBin = "$InstallDir"
    } else {
        # $FulilianHome\bin is the managed binary dir (shared with the managed
        # uv), OUTSIDE the git checkout: `fulilian update`'s autostash
        ...
        # installer staged under fulilian-agent\bin. No git operation can ever
        # touch this dir. Staging and verification live in
        # Install-FulilianCommandLaunchers, which throws BEFORE any PATH
        # mutation when the launchers cannot be staged.
        $fulilianBin = "$FulilianHome\bin"
        Install-FulilianCommandLaunchers -Root $InstallDir -Destination $fulilianBin | Out-Null
    }
    ...
    #   fulilian-agent\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The fulilian-agent\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept fulilian via $fulilianBin)"
        }
    }
    
    if ($currentPath -notlike "*$fulilianBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$fulilianBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $fulilianBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set FULILIAN_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\fulilian instead
    # of the Unix default ~/.fulilian
    $currentFulilianHome = [Environment]::GetEnvironmentVariable("FULILIAN_HOME", "User")
    if (-not $currentFulilianHome -or $currentFulilianHome -ne $FulilianHome) {
        [Environment]::SetEnvironmentVariable("FULILIAN_HOME", $FulilianHome, "User")
        Write-Success "Set FULILIAN_HOME=$FulilianHome"
    }
    $env:FULILIAN_HOME = $FulilianHome
    
    # Update current session
    $env:Path = "$fulilianBin;$env:Path"
    
    Write-Success "fulilian command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.fulilian-bootstrap-complete which tells the Fulilian
    ...
    # Fulilian-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `fulilian update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".fulilian-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the FULILIAN_HOME directory structure ($FulilianHome, default %LOCALAPPDATA%\fulilian)
    New-Item -ItemType Directory -Force -Path "$FulilianHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$FulilianHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$FulilianHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$FulilianHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$FulilianHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$FulilianHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$FulilianHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$FulilianHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$FulilianHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$FulilianHome\.env"
    ...
    # Create config.yaml
    $configPath = "$FulilianHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Fulilian's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$FulilianHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in fulilian_cli/default_soul.py. The runtime
        ...
        $soulContent = @"
You are FuLiLian, an open-source intelligent AI assistant. You are helpful, knowledgeable, and direct. You assist users with a wide range of tasks including answering questions, writing and editing code, analyzing information, creative work, and executing actions via your tools. You communicate clearly, admit uncertainty when appropriate, and prioritize being genuinely useful over being verbose unless otherwise directed below. Be targeted and efficient in your exploration and investigations.
"@
        $utf8NoBom = New-Object System.Text.UTF8Encoding($false)
    ...
    Write-Success "Configuration directory ready: $FulilianHome"
    
    # Seed bundled skills into $FulilianHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $FulilianHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $FulilianHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$FulilianHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $FulilianHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Fulilian-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'fulilian setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\fulilian-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\fulilian-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\fulilian-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `fulilian tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $FulilianHome "bin"
    $managedBu = Join-Path $managedBin "browser-use.exe"
    # MANAGED-FIRST: only Fulilian' managed copy short-circuits. A browser-use
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside Fulilian' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'fulilian tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `fulilian
...
# paths still lazy-install via install_cua_driver() (fulilian_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `fulilian computer-use install` runs. Bounded
        ...
        # Fulilian' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'fulilian tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: fulilian computer-use install"
            }
        } else {
...
# the final `electron` -> `Fulilian` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Fulilian.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_fulilian_desktop command
    # resolves apps/desktop/release/win-unpacked/Fulilian.exe directly,
    ...
    # enforces the build floor (Node >=26) and prepends the Fulilian-managed
    ...
    # The --dir mode produces an unpacked Fulilian.exe in
    ...
    # are NOT trying to work around). The Fulilian icon + product name are
    # stamped onto Fulilian.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\fulilian-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Fulilian.exe",
        "$desktopDir\release\win-arm64-unpacked\Fulilian.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Fulilian.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Fulilian icon + identity are stamped onto Fulilian.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, fulilian-agent#38216).
    ...
    #    Fulilian.exe. We deliberately do NOT point them at `fulilian desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Fulilian.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Fulilian.lnk')
        )
        ...
        # with the Fulilian icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.fulilian/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'fulilian setup'."
        return
    }
    ...
    # Run fulilian setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m fulilian_cli.main setup
    } else {
        python -m fulilian_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $fulilianCmd = "$InstallDir\venv\Scripts\fulilian.exe"
    if (-not (Test-Path $fulilianCmd)) {
        $fulilianCmd = "fulilian"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$FulilianHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'fulilian whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: fulilian gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$FulilianHome\logs\gateway.log"
            Start-Process -FilePath $fulilianCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$FulilianHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: fulilian gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: fulilian gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$FulilianHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$FulilianHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$FulilianHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$FulilianHome\fulilian-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   fulilian              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   fulilian setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   fulilian config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   fulilian config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   fulilian gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   fulilian update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Fulilian repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Fulilian-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Fulilian to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/xujuan-cyber/fulilian/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
