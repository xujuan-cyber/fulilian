#!/bin/bash
# repro.sh -- reproduce desktop-update paths against a sandboxed FULILIAN_HOME.
#
# Nothing here touches your real ~/.fulilian or checkout. Each mode builds (or
# reuses) a disposable install under /tmp and drives the REAL code path --
# the actual installer, the actual orchestrator, the actual `fulilian update`.
#
#   repro.sh shim          shim UI only: success event after 6s
#   repro.sh shim-fail     shim UI only: error event after 6s
#   repro.sh fresh         fresh install into a sandbox FULILIAN_HOME
...
SANDBOX="${FULILIAN_UPDATE_REPRO_HOME:-/tmp/fulilian-update-repro}"
SANDBOX_ROOT="$SANDBOX/fulilian-agent"

say() { printf '\n\033[1m== %s ==\033[0m\n' "$1"; }

ensure_sandbox_install() {
  if [ -x "$SANDBOX_ROOT/venv/bin/fulilian" ]; then
...
case "$MODE" in
  shim)
    FULILIAN_SELFTEST_HOLD_SECONDS="${FULILIAN_SELFTEST_HOLD_SECONDS:-6}" \
      bash "$SCRIPT_DIR/posix.sh" --self-test-ui
    ;;
  shim-fail)
    FULILIAN_SELFTEST_FAIL=1 FULILIAN_SELFTEST_HOLD_SECONDS="${FULILIAN_SELFTEST_HOLD_SECONDS:-6}" \
      bash "$SCRIPT_DIR/posix.sh" --self-test-ui
    ;;
  fresh)
    rm -rf "$SANDBOX"
    ensure_sandbox_install
    say "fresh install OK: $("$SANDBOX_ROOT/venv/bin/fulilian" --version 2>/dev/null || echo '?')"
    ;;
  behind)
    N="${2:-25}"
    ensure_sandbox_install
    ...
    FULILIAN_HOME="$SANDBOX" bash "$SCRIPT_DIR/posix.sh" \
      --install-root "$SANDBOX_ROOT" --branch main --desktop-pid 0 || true
    say "result file:"
    cat "$SANDBOX/.fulilian-update-result.json" 2>/dev/null || echo "(none written)"
    echo
  ...
  gate)
    ...
    G="/tmp/fulilian-gate-test.$$"
    UNPACKED="$G/fulilian-agent/apps/desktop/release/linux-unpacked"
    mkdir -p "$UNPACKED"
    touch "$UNPACKED/fulilian" && chmod +x "$UNPACKED/fulilian"
    ...
    decide() { bash "$SCRIPT_DIR/posix.sh" --self-test-gate --install-root "$G/fulilian-agent" "$@" | cut -d: -f1; }

    expect "appimage (not under unpacked)"      skew     "$(decide --relaunch-target /opt/Fulilian/fulilian)"
    expect "sibling-prefix dir not fooled"      skew     "$(decide --relaunch-target "$UNPACKED-evil/fulilian")"
    expect "no chrome-sandbox (namespace)"      relaunch "$(decide --relaunch-target "$UNPACKED/fulilian")"

    touch "$UNPACKED/chrome-sandbox"
    expect "sandbox not root/setuid"            manual   "$(decide --relaunch-target "$UNPACKED/fulilian")"
    expect "opt-out: --sandbox-fallback"        relaunch "$(decide --relaunch-target "$UNPACKED/fulilian" --sandbox-fallback)"
    expect "opt-out: --no-sandbox launch arg"   relaunch "$(decide --relaunch-target "$UNPACKED/fulilian" -- --no-sandbox)"
    expect "opt-out: ELECTRON_DISABLE_SANDBOX"  relaunch "$(ELECTRON_DISABLE_SANDBOX=1 decide --relaunch-target "$UNPACKED/fulilian")"
    ...
    QHOME="$G/qhome"; mkdir -p "$QHOME/fulilian-agent"
    bash "$SCRIPT_DIR/posix.sh" --no-ui --no-marker-cleanup --desktop-pid 0 \
      --install-root "$QHOME/fulilian-agent" --branch 'evil"branch\n$(x)' >/dev/null 2>&1 || true
    if python3 -c "import json,sys; d=json.load(open('$QHOME/.fulilian-update-result.json')); sys.exit(0 if d['branch']=='evil\"branch\\\\n\$(x)' and d['ok']==False else 1)"; then
  ...
  launch)
    ...
    # a fake install whose `fulilian` stub exits 0 instantly, so the flow
    # reaches finish() with FINAL_CODE=0 and exercises the launch leg.
    L="/tmp/fulilian-launch-test.$$"
    fails=0
    expect_msg() { # name python-expr
      if python3 -c "import json,sys; d=json.load(open('$L/.fulilian-update-result.json')); sys.exit(0 if ($2) else 1)"; then
        printf 'ok   %s\n' "$1"
      else
        printf 'FAIL %s -> %s\n' "$1" "$(cat "$L/.fulilian-update-result.json" 2>/dev/null)"; fails=$((fails+1))
      fi
    }
    stub_install() { # creates a fake install whose fulilian update succeeds
      rm -rf "$L"; mkdir -p "$L/fulilian-agent/venv/bin"
      printf '#!/bin/sh\nexit 0\n' > "$L/fulilian-agent/venv/bin/fulilian"
      chmod +x "$L/fulilian-agent/venv/bin/fulilian"
    }

    # 1. linux relaunch target dies instantly -> manual downgrade in result
    stub_install
    UNPACKED="$L/fulilian-agent/apps/desktop/release/linux-unpacked"
    mkdir -p "$UNPACKED"
    printf '#!/bin/sh\nexit 1\n' > "$UNPACKED/fulilian"; chmod +x "$UNPACKED/fulilian"
    if [ "$(uname)" != "Darwin" ]; then
      bash "$SCRIPT_DIR/posix.sh" --no-ui --desktop-pid 0 --install-root "$L/fulilian-agent" \
        --relaunch-target "$UNPACKED/fulilian" >/dev/null 2>&1 || true
      expect_msg "instant-exit relaunch downgrades to manual" "d['ok']==True and d['manual']==True and 'Reopen Fulilian' in d['message']"
    else
      ...
      expect_msg "missing bundle downgrades to manual" "d['ok']==True and d['manual']==True and 'Reopen Fulilian' in d['message']"
    fi
    ...
    stub_install
    bash "$SCRIPT_DIR/posix.sh" --no-ui --desktop-pid 0 --install-root "$L/fulilian-agent" \
      --relaunch-target /opt/Fulilian/fulilian >/dev/null 2>&1 || true
    if [ "$(uname)" != "Darwin" ]; then
