...
# tree generates llms.txt — the index every LLM (Fulilian included, via the
# fulilian-agent skill) reads to learn what Fulilian can do — and
...
_MCP_CATALOG_FILES = {"fulilian_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (Fulilian-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``fulilian serve`` backend, the Docker image)
