# Behavioral test for install.ps1's fulilian launcher staging (PR #92092,
...
#
# The test lifts the real Install-FulilianCommandLaunchers function from the
...
# the caller (Set-PathVariable passes $FulilianHome\bin -- the managed binary
...
$fn = $ast.Find({
    ...
    $n.Name -eq 'Install-FulilianCommandLaunchers'
}, $true)

if (-not $fn) {
    throw "Install-FulilianCommandLaunchers not found in $installPs1"
}
...
$caseRoot = [System.IO.Path]::GetFullPath((Join-Path $tempBase (
    'fulilian-cli-launcher-test-' + [guid]::NewGuid().ToString('N')
)))
...
try {
    $installRoot = Join-Path $caseRoot 'fulilian-agent'
    ...
    $missingThrew = $false
    try {
        Install-FulilianCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    } catch {
        $missingThrew = $_.Exception.Message -like '*required launcher not found*'
    }
    Assert-True $missingThrew 'missing fulilian.exe fails the launcher stage'
    ...
    $fulilianV1 = [byte[]](77, 90, 1)
    $fulilianV2 = [byte[]](77, 90, 2)
    $acp = [byte[]](77, 90, 3)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'fulilian.exe'), $fulilianV1)
    Set-Content -Path (Join-Path $installRoot 'venv\pyvenv.cfg') `
        -Value "home = X" -Encoding Ascii

    $staged = Install-FulilianCommandLaunchers -Root $installRoot -Destination $binDir
    Assert-True ($staged -eq $binDir) 'returns the destination it staged into'
    Assert-BytesEqual $fulilianV1 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'fulilian.exe'))) `
        'normal venv: exe copy lands in the destination'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'fulilian-acp.exe'))) `
        'optional ACP launcher may be absent'

    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'fulilian.exe'), $fulilianV2)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'fulilian-acp.exe'), $acp)
    Install-FulilianCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-BytesEqual $fulilianV2 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'fulilian.exe'))) `
        'installer refreshes an existing Fulilian launcher'
    Assert-BytesEqual $acp `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'fulilian-acp.exe'))) `
    ...
    Install-FulilianCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-True (Test-Path -LiteralPath (Join-Path $binDir 'fulilian.cmd')) `
        'relocatable venv: .cmd delegator staged'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'fulilian.exe'))) `
        'relocatable venv: stale exe copy removed'
    $cmdBody = [System.IO.File]::ReadAllText((Join-Path $binDir 'fulilian.cmd'))
    Assert-True ($cmdBody.Contains((Join-Path $scriptsDir 'fulilian.exe')) -and $cmdBody.Contains('%*')) `
