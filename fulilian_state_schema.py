...
``fulilian_state.SessionDB``. It defines no ``__init__`` and no state of its
...
``SessionDB.__init__``. It must never import fulilian_state (cycle) — shared
module-level constants live in fulilian_state_common.
"""
...
# Moved methods logged under the "fulilian_state" logger before the split;
# keep that logger identity so log filtering/capture behavior is unchanged.
logger = logging.getLogger("fulilian_state")
...
def schema_read_probe_statements() -> tuple:
    ...
    ``fulilian_cli/web_server.py``) run these probes right after a read-only
    ...
    served an empty session list after `fulilian update` until the user's
...
class SessionSchemaMixin:
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._fulilian_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._fulilian_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
            ...
            if escalated:
                ...
                if foreign_holders:
                    logger.error(
                        ...
                        "`fulilian sessions optimize-storage` with the gateway stopped. "
                        "`fulilian doctor` reports this degraded state.",
                        attempts,
                        foreign_holders,
                    )
    ...
    def _parse_schema_columns(schema_sql: str) -> Dict[str, Dict[str, str]]:
        ...
        try:
            from fulilian_constants import get_fulilian_home
            cache_path = get_fulilian_home() / "cache" / "schema_columns.json"
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 23:
                ...
                # optimization is available. `fulilian sessions optimize-storage`
        ...
        if fts5_available:
            ...
            # not yet opted into `fulilian db optimize`) must keep its EXISTING
