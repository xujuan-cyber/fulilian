...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `fulilian backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - fulilian_home (str): The active FULILIAN_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.fulilian``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "fulilian").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'fulilian memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], fulilian_home: str) -> None:
        ...
        Called by 'fulilian memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``fulilian_home`` is the active FULILIAN_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE FULILIAN_HOME.

        ``fulilian backup`` only walks FULILIAN_HOME, so any provider state kept
        ...
        of the archive, and ``fulilian import`` restores them to their original
