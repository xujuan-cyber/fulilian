"""Preventive SSL CA certificate checks for FuLiLian.
...
def _skip_ssl_guard_enabled() -> bool:
    return os.getenv("FULILIAN_SKIP_SSL_GUARD", "").strip().lower() in _SKIP_VALUES


def _repair_hint() -> str:
    return (
        "Repair: run `fulilian doctor --fix` (auto-reinstalls certifi), or "
...
def verify_ca_bundle() -> None:
    ...
    if _skip_ssl_guard_enabled():
        logger.debug("SSL CA bundle guard skipped via FULILIAN_SKIP_SSL_GUARD")
        return
