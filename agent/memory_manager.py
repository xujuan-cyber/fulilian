...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_FULILIAN_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Fulilian expands the turn into
    ...
    def _prefetch_provider(
        ...
        # Propagate the caller's contextvars (profile FULILIAN_HOME override)
    ...
    def _submit_background(self, fn, *, kind: str = "write") -> None:
        ...
        dashboard, cron) is a ContextVar-scoped FULILIAN_HOME override, and
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``fulilian_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_fulilian_home()`` themselves.
        """
        if "fulilian_home" not in kwargs:
            from fulilian_constants import get_fulilian_home
            kwargs["fulilian_home"] = str(get_fulilian_home())
        for provider in self._providers:
            try:
