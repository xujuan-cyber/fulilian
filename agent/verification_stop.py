...
def verify_on_stop_enabled(config: dict[str, Any] | None = None) -> bool:
    ...
    Precedence: an explicit ``FULILIAN_VERIFY_ON_STOP`` env var wins, then an
...
def _workspace_has_runnable_recipe(root: Any) -> bool:
    """Whether the workspace has a runtime verify recipe ``fulilian verify`` can run.

    True when a saved ``.fulilian/environment.json`` manifest exists, or when
    ...
    if verify_commands:
        ...
        if _workspace_has_runnable_recipe(facts.get("root")):
            command_instruction += (
                ...
                "start + readiness), prefer `fulilian verify --json` — a passing "
        ...
        if _workspace_has_runnable_recipe(facts.get("root")):
            command_instruction = (
                ...
                "project has a runnable verification recipe. Run `fulilian verify "
            ...
            command_instruction = (
                ...
                "`tempfile` path with a `fulilian-verify-` filename prefix, run it "
