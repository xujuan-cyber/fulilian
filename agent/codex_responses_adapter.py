...
def _clamp_responses_call_id(call_id: str) -> str:
    ...
    component the built-in ``fulilian-tools`` server already overflows 64 chars,
...
# output items.  Fulilian injects xAI's native ``web_search`` for the xAI
...
# server-assigned base64 blobs that can run 400+ chars, while Fulilian-minted
...
def _chat_messages_to_responses_input(
    ...
    *,
    ...
    explicitly relies on Fulilian threading encrypted reasoning back across
    ...
    ``convert_messages``). Dropping the checkpoint costs nothing: Fulilian'
                ...
                if isinstance(codex_reasoning, list):
                    for ri in codex_reasoning:
                        if isinstance(ri, dict) and ri.get("encrypted_content"):
                            ...
                            # it is a Fulilian-side metadata key and not part
