...
``auth.json``.  They intentionally have no dependency on ``fulilian_cli.auth`` so
...
# Sources Fulilian owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Fulilian-owned OAuth/device-code state)
