...
from fulilian_cli.timeouts import get_provider_request_timeout, get_provider_stale_timeout
from fulilian_constants import PARTIAL_STREAM_STUB_ID, FINISH_REASON_LENGTH
...
def _check_stale_giveup(agent) -> None:
    ...
    _giveup = env_int("FULILIAN_STREAM_STALE_GIVEUP", 5)
...
def _derive_stream_stale_timeout(agent, api_kwargs: dict) -> float:
    ...
    if _cfg_stale is not None:
        ...
        _base = env_float("FULILIAN_STREAM_STALE_TIMEOUT", 180.0)
...
def _resolve_direct_stale_timeout(agent, api_kwargs: dict) -> float:
    ...
    ``FULILIAN_API_CALL_STALE_TIMEOUT`` → reasoning-model floor → context-size
...
def interruptible_api_call(agent, api_kwargs: dict):
    ...
    # activity. Operators can tune via FULILIAN_CODEX_TTFB_TIMEOUT_SECONDS and
    # FULILIAN_CODEX_EVENT_STALE_TIMEOUT_SECONDS (0 disables each).
    ...
    # limit. Tunable via FULILIAN_CODEX_HARD_TIMEOUT_SECONDS (set to 0 to
    # disable the ceiling entirely; that restores the pre-fix behavior).
    _codex_hard_timeout = _env_float("FULILIAN_CODEX_HARD_TIMEOUT_SECONDS", 1500.0)
    if (
    ...
    # FULILIAN_CODEX_TTFB_TIMEOUT_SECONDS=0 to disable this watchdog entirely.
    _ttfb_enabled = _codex_watchdog_enabled
    _ttfb_timeout = _env_float("FULILIAN_CODEX_TTFB_TIMEOUT_SECONDS", 120.0)
    if _ttfb_timeout <= 0:
        _ttfb_enabled = False
    elif _openai_codex_backend:
        _ttfb_disable_above = _env_float("FULILIAN_CODEX_TTFB_DISABLE_ABOVE_TOKENS", 10_000.0)
        _ttfb_strict = os.environ.get("FULILIAN_CODEX_TTFB_STRICT", "").strip().lower() in {
            "1", "true", "yes", "on"
        }
        if (
            not _ttfb_strict
            ...
            if _ttfb_timeout < _large_request_ttfb_timeout:
                logger.info(
                    ...
                    "Set FULILIAN_CODEX_TTFB_STRICT=1 to keep the smaller cutoff.",
                    _ttfb_timeout,
        ...
        _ttfb_cap = _env_float("FULILIAN_CODEX_TTFB_MAX_SECONDS", 120.0)
        if _ttfb_cap > 0 and _ttfb_timeout > _ttfb_cap:
            logger.info(
                "Capping openai-codex no-byte TTFB timeout from %.0fs to %.0fs "
                "(context=~%s tokens). Set FULILIAN_CODEX_TTFB_MAX_SECONDS to tune.",
                _ttfb_timeout,
                _ttfb_cap,
                f"{_est_tokens_for_codex_watchdog:,}",
            )
    ...
    _codex_idle_timeout = _env_float(
        "FULILIAN_CODEX_EVENT_STALE_TIMEOUT_SECONDS",
...
def build_api_kwargs(agent, api_messages: list, tools_for_api: list | None = None) -> dict:
    ...
    if _is_qwen:
        _qwen_meta = {
            "sessionId": agent.session_id or "fulilian",
...
def build_assistant_message(agent, assistant_message, finish_reason: str) -> dict:
    ...
    # Respects FULILIAN_REDACT_SECRETS via redact_sensitive_text — no-op
    # when disabled. (#19798)
    if isinstance(_san_content, str) and _san_content:
        from agent.redact import redact_sensitive_text
...
def handle_max_iterations(agent, messages: list, api_call_count: int) -> str:
    ...
    if getattr(agent, "suppress_status_output", False):
        # Strict machine-readable mode (fulilian chat -Q, oneshot, background
    ...
    try:
        ...
        for msg in messages:
            ...
            # and every Fulilian-internal underscore-prefixed scaffolding key.
            ...
            # substitute it before dropping the key (Fulilian bookkeeping,
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
        ...
        try:
            ...
            while t.is_alive():
                ...
                if _stale_elapsed > _bedrock_stale_timeout:
                    ...
                    # crosses FULILIAN_STREAM_STALE_GIVEUP, so a persistently wedged
    ...
    def _call_chat_completions(stream_attempt_id: int):
        ...
        # wins over the FULILIAN_API_TIMEOUT env default if the user set it.
        _provider_timeout_cfg = get_provider_request_timeout(agent.provider, agent.model)
        _base_timeout = (
            _provider_timeout_cfg
            if _provider_timeout_cfg is not None
            else env_float("FULILIAN_API_TIMEOUT", 1800.0)
        )
        # Read timeout: config wins here too.  Otherwise use
        # FULILIAN_STREAM_READ_TIMEOUT (default 120s) for cloud providers.
        if _provider_timeout_cfg is not None:
            _stream_read_timeout = _provider_timeout_cfg
        else:
            _stream_read_timeout = env_float("FULILIAN_STREAM_READ_TIMEOUT", 120.0)
            # Local providers (Ollama, llama.cpp, vLLM) can take minutes for
            # prefill on large contexts before producing the first token.
            # Auto-increase the httpx read timeout unless the user explicitly
            # overrode FULILIAN_STREAM_READ_TIMEOUT.
        ...
        def _accept_stream_chunk(_chunk: Any) -> bool:
            ...
            # already-received tool-call chunk back to Fulilian. Preserve only
        ...
        if agent.provider == "moa":
            # Fulilian interrupts the managed stream; Relay retains sole
        ...
        # completed response. Relay records that attempt while Fulilian preserves
    ...
    def _call():
        ...
        _max_stream_retries = env_int("FULILIAN_STREAM_RETRIES", 2)
    ...
    if _cfg_stale is not None:
        ...
        _stream_stale_timeout_base = env_float("FULILIAN_STREAM_STALE_TIMEOUT", 180.0)
    ...
    # unless the user explicitly set FULILIAN_STREAM_STALE_TIMEOUT; override the
    # local ceiling with FULILIAN_LOCAL_STREAM_STALE_TIMEOUT (documented in
    # website/docs/reference/environment-variables.md).
    if _stream_stale_timeout_base == 180.0 and agent.base_url and is_local_endpoint(agent.base_url):
        # Read config.yaml ``agent.local_stream_stale_timeout`` (default 900),
        # env var ``FULILIAN_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch.
        ...
        try:
            ...
            pass
        _stream_stale_timeout = env_float("FULILIAN_LOCAL_STREAM_STALE_TIMEOUT", _local_default)
        logger.debug(
