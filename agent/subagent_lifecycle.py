"""Public, plugin-safe lifecycle API for delegated Fulilian subagents.
...
_EXECUTOR = _DaemonExecutor(max_workers=8, thread_name_prefix="fulilian-lifecycle")
_SECRET = secrets.token_bytes(32)
_ACTIVE_PARENT_AGENT: contextvars.ContextVar[Any] = contextvars.ContextVar(
    "fulilian_subagent_lifecycle_parent", default=None
)
...
class SubagentLifecycleService:
    ...
    def launch(self, request: SubagentLaunchRequest) -> SubagentHandle:
        ...
        if parent is None:
            raise SubagentLifecycleError(
                "No active Fulilian parent session is available."
            )
        ...
        if not subagent_id:
            raise SubagentLifecycleError("Fulilian failed to assign a child identity.")
        created = time.time()
        handle = SubagentHandle(
    ...
    def _validate_request(request: SubagentLaunchRequest, parent: Any) -> None:
        ...
        if request.working_directory is not None:
            raise SubagentLifecycleError(
                "working_directory is not supported because Fulilian delegates use isolated task environments."
            )
        if request.blocked_tools:
            raise SubagentLifecycleError(
                "Per-tool blocking is not supported; use allowed_toolsets. Fulilian always blocks unsafe child tools."
            )
        try:
            metadata_bytes = len(
