...
``fulilian pause`` writes a sentinel file at ``$FULILIAN_HOME/ESTOP``;
``fulilian resume`` removes it. While the sentinel exists:
...
* new gateway turns get a brief "Fulilian is paused" reply instead of an
...
hold even if the file was created by ``touch ~/.fulilian/ESTOP``.
...
def _fulilian_home() -> Path:
    """Resolve the active FULILIAN_HOME (profile-aware) at call time."""
    try:
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel under the active FULILIAN_HOME."""
    return _fulilian_home() / SENTINEL_NAME
...
def is_engaged() -> bool:
    ...
    exists (permission error, transient I/O failure on FULILIAN_HOME), report
...
def paused_reply() -> Optional[str]:
    ...
    if reason:
        return (
            f"⏸️ Fulilian is paused ({reason}). New work is on hold; "
            "run `fulilian resume` to pick things back up."
        )
    return (
        "⏸️ Fulilian is paused. New work is on hold; "
        "run `fulilian resume` to pick things back up."
    )
