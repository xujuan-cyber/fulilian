...
row taxonomy is inferred from the atlas shape so Fulilian can render both legacy
...
# rows (1536x1872), while older Fulilian/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Fulilian' activity state names. They are not always identical to the
...
# Legacy Fulilian/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Fulilian activity names -> accepted row-name aliases in descending
