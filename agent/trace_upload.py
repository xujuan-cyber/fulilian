"""Upload a Fulilian session transcript to Hugging Face as an agent trace.

Fulilian stores sessions in its own SQLite store (``fulilian_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``fulilian trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Fulilian' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "fulilian-traces"
_FULILIAN_VERSION = "fulilian-agent"
...
# ---------------------------------------------------------------------------
# Conversion: Fulilian OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Fulilian' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Fulilian conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.fulilian/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `fulilian trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/fulilian-traces`` dataset. Returns a
