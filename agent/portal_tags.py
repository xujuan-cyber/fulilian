...
Every Fulilian request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to FuLiLian and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=fulilian-agent",
        "client=fulilian-client-v<__version__>",
    ]

The version is sourced live from ``fulilian_cli.__version__`` so it auto-aligns
...
``fulilian_cli.__version__`` is the canonical source of truth.
"""
...
def _fulilian_version() -> str:
    """Return the current Fulilian release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``fulilian_cli`` cannot be imported (should
...
def fulilian_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=fulilian-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=fulilian-client-v{_fulilian_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Fulilian session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Fulilian conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.fulilian/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific Fulilian
    ...
    """
    tags = ["product=fulilian-agent", fulilian_client_tag()]
