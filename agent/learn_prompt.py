...
# (HARDLINE)" and the fulilian-agent-dev new-skill salvage reference. Embedded in
# the prompt so the agent authors skills the way a maintainer would by hand.
_AUTHORING_STANDARDS = """\
Follow the Fulilian skill-authoring standards exactly. These are the same
...
- author: always the literal value `Fulilian`. NEVER fill it from the host
  ...
  a privacy leak the user never opted into; the skill names itself as Fulilian.
- platforms: declare `[macos]`, `[linux]`, and/or `[windows]` IF the skill
  ...
  skills.
- metadata.fulilian.tags: a few Capitalized, Relevant, Tags.
...
4. "## How to Run" — the canonical invocation, framed through Fulilian tools.
...
Fulilian-tool framing (this is what makes it a skill, not shell docs):
- Frame running scripts as "invoke through the `terminal` tool".
- Reference Fulilian tools by name in backticks: `terminal`, `read_file`,
