...
Storage
-------
Bundles live in ``~/.fulilian/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``FULILIAN_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``fulilian bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under FULILIAN_HOME.

    Honors ``FULILIAN_BUNDLES_DIR`` for tests; falls back to
    ``<FULILIAN_HOME>/skill-bundles``.
    """
    override = os.environ.get("FULILIAN_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_fulilian_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `fulilian bundles` CLI subcommand.
# ---------------------------------------------------------------------------
