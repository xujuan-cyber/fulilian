...
Owns one Codex thread per Fulilian session. Drives `turn/start`, consumes
...
# Fulilian' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_FULILIAN_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Fulilian/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Fulilian session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _FULILIAN_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("FULILIAN_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="fulilian",
            client_title="FuLiLian",
            client_version=_get_fulilian_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Fulilian' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Fulilian' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own fulilian-tools callback we
            # auto-accept — the user already approved Fulilian' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Fulilian
        approval-mode/timeout logic. The Fulilian-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Fulilian approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Fulilian approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Fulilian returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Fulilian approval mode/timeout resolution
    ...
    # "prompt expired"; the Fulilian-side messaging already distinguishes them.
    return "decline"
...
def _get_fulilian_version() -> str:
    """Best-effort Fulilian version string for codex's userAgent line."""
    try:
