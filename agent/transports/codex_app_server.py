...
projection into Fulilian' display, approval bridging, transcript projection into
...
"codex_app_server"`. Fulilian' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Fulilian secret — gateway
        ...
        spawn_env = fulilian_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("FULILIAN_KANBAN_TASK"):
            kanban_db = spawn_env.get("FULILIAN_KANBAN_DB")
            kanban_root = (
                os.path.dirname(kanban_db)
                if kanban_db
                else spawn_env.get(
                    "FULILIAN_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("FULILIAN_HOME", os.path.expanduser("~/.fulilian")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "fulilian",
        client_title: str = "FuLiLian",
