...
The parent Fulilian process may itself be a Kanban dispatcher worker with
FULILIAN_KANBAN_* variables in process env. delegate_task children run inside the
...
# though the worker's FULILIAN_KANBAN_* vars are legitimately in os.environ (cron
...
_NON_DISPATCHER_OWNED_CONTEXT: ContextVar[bool] = ContextVar(
    "fulilian_non_dispatcher_owned_context",
    default=False,
)

DELEGATED_CHILD_ENV_MARKER = "FULILIAN_DELEGATED_CHILD_CONTEXT"
...
def non_dispatcher_owned_context() -> Iterator[None]:
    ...
    own process, where ``FULILIAN_KANBAN_TASK`` is legitimately set.  Without this
    ...
    ``kanban_complete`` defaults ``task_id`` to ``$FULILIAN_KANBAN_TASK`` — letting
...
def is_dispatcher_owned_worker_context() -> bool:
    ...
    The single predicate every ``FULILIAN_KANBAN_*`` identity gate should use
...
def delegated_child_subprocess_env(
    ...
    parent dispatcher ``FULILIAN_KANBAN_*`` vars while losing the ContextVar in
