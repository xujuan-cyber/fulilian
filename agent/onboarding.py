...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Fulilian starts and finds ``~/.openclaw/``.

    Points users at ``fulilian claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``fulilian claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Fulilian, run "
        "`fulilian claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `fulilian claw cleanup` (renames it to ~/.openclaw.pre-migration — "
