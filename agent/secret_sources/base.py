...
``~/.fulilian/.env`` has loaded and BEFORE the rest of Fulilian reads
``os.environ``.
...
* **Startup-time, synchronous.**  ``fetch()`` is called once per process
  (per FULILIAN_HOME) by the orchestrator in
...
_SOURCE_ENVIRONMENT = ContextVar("fulilian_secret_source_environment", default=None)
...
class ErrorKind(str, Enum):
    ...
    A fixed vocabulary keeps startup warnings and ``fulilian secrets status``
...
class SecretSource(ABC):
    ...
    Attributes:
        ...
        label: Human-readable name used in startup messages and
            ``fulilian secrets status`` (e.g. ``"Bitwarden Secrets Manager"``).
    ...
    def fetch(self, cfg: dict, home_path: Path) -> FetchResult:
        ...
        may be malformed.  ``home_path`` is the resolved FULILIAN_HOME.
        """
    ...
    def remediation(self, kind: Optional["ErrorKind"], cfg: dict) -> str:
        ...
        Called by the startup status printer (and ``fulilian secrets ...
        ...
        *what to run* next to fix it — not just what broke.  Sources
        ...
        ``fulilian secrets bitwarden token`` for AUTH_FAILED).  Return an
        ...
        generic = {
            ErrorKind.NOT_CONFIGURED: (
                f"Run `fulilian secrets {self.name} setup` to finish configuration."
            ),
            ErrorKind.BINARY_MISSING: (
                f"Run `fulilian secrets {self.name} setup` to install the helper CLI."
            ),
            ErrorKind.AUTH_FAILED: (
                f"Credentials rejected — run `fulilian secrets {self.name} setup` "
                "to re-authenticate."
            ),
            ErrorKind.AUTH_EXPIRED: (
                f"Credentials expired — run `fulilian secrets {self.name} setup` "
                "to re-authenticate."
            ),
            ErrorKind.NETWORK: (
...
# codes that must not reach Fulilian' own startup output.
...
def run_secret_cli(
    ...
    * The child gets ``PATH``/``HOME``/locale basics plus only the env
      ...
      this point holds every credential Fulilian knows about.
    * ``NO_COLOR=1`` is set and stderr/stdout are ANSI-scrubbed so
      helper diagnostics can't smuggle escape sequences into Fulilian
      output.
