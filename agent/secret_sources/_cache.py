...
cache problem must never block Fulilian startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Fulilian home used for cache paths.

    ``home_path`` is whatever ``load_fulilian_dotenv()`` already resolved;
    falling back to ``$FULILIAN_HOME`` / ``~/.fulilian`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from fulilian_constants import get_fulilian_home

        home_path = get_fulilian_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<fulilian_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if FULILIAN_HOME is ever made traversable.
            try:
