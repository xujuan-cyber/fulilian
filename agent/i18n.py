"""Lightweight internationalization (i18n) for Fulilian static user-facing messages.

Scope (thin slice, by design): only the highest-impact static strings shown
to the user by Fulilian itself -- approval prompts, a handful of gateway slash
...
Language resolution order:
    ...
    2. ``FULILIAN_LANGUAGE`` environment variable (for tests / quick override)
...
def _locales_dir() -> Path:
    ...
    1. ``FULILIAN_BUNDLED_LOCALES`` env var -- set by the Nix wrapper (or any
    ...
    """
    override = os.getenv("FULILIAN_BUNDLED_LOCALES", "").strip()
    if override:
        candidate = Path(override)
        if candidate.is_dir():
            return candidate
        logger.warning(
            "FULILIAN_BUNDLED_LOCALES points to a non-directory path (%s); "
...
def _load_catalog(lang: str) -> dict[str, str]:
    ...
    try:
        import yaml  # PyYAML is already a fulilian dependency
...
def reset_language_cache() -> None:
    ...
    Call after :func:`fulilian_cli.config.save_config` if a running process
...
def get_language() -> str:
    ...
    env_lang = os.environ.get("FULILIAN_LANGUAGE")
    if env_lang:
