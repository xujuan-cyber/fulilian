...
Plugins built on FuLiLian often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Fulilian
...
:class:`~fulilian_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Fulilian' main config
...
handles every provider, fallback chain, and per-task override Fulilian
supports.
"""
...
def _resolve_task_ownership(plugin_id: str) -> tuple[frozenset, frozenset]:
    ...
    try:
        ...
        pass
    try:
        from fulilian_cli.main import _AUX_TASKS
...
class PluginLlm:
    ...
    Instances are constructed by :class:`fulilian_cli.plugins.PluginContext`
