...
Extracted verbatim from fulilian_state.py so the SessionDB mixin modules
(fulilian_state_search / fulilian_state_schema / fulilian_state_portability) can
reference them without importing fulilian_state (which would be a cycle).
fulilian_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``fulilian sessions optimize-storage``. A legacy DB sits at
...
# `fulilian sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `fulilian db optimize`. This is
...
# service, the Desktop app's `fulilian serve` backend, interactive CLI sessions,
...
#
# Semantics mirror `fulilian_state._cross_process_repair_lock` (the schema-
...
# rebuilder cannot wedge future rebuilds. It lives here (not fulilian_state)
# because the search/schema mixins cannot import fulilian_state (cycle).
#
