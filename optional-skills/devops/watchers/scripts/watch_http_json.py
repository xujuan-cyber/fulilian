...
Usage (via cron with --no-agent):
    ...
    fulilian cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$FULILIAN_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Fulilian-Watcher/1.0"})
