...
is typically only in `~/.fulilian/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike Fulilian,
...
`fulilian-custom-providers` skill, pitfall 2). Fulilian hits this because it forwards
