#!/usr/bin/env python3
"""Canvas LMS API CLI for FuLiLian.
...
def _check_config():
    ...
    if missing:
        fulilian_env = os.path.join(
            os.environ.get("FULILIAN_HOME", os.path.expanduser("~/.fulilian")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {fulilian_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for FuLiLian"
    )
