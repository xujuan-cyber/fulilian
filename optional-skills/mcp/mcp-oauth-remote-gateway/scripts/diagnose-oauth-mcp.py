...
Usage:
  ...
  <server_name> matches the files in $FULILIAN_HOME/mcp-tokens/<server>.json etc.
...
def _fulilian_home():
    # Prefer Fulilian' own resolver (profile-safe); fall back to env then ~/.fulilian.
    try:
        from fulilian_constants import get_fulilian_home
        return str(get_fulilian_home())
    except Exception:
        return os.environ.get("FULILIAN_HOME") or os.path.expanduser("~/.fulilian")


def _tokens_dir():
    return os.path.join(_fulilian_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "fulilian-diag", "version": "1.0"}},
        },
        headers={
