...
the actual `setup.sh` that creates the project workspace, configures Fulilian
...
2. **Create profiles** — `fulilian profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.fulilian/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `fulilian kanban create` assigned to the director
...
```bash
fulilian profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.fulilian/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.fulilian/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Fulilian
...
Each profile gets a `SOUL.md` at `~/.fulilian/profiles/<name>/SOUL.md` that
...
```bash
fulilian kanban create "Direct production of <video title>" \
    --assignee director \
...
the Fulilian `.env` (`${FULILIAN_HOME:-$HOME/.fulilian}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _fulilian_env="${FULILIAN_HOME:-$HOME/.fulilian}/.env"
    if grep -q "^${var}=" "$_fulilian_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_fulilian_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_fulilian_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY fulilian ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY fulilian OPENROUTER_API_KEY || exit 1
# ...
```
