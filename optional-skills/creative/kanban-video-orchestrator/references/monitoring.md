...
fulilian kanban watch --tenant <project-slug>

# Snapshot of the board
fulilian kanban list --tenant <project-slug>
fulilian kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
fulilian kanban stats --tenant <project-slug>

# Visual dashboard (browser)
fulilian dashboard

# Inspect a specific task (includes comments + events)
fulilian kanban show <task-id>

# Follow a single task's event stream
fulilian kanban tail <task-id>
```

Verify available subcommands with `fulilian kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `fulilian kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `fulilian kanban block <id>` then `fulilian kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `fulilian kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `fulilian kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `fulilian profile list` (profile exists?), `fulilian status` (gateway/dispatcher up?), and verify tenant. |
...
fulilian kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
fulilian kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(fulilian kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
fulilian kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
fulilian kanban block <task-id>
fulilian kanban archive <task-id>

# Diagnose what happened
fulilian kanban show <task-id>      # task body, comments, recent events
fulilian kanban tail <task-id>      # follow the live event stream
fulilian kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `fulilian kanban create` for the director
...
while true; do
    clear
    fulilian kanban list --tenant <slug>
    echo "---"
    fulilian kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `fulilian kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `fulilian profile list` and start
