...
Maps each role archetype to the Fulilian skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public fulilian-agent
...
## Fulilian skills relevant to video production

### Visual / rendering skills (`fulilian-agent/skills/creative/`)
...
### Design / pre-production skills (`fulilian-agent/skills/creative/`)
...
### Audio / media skills (`fulilian-agent/skills/creative/` + `skills/media/`)
...
These are **not** Fulilian skills but external CLIs / APIs that profiles invoke.
...
## Built-in Fulilian tools for media review

These are native Fulilian tools — not invoked via terminal but through their own
...
key is present in `${FULILIAN_HOME:-~/.fulilian}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Fulilian profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${FULILIAN_HOME:-~/.fulilian}/.env` → environment variable.
...
When a new Fulilian-public video skill ships:
