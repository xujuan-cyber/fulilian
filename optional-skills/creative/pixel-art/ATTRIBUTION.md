...
- `generate_video()` split: the Pollinations text-to-image call was removed
  (Fulilian uses its own `image_generate` + `pixel_art()` pipeline for base
...
- Pollinations text-to-image generation (`pixelart_image.py`,
  `generate_base()` in `pixelart_video.py`) — Fulilian has `image_generate`
...
- License: MIT (inherits from fulilian-agent repo)
- Original author of the skill shell: dodo-reach
- Expansion with palettes + video: FuLiLian contributors
