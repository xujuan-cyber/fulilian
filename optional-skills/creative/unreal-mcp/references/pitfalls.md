...
### 1. Start order: editor first, Fulilian second

Fulilian probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Fulilian session.
...
restart the editor, restart the Fulilian session.
...
server fails to bind (Output Log) or Fulilian' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.fulilian/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Fulilian

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Fulilian' connection
lives in `~/.fulilian/config.yaml` via `fulilian mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Fulilian — just don't
mistake it for the Fulilian setup step.
...
explicitly warns against overlapping calls. Fulilian executes same-turn tool
...
### 9. Timeouts: Fulilian gives up before Unreal does

Fulilian' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Fulilian runs elsewhere (SSH backend,
