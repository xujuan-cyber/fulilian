...
|--------|----------|--------|
| Metadata namespace | `openclaw` | `fulilian` (with `tags` + `homepage`) |
| Trigger | Slash commands / CLI flags | Natural language skill matching |
| User config | EXTEND.md file (project/user/XDG paths) | Removed — not part of Fulilian infra |
...
| File operations | Generic instructions | Fulilian file tools (`write_file`, `read_file`) |
...
- **`scripts/` directory** (removed entirely): upstream's `merge-to-pdf.ts` depended on `pdf-lib`, which is not declared anywhere in the Fulilian repo. Rather than add a new dependency, the port drops PDF assembly and delivers per-page PNGs.
