#!/usr/bin/env python3
"""OpenClaw -> Fulilian migration helper.

This script migrates the parts of an OpenClaw user footprint that map cleanly
into FuLiLian, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the OpenClaw persona file into Fulilian.",
    },
    "workspace-agents": {
    ...
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into Fulilian memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into Fulilian memories.",
    },
    "messaging-settings": {
        "label": "Messaging settings",
        "description": "Import Fulilian-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of Fulilian-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
        "label": "Command allowlist",
        "description": "Merge OpenClaw exec approval patterns into Fulilian command_allowlist.",
    },
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.fulilian/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.fulilian/tts/.",
    },
    "discord-settings": {
        "label": "Discord settings",
        "description": "Import Discord bot token and allowlist into Fulilian .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into Fulilian .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into Fulilian .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into Fulilian .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into Fulilian .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into Fulilian config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into Fulilian config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared OpenClaw skills from ~/.openclaw/skills/ into Fulilian.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into Fulilian MEMORY.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from OpenClaw into Fulilian config.yaml.",
    },
    "plugins-config": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'fulilian cron'.",
    },
    "hooks-config": {
    ...
    "agent-config": {
        ...
        "description": "Import agent defaults (compaction, context, thinking) into Fulilian config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "session-config": {
        ...
        "description": "Import session reset policies (daily/idle) into Fulilian session_reset config.",
    },
    "full-providers": {
        "label": "Full model provider definitions",
        "description": "Import custom model providers (baseUrl, apiType, headers) into Fulilian custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into Fulilian config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into Fulilian config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into Fulilian config.yaml approvals section.",
    },
    "memory-backend": {
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`fulilian config edit` (or move it aside), then re-run the migration."
        )
    return data
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    ...
    ``~/.fulilian/config.yaml`` into a dotfiles repo or profile package.
    """
    if yaml is None:
        raise RuntimeError("PyYAML is required to update Fulilian config.yaml")
    ensure_parent(path)
...
# ── Brand rewriting ─────────────────────────────────────────
# Replace OpenClaw brand names with Fulilian in migrated text so that
# memory entries, user profiles, SOUL.md, and workspace instructions
# read as self-referential to the new agent identity.
#
# Case-preserving: ``OpenClaw`` → ``Fulilian`` (prose), but lowercase matches
# like ``openclaw`` → ``fulilian`` (so filesystem paths like ``~/.openclaw``
# become ``~/.fulilian`` — the real Fulilian home — not the broken ``~/.Fulilian``).
_REBRAND_PATTERNS: List[Tuple[re.Pattern, str]] = [
    (re.compile(r'\bOpen[\s-]?Claw\b', re.IGNORECASE), 'Fulilian'),
    (re.compile(r'\bClawdBot\b', re.IGNORECASE), 'Fulilian'),
    (re.compile(r'\bMoltBot\b', re.IGNORECASE), 'Fulilian'),
]
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``Fulilian`` but maps ``openclaw`` → ``fulilian`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.fulilian/config.yaml`` (the real Fulilian home) instead of the broken
...
def rebrand_text(text: str) -> str:
    """Replace OpenClaw / ClawdBot / MoltBot brand names with Fulilian.
...
def parse_existing_memory_entries(path: Path) -> List[str]:
    """Parse a DESTINATION Fulilian memory store (memories/MEMORY.md, USER.md).
...
def write_report(output_dir: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# OpenClaw -> Fulilian Migration Report",
        "",
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-fulilian/apply.ts "blocked by earlier
    ...
    # Option ids that mutate the Fulilian config.yaml file.  Once any one of
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-fulilian/plan.ts warnings[].
    ...
    def _build_next_steps(self, summary: Dict[str, int]) -> List[str]:
        ...
        if summary.get("migrated", 0) > 0:
            ...
            steps.append(
                "Start a new Fulilian session (or /reset) to pick up the imported config."
            )
        if summary.get("conflict", 0) > 0:
            steps.append(
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "Fulilian config.yaml does not exist yet")
            return
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # that path will be stale after migration and would cause the Fulilian
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No Fulilian-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted Fulilian-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in {"file", "exec"}:
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: fulilian config set",
                        )
                    continue
        ...
        for oc_key, fulilian_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and fulilian_key not in secret_additions:
                secret_additions[fulilian_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, fulilian_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and fulilian_key not in secret_additions:
                        secret_additions[fulilian_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        fulilian_config = load_yaml_file(destination)
        current_model = fulilian_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = fulilian_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                fulilian_config["model"] = {"default": model_str}
            dump_yaml_file(destination, fulilian_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(provider, str) and provider in {"elevenlabs", "openai", "edge", "microsoft"}:
            # OpenClaw renamed "edge" to "microsoft"; Fulilian still uses "edge"
        ...
        fulilian_config = load_yaml_file(destination)
        existing_tts = fulilian_config.get("tts", {})
        ...
        if self.execute:
            ...
            fulilian_config["tts"] = merged_tts
            dump_yaml_file(destination, fulilian_config)
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct Fulilian destination; archived for manual review")
        ...
        partially_extracted = [
            ("openclaw.json", "Selected Fulilian-compatible values were extracted; raw OpenClaw config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected Fulilian-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        fulilian_cfg_path = self.target_root / "config.yaml"
        fulilian_cfg = load_yaml_file(fulilian_cfg_path)
        existing_mcp = fulilian_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            if srv.get("url"):
                ...
                if srv.get("headers"):
                    fulilian_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    fulilian_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                fulilian_srv["enabled"] = False
            if srv.get("timeout"):
                fulilian_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                fulilian_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                fulilian_srv["tools"] = {}
                if tools_cfg.get("include"):
                    fulilian_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    fulilian_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
            sampling = srv.get("sampling")
            if sampling and isinstance(sampling, dict):
                fulilian_srv["sampling"] = {
                    k: v for k, v in {
            ...
            existing_mcp[name] = fulilian_srv
            added += 1
        ...
        if added > 0 and self.execute:
            self.maybe_backup(fulilian_cfg_path)
            fulilian_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(fulilian_cfg_path, fulilian_cfg)
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'fulilian cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = fulilian_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> Fulilian reasoning_effort
        ...
        if compaction:
            compression = fulilian_cfg.get("compression") or {}
            ...
            if compaction.get("model"):
                aux = fulilian_cfg.setdefault("auxiliary", {})
                aux_comp = aux.setdefault("compression", {})
                aux_comp["model"] = compaction["model"]
            fulilian_cfg["compression"] = compression
            changes = True

        # Map humanDelay
        human_delay = defaults.get("humanDelay") or {}
        if human_delay:
            hd = fulilian_cfg.get("human_delay") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'fulilian gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("FULILIAN_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        fulilian_cfg_path = self.target_root / "config.yaml"
        fulilian_cfg = load_yaml_file(fulilian_cfg_path)
        sr = fulilian_cfg.get("session_reset") or {}
        changes = False
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        fulilian_cfg_path = self.target_root / "config.yaml"
        fulilian_cfg = load_yaml_file(fulilian_cfg_path)
        custom_providers = fulilian_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(fulilian_cfg_path)
            fulilian_cfg["custom_providers"] = custom_providers
            dump_yaml_file(fulilian_cfg_path, fulilian_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map Discord-specific settings to Fulilian config
        discord_cfg = channels.get("discord") or {}
        if discord_cfg:
            fulilian_cfg_path = self.target_root / "config.yaml"
            fulilian_cfg = load_yaml_file(fulilian_cfg_path)
            discord_fulilian = fulilian_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_fulilian["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_fulilian["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                fulilian_cfg["discord"] = discord_fulilian
                dump_yaml_file(fulilian_cfg_path, fulilian_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        fulilian_cfg_path = self.target_root / "config.yaml"
        fulilian_cfg = load_yaml_file(fulilian_cfg_path)
        browser_fulilian = fulilian_cfg.get("browser") or {}
        changed = False

        # Map fields that have Fulilian equivalents
        if browser.get("cdpUrl"):
            browser_fulilian["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_fulilian["headless"] = browser["headless"]
            changed = True

        if changed:
            fulilian_cfg["browser"] = browser_fulilian
            if self.execute:
                self.maybe_backup(fulilian_cfg_path)
                dump_yaml_file(fulilian_cfg_path, fulilian_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            fulilian_mode = mode_map.get(mode, "manual")
            fulilian_cfg.setdefault("approvals", {})["mode"] = fulilian_mode
            if self.execute:
                self.maybe_backup(fulilian_cfg_path)
                dump_yaml_file(fulilian_cfg_path, fulilian_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{fulilian_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        if not self.output_dir:
            return
        notes = [
            "# OpenClaw -> Fulilian Migration Notes",
            "",
        ...
        if archived:
            notes.extend([
                ...
                "direct 1:1 mapping in Fulilian. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing Fulilian Config Not Overwritten)",
                "",
                "These items already existed in your Fulilian config. Re-run with",
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the Fulilian agent discovers these",
            "directories, it may read/write to them instead of the Fulilian state, causing",
            "confusion (e.g., cron jobs reading a different todo list than interactive sessions).",
            "",
            "**Strongly recommended:** Run `fulilian claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `fulilian claw cleanup` to fix it.",
            "",
            "## Fulilian-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `fulilian claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `fulilian setup` to configure any remaining settings",
            "- Run `fulilian mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `fulilian cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `fulilian cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `fulilian gateway install` if you need the gateway service",
            "- Review `~/.fulilian/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into FuLiLian.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=os.environ.get("FULILIAN_HOME") or str(Path.home() / ".fulilian"), help="Fulilian home directory")
    parser.add_argument(
        "--workspace-target",
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing Fulilian targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of Fulilian-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
...
def main() -> int:
    ...
    print()
    print("  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║   OpenClaw -> Fulilian Migration   [{mode_label:>8s}]   ║")
    print("  ╠══════════════════════════════════════════════════════╣")
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.fulilian/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.fulilian/config.yaml")
        print("    2. Run: fulilian mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: fulilian cron")
