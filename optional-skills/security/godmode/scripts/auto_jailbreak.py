...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("FULILIAN_HOME", os.path.expanduser("~/.fulilian")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("FULILIAN_HOME", Path.home() / ".fulilian")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Fulilian config paths
# ═══════════════════════════════════════════════════════════════════

FULILIAN_HOME = Path(os.getenv("FULILIAN_HOME", Path.home() / ".fulilian"))
CONFIG_PATH = FULILIAN_HOME / "config.yaml"
PREFILL_PATH = FULILIAN_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Fulilian models — already uncensored, just needs clean prompt
    "fulilian": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "fulilian" in model_lower or "nous" in model_lower:
        return "fulilian"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Fulilian config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.fulilian/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Fulilian for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Fulilian for changes to take effect.")
