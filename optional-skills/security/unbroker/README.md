...
The design is **Fulilian-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
fulilian skills install official/security/unbroker
```

Then start a new Fulilian session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Fulilian session:
...
- **PII handling.** Dossiers live under the Fulilian home directory (`0600`, optionally
