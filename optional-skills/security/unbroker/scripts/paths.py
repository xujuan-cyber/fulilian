...
All per-subject data lives under PDD_DATA_DIR (default: $FULILIAN_HOME/unbroker),
which is the same trust boundary Fulilian uses for .env and OAuth tokens.
"""
...
def fulilian_home() -> Path:
    return Path(os.environ.get("FULILIAN_HOME") or (Path.home() / ".fulilian"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else fulilian_home() / "unbroker"
