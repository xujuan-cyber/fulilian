# FuLiLian - Development Guide

Instructions for AI coding assistants and developers working on the fulilian-agent codebase.

**Never give up on the right solution.**

## What Fulilian Is

Fulilian is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Fulilian ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`fulilian tools`, `fulilian setup`, auto-install) rather than bolting on a raw
  env var.
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `FULILIAN_HOME`. Mocks hide integration bugs.
...
- **New `FULILIAN_*` env vars for non-secret config.** `.env` is for secrets
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `fulilian tools`
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.fulilian/plugins/` (or via a
  pip entry point), and promote them via GitHub Discussions
  (github.com/xujuan-cyber/fulilian/discussions). This is a coupling-and-maintenance decision, not
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `fulilian <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `fulilian webhook`, `fulilian cron`, `fulilian tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.fulilian/plugins/` or a pip package, discovered at runtime.
...
one behind a plain URL + token, or Fulilian Cloud. Only the first two are spawned
by us and carry `FULILIAN_DESKTOP=1`. Every env-keyed GUI gate is therefore a
...
Fulilian desktop app."*

The pattern that works:

- **The toolset is the surface gate.** Keep the tools off `_FULILIAN_CORE_TOOLS`
...
- **Ask which identity you actually mean.** `FULILIAN_DESKTOP=1` legitimately
  ...
  and the embedded terminal pane (`fulilian --tui` against that same backend) is
...
`$HOME/.fulilian/fulilian-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── toolsets.py           # Toolset definitions, _FULILIAN_CORE_TOOLS list
├── cli.py                # FulilianCLI class — interactive CLI orchestrator (~11k LOC)
├── fulilian_state.py       # SessionDB — SQLite session store (FTS5 search)
├── fulilian_constants.py   # get_fulilian_home(), display_fulilian_home() — profile-aware paths
├── fulilian_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── fulilian_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   ├── fulilian-achievements/  # Gamified achievement tracking
...
├── ui-tui/               # Ink (React) terminal UI — `fulilian --tui`
...
**User config:** `~/.fulilian/config.yaml` (settings), `~/.fulilian/.env` (API keys only).
**Logs:** `~/.fulilian/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_fulilian_home()`.
Browse with `fulilian logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across Fulilian: desktop, TUI, website, and future TS packages.
...
- **Skin engine** (`fulilian_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `FulilianCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.fulilian/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`fulilian_cli/commands.py`)
...
- **Slack** — `slack_subcommand_map()` generates `/fulilian` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `fulilian_cli/commands.py`:
```python
CommandDef("mycommand", "Description of what it does", "Session",
           aliases=("mc",), args_hint="[arg]"),
```
2. Add handler in `FulilianCLI.process_command()` in `cli.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `fulilian --tui` or `FULILIAN_TUI=1`.
...
npm run dev       # watch mode (rebuilds fulilian-ink + tsx --watch)
npm start         # production
npm run build     # full build (fulilian-ink + tsc)
...
### TUI in the Dashboard (`fulilian dashboard` → `/chat`)

The dashboard embeds the real `fulilian --tui` — **not** a rewrite.  See `fulilian_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `fulilian_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `fulilian --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `fulilian --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.

**Structured React UI around the TUI is allowed when it is not a second chat surface.** Sidebar widgets, inspectors, summaries, status panels, and similar supporting views (e.g. `ChatSidebar`, `ModelPickerDialog`, `ToolCall`) are fine when they complement the embedded TUI rather than replacing the transcript / composer / terminal. Keep their state independent of the PTY child's session and surface their failures non-destructively so the terminal pane keeps working unimpaired.

### Electron Desktop Chat App (`apps/desktop/`)

A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). The WebSocket/JSON-RPC transport lives in the framework-agnostic `apps/shared` package (`@fulilian/shared` — `JsonRpcGatewayClient` + WS URL helpers), which the web dashboard (`web/`) also consumes; **desktop has no build/runtime dependency on the dashboard frontend** — it spawns a headless `fulilian serve` backend server (the same gateway `dashboard` serves, minus the browser UI entirely: `serve` sets `headless_backend=True`, so `cmd_dashboard` skips `_build_web_ui` AND exports `FULILIAN_SERVE_HEADLESS=1` so `mount_spa()` disables the SPA even if a stray `web_dist/` exists — only the JSON-RPC/WS/API surface is reachable). `dashboard` and `serve` share `cmd_dashboard`/`start_server` but are independent surfaces — neither launches the other. The one exception is a backward-compat *fallback*: `serve` is newer, so the desktop spawn (`electron/backend-command.ts` + `backendSupportsServe()` in `electron/main.ts`) detects whether the resolved runtime registers `serve` and, only when it does not (an older managed install / PATH `fulilian` the app hasn't updated yet), rewrites the argv to the legacy `dashboard --no-open`. Without that, a new app against an un-upgraded runtime would crash on an unknown subcommand and brick every mid-upgrade user. It does NOT embed `fulilian --tui` — it has its own composer, transcript, and slash-command pipeline. For scoped Desktop architecture, state, resolver, transport, and testing rules, read `apps/desktop/AGENTS.md`.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMAND_SPECS` (the built-ins and their Desktop surfaces) plus `NO_DESKTOP_SURFACE` block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Fulilian built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Fulilian core.
Use the plugin route instead: create `~/.fulilian/plugins/<name>/plugin.yaml`
and `~/.fulilian/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core Fulilian tool that should ship in the base system.
...
**2. Add to `toolsets.py`** — either `_FULILIAN_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_FULILIAN_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual import list to maintain. Wiring into a toolset is still a deliberate, manual step.

The registry handles schema collection, dispatch, availability checking, and error wrapping. All handlers MUST return a JSON string.

**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_fulilian_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `FULILIAN_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_fulilian_home()` for the base directory — never `Path.home() / ".fulilian"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `fulilian_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `fulilian_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `fulilian tools`, `fulilian setup`, most CLI subcommands | `fulilian_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`fulilian_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
fulilian_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.fulilian/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
Add to `_BUILTIN_SKINS` dict in `fulilian_cli/skin_engine.py`:
...
Users create `~/.fulilian/skins/<name>.yaml`:
...
Fulilian has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.fulilian/plugins/` and pip-installed entry points.

### General plugins (`fulilian_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.fulilian/plugins/`, `./.fulilian/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `fulilian` at startup so
  `fulilian <pluginname> <subcmd>` works with no change to `main.py`
...
bundled, `$FULILIAN_HOME/plugins/`, `./.fulilian/plugins/` (opt-in via
`FULILIAN_ENABLE_PROJECT_PLUGINS`), and `fulilian_agent.memory_providers` entry
...
`post_setup(fulilian_home, config)` for setup-wizard integration.

**CLI commands via `plugins/memory/<name>/cli.py`:** if a memory plugin
defines `register_cli(subparser)`, `discover_plugin_cli_commands()` finds
it at argparse setup time and wires it into `fulilian <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `fulilian --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `fulilian_cli/main.py`, etc.).
...
into `~/.fulilian/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `fulilian memory setup` / `post_setup()` without
...
`~/.fulilian/plugins/` (or via pip entry points). They register through
...
(github.com/xujuan-cyber/fulilian/discussions). PRs that add such a directory under
...
2. User: `$FULILIAN_HOME/plugins/model-providers/<name>/`
...
[`fulilian-example-plugins`](https://github.com/NousResearch/hermes-example-plugins)
companion repo, not in this tree.

### Bot Mode (`apps/desktop/src/plugins/fulilian-bots/`)

The desktop "Bots" experience ships bundled in-tree. Each bot is a Fulilian
...
`ui_meta['fulilian-bots'].chat` and verified it per click; five hardening
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `fulilian skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.fulilian.tags`, `metadata.fulilian.category`,
`metadata.fulilian.related_skills`, `metadata.fulilian.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.fulilian.*` by the loader.
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "FuLiLian" is the secondary collaborator. If the
   contributor's commit shows "FuLiLian" as author (because they
   used Fulilian to draft the skill), replace it with their actual name
...
lives in the `fulilian-agent-dev` skill at
...
`"messaging"`); `_FULILIAN_CORE_TOOLS` is the default bundle most
...
Enable/disable per platform via `fulilian tools` (the curses UI) or the
...
Fulilian returns a delegation id immediately and the result re-enters the
...
go to `~/.fulilian/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `fulilian_cli/curator.py` wires `fulilian curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.fulilian/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `fulilian cron <verb>`
...
- File lock at `~/.fulilian/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `fulilian kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `fulilian_cli/kanban.py` wires `fulilian kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`fulilian-kanban-dispatcher.service` for
  standalone dispatcher deployment).

Isolation model:
- **Board** is the hard boundary — workers are spawned with
  `FULILIAN_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
## Update Pipeline (`fulilian update`)
...
- **Plan** (`fulilian_cli/update_inventory.py`, `fulilian update --plan`):
...
- **Snapshot** (`fulilian_cli/backup.py`): pre-update quick snapshot for
...
- **Restart-per-kind**: systemd and launchd restarts are FLEET-WIDE
  (every `fulilian-gateway*` unit / `ai.fulilian.gateway*` LaunchAgent),
...
- **Report**: every run writes a machine-readable receipt to
  `~/.fulilian/logs/update_receipts/` (`latest.json` pointer; steps,
...
`fulilian serve` (control plane, desktop-spawned child) dies with the app
— by design. The messaging gateway (`gateway run`) SURVIVES the app: the
serve backend's `/api/gateway/*` endpoints spawn it detached
(`_spawn_fulilian_action` — `start_new_session` / `DETACHED_PROCESS`), so
...
Fulilian-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `FULILIAN_BACKGROUND_NOTIFICATIONS` env var):
...
Fulilian supports **profiles** — multiple fully isolated instances, each with its own
`FULILIAN_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `fulilian_cli/main.py` sets
`FULILIAN_HOME` before any module imports. All `get_fulilian_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_fulilian_home()` for all FULILIAN_HOME paths.** Import from `fulilian_constants`.
   NEVER hardcode `~/.fulilian` or `Path.home() / ".fulilian"` in code that reads/writes state.
   ```python
   # GOOD
   ...
   # BAD — breaks profiles
   config_path = Path.home() / ".fulilian" / "config.yaml"
   ```

2. **Use `display_fulilian_home()` for user-facing messages.** Import from `fulilian_constants`.
   This returns `~/.fulilian` for default or `~/.fulilian/profiles/<name>` for profiles.
   ```python
   # GOOD
   from fulilian_constants import display_fulilian_home
   print(f"Config saved to {display_fulilian_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.fulilian/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_fulilian_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_fulilian_home()`,
   not `Path.home() / ".fulilian"`.

4. **Tests that mock `Path.home()` must also set `FULILIAN_HOME`** — since code now uses
   `get_fulilian_home()` (reads env var), not `Path.home() / ".fulilian"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"FULILIAN_HOME": str(tmp_path / ".fulilian")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not FULILIAN_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".fulilian" / "profiles"`, NOT `get_fulilian_home() / "profiles"`.
   This is intentional — it lets `fulilian -p coder profile list` see all profiles regardless
...
- Use the canonical matchers: `gateway.status.looks_like_gateway_command_line`
  (gateway run), `fulilian_cli.update_cmd._fulilian_holder_subcommand`
  (top-level subcommand of any Fulilian argv). Never hand-roll token scans.
...
### DO NOT hardcode `~/.fulilian` paths
Use `get_fulilian_home()` from `fulilian_constants` for code paths. Use `display_fulilian_home()`
for user-facing print/log messages. Hardcoding `~/.fulilian` breaks profiles — each profile
has its own `FULILIAN_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### All CLI menu-pickers MUST use curses.
Interactive menus must use `fulilian_cli/curses_ui.py`. See `fulilian_cli/tools_config.py` for an example.
...
canary ledger, fulilian#85796 / gateway-gateway#210). Violating any of them
...
with actual imports (not mocks) against a temp `FULILIAN_HOME`.

### Tests must not write to `~/.fulilian/`
The `_isolate_fulilian_home` autouse fixture in `tests/conftest.py` redirects `FULILIAN_HOME` to a temp dir. Never hardcode `~/.fulilian/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_fulilian_home()` resolve within the temp dir.
Use the pattern from `tests/fulilian_cli/test_profiles.py`:
```python
@pytest.fixture
...
subprocess (`--file-retries`, default 1; `FULILIAN_TEST_FILE_RETRIES=0` to
...
| HOME / `~/.fulilian/` | Your real config+auth.json                  | Temp dir per test                         |
...
Fulilian supports Linux, macOS and native Windows, and plenty of its behaviour
...
`tests/fulilian_cli/test_venv_holder_windows_live.py` on a real
