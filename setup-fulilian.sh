...
# 5. Symlinks the 'fulilian' CLI command into a user-facing bin dir
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/fulilian-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink fulilian into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up fulilian command..."

FULILIAN_BIN="$SCRIPT_DIR/venv/bin/fulilian"
...
# ============================================================================
# Seed bundled skills into ~/.fulilian/skills/
# ============================================================================

FULILIAN_SKILLS_DIR="${FULILIAN_HOME:-$HOME/.fulilian}/skills"
mkdir -p "$FULILIAN_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.fulilian/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$FULILIAN_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  fulilian status        # Check configuration"
if is_termux; then
    echo "  fulilian gateway       # Run gateway in foreground"
else
    echo "  fulilian gateway install # Install gateway service (messaging + cron)"
fi
echo "  fulilian cron list     # View scheduled jobs"
echo "  fulilian doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m fulilian_cli.main setup
fi
