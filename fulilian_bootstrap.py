"""Windows UTF-8 bootstrap for Fulilian entry points.
...
should be imported at the very top of every Fulilian entry point
(``fulilian``, ``fulilian-agent``, ``fulilian-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``fulilian_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``fulilian_bootstrap``, never ``fulilian_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Fulilian modules.

    Fulilian ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Fulilian source root
    ...
    """
    root = src_root or os.environ.get("FULILIAN_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``FULILIAN_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("FULILIAN_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import fulilian_bootstrap``
# (or ``from fulilian_bootstrap import apply_windows_utf8_bootstrap``) at
