# Contributing to FuLiLian

Thank you for contributing to FuLiLian! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Fulilian to work everywhere.
...
Related: #38284 covers the agent-side analog — Fulilian itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every Fulilian install. They should be **broadly useful to most users**:
...
**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.fulilian/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(fulilian_home, config)` for setup-wizard integration
...
- Integrate with `fulilian memory setup` via `post_setup()` — no need to touch core code
...
The installer creates the Fulilian venv, wires the `fulilian` command, stamps the
install method for `fulilian update`, and clones the full git project into
`$FULILIAN_HOME/fulilian-agent` (usually `~/.fulilian/fulilian-agent`). That keeps your
...
Use this only if you intentionally do not want Fulilian' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `fulilian` entrypoint from this venv; running the
system `python3 -m fulilian_cli.main` can pick up unrelated system Python
packages.
...
uv venv ~/.fulilian/venvs/fulilian-dev --python 3.11
...
```bash
mkdir -p ~/.fulilian/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.fulilian/config.yaml
touch ~/.fulilian/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.fulilian/.env
```

### Run

```bash
# The standard installer already put `fulilian` on PATH.
fulilian doctor
fulilian chat -q "Hello"
```

If you used the manual clone fallback, run `./fulilian` from the checkout or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/fulilian" ~/.local/bin/fulilian
```
...
├── cli.py                    # FulilianCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (fulilian-cli, fulilian-telegram, etc.)
├── fulilian_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── fulilian_cli/               # CLI command implementations
...
### User configuration (stored in `~/.fulilian/`)

| Path | Purpose |
|------|---------|
| `~/.fulilian/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.fulilian/.env` | API keys and secrets |
| `~/.fulilian/auth.json` | OAuth credentials (Nous Portal) |
| `~/.fulilian/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.fulilian/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.fulilian/state.db` | SQLite session database |
| `~/.fulilian/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.fulilian/cron/` | Scheduled job data |
| `~/.fulilian/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`fulilian_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.fulilian/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
(for example `_FULILIAN_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.fulilian`:
...
The user may skip setup and keep loading the skill. Fulilian only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `fulilian setup` or update `~/.fulilian/.env` locally.
...
1. **`description` ≤ 1024 characters, one sentence, ends with a period.** Long descriptions bloat the skill listing UI and dilute the model's attention when many skills are loaded, so avoid padding — but don't compress away the trigger to hit some arbitrary shorter limit. State the capability, not the implementation. No marketing words ("powerful", "comprehensive", "seamless", "advanced"). Don't repeat the skill name. Verify with:
   ```python
   import re, pathlib
...
2. **Tools referenced in SKILL.md prose must be native Fulilian tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "FuLiLian" is the secondary collaborator. If the contributor's commit shows "FuLiLian" as author because they used Fulilian to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Fulilian tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `fulilian --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Fulilian uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.fulilian/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `fulilian_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `fulilian_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Fulilian runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the fulilian wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `fulilian_cli/gateway.py::_scan_gateway_pids` for the pattern.
   ```
...
9. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `fulilian_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
15. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `fulilian_cli/gateway_windows.py::
...
Fulilian has terminal access. Security matters.
...
2. **Test manually**: Run `fulilian` and exercise the code path you changed
