...
/** Why a known Fulilian command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Fulilian profile', surface: action('profile') },
  {
    name: '/skin',
...
function isKnownFulilianSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Fulilian' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownFulilianSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with Fulilian and were never asked for. It is
