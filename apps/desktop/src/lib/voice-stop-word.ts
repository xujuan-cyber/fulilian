...
//
// When someone is in a hands-free "Hey Fulilian" voice chat, the natural way to
...
// phrase (optionally addressed to Fulilian), so a real turn that merely contains
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("fulilian") is not a stop command on its own.
      continue
    }
 ...
 * to Fulilian). Returns false for anything that merely contains "stop" as part of
