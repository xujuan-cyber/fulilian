import { fulilianApi } from '@/api/client'
import type {
  FulilianConnection,
  FulilianReadDirResult,
  FulilianReadFileTextResult,
  FulilianSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: FulilianConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: FulilianConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<FulilianReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<FulilianReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<FulilianReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<FulilianReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.fulilianDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: FulilianSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
