 ...
 * `FulilianPlugin`; it never touches the registry directly — it receives a
...
import { pluginRest, type PluginRestOptions, pluginSocket } from '@/fulilian'
...
export type { PluginRestOptions } from '@/fulilian'
 ...
 *  under `fulilian.plugin.<id>.` — plugins can't read or clobber each other. */
 ...
 *  in one attributed namespace instead of the raw `window.fulilianDesktop`
...
export interface PluginOs {
   ...
   *  the user is away from Fulilian — use `host.notify` for the in-app toast.
   *  Throttled per plugin; reserve it for genuinely notable events.
   *  Supports `icon`, `activate` (e.g. `fulilian://index-network/intent/1`),
...
export interface FulilianPlugin {
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `fulilian.plugin.${pluginId}.${key}`
...
function createPluginOs(pluginId: string): PluginOs {
  const attempt = async (run: (bridge: NonNullable<typeof window.fulilianDesktop>) => Promise<boolean>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.fulilianDesktop
