...
import { capabilityScoped, fulilianApi, type ProfileScope, STARTUP_REQUEST_TIMEOUT_MS } from './client'

export function getProfiles(): Promise<ProfilesResponse> {
  return fulilianApi<ProfilesResponse>({
    path: '/api/profiles',
...
export function createProfile(body: ProfileCreatePayload): Promise<{ name: string; ok: boolean; path: string }> {
  return fulilianApi<{ name: string; ok: boolean; path: string }>({
    path: '/api/profiles',
    method: 'POST',
    body
  })
...
export function deleteProfile(name: string, scope?: ProfileScope): Promise<{ ok: boolean; path: string }> {
  ...
  return fulilianApi<{ ok: boolean; path: string }>({
    ...profileOwnerScoped(scope),
...
export function getProfileSoul(name: string, scope?: ProfileScope): Promise<ProfileSoul> {
  return fulilianApi<ProfileSoul>({
    ...profileOwnerScoped(scope),
...
export function getProfileSetupCommand(name: string): Promise<ProfileSetupCommand> {
  return fulilianApi<ProfileSetupCommand>({
...
export function exportProfileArchive(
  ...
  return fulilianApi<{ archive: string; ok: boolean }>({
...
export function importProfileArchive(
  ...
  return fulilianApi<{ desktop: null | ProfileDesktopOverlay; name: string; ok: boolean; path: string }>({
