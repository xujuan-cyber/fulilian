...
test('classifier: raw-text fallback maps current gateway error strings', () => {
  ...
  assert.equal(
    attentionReasonFromError('No LLM provider configured. Run fulilian model to pick one.'),
    'missing_config'
  )
...
test('render: roster row shows an amber warning glyph with a per-class hint', () => {
  ...
  assert.match(pluginSource, /Provider not configured — run fulilian model/)
