...
function runtime() {
  ...
  const code = source
    .replace(/^import \* as sdk from '@fulilian\/plugin-sdk'\r?\n/m, '')
...
test('default rows use source identity without borrowing another source title', () => {
  ...
  // label. Titled: the title wins. Untitled: it stays "Fulilian". Regression:
  ...
  assert.equal(name(active, undefined), 'Fulilian')
})
...
test('botHandle: precomputed multi-source handle wins; default stays fulilian', () => {
  ...
  assert.equal(botHandle('default'), 'fulilian')
})
...
test('displayName: local default stays Fulilian; remote default uses the device label', () => {
...
test('resolveRosterMentions: @fulilian in this chat is not a handoff to yourself', () => {
  const { __resolveRosterMentions: resolve } = runtime()
  const roster = [
    { name: 'default', connectionId: 'local', handle: 'fulilian' },
    {
      name: 'default',
      connectionId: 'mac-mini',
      connectionLabel: 'Mac Mini',
  ...
  const hits = resolve('ask @fulilian and @default-mac-mini', roster, {
    name: 'default',
    connectionId: 'local'
  })
