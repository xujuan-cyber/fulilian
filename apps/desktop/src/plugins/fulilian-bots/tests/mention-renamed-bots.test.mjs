...
// Bot Mode title or `fulilian profile rename` display_name — must update what
...
test('mentionNameForms: slugged + collapsed, reserved tokens dropped', () => {
  ...
  // A bot renamed "Fulilian" or "everyone" cannot hijack reserved tags.
  assert.equal(mentionNameForms('Fulilian').length, 0)
...
test('botMentionTag: renamed slug wins, profile handle is the fallback', () => {
  ...
  assert.equal(botMentionTag({ name: 'default' }), 'fulilian')
  // display_name (fulilian profile rename) drives the tag too.
...
test('parseGroupChatMentions: display_name and ui_meta title forms address a member', () => {
  ...
  const members = [
    ...
    { name: 'ops', title: '', ui_meta: { 'fulilian-bots': { title: 'Night Watch' } } }
  ]
