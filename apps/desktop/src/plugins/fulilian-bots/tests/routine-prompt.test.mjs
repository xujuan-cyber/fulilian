...
test('integration: a different active profile retains the delegated routine wrapper', () => {
  ...
  assert.match(prompt, /fulilian -p 'research' chat/)
...
test('security: delegated routine arguments remain literal shell values', () => {
  ...
  const command = prompt.slice(prompt.indexOf('fulilian '), prompt.lastIndexOf('\n\nIf the command'))
  const script = `fulilian() { printf '%s\\037' "$@"; }\n${command}`
...
test('security: upgrade pauses persisted delegated routines before they can execute', async () => {
  const titleSentinel = `/tmp/fulilian-bot-mode-title-${process.pid}`
  const taskSentinel = `/tmp/fulilian-bot-mode-task-${process.pid}`
  ...
  const oldPrompt =
    ...
    `fulilian -p research chat -c "Routine: ${title}" -q ${JSON.stringify(`[Scheduled routine] ${instruction}`)}\n\n` +
  ...
  const command = recreated.slice(recreated.indexOf('fulilian '), recreated.lastIndexOf('\n\nIf the command'))
  const script = `fulilian() { printf '%s\\037' "$@"; }\n${command}`
...
test('regression: Create Cronjob passes the active profile to routinePrompt', () => {
  ...
  assert.doesNotMatch(__routines.routinePrompt('ops', 'Check', instruction, 'ops'), /fulilian -p/)
})
