...
function load(turnScript, { busyUntilResumeCall, clarifyUntilResumeCall, approvalUntilResumeCall, conflictOnce = false, deferredTimers = false } = {}) {
      ...
      request: async (method, params) => {
        ...
        if (method === 'profiles.configure') {
          if (conflictOnce && !injectedConflict) {
            ...
            sharedUiMeta['fulilian-bots-groups'] = {
              version: 2,
              rooms: {
                Shared: {
                  revision: 1,
            ...
            uiMetaRevisions['fulilian-bots-groups'] = 1
            return {
              applied: {
                ui_meta: false,
                ui_meta_conflicts: { 'fulilian-bots-groups': { expected: 0, actual: 1 } },
                ui_meta_revisions: { 'fulilian-bots-groups': 1 }
              }
            }
          }
  ...
  const source = pluginSource
    .replace(/^import\s+\*\s+as\s+sdk\s+from '@fulilian\/plugin-sdk'\r?\n/m, '')
    .replace(/^import\s+\{[\s\S]*?\}\s+from '@fulilian\/plugin-sdk'\r?\n/m, '')
...
test('group room messages and members mirror through bounded gateway profile metadata', async () => {
  ...
  const envelope = configure.params.ui_meta['fulilian-bots-groups']
...
test('an explicit final-room disband may clear the gateway room mirror', async () => {
  ...
  const configure = gc.requests.filter(call => call.method === 'profiles.configure').at(-1)
  assert.deepEqual(Object.keys(configure.params.ui_meta['fulilian-bots-groups'].rooms), [])
  assert.ok(configure.params.ui_meta['fulilian-bots-groups'].deleted['name:Research'] > 0)
})
...
test('sync worker retries a gateway CAS conflict and publishes the merged room', async () => {
  ...
  for (let i = 0; i < 20 && (gc.uiMetaRevisions['fulilian-bots-groups'] || 0) < 2; i++) {
    await new Promise(resolve => setImmediate(resolve))
  }

  const stored = gc.sharedUiMeta['fulilian-bots-groups']
  assert.equal(gc.uiMetaRevisions['fulilian-bots-groups'], 2)
  assert.equal(
...
test('fan-out class: a room write is mirrored to every reachable default-profile gateway', async () => {
  ...
  gc.host.requestProfile = async (route, method) => {
    ...
    if (method === 'profiles.configure') {
      return { applied: { ui_meta: true, ui_meta_revisions: { 'fulilian-bots-groups': 1 } } }
    }
    return {}
  }
...
test('default profile speaks as Fulilian in room transcripts, not @default', () => {
  const gc = load(() => '(pass)')
  const line = gc.formatGroupChatLine({ from: { kind: 'member', name: 'default' }, text: 'hello room' }, 'builder')
  assert.equal(line, 'Fulilian: hello room')
  ...
  assert.equal(you, 'Fulilian (you): hi')
  const plain = gc.formatGroupChatLine({ from: { kind: 'member', name: 'builder' }, text: 'yo' }, 'research')
  assert.equal(plain, 'builder: yo')
})

test('speaker labels honor friendly identity: Bot Mode title, then display_name, never a stale Fulilian', () => {
  const gc = load(() => '(pass)')

  // A renamed default (core display_name via `fulilian profile rename`) must
  // read as its new name — the community report was "Lucy" still showing
  // "Fulilian is thinking…" in group rooms.
  ...
  // Untitled rows keep today's behavior: default → Fulilian, others verbatim.
  gc.$botMeta.set({})
  assert.equal(gc.groupSpeakerLabel('default'), 'Fulilian')
...
test('turn prompt addresses the default profile as @fulilian', () => {
  const gc = load(() => '(pass)')
  ...
  assert.match(prompt, /You are @fulilian,/)
  ...
  assert.match(peerView, /group chat with @fulilian/)
})

test('mention routing: @fulilian resolves to the default member', () => {
  const gc = load(() => '(pass)')
  const members = [{ name: 'default', title: '' }, { name: 'builder', title: '' }]
  const parsed = gc.parseGroupChatMentions('@fulilian take a look', members)
...
test('source contract: workspace speaker labels use displayName with a click-to-reveal handle', () => {
  // Speaker labels come from the roster displayName (default → Fulilian)…
...
test('group room preview renders the bot HANDLE, not the raw profile name', () => {
  ...
  // @fulilian, so users concluded mention routing was broken.
