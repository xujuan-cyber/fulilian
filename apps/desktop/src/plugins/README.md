...
Drop a `<name>/plugin.{ts,tsx}` here that default-exports a `FulilianPlugin` and
...
`$FULILIAN_HOME/desktop-plugins/<name>/plugin.js` (the disk door) — see the
`fulilian-desktop-plugins` skill.
