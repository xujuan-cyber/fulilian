...
import type { FulilianConfigRecord, SessionInfo } from '@/types/fulilian'
...
function AutoArchiveSetting() {
  ...
  const persist = useCallback(
    async (autoArchive: boolean, archiveDays: number) => {
      ...
      try {
        await saveFulilianConfig(updated)
      } catch (err) {
...
// / Program Files), which buried any files Fulilian wrote there.
function DefaultProjectDirSetting() {
  ...
  useEffect(() => {
    ...
    // / Storybook / non-Electron context `window.fulilianDesktop` is
    ...
    const settings = window.fulilianDesktop?.settings
