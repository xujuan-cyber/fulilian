...
// Fulilian Cloud discovery lifecycle for the cloud-mode panel.
...
export function GatewaySettings({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const setKeychainEncryption = async (on: boolean) => {
    ...
    try {
      const res = await window.fulilianDesktop.setSecretStorageEncryption(on)
  ...
  // --- Fulilian Cloud (cloud mode) state ---
  ...
  useEffect(() => {
    if (state.mode !== 'ssh' || !window.fulilianDesktop?.sshConfigHosts) {
      return
    }
  ...
  const payload = (allowPlainTextToken?: boolean) => ({
    ...
    sshRemoteFulilianPath: state.sshRemoteFulilianPath.trim(),
  ...
  const performSave = async (apply: boolean, allowPlainTextToken: boolean) => {
    ...
    try {
      const next = apply
        ? await window.fulilianDesktop.applyConnectionConfig(payload(allowPlainTextToken))
        : await window.fulilianDesktop.saveConnectionConfig(payload(allowPlainTextToken))
      ...
      const errors = {
        ...
        'fulilian-not-found': g.sshErrNotInstalled,
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const saved = await window.fulilianDesktop.saveConnectionConfig({
        mode: state.mode,
      ...
      const result = await window.fulilianDesktop.oauthLoginConnectionConfig(trimmedUrl)
      ...
      if (result.connected) {
        const refreshed = await window.fulilianDesktop.getConnectionConfig(null)
  ...
  const signOut = async () => {
    ...
    try {
      await window.fulilianDesktop.oauthLogoutConnectionConfig(trimmedUrl)
      const refreshed = await window.fulilianDesktop.getConnectionConfig(null)
  ...
  const resolveSshHost = async (host: string) => {
    if (!host || !window.fulilianDesktop?.sshResolveHost) {
      return
    }
    ...
    try {
      const resolved = await window.fulilianDesktop.sshResolveHost(host)
  ...
  const testSsh = async () => {
    ...
    try {
      const result = await window.fulilianDesktop.testConnectionConfig(payload())
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.fulilianDesktop.testConnectionConfig({
        mode: 'remote',
        remoteAuthMode: authMode,
        remoteToken: authMode === 'token' ? remoteToken.trim() || undefined : undefined,
        remoteUrl: trimmedUrl
  ...
  if (!window.fulilianDesktop?.getConnectionConfig) {
    ...
    <SettingsContent bare={embedded}>
          ...
          <ListRow
            action={
              <Input
                ...
                placeholder="https://gateway.example.com/fulilian"
        ...
        <div className="mt-5 grid gap-1">
          ...
          <ListRow
            action={
              <Input
                className={cn('h-8 font-mono', CONTROL_TEXT)}
                onChange={event => setState(current => ({ ...current, sshRemoteFulilianPath: event.target.value }))}
                placeholder={g.sshFulilianPathPlaceholder}
                value={state.sshRemoteFulilianPath}
              />
            }
            description={g.sshFulilianPathDesc}
            title={g.sshFulilianPathTitle}
          />
        </div>
      ) : null}
      ...
      {embedded ? null : (
        <div className="mt-6 grid gap-1">
          ...
          <ListRow
            action={
              <Button onClick={() => void window.fulilianDesktop?.revealLogs()} size="sm" variant="textStrong">
                <FileText />
                {g.openLogs}
              </Button>
            }
