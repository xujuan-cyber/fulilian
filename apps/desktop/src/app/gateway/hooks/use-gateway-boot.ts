import { isGatewayReauthRequired, JsonRpcGatewayError, resolveGatewayWsUrl } from '@fulilian/shared'
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.fulilianDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: FulilianGateway | null) => void
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: FulilianConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Fulilian...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Fulilian…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Fulilian gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshFulilianConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshFulilianConfig(false, ownsSwitch).catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startFulilian re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new FulilianGateway()
    ...
    async function boot() {
      try {
        ...
        // hanging "Starting Fulilian…" forever.
        const wsUrl = await withTimeout(
          resolveGatewayWsUrl(desktop, conn),
          RECONNECT_ATTEMPT_TIMEOUT_MS,
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshFulilianConfig(),
          ...
          // retry it), not brick boot behind the "Fulilian couldn't start"
