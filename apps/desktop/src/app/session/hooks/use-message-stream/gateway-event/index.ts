...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  const { activeSessionIdRef, compactedTurnRef, refreshFulilianConfig, sessionStateByRuntimeIdRef } = deps
  ...
  // refreshFulilianConfig — two REST calls (config + defaults) per event, per
