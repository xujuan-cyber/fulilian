...
import { pathFromFulilianDeepLink, resolveFulilianOpenPath } from '@/lib/fulilian-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.fulilianDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    window.fulilianDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.fulilianDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.fulilianDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // fulilian://index-network/intent/1 vocabulary with deep links.
  useEffect(() => {
    const unsubscribe = window.fulilianDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        // the pre-IPC validation — any future fulilianDesktop.notify caller gets
        // funneled through the same resolver.
        const path = resolveFulilianOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.fulilianDesktop?.onDeepLink?.(payload => {
      if (!payload?.kind) {
        return
      }
      ...
      // link (fulilian://index-network/intent/1, fulilian://open/…). The resolver
      // rejects reserved kinds and unsafe paths.
      const path = pathFromFulilianDeepLink(payload.kind, payload.name || '', payload.params || {})
    ...
    void window.fulilianDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.fulilianDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.fulilianDesktop?.onPreviewNav?.(command => {
  ...
  useEffect(() => {
    const unsubscribe = window.fulilianDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
