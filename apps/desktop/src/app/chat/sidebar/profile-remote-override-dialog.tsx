...
export function ProfileRemoteOverrideDialog({ profileNames }: { profileNames: string[] }) {
  ...
  useEffect(() => {
    ...
    window.fulilianDesktop?.connections
      ?.list()
      .then(registry => {
        if (cancelled) {
          return
  ...
  const performSave = async () => {
    ...
    try {
      await window.fulilianDesktop.applyConnectionConfig({
        mode: 'remote',
        profile,
  ...
  const removeOverride = async () => {
    ...
    try {
      await window.fulilianDesktop.applyConnectionConfig({ mode: 'local', profile })
