...
export const ja = defineLocale({
  ...
  boot: {
    ready: 'Fulilian Desktop の準備ができました',
    desktopBootFailedWithMessage: message => `デスクトップの起動に失敗しました: ${message}`,
    steps: {
      connectingGateway: 'ライブデスクトップゲートウェイに接続中',
      loadingSettings: 'Fulilian の設定を読み込み中',
      loadingSessions: '最近のセッションを読み込み中',
      retryingRemoteBackend: 'リモート Fulilian バックエンドに再接続中…',
      startingDesktopConnection: 'デスクトップ接続を開始中',
      startingFulilianDesktop: 'Fulilian Desktop を起動中…'
    },
    errors: {
      backgroundExited: 'Fulilian バックグラウンドプロセスが終了しました。',
      backgroundExitedDuringStartup: '起動中に Fulilian バックグラウンドプロセスが終了しました。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage:
      'Fulilian バックエンドがこのデスクトップビルドより古く、正常に動作しない場合があります。更新して揃えてください。',
    installMethodUnsupportedTitle: 'サポート対象外のインストール方法',
    updateFulilian: 'Fulilian を更新',
    ...
    errors: {
      ...
      methodNotAllowed:
        'デスクトップバックエンドがそのリクエストを拒否しました (405 Method Not Allowed)。Fulilian Desktop を再起動してください。',
    ...
    native: {
      ...
      rejectAction: '拒否',
      inputTitle: '入力が必要です',
      inputBody: 'Fulilian が応答を待っています。',
      turnDoneTitle: 'Fulilian が完了しました',
      turnDoneBody: '',
  ...
  sendDiagnostics: {
    ...
    failedHint:
      'ターミナルから `fulilian debug share --nous` を実行するか、`fulilian debug share --local` でアップロードせずにレポートを表示することもできます。',
  ...
  settings: {
    ...
    resetConfirm: 'すべての設定を Fulilian のデフォルトに戻しますか？',
    ...
    notifications: {
      ...
      focusedHint: '完了通知は Fulilian がバックグラウンドにあるときのみ表示されます。',
      kinds: {
        approval: {
          label: '承認が必要',
        ...
        input: {
          label: '入力が必要',
          description: 'Fulilian が質問したか、パスワードやシークレットを必要としています。'
        },
        turnDone: {
          label: '応答完了',
          description: 'Fulilian がバックグラウンドのときにターンが完了しました。'
        },
        turnError: {
          label: 'ターン失敗',
        ...
        plugin: {
          label: 'プラグイン通知',
          description: 'Fulilian がバックグラウンドの間に、デスクトッププラグインが通知を送信しました。'
        }
      },
      test: 'テスト通知を送信',
      testTitle: 'Fulilian',
    ...
    appearance: {
      ...
      colorMode: 'カラーモード',
      colorModeDesc: '固定モードを選ぶか、Fulilian をシステム設定に合わせます。',
      ...
      reactionsDesc:
        'iMessage風の絵文字タップバック — メッセージにリアクションでき、Fulilianもあなたのメッセージにリアクションします。',
      tipsTitle: 'アプリ内ヒント',
      tipsDesc:
        'アプリの一部を指す小さな吹き出し。待機中にときどき、また役に立つときは Fulilian からも表示します。閉じたヒントは二度と表示されません。',
      tipsReset: (count: number) => `閉じた${count}件のヒントを元に戻す`,
      toursTitle: 'ガイドツアー',
      toursDesc: '画面を暗くして各ステップを強調しながら、Fulilian がアプリを案内します。',
      ...
      pet: {
        title: 'ペット',
        intro:
          'アプリ上に浮かぶ petdex のアニメーションマスコットを採用しましょう。ツール実行中は走り、成功すると喜び、エラーでしょんぼりと、Fulilian の状態に反応します。',
        restartHint:
          'ペット機能には再起動が必要です。この機能が追加される前に起動したアプリが動作中です。Fulilian を終了して再度開き、このページに戻ってください。',
        scaleTitle: 'サイズ',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone:
        'Fulilian がローカル時刻のコンテキストを必要とするときに使用します。空欄ならシステムのタイムゾーンを使います。',
      agent: {
        imageInputMode: '画像添付をモデルへ送る方法を制御します。',
        maxTurns: 'Fulilian が 1 回の実行を停止するまでのツール呼び出しターン上限です。'
      },
      terminal: {
      ...
      fileReadMaxChars: 'Fulilian が 1 回のファイル読み取りで取得できる最大文字数です。',
      approvals: {
        mode: '明示的な承認が必要なコマンドを Fulilian がどう扱うかを設定します。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'アプリから Fulilian 自身を更新するとき、ローカルのソース変更を保持するか破棄するかを選びます。ターミナル更新では常に確認されます。'
      }
    }),
    about: {
      ...
      bundleOutOfSyncDesc:
        'Fulilian ランタイムは更新されましたが、デスクトップアプリ自体は古いビルドのままです。アプリを更新するまで、新しいインターフェース機能(Bot Mode など)は表示されません。下の更新を実行してアプリを再ビルドしてください。それでもこの警告が消えない場合は、最新のデスクトップインストーラーから再インストールしてください。',
      ...
      automaticUpdatesDesc: 'Fulilian はバックグラウンドで自動的に更新を確認し、利用可能になったら通知します。',
    ...
    gateway: {
      ...
      intro:
        'Fulilian Desktop はデフォルトで独自のローカルゲートウェイを起動します。別のマシンや信頼できるプロキシの背後で既に動作している Fulilian バックエンドをこのアプリで制御する場合は、リモートゲートウェイを使用してください。ゲートウェイ接続はマシン単位の設定で、プロファイルは接続したゲートウェイから検出されます。',
      envOverrideTitle: '環境変数がこのデスクトップセッションを制御しています。',
      envOverrideDesc:
        '保存された設定を使用するには FULILIAN_DESKTOP_REMOTE_URL と FULILIAN_DESKTOP_REMOTE_TOKEN の設定を解除してください。',
      ...
      remoteUrlDesc:
        'リモートダッシュボードバックエンドのベース URL。/fulilian などのパスプレフィックスもサポートしています。',
      ...
      restartingMessage: 'Fulilian Desktop は保存された設定を使用して再接続します。',
      savedMessage: '次回起動時に保存されます。',
      connectedTo: (baseUrl, version) => `${baseUrl}${version ? ` · Fulilian ${version}` : ''} に接続しました`,
      ...
      sshTitle: 'SSH で接続',
      sshDesc:
        'Fulilian は SSH 経由でリモート上に起動され、このアプリにトンネルされます。リモート側で何かを起動・公開する必要はありません。ホストへの鍵ベースの SSH アクセスが前提です。',
      ...
      sshFulilianPathTitle: 'Fulilian パス（任意）',
      sshFulilianPathDesc: 'リモートの fulilian バイナリへのフルパス。空欄 = 自動検出。',
      sshFulilianPathPlaceholder: '自動検出',
      sshTestConnection: 'SSH をテスト',
      sshConnect: '接続',
      sshButtonsHint: '「保存」は次回起動時に適用され、「接続」は今すぐ再接続します。',
      sshReachable: (host, platform) => `接続可能: ${host}（${platform}）— Fulilian を検出`,
      sshIncompleteHost: '接続する前に SSH ホストを入力してください。',
      sshErrUnreachable: 'SSH でそのホストに到達できませんでした。ホスト、ポート、ネットワークを確認してください。',
      sshErrAuth:
        'SSH 認証に失敗しました。鍵を ssh-agent に読み込む（ssh-add）か、~/.ssh/config に IdentityFile を設定してください。Fulilian は非対話的に ssh を実行します。',
      sshErrHostKey:
        '前回の接続以降、ホスト鍵が変更されています。想定どおりか確認し、ssh-keygen -R <host> を実行してから再接続してください。',
      sshErrNotInstalled:
        'リモートホストに Fulilian がインストールされていません。リモートでインストールする（curl -fsSL https://raw.githubusercontent.com/xujuan-cyber/fulilian/main/scripts/install.sh | sh）か、Fulilian パスを設定してください。',
      sshErrPlatform:
        'サポートされていないリモートプラットフォームです。Fulilian Desktop の SSH モードは Linux、macOS、Windows のリモートホストに対応しています。',
      sshErrTimeout: 'SSH 接続がタイムアウトしました。ホストが到達不能、またはスリープ中の可能性があります。',
      sshErrUpdateRequired: 'Desktop SSH で接続する前に、リモートホストの Fulilian を更新してください。',
    ...
    mcp: {
      ...
      deepLinkDescription:
        'リンクがこの MCP サーバーを Fulilian に追加するよう要求しました。下の設定はリンク側から来たものです。内容を必ず確認してください。',
    ...
    providers: {
      ...
      intro:
        'サブスクリプションでサインインします。API キーのコピーは不要です。Fulilian がアプリ内でブラウザーサインインを代行します。',
      connected: '接続済み',
      collapse: '折りたたむ',
  ...
  skills: {
    ...
    skillArchivedMessage: 'fulilian curator restore で復元できます。'
  },
  ...
  starmap: {
    ...
    emptyDesc: 'Fulilian がスキルやメモリを蓄積すると、ここに表示されます。'
  },
  agents: {
    close: 'エージェントを閉じる',
    title: 'スポーンツリー',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: 'ペット機能を使うには Fulilian を再起動してください。',
      empty: '一致するペットがありません。',
      turnOff: 'オフ',
      turnOn: 'オン',
      installed: 'インストール済み',
    ...
    generatePet: {
      ...
      staleBackend: 'ペットを生成するには Fulilian を更新してください。',
      backgroundHint: 'このウィンドウは閉じても大丈夫です。完了したら Fulilian が通知します。',
    ...
    nav: {
      ...
      settings: { title: '設定', detail: 'Fulilian デスクトップを設定' },
    ...
    fulilianActiveSessions: (version, count) => `Fulilian ${version} · アクティブセッション ${count}`,
  ...
  messaging: {
    ...
    fieldCopy: {
      ...
      MATRIX_USER_ID: { label: 'ボットユーザー ID', placeholder: '@fulilian:example.org' },
  ...
  profiles: {
    ...
    remoteOverride: {
      ...
      description: 'このプロファイルのセッションは、このパソコンではなく指定したリモートの Fulilian で実行されます。',
      urlLabel: 'リモートアドレス',
    ...
    deleting: '削除中...',
    createDesc: 'プロファイルは独立した Fulilian 環境です：設定、スキル、SOUL.md が別々になります。',
    nameLabel: '名前',
    cloneFrom: '複製元',
    cloneFromNone: 'なし（空）',
  ...
  cron: {
    ...
    modelImpact: {
      ...
      saveFailed: 'Fulilian はモデルの変更を保存しませんでした。',
    ...
    newCron: '新しい Cron',
    emptyDescNew:
      'Cron 式でプロンプトを実行するスケジュールを設定します。Fulilian が実行して、選択した宛先に結果を送信します。',
  ...
  sidebar: {
    ...
    projects: {
      ...
      staleBackend:
        'プロジェクトを作成するには Fulilian バックエンドを更新してください。バックエンドがこのデスクトップアプリより古いです（設定 → 更新 → バックエンド）。',
      deleteConfirm:
        'Fulilian から保存済みプロジェクトを削除します。ファイル・git リポジトリ・ワークツリーはそのまま残ります。',
      startWork: '新しいワークツリー',
      ...
      worktreeStaleBackend:
        'このリモート接続でワークツリーを作成するには Fulilian バックエンドを更新してください — git ワークツリー API 以前のバージョンです。',
  ...
  composer: {
    ...
    placeholderStarting: 'Fulilian を起動中...',
    placeholderReconnecting: 'Fulilian に再接続中…',
    ...
    attachUrlDesc: 'Fulilian がページを取得し、このターンのコンテキストとして含めます。',
  ...
  statusStack: {
    ...
    coding: {
      ...
      agentShip: 'Fulilian にコミットと PR を任せる',
  ...
  updates: {
    stages: {
      ...
      pull: 'もうすぐ完了…',
      pydeps: '仕上げ中…',
      update: 'Fulilian を更新中…',
      rebuild: 'デスクトップアプリを再ビルド中…',
      restart: 'Fulilian を再起動中…',
      done: '更新が完了しました',
      manual: 'ターミナルから更新',
    ...
    unsupportedMessage: 'このバージョンの Fulilian はアプリ内から自分を更新できません。',
    ...
    availableBody: '新しいバージョンの Fulilian をインストールする準備ができています。',
    availableTitleBackend: 'バックエンドの更新があります',
    availableBodyBackend: '接続中の Fulilian バックエンドの新しいバージョンをインストールできます。',
    ...
    manualBody:
      'Fulilian をコマンドラインからインストールしたため、更新もそこで実行されます。これをターミナルに貼り付けてください:',
    manualPickedUp: 'Fulilian は次回起動時に新しいバージョンを読み込みます。',
    guiSkewTitle: 'デスクトップアプリを更新してください',
    guiSkewBody:
      'バックエンドは更新されましたが、このデスクトップアプリのパッケージは変更されていません。一致させるために Fulilian デスクトップアプリ（AppImage / .deb / .rpm）を更新または再インストールしてください。',
    copy: 'コピー',
    copied: 'コピーしました',
    done: '完了',
    applyingBody:
    ...
    blockerTitle: 'Fulilian を更新するためにローカルプレビューを閉じますか？',
    blockerBody:
      '更新する前に、これらのローカルプレビューを停止する必要があります。ファイルが変更または削除されることはありません。',
    foreignBlockerTitle: '他のプロセスを閉じて Fulilian を更新',
    foreignBlockerBody:
      'Fulilian はこれらのプロセスを安全に自動終了できません。各プロセスを所有するアプリ、ターミナル、またはサービスを閉じてから、もう一度更新してください。',
    mixedBlockerBody:
      'Fulilian は以下のローカルプレビューを閉じることができます。更新を続けるには、他のプロセスを手動で閉じる必要があります。',
  ...
  install: {
    ...
    oneTimeTitle: 'Fulilian には一度限りのインストールが必要です',
    ...
    setupChoiceTitle: 'Fulilian Desktop をセットアップ',
    setupChoiceDesc:
      'すでに実行している Fulilian ゲートウェイに接続するか、このコンピューターに Fulilian をローカルインストールします。',
    connectExistingTitle: '既存の Fulilian に接続',
    ...
    remoteSetupDesc:
      'ゲートウェイ URL を入力してください。Fulilian Desktop がトークンとブラウザーサインインのどちらが必要かを検出します。',
    remoteUrlTitle: 'ゲートウェイ URL',
    remoteUrlDesc: 'Fulilian ゲートウェイのベース URL を使用します。リモートの場合は https:// を含めてください。',
    remoteUrlPlaceholder: 'https://gateway.example.com/fulilian',
    probing: 'ゲートウェイ認証方式を検出中...',
    probeError: 'その Fulilian ゲートウェイに到達できませんでした。',
    identityProvider: 'ID プロバイダー',
    authTitle: '認証',
    ...
    settingUpTitle: 'FuLiLian を設定中',
    finishingTitle: '仕上げ中',
    failedDesc:
      'インストール手順のいずれかが失敗しました。Windows では、別の Fulilian CLI またはデスクトップインスタンスが実行中の場合に発生することがあります。実行中の Fulilian インスタンスをすべて停止してから再試行してください。詳細は以下またはデスクトップログで確認できます。',
    activeDesc:
      'これは一回限りのセットアップです。Fulilian インストーラーが依存関係をダウンロードしてマシンを設定しています。以降の起動ではこの手順はスキップされます。',
  ...
  onboarding: {
    headerTitle: 'FuLiLian のセットアップをしましょう',
    headerDesc: 'チャットを始めるにはモデルプロバイダーを接続してください。ほとんどのオプションはワンクリックです。',
    preparingInstall: 'Fulilian はインストールを完了中です。初回実行では通常 1 分以内に完了します。',
    starting: 'Fulilian を起動中…',
    ...
    recommended: '推奨',
    connected: '接続済み',
    featuredPitch: '1 つのサブスクリプションで 300 以上の最先端モデル — Fulilian を実行するための推奨方法',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: 'セルフホスト',
        description:
          'ローカルまたはセルフホストの OpenAI 互換エンドポイント（vLLM、llama.cpp、Ollama など）に Fulilian を接続。'
      }
    },
    ...
    flowSubtitles: {
      ...
      device_code: 'ブラウザーで確認ページを開きます — Fulilian が自動接続します',
    ...
    authorizeThere: 'そこで Fulilian を承認してください。',
    copyAuthCode: '認証コードをコピーして以下に貼り付けてください。',
    pasteAuthCode: '認証コードを貼り付け',
    reopenAuthPage: '認証ページを再度開く',
    autoBrowser: provider =>
      `${provider} をブラウザーで開きました。Fulilian をそこで承認すれば自動接続されます。コピーや貼り付けは不要です。`,
  ...
  preview: {
    ...
    largeBody: (label, size) => `${label} は ${size} です。Fulilian は最初の 512 KB のみを表示します。`,
    ...
    web: {
      ...
      tryAgain: '再試行',
      restarting: 'Fulilian を再起動中...',
      askRestart: 'Fulilian にサーバーの再起動を依頼',
      lookingRestart: taskId => `Fulilian は再起動するプレビューサーバーを検索中です (${taskId})`,
      restartingTitle: 'プレビューサーバーを再起動中',
      restartingMessage: 'Fulilian はバックグラウンドで作業中です。進捗はプレビューコンソールで確認してください。',
      ...
      blankPageBody: '上のアドレス欄に入力するか、Fulilian にページを開くよう頼んでください。',
      finishedRestarting: message =>
        `Fulilian がプレビューサーバーの再起動を完了しました${message ? `: ${message}` : ''}`,
      ...
      restartFailedMessage: 'Fulilian がサーバーを再起動できませんでした。',
      stillWorking:
        'Fulilian はまだ作業中ですが、再起動の結果がまだ届いていません。サーバーコマンドがフォアグラウンドで実行されている可能性があります。',
  ...
  assistant: {
    thread: {
      ...
      loadingResponse: 'Fulilian が応答を読み込み中',
    ...
    approval: {
      gatewayDisconnected: 'Fulilian ゲートウェイが接続されていません',
      ...
      alwaysDescription: pattern =>
        `これにより "${pattern}" パターンが永続的な許可リスト (~/.fulilian/config.yaml) に追加されます。Fulilian はこのセッションや将来のセッションで、このようなコマンドについて再度尋ねません。`,
      alwaysAllow: '常に許可'
    },
    clarify: {
  ...
  prompts: {
    ...
    sudoTitle: '管理者パスワード',
    sudoDesc:
      'Fulilian は特権コマンドを実行するために sudo パスワードが必要です。ローカルエージェントにのみ送信されます。',
    sudoPlaceholder: 'sudo パスワード',
    secretTitle: 'シークレットが必要です',
    secretDesc: 'Fulilian は続行するための認証情報が必要です。',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '画像を保存するには Fulilian Desktop を再起動してください。',
    restartToSaveImages: '画像を保存するには Fulilian Desktop を再起動してください',
    ...
    handoff: {
      ...
      timedOut: 'ゲートウェイの待機がタイムアウトしました。`fulilian gateway` は起動していますか？'
    }
  },
  ...
  tips: {
    ...
    items: {
      ...
      skills: {
        title: '一度教えれば覚えます',
        text: 'スキルは手順書のフォルダで、必要な場面で Fulilian が自分で読み込みます。'
      },
      messaging: {
        title: 'デスクを離れても Fulilian',
        text: 'Telegram、Discord、Slack などに接続。同じエージェント、同じ記憶のままです。'
      },
      artifacts: {
        title: 'Fulilian が作ったものすべて',
      ...
      profiles: {
        ...
        text: 'それぞれが独自のキー・メモリ・セッションを持つ、別の Fulilian です。'
      },
      'composer-mentions': {
        title: 'ファイルとコマンド',
