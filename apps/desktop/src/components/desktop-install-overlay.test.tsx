...
describe('DesktopInstallOverlay first-run setup', () => {
  it('shows the remote/local choice without installer progress', async () => {
    installDesktopMock(
      bootstrapState({
        setupChoice: { platform: 'win32', activeRoot: 'C:\\Users\\me\\AppData\\Local\\fulilian\\fulilian-agent' }
      })
    )

    render(<DesktopInstallOverlay />)

    expect(await screen.findByText('Set up Fulilian Desktop')).toBeTruthy()
    expect(screen.getByText('Connect to existing Fulilian')).toBeTruthy()
    expect(screen.getByText('Install Fulilian locally')).toBeTruthy()
    expect(screen.queryByText(/steps complete/i)).toBeNull()
    expect(screen.queryByText(/Fetching installer manifest/i)).toBeNull()
  })

  it('continues local bootstrap only when Install Fulilian locally is selected', async () => {
    ...
    fireEvent.click(await screen.findByText('Install Fulilian locally'))

    expect(desktop.continueBootstrapLocal).toHaveBeenCalledTimes(1)
    expect(screen.getByText('Set up Fulilian Desktop')).toBeTruthy()

    act(() => {
      desktop.emitBootstrapEvent({ type: 'manifest', protocolVersion: 1, stages: [] })
    })

    await waitFor(() => expect(screen.queryByText('Set up Fulilian Desktop')).toBeNull())
  ...
  it('surfaces a recoverable error when the local-bootstrap bridge is unavailable', async () => {
    ...
    const install = (await screen.findByText('Install Fulilian locally')).closest('button') as HTMLButtonElement
    fireEvent.click(install)

    expect(
      await screen.findByText('Local installation could not start. Restart Fulilian Desktop and try again.')
    ).toBeTruthy()
  ...
  it('keeps the local-start error when the first snapshot commits under the click', async () => {
    ...
    const install = (await whenPresent('Install Fulilian locally')).closest('button') as HTMLButtonElement
    fireEvent.click(install)
    ...
    expect(screen.queryByText('Local installation could not start. Restart Fulilian Desktop and try again.')).toBeTruthy()
  })
  ...
  it('clears a stale local-start error when a repair presents a different root', async () => {
    ...
    fireEvent.click((await screen.findByText('Install Fulilian locally')).closest('button') as HTMLButtonElement)
    expect(
      await screen.findByText('Local installation could not start. Restart Fulilian Desktop and try again.')
    ).toBeTruthy()
    ...
    act(() => {
      desktop.emitBootstrapEvent({
        ...
        active: false,
        platform: 'win32',
        activeRoot: 'C:\\Users\\me\\AppData\\Local\\fulilian\\fulilian-agent-repaired'
      })
    })

    expect(screen.queryByText('Local installation could not start. Restart Fulilian Desktop and try again.')).toBeNull()
  })

  it('opens the remote connection form from the first-run choice', async () => {
    installDesktopMock(
      bootstrapState({
        setupChoice: { platform: 'linux', activeRoot: '/home/me/.fulilian/fulilian-agent' }
      })
    )

    render(<DesktopInstallOverlay />)

    fireEvent.click(await screen.findByText('Connect to existing Fulilian'))
  ...
  it('requires a successful token connection test before applying remote config', async () => {
    ...
    desktop.probeConnectionConfig.mockResolvedValue({
      authMode: 'token',
      baseUrl: 'https://gateway.example.com/fulilian',
      error: null,
      providers: [],
      reachable: true,
      version: '0.17.0'
    ...
    fireEvent.click(await screen.findByText('Connect to existing Fulilian'))
    fireEvent.change(await screen.findByPlaceholderText('https://gateway.example.com/fulilian'), {
      target: { value: 'https://gateway.example.com/fulilian' }
    })
    ...
    await waitFor(() => {
      expect(desktop.testConnectionConfig).toHaveBeenCalledWith({
        ...
        remoteUrl: 'https://gateway.example.com/fulilian'
      })
    })

    await screen.findByText('Connected to https://gateway.example.com/fulilian (0.17.0).')
  ...
  it('ignores a completed probe after the gateway URL becomes invalid', async () => {
    ...
    fireEvent.click(await screen.findByText('Connect to existing Fulilian'))
    const urlInput = await screen.findByPlaceholderText('https://gateway.example.com/fulilian')
    fireEvent.change(urlInput, { target: { value: 'https://gateway.example.com/fulilian' } })
  ...
  it('does not enable Apply when credentials change during a connection test', async () => {
    ...
    await act(async () => {
      resolveTest?.({ baseUrl: 'https://gateway.example.com/fulilian', ok: true, version: '0.17.0' })
      await pendingTest
    })

    expect(screen.queryByText('Connected to https://gateway.example.com/fulilian (0.17.0).')).toBeNull()
    expect(apply.disabled).toBe(true)
  })
  ...
  it('signs in, tests, and applies a password-style remote gateway', async () => {
    ...
    await waitFor(() => {
      expect(desktop.oauthLoginConnectionConfig).toHaveBeenCalledWith('https://gateway.example.com/fulilian')
    })
    ...
    await screen.findByText('Connected to https://gateway.example.com/fulilian.')
    const apply = screen.getByText('Apply and reconnect').closest('button') as HTMLButtonElement
    expect(apply.disabled).toBe(false)
    fireEvent.click(apply)
  ...
  it('offers remote connection from the unsupported packaged install screen', async () => {
    const desktop = installDesktopMock(
      bootstrapState({
        unsupportedPlatform: {
          platform: 'darwin',
          activeRoot: '/Users/me/.fulilian/fulilian-agent',
    ...
    expect(await screen.findByText('Fulilian needs a one-time install')).toBeTruthy()
    ...
    fireEvent.change(screen.getByPlaceholderText('https://gateway.example.com/fulilian'), {
      target: { value: 'https://gateway.example.com/fulilian' }
    })
    ...
    await waitFor(() => expect(screen.queryByText('Gateway URL')).toBeNull())
    expect(screen.queryByText('Fulilian needs a one-time install')).toBeNull()
  })
})
