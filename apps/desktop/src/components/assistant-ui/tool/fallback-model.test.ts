...
describe('buildToolView web-search query', () => {
  it('keeps the query separate from structured search results', () => {
    const view = buildToolView(
      part({
        args: { query: 'FuLiLian Desktop tool calls' },
        result: { web: [{ snippet: 'Desktop docs', title: 'Fulilian docs', url: 'https://example.com/docs' }] },
        toolName: 'web_search'
      }),
      ''
    )

    expect(view.searchQuery).toBe('FuLiLian Desktop tool calls')
    expect(view.searchHits).toEqual([
      { snippet: 'Desktop docs', title: 'Fulilian docs', url: 'https://example.com/docs' }
    ])
  })
})
  ...
  it('shows failed title when navigate returns success=false', () => {
    const view = buildToolView(
      part({
        ...
        args: { url: 'https://github.com/xujuan-cyber/fulilian/docs' },
  ...
  it('shows opened title on success', () => {
    const view = buildToolView(
      part({
        toolName: 'browser_navigate',
        args: { url: 'https://github.com/xujuan-cyber/fulilian/docs' },
        result: { success: true, url: 'https://github.com/xujuan-cyber/fulilian/docs', title: 'Docs' }
      }),
      ''
    )
