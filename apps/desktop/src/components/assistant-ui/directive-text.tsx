...
const FULILIAN_REF_TYPES = WIRE_REFERENCE_KINDS
type FulilianRefType = (typeof FULILIAN_REF_TYPES)[number]
...
const FULILIAN_DIRECTIVE_RE = referenceRe()
...
export const fulilianDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(FULILIAN_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...fulilianDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Fulilian directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.fulilianDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.fulilianDesktop?.readFileDataUrl(id)
