...
function sessionsBotsTree() {
  ...
  registerPane('fulilian-bots:pane', { placement: 'left', hideOnly: true })
...
describe('hide-only strip tabs', () => {
  it('hides and shows a chrome tab, keeping the pane in the tree', () => {
    ...
    expect(setStripTabHidden('fulilian-bots:pane', true)).toBe(true)
    expect($hiddenTreePanes.get()).toContain('fulilian-bots:pane')
    expect($hiddenStripTabs.get()).toContain('fulilian-bots:pane')
    // Hidden, not dismissed: the pane stays in the layout tree.
    expect(allPaneIds($layoutTree.get()!)).toContain('fulilian-bots:pane')

    expect(setStripTabHidden('fulilian-bots:pane', false)).toBe(true)
    expect($hiddenTreePanes.get()).not.toContain('fulilian-bots:pane')
    expect($hiddenStripTabs.get()).not.toContain('fulilian-bots:pane')
  })

  it('refuses to hide the zone last visible tab', () => {
    sessionsBotsTree()
    setStripTabHidden('fulilian-bots:pane', true)
  ...
  it('persists hides and clears them on reveal', () => {
    ...
    const persisted = JSON.parse(window.localStorage.getItem('fulilian.desktop.hiddenStripTabs.v1') ?? '[]')

    expect(persisted).toContain('fulilian-bots:pane')
    ...
    revealTreePane('fulilian-bots:pane')
    expect($hiddenTreePanes.get()).not.toContain('fulilian-bots:pane')
    expect(window.localStorage.getItem('fulilian.desktop.hiddenStripTabs.v1')).toBeNull()
  })
  ...
  it('lists the zone hide-only tabs with live hidden state for the menu', () => {
    ...
    expect(hideOnlyZoneTabs('g-side')).toEqual([
      ...
      { hidden: true, id: 'fulilian-bots:pane', title: 'fulilian-bots:pane' }
    ])
  ...
  it('excludes hide-only tabs from every close verb', () => {
    ...
    $layoutTree.set(group(['sessions', 'fulilian-bots:pane', 'session-tile:x'], { active: 'sessions', id: 'g-mixed' }))
