...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Fulilian doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in fulilian_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `fulilian model` uses — instead of
...
export interface FulilianConfig {
  agent?: {
...
export type FulilianConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `fulilian model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// non-desktop) Fulilian simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (fulilian.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors fulilian_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Fulilian, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `fulilian tools` active-provider detection). */
  is_active: boolean
 ...
 *  not Fulilian); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** fulilian-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** fulilian-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/fulilian/update/check` — the backend's own update state.
