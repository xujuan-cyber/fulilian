...
// /api/profiles/active `current`). "default" is the root ~/.fulilian. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'fulilian.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'fulilian.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await fulilianApi<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new FULILIAN_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.fulilianDesktop.profile.set(name)
}
...
async function resolveConnectionForProfile(profile: string): Promise<FulilianConnection | null> {
  const getConnection = window.fulilianDesktop?.getConnection
...
// previous descriptor.
async function resolveConnectionForAgent(connectionId: string, profile: string): Promise<FulilianConnection | null> {
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'fulilian.desktop.showAllProfiles'
...
export function selectProfile(name: string): void {
  ...
  void Promise.all([activateOnCurrentSource(target), shouldRememberStartupProfile])
    .then(([, shouldRemember]) => {
      if (shouldRemember) {
        return window.fulilianDesktop?.profile?.remember(target)
      }
...
// Switch to the default (root ~/.fulilian) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.fulilianDesktop?.touchBackend?.(target).catch(() => undefined)
}
