...
import { type FulilianOpenTarget, resolveFulilianOpenPath } from '@/lib/fulilian-open-target'
...
export type { FulilianOpenTarget }
...
const STORAGE_KEY = 'fulilian:native-notifications'
...
// "Backgrounded" = the user isn't on Fulilian. `document.hidden` only flips when
...
export interface NativeNotificationInput {
   ...
   * `sessionId` (plugins). Same vocabulary as `fulilian://index-network/intent/1`.
   */
  activate?: string
...
export function dispatchNativeNotification(input: NativeNotificationInput): boolean {
  ...
  void window.fulilianDesktop?.notify({
...
export interface PluginNotificationAction {
  id: string
  label: string
  /** Navigate here on button press (path or `fulilian://index-network/intent/1`). */
  activate?: FulilianOpenTarget
 ...
 *  user is away from Fulilian — the in-app toast (`host.notify`) covers the
 *  foreground case. */
export function dispatchPluginNativeNotification(pluginId: string, input: PluginNativeNotificationInput): void {
  const activate = resolveFulilianOpenPath(input.activate) ?? undefined
  ...
  const actions: NativeNotificationAction[] | undefined = input.actions?.map(action => ({
    activate: resolveFulilianOpenPath(action.activate) ?? undefined,
    id: action.id,
    text: action.label
  }))
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.fulilianDesktop
