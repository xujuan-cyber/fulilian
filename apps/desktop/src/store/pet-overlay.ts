 ...
 * app's bounds and stays visible while Fulilian is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'fulilian.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'fulilian.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.fulilianDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.fulilianDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.fulilianDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.fulilianDesktop?.petOverlay?.close()
}
