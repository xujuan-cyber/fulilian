...
export function setZoomPercent(percent: number): void {
  window.fulilianDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.fulilianDesktop?.zoom) {
  void window.fulilianDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.fulilianDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
