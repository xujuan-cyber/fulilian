...
export function canOpenSessionWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.fulilianDesktop?.openSessionWindow === 'function'
}

// True when the shell can open a full peer app window (⌘⇧N / "New Window").
export function canOpenNewWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.fulilianDesktop?.openWindow === 'function'
}

// True when the shell can pop the in-app Browser into its own OS window.
export function canOpenBrowserWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.fulilianDesktop?.openBrowserWindow === 'function'
}
...
export function canOpenSessionInTerminal(): boolean {
  return typeof window !== 'undefined' && typeof window.fulilianDesktop?.openSessionInTerminal === 'function'
}
...
export async function openSessionInNewWindow(sessionId: string, opts?: { watch?: boolean }): Promise<void> {
  ...
  await runWindowOpen(
    () => window.fulilianDesktop.openSessionWindow(sessionId, opts),
...
export async function openNewWindow(): Promise<void> {
  ...
  await runWindowOpen(() => window.fulilianDesktop.openWindow(), 'Could not open a new window')
}
...
export async function openBrowserInNewWindow(tabId: string): Promise<boolean> {
  ...
  return runWindowOpen(() => window.fulilianDesktop.openBrowserWindow(tabId), 'Could not pop out browser')
}
...
export async function openSessionInTerminal(
  ...
  await runWindowOpen(
    () => window.fulilianDesktop.openSessionInTerminal(sessionId, opts),
