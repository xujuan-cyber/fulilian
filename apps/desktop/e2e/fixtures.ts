/**
 * Shared E2E fixtures for the Fulilian desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → fulilian serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `fulilian desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `fulilian-e2e-${prefix}-${Math.random()}`))
  const fulilianHome = path.join(root, 'fulilian-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(fulilianHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(fulilianHome, 'config.yaml')
 ...
 */
export function writeEnvFile(fulilianHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(fulilianHome, '.env')
 ...
 */
function writeEmptyConfig(fulilianHome: string): void {
  const configPath = path.join(fulilianHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - FULILIAN_HOME → sandbox fulilian-home (isolated config/sessions)
 *  - FULILIAN_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - FULILIAN_DESKTOP_IGNORE_EXISTING=1 → don't pick up `fulilian` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - FULILIAN_DESKTOP_FULILIAN_ROOT → REPO_ROOT (dev checkout resolution)
 *  - FULILIAN_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    FULILIAN_HOME: sandbox.fulilianHome,
    FULILIAN_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    FULILIAN_DESKTOP_IGNORE_EXISTING: '1',
    FULILIAN_DESKTOP_FULILIAN_ROOT: REPO_ROOT,
    FULILIAN_DESKTOP_APP_NAME: `FulilianE2E-${Date.now()}`,
    ...
    FULILIAN_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated FULILIAN_HOME + userData
 * @param env      - the process environment (already has FULILIAN_HOME etc.)
...
export async function setupMockBackend(options: MockBackendOptions = {}): Promise<MockBackendFixture> {
  ...
  writeMockProviderConfig(
    sandbox.fulilianHome,
    mock.url,
    options.extraDisplayConfig,
    options.extraConfig,
    options.modelContextLength,
  )
  writeEnvFile(sandbox.fulilianHome)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.fulilianHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via FULILIAN_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`fulilian serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.fulilianHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { FULILIAN_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Fulilian backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Fulilian.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Fulilian.app', 'Contents', 'MacOS', 'Fulilian')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'fulilian')
}
 ...
 * progress without spawning a real Fulilian backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * FULILIAN_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    FULILIAN_DESKTOP_BOOT_FAKE: '1',
    FULILIAN_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + fulilian-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).FULILIAN_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).FULILIAN_DESKTOP_FULILIAN
  delete (env as Record<string, string | undefined>).FULILIAN_DESKTOP_FULILIAN_ROOT
