 ...
 * second gateway, "Homelab", is a REAL second `fulilian serve` this spec spawns
 * with its own FULILIAN_HOME, profiles and session token, registered in the v2
...
function findFulilianBinary(): string {
  const venv = path.join(REPO_ROOT, '.venv', 'bin', 'fulilian')

  if (fs.existsSync(venv)) {
    return venv
  }

  const result = spawnSync('which', ['fulilian'], { encoding: 'utf8' })

  if (result.status === 0 && result.stdout.trim()) {
    return result.stdout.trim()
  }

  throw new Error('fulilian binary not found: create the repo venv (uv sync) or put fulilian on PATH')
}
...
/**
 * Spawn a second, fully real `fulilian serve` as the remote gateway. Its
 * session token is pinned through FULILIAN_DASHBOARD_SESSION_TOKEN so the
...
async function startRemoteGateway(root: string, mockUrl: string, profiles: string[]): Promise<RemoteGateway> {
  ...
  const child: ChildProcess = spawn(
    ...
    ['serve', '--host', '127.0.0.1', '--port', String(port), '--skip-build'],
    {
      ...
      env: {
        ...process.env,
        FULILIAN_HOME: home,
        FULILIAN_DASHBOARD_SESSION_TOKEN: REMOTE_TOKEN,
      },
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(`remote fulilian serve exited early (${child.exitCode}):\n${log}`)
    }

    try {
      const response = await fetch(`${url}/api/status`, {
        headers: { 'X-Fulilian-Session-Token': REMOTE_TOKEN },
      })
  ...
  if (Date.now() >= deadline) {
    throw new Error(`remote fulilian serve never became ready:\n${log}`)
  }
...
test.describe('fleet profile rail — two registered gateways', () => {
  ...
  test.beforeAll(async () => {
    ...
    writeMockProviderConfig(sandbox.fulilianHome, mock.url)
    ...
    seedProfiles(sandbox.fulilianHome, ['research'])
