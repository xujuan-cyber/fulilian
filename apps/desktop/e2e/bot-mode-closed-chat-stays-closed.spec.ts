...
async function seedBot(fulilianHome: string, mockUrl: string, name: string): Promise<void> {
  const dir = path.join(fulilianHome, 'profiles', name)
...
test.beforeAll(async () => {
  ...
  await seedBot(sandbox.fulilianHome, mock.url, 'alpha')
  await seedBot(sandbox.fulilianHome, mock.url, 'beta')
