...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  fs.appendFileSync(
    path.join(sandbox.fulilianHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.fulilianHome)
  const builder = await RealSessionBuilder.start(sandbox.fulilianHome)
  try {
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.fulilianHome, mock.url)
  fs.appendFileSync(path.join(sandbox.fulilianHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
  writeEnvFile(sandbox.fulilianHome)
  const { app, page } = await launchDesktop(buildAppEnv(sandbox))
