...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'Fulilian E2E'], { cwd: repo })
...
function configureRepoCwd(fulilianHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(fulilianHome, mockUrl)
  fs.appendFileSync(path.join(fulilianHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(fulilianHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.fulilianHome, mock.url, repo)
