...
<p align="center">
  ...
  <a href="https://github.com/xujuan-cyber/fulilians/"><img src="https://img.shields.io/badge/Docs-hermes--agent.nousresearch.com-FFD700?style=for-the-badge" alt="Documentation"></a>
  <a href="https://discord.gg/NousResearch"><img src="https://img.shields.io/badge/Discord-5865F2?style=for-the-badge&logo=discord&logoColor=white" alt="Discord"></a>
  <a href="https://github.com/NousResearch/hermes-agent/blob/main/LICENSE"><img src="https://img.shields.io/badge/License-MIT-green?style=for-the-badge" alt="License: MIT"></a>
</p>

**The native desktop app for [FuLiLian](../../README.md) — the self-improving AI agent from [Nous Research](https://nousresearch.com).** Same agent, same skills, same memory as the CLI and gateway, in a polished native window — chat with streaming tool output, side-by-side previews, a file browser, voice, and settings, no terminal required. Available for **macOS, Windows, and Linux**.

<table>
<tr><td><b>Chat with the full agent</b></td><td>Streaming responses, live tool activity, structured tool summaries, and the same conversation history as every other Fulilian surface.</td></tr>
<tr><td><b>Side-by-side previews</b></td><td>Render web pages, files, and tool outputs in a right-hand pane while you keep chatting.</td></tr>
<tr><td><b>File browser</b></td><td>Explore and preview the working directory without leaving the app.</td></tr>
<tr><td><b>Voice</b></td><td>Talk to Fulilian and hear it back.</td></tr>
...
### Install with Fulilian (recommended)

Already have the Fulilian CLI? Just run:

```bash
fulilian desktop
```

It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing Fulilian gateway or install Fulilian locally. Local onboarding then walks you through choosing a provider and model.

### Prebuilt installers

Prebuilt installers are built and distributed via [the Fulilian Desktop website.](https://github.com/xujuan-cyber/fulilian).
...
```bash
# throwaway FULILIAN_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the FuLiLian runtime into `FULILIAN_HOME`
(`~/.fulilian`, or `%LOCALAPPDATA%\fulilian` on Windows), using the same layout as a
CLI install.
...
- **FuLiLian** runs as a headless `fulilian serve` process and exposes the
...
1. `FULILIAN_DESKTOP_FULILIAN_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `FULILIAN_DESKTOP_FULILIAN`, or `fulilian` on `PATH`
5. a system Python that can import the Fulilian runtime
...
Desktop supports a managed local backend, explicit remote gateways, and Fulilian
...
screen offers **Connect to existing Fulilian** before starting the local installer.
...
terminal commands, and file operations run against the remote Fulilian host, not
...
transport- or Fulilian-managed header names such as `Authorization`, `Cookie`,
`Host`, `Origin`, `Referer`, and `X-Fulilian-Session-Token`.
...
Boot logs land in `FULILIAN_HOME/logs/desktop.log` (includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure.
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\fulilian\fulilian-agent\.fulilian-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\fulilian\fulilian-agent\venv"
```

> The default Fulilian home on Windows is `%LOCALAPPDATA%\fulilian`. Set the `FULILIAN_HOME` env var if you've relocated it.
