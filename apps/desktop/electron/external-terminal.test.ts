...
test('terminalScriptEnv drops PATH in any casing and keeps the rest', () => {
  ...
  assert.deepEqual(env, { PYTHONPATH: '/repo', PYTHONUTF8: '1', FULILIAN_HOME: '/home/b/.fulilian' })
})
...
test('buildTerminalScript execs the resolved runtime with its env', () => {
  const script = buildTerminalScript({
    args: ['-m', 'fulilian_cli.main', '--tui', '--resume', 'sess'],
    command: '/home/b/.fulilian/fulilian-agent/venv/bin/python',
    cwd: "/home/b/o'brien",
    env: { PYTHONPATH: '/home/b/.fulilian/fulilian-agent' },
    platform: 'darwin'
  })
  ...
  assert.equal(
    ...
    [
      ...
      `export PYTHONPATH='/home/b/.fulilian/fulilian-agent'`,
      `exec '/home/b/.fulilian/fulilian-agent/venv/bin/python' '-m' 'fulilian_cli.main' '--tui' '--resume' 'sess'`,
      ''
    ].join('\n')
  )
})
...
test('buildTerminalScript emits a cmd script on Windows', () => {
  const script = buildTerminalScript({
    ...
    command: 'C:\\fulilian\\venv\\Scripts\\fulilian.exe',
    cwd: 'C:\\Users\\b',
    env: { PYTHONUTF8: '1' },
    platform: 'win32'
  })
  ...
  assert.deepEqual(script.split('\r\n'), [
    ...
    'cd /d "C:\\Users\\b"',
    'set "PYTHONUTF8=1"',
    '"C:\\fulilian\\venv\\Scripts\\fulilian.exe" "--tui" "--resume" "sess"',
    ''
  ])
})
