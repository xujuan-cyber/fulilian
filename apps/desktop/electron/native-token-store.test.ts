...
const CRED_GATEWAY = 'https://alice:supersecret@gw.example.com/fulilian'
...
test('a decryption failure logs the gateway host and path but not its credentials', () => {
  ...
  assert.match(locked.logs[0], /failed to decrypt stored tokens for https:\/\/gw\.example\.com\/fulilian/)
  assert.match(locked.logs[0], /keeping stored entry for retry/)
...
test('a parsing failure logs the gateway host and path but not its credentials', () => {
  ...
  assert.match(disk.logs[0], /failed to load stored tokens for https:\/\/gw\.example\.com\/fulilian/)
...
test('the credential-bearing base URL stays the exact store key', () => {
  ...
  assert.equal(loadNativeTokenSet('https://gw.example.com/fulilian', createFakeDisk(first.fileText()).io), null)
})
