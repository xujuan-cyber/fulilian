...
const READY_RE = /^FULILIAN_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitFulilianPath = '') {
  const explicit = psLiteral(explicitFulilianPath)
  ...
  const script = [
    ...
    '$fulilianHome=$env:FULILIAN_HOME',
    'if(-not $fulilianHome){$fulilianHome=Join-Path $env:LOCALAPPDATA "fulilian"}',
    'Assert-NoReparse $fulilianHome $true',
    '$candidate=[IO.Path]::Combine($fulilianHome, "fulilian-agent\\venv\\Scripts\\fulilian.exe")',
    ...
    '$profileCandidate=[IO.Path]::Combine($HOME, "fulilian-agent\\.venv\\Scripts\\fulilian.exe")',
    ...
    '$fallbackHomeCandidate=Join-Path $fulilianHome "fulilian-agent\\venv\\Scripts\\fulilian.exe"',
    '$fallbackProfileCandidate=Join-Path $HOME "fulilian-agent\\.venv\\Scripts\\fulilian.exe"',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command fulilian.exe -ErrorAction SilentlyContinue',
    ...
    '$fulilian=$null',
    'foreach($candidate in $candidates){Assert-NoReparse $candidate $true;$candidatePython=[IO.Path]::Combine([IO.Path]::GetDirectoryName($candidate), "python.exe");Assert-NoReparse $candidatePython $true;try{$item=Get-Item -LiteralPath $candidate -Force -ErrorAction Stop;if(($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0 -and -not $item.PSIsContainer){$fulilian=$item.FullName;break}}catch [Management.Automation.ItemNotFoundException]{continue}}',
    'if(-not $fulilian){throw "Fulilian is not installed on the remote Windows host."}',
    'Assert-NoReparse $fulilian $false',
    'if($explicit -and $fulilian -ne $explicit){throw "The configured Fulilian path is not an executable file."}',
    '$python=[IO.Path]::Combine([IO.Path]::GetDirectoryName($fulilian), "python.exe")',
    'Assert-NoReparse $python $false',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;fulilianHome=$fulilianHome;fulilianPath=$fulilian;python=$python}|ConvertTo-Json -Compress'
  ].join(';')

  return JSON.parse((await ssh.exec(powerShellCommand(script))).trim())
}

function windowsUpdateMarkerProbeCommand(fulilianHome) {
  const script = [
    '$ErrorActionPreference="Stop"',
...
public static class FulilianMarkerNoFollow {
...
`,
    ...
    '}',
    '}',
    `$home=${psLiteral(fulilianHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".fulilian-update-in-progress"',
    '$result="UNCERTAIN"',
    '$stream=$null;$memory=$null',
    'try{',
    'Assert-NoReparse $marker $true',
    'if(-not (Test-Path -LiteralPath $marker -PathType Leaf)){$result="CLEAR"}else{$stream=[FulilianMarkerNoFollow]::OpenRead($marker)',
 ...
 */
async function assertWindowsRemoteInstallUpdateClear(ssh, fulilianHome) {
  let observation = ''

  try {
    observation =
      String(await ssh.exec(windowsUpdateMarkerProbeCommand(fulilianHome)))
        .replace(/^\uFEFF/, '')
        .trim()
        .split(/\r?\n/)
        .pop() || ''
...
async function detectRemotePlatform(ssh, explicitFulilianPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitFulilianPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'fulilian_cli.windows_ssh_runtime', operation, ...args]
...
function atomicWindowsSpawnCommand(runtime, reservation: any = {}) {
  const argv = [runtime.python, '-m', 'fulilian_cli.windows_ssh_runtime', 'spawn']
  const helper = operation => [runtime.python, '-m', 'fulilian_cli.windows_ssh_runtime', operation]

  const script = [
    '$ErrorActionPreference="Stop"',
    `$home=${psLiteral(runtime.fulilianHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".fulilian-update-in-progress"',
    ...
    reservation.ownershipId
      ...
      : '  if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}',
    reservation.ownershipId
      ? `  $spawned=$spawnLines[-1]|ConvertFrom-Json; $lock=[ordered]@{schemaVersion=2;protocolVersion=1;ownershipId=${psLiteral(reservation.ownershipId)};spawnNonce=${psLiteral(reservation.spawnNonce)};pid=[int]$spawned.pid;creationTimeNs=[string]$spawned.creationTimeNs;port=0;profile=${psLiteral(reservation.profile)};fulilianPath=${psLiteral(reservation.fulilianPath)};fulilianHome=${psLiteral(reservation.fulilianHome)};tokenFingerprint=${psLiteral(reservation.tokenFingerprint)};startedAt=${psLiteral(reservation.startedAt)}}|ConvertTo-Json -Compress; ` +
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.fulilianPath === 'string' &&
    typeof lock.fulilianHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.fulilianPath === runtime.fulilianPath &&
    lock.fulilianHome === runtime.fulilianHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertBootstrapNotSuperseded(signal)
  const runtime = await probeWindowsRemote(ssh, remoteFulilianPath)
  await assertWindowsRemoteInstallUpdateClear(ssh, runtime.fulilianHome)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.fulilianPath])

  if (!inspection.supported) {
    const error: any = new Error('Update Fulilian on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.fulilianPath = inspection.path
  const fulilianVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located fulilian at ${runtime.fulilianPath}`)
  ...
  try {
    ...
    spawned = await atomicWindowsSpawn(
      ssh,
      runtime,
      JSON.stringify({ ownershipId, spawnNonce, profile, fulilianPath: runtime.fulilianPath }),
      {
        ownershipId,
        spawnNonce,
        profile,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForFulilian(baseUrl, token)
    assertBootstrapNotSuperseded(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Fulilian SSH"', 'powershell.exe -NoLogo')
