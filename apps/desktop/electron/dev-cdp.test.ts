...
test('a packaged build never opens the port, however loudly the env asks', () => {
  const decision = resolveDevCdpPort({ ...devRun, env: { FULILIAN_DESKTOP_CDP_PORT: '9222' }, isPackaged: true })
...
test('packaged is checked before every other gate', () => {
  ...
  for (const value of ['9222', '', 'off', 'garbage']) {
    const decision = resolveDevCdpPort({
      env: { FULILIAN_DESKTOP_CDP_PORT: value },
      isPackaged: true,
...
test('the port is overridable', () => {
  assert.equal(resolveDevCdpPort({ ...devRun, env: { FULILIAN_DESKTOP_CDP_PORT: '9333' } }).port, 9333)
})
...
test('can be switched off on a dev run', () => {
  for (const value of ['0', 'off', 'OFF', 'false', 'no']) {
    const decision = resolveDevCdpPort({ ...devRun, env: { FULILIAN_DESKTOP_CDP_PORT: value } })
