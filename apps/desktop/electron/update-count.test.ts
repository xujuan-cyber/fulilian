...
function createTempGitRepo() {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'fulilian-update-count-'))
  ...
  try {
    ...
    git('config', 'user.name', 'Fulilian Test')
    git('config', 'user.email', 'fulilian@example.invalid')
...
test('compareApiUrl builds the GitHub compare URL for HTTPS origins', () => {
  assert.equal(
    compareApiUrl({
      currentSha: SHA_A,
      originUrl: 'https://github.com/xujuan-cyber/fulilian.git',
      targetSha: SHA_B
    }),
    `https://api.github.com/repos/xujuan-cyber/fulilian/compare/${SHA_A}...${SHA_B}`
  )
})
...
test('compareApiUrl handles SSH origin forms', () => {
  for (const originUrl of [
    ...
    'ssh://git@github.com/xujuan-cyber/fulilian.git',
