...
// hardening, FULILIAN_HOME resolution, and the git binary stay injected.
...
export function registerFsIpc({
  ...
  ipcMain.handle('fulilian:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

  ipcMain.handle('fulilian:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

  // Reveal a path in the OS file manager (Finder / Explorer / Files).
  ipcMain.handle('fulilian:fs:reveal', async (_event, targetPath) => {
  ...
  ipcMain.handle('fulilian:fs:openDir', async (_event, dirPath) => {
  ...
  // The LOCAL Desktop runtime-plugin root: `<FULILIAN_HOME>/desktop-plugins`,
  // resolved from the main-process FULILIAN_HOME (see resolveFulilianHome) — NOT from
  // the connected backend. A remote backend reports its own `fulilian_home` over
  ...
  async function localPluginsRoot(dirName: string): Promise<string> {
    ...
    // profiles/<name>/, matching the profile-scoped fulilian_home the backend
    // reported before this resolver existed. 'default'/unset pins the global root.
    const profile = readActiveDesktopProfile()
    const base = profile && profile !== 'default' ? path.join(fulilianHome, 'profiles', profile) : fulilianHome
  ...
  ipcMain.handle('fulilian:fs:desktopPluginsRoot', async () => localPluginsRoot('desktop-plugins'))

  // The LOCAL logs root (`<FULILIAN_HOME>/logs`, profile-aware) — the error
  // card's "Open Logs" action reveals agent.log/gateway.log without the user
  // knowing where FULILIAN_HOME lives. Same Electron-local resolution as the
  // plugin roots: valid in every connection mode, created on demand.
  ipcMain.handle('fulilian:fs:logsRoot', async () => localPluginsRoot('logs'))

  // The LOCAL agent-plugin root (`<FULILIAN_HOME>/plugins`), same Electron-local
  ...
  ipcMain.handle('fulilian:fs:agentPluginsRoot', async () => localPluginsRoot('plugins'))

  ipcMain.handle('fulilian:plugin:probe', async (_event, payload) => {
  ...
  ipcMain.handle('fulilian:plugin:installDesktop', async (_event, payload) => {
    const identifier = String(payload?.identifier || payload?.repo || '').trim()
  ...
  ipcMain.handle('fulilian:fs:rename', async (_event, targetPath, newName) => {
  ...
  ipcMain.handle('fulilian:fs:writeText', async (_event, filePath, content) => {
  ...
  ipcMain.handle('fulilian:fs:trash', async (_event, targetPath) => {
    const target = String(targetPath || '').trim()
