 ...
 *   1. FULILIAN_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\fulilian\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect FULILIAN_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.FULILIAN_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'fulilian', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'fulilian', 'git', 'usr', 'bin', 'bash.exe'))
  }
