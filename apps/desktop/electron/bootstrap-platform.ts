 ...
 * null to keep it enabled. `FULILIAN_DESKTOP_DISABLE_GPU` overrides detection
...
function detectRemoteDisplay(options: { env?: NodeJS.ProcessEnv; platform?: NodeJS.Platform } = {}) {
  ...
  const override = String(env.FULILIAN_DESKTOP_DISABLE_GPU || '')
    .trim()
    .toLowerCase()

  if (GPU_OVERRIDE_ON.has(override)) {
    return 'override (FULILIAN_DESKTOP_DISABLE_GPU)'
  }
 ...
 * gateway tokens. The `fulilian desktop` launcher detects the session keychain
 * (or reads `desktop.password_store` from config.yaml) and bridges the value
 * in via FULILIAN_DESKTOP_PASSWORD_STORE.
 *
...
function resolveLinuxPasswordStore(options: { env?: NodeJS.ProcessEnv; platform?: NodeJS.Platform } = {}) {
  ...
  const requested = String(env.FULILIAN_DESKTOP_PASSWORD_STORE || '').trim()
  ...
  if (!LINUX_PASSWORD_STORES.has(requested)) {
    return { store: null, warning: `ignoring unknown FULILIAN_DESKTOP_PASSWORD_STORE value: ${requested}` }
  }
