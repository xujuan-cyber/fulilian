...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'fulilian serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/fulilian serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m fulilian_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/fulilian --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\Fulilian Runtime\\fulilian.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('fulilian chat --query serve'), false)
