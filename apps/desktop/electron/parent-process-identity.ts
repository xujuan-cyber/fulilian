export type ParentWatchdogEnv = {
  FULILIAN_PARENT_PID: string
  FULILIAN_PARENT_START_MARKER?: string
  FULILIAN_PARENT_NONCE?: string
  /** Spawn tag consumed by fulilian_cli.process_identity (`v1:<install>:<purpose>:<spawner_pid>:<spawner_create_s>`).
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { FULILIAN_PARENT_PID: String(pid) }
  env.FULILIAN_SPAWN = spawnTag(pid, startMarker)
  ...
  env.FULILIAN_PARENT_START_MARKER = startMarker
  env.FULILIAN_PARENT_NONCE = nonce

  return env
}

/** Build the `FULILIAN_SPAWN` tag mirrored by fulilian_cli/process_identity.py.
