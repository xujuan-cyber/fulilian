...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(FULILIAN_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Fulilian-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.fulilian/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'fulilian-desktop-ssh')
  }

  return path.join(os.homedir(), '.fulilian', 'desktop-ssh')
}
