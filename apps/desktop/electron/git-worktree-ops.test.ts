...
test('parseWorktrees: main checkout + linked worktree', () => {
  const out = [
    ...
    'HEAD def456',
    'branch refs/heads/fulilian/feat',
    ''
  ].join('\n')
  ...
  assert.equal(trees[1].branch, 'fulilian/feat')
})
...
test('ensureGitRepo: inits a plain dir with a root commit so worktrees branch', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'fulilian-wt-'))
...
test('switchBranch: switches a normal checkout branch', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'fulilian-switch-'))
  const git = (...args) => execFileSync('git', args, { cwd: dir }).toString().trim()
...
test('listBranches: lists locals and flags the checked-out branch', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'fulilian-branches-'))
...
test('listBranches: flags a free default branch as default, not checked out', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'fulilian-branches-default-'))
  const git = (...args) => execFileSync('git', args, { cwd: dir }).toString().trim()
...
test('listBranches: a branch claimed by a worktree is flagged checked out', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'fulilian-branches-wt-'))
...
test('listBranches: empty on a non-repo path', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'fulilian-nonrepo-'))
...
test('addWorktree: existingBranch checks the branch out without a new branch', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'fulilian-convert-'))
  const git = (...args) => execFileSync('git', args, { cwd: dir }).toString().trim()
...
test('addWorktree: existing default branch switches the main checkout, not .worktrees/main', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'fulilian-convert-default-'))
  const git = (...args) => execFileSync('git', args, { cwd: dir }).toString().trim()
...
test('listBaseBranches: lists local branches and flags the default', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'fulilian-base-branches-'))
  const git = (...args) => execFileSync('git', args, { cwd: dir }).toString().trim()
...
test('listBaseBranches: empty on a non-repo path', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'fulilian-base-nonrepo-'))
...
test('addWorktree: base param branches off a specified local branch', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'fulilian-base-add-'))
  const git = (...args) => execFileSync('git', args, { cwd: dir }).toString().trim()
...
test('addWorktree: base origin/main does not set up upstream tracking', async () => {
  ...
  const remoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'fulilian-remote-'))
  const cloneDir = fs.mkdtempSync(path.join(os.tmpdir(), 'fulilian-clone-'))
...
function seedRemoteAndClone(label, branches) {
  const remoteDir = fs.mkdtempSync(path.join(os.tmpdir(), `fulilian-${label}-remote-`))
  const cloneDir = fs.mkdtempSync(path.join(os.tmpdir(), `fulilian-${label}-clone-`))
  ...
  execFileSync('git', ['init', '-b', 'main', remoteDir])
  remoteGit('-c', 'user.email=fulilian@localhost', '-c', 'user.name=Fulilian', 'commit', '--allow-empty', '-m', 'root')
...
test('switchBranch: non-repo dir short-circuits instead of throwing', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'fulilian-sw-'))
