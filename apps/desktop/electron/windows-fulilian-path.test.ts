// Unit tests for the pure Windows `fulilian` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvFulilianCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `fulilian` shim shadows
//      the real fulilian.cmd/fulilian.exe.
...
//   3. resolveVenvFulilianCommand() — must probe the venv python via
//      canImportFulilianCli() before trusting it, or a broken venv gets
...
test('chooseUpdaterArgs: marker-only install uses --repair when the venv is gone', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvFulilian: false, hasVenvPython: false }, 'main'),
    ['--repair', '--branch', 'main']
  )
})
...
test('chooseUpdaterArgs: passes the branch through unchanged in both modes', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvFulilian: true, hasVenvPython: true }, 'release/1.2'),
    ['--update', '--branch', 'release/1.2']
  )
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvFulilian: false, hasVenvPython: false }, 'release/1.2'),
    ['--repair', '--branch', 'release/1.2']
  )
})

function makeDeps(overrides: Partial<Parameters<typeof resolveVenvFulilianCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportFulilianCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    fulilianHome: '/fake/fulilian-home',
...
test('resolveVenvFulilianCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvFulilianCommand('/root/venv/Scripts/fulilian.exe', [], deps), null)
})

test('resolveVenvFulilianCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvFulilianCommand('/root/venv/Scripts/fulilian.cmd', [], deps), null)
})

test('resolveVenvFulilianCommand: returns null when the basename is not fulilian/fulilian.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvFulilianCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvFulilianCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvFulilianCommand('/root/venv/bin/fulilian.exe', [], deps), null)
})

test('resolveVenvFulilianCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvFulilianCommand('/root/venv/Scripts/fulilian.exe', [], deps), null)
})

test('resolveVenvFulilianCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportFulilianCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvFulilianCommand('/root/venv/Scripts/fulilian.exe', ['serve'], deps)
...
test('resolveVenvFulilianCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvFulilianCommand('/root/venv/Scripts/fulilian.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'fulilian_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvFulilianCommand: is case-insensitive on fulilian.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvFulilianCommand('/root/venv/Scripts/FULILIAN.EXE', [], deps))
  assert.ok(resolveVenvFulilianCommand('/root/venv/SCRIPTS/fulilian.exe', [], deps))
})
