 ...
 *
 * The staged Tauri `fulilian-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `fulilian update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `fulilian update`, swaps/relaunches
 * the app, and writes .fulilian-update-result.json. With the app gone before
 * the update starts, the FULILIAN_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `fulilian update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into FULILIAN_HOME on *every* platform
 * (`fulilian-setup.exe` on Windows, `fulilian-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_fulilian_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `fulilian update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(fulilianHome, 'fulilian-setup.exe')
 ...
 *
 * `copy_self_to_fulilian_home` deliberately no-ops during `--update`
 ...
 * "Another Fulilian update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `fulilian update`, which
 ...
 * the installer is written to FULILIAN_HOME at install/repair time, making mtime
