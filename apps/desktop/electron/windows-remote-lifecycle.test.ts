...
test('Windows spawn holds the update mutex across marker check and helper spawn', () => {
  const command = atomicWindowsSpawnCommand({
    fulilianHome: 'C:\\Users\\andre\\.fulilian',
    python: 'C:\\Users\\andre\\.fulilian\\python.exe'
  })

  const encoded = command.match(/-EncodedCommand\s+([^\s]+)$/)?.[1]
  const script = encoded ? Buffer.from(encoded, 'base64').toString('utf16le') : ''
  assert.match(script, /\.fulilian-update-in-progress/)
...
test('Windows spawn publishes the initial ownership record before releasing the mutex', () => {
  const command = atomicWindowsSpawnCommand(
    ...
    },
    {
      ...
      profile: 'default',
      fulilianPath: 'C:\\Fulilian\\fulilian.exe',
      fulilianHome: 'C:\\Users\\andre\\.fulilian',
...
test('Windows relaunch gate refuses live and uncertain markers before executing the remote runtime', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = sshWith(async command => {
      ...
      if (script.includes('Get-Command fulilian.exe')) {
        return JSON.stringify({
          os: 'Windows',
          arch: 'AMD64',
          fulilianHome: 'C:\\Users\\alice\\.fulilian',
          fulilianPath: 'C:\\Fulilian\\fulilian.exe',
          python: 'C:\\Fulilian\\python.exe'
        })
      }

      if (script.includes('.fulilian-update-in-progress')) {
        return observation
      }
    ...
    assert.equal(
      scripts.some(script => script.includes('fulilian_cli.windows_ssh_runtime')),
      false
    )
  }
})
...
test('Windows relaunch gate uses strict install-wide marker parsing and fail-closed PID probing', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(ssh, 'C:\\Users\\alice\\.fulilian\\profiles\\research')
  assert.match(script, /\.fulilian-update-in-progress/)
...
test('Windows probe validates Fulilian and Python topology before selection', async () => {
  let script = ''
  ...
  const fallbackJoin = script.indexOf('Join-Path $fulilianHome')
  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('Fulilian is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /Fulilian is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\Fulilian's\\python.exe" }, 'inspect', [
    'C:\\x y\\fulilian.exe'
  ])

  const encoded = command.split(' ').pop()!
  const script = Buffer.from(encoded, 'base64').toString('utf16le')
  assert.match(script, /-m' 'fulilian_cli\.windows_ssh_runtime' 'inspect'/)
  assert.match(script, /Fulilian''s/)
  assert.match(script, /C:\\x y\\fulilian\.exe/)
})
...
test('Windows SSH reuse requires the requested remote profile to match the lock', () => {
  ...
  const runtime = { fulilianPath: lock.fulilianPath, fulilianHome: lock.fulilianHome }
