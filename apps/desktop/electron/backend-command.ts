// Backend subcommand routing for the desktop-managed Fulilian process.
//
// The desktop app launches its own headless backend via `fulilian serve` — it
...
// app hasn't updated yet, or an older `fulilian` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Fulilian profile to pin via `--profile`.
 */
 ...
 * `-m fulilian_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `fulilian_cli/subcommands/dashboard.py` source registers
