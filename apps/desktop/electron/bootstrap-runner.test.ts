...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'fulilian-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/fulilian-runner-test',
    sourceRepoRoot: null,
    fulilianHome: '/tmp/fulilian-runner-test',
    logRoot: '/tmp/fulilian-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'fulilian-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'fulilian-agent')
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/fulilian-agent', '--fulilian-home', '/tmp/fulilian', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/fulilian',
      fulilianHome: '/tmp/home'
    }),
    ['--dir', '/tmp/fulilian', '--fulilian-home', '/tmp/home', '--branch', 'main']
  )
})
