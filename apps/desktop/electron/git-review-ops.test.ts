...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'fulilian-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'fulilian-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Fulilian Test'], { cwd: dir })
