...
function withTempDir(run: (dir: string) => void) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'fulilian-secret-file-'))
...
test('attachment data URL helper reads bytes above the preview default without changing that limit', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'fulilian-desktop-large-attachment-'))
...
test('resolveRequestedPathForIpc resolves relative paths from the trimmed base directory', () => {
  const baseDir = path.join(os.tmpdir(), 'fulilian-desktop-base')
...
test('resolveReadableFileForIpc validates existence type size and sensitivity', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'fulilian-desktop-hardening-'))
...
test('resolveReadableFileForIpc blocks common sensitive files', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'fulilian-desktop-sensitive-'))
...
test('resolveReadableFileForIpc blocks symlinks whose realpath is sensitive', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'fulilian-desktop-realpath-'))
...
test('resolveDirectoryForIpc accepts directories and rejects invalid directory targets', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'fulilian-desktop-dir-'))
...
test('resolveDirectoryForIpc accepts directory symlinks or junctions', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'fulilian-desktop-dir-link-'))
...
// (see windows-fulilian-resolution.test.ts). These pin the propagation the PR
...
test('connection-config save and apply IPC handlers route payloads through coerceDesktopConnectionConfig', () => {
  ...
  for (const channel of ['fulilian:connection-config:save', 'fulilian:connection-config:apply']) {
