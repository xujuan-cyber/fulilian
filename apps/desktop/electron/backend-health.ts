...
export interface FulilianReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForFulilianReady`).
   */
 ...
 *  - readiness-probe exhaustion in waitForFulilianReady().
 *
...
/**
 * True when the backend URL points at a Nous-managed Fulilian Cloud instance
 ...
 * `isReauthRequired` so startFulilian latches; `needsOauthLogin` alone only
...
export async function waitForFulilianReady(baseUrl: string, options: FulilianReadyOptions): Promise<void> {
  ...
  throw new Error(`Fulilian backend did not become ready: ${detail}`)
}
