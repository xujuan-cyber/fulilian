...
export function registerGitIpc({ resolveGitBinary, resolveGhBinary }: GitIpcDeps) {
  ...
  ipcMain.handle('fulilian:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

  ipcMain.handle('fulilian:git:worktreeAdd', async (_event, repoPath, options) =>
    addWorktree(repoPath, options || {}, resolveGitBinary())
  )

  ipcMain.handle('fulilian:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
    removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
  )

  ipcMain.handle('fulilian:git:branchSwitch', async (_event, repoPath, branch) =>
    switchBranch(repoPath, branch, resolveGitBinary())
  )

  ipcMain.handle('fulilian:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))

  ipcMain.handle('fulilian:git:baseBranchList', async (_event, repoPath) =>
  ...
  ipcMain.handle('fulilian:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
  ...
  ipcMain.handle('fulilian:git:review:list', async (_event, repoPath, scope, baseRef) =>
    reviewList(repoPath, scope, baseRef, resolveGitBinary())
  )
  ipcMain.handle('fulilian:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
  ...
  ipcMain.handle('fulilian:git:fileDiff', async (_event, repoPath, filePath) =>
    fileDiffVsHead(repoPath, filePath, resolveGitBinary())
  )
  ipcMain.handle('fulilian:git:review:stage', async (_event, repoPath, filePath) =>
    reviewStage(repoPath, filePath ?? null, resolveGitBinary())
  )
  ipcMain.handle('fulilian:git:review:unstage', async (_event, repoPath, filePath) =>
    reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
  )
  ipcMain.handle('fulilian:git:review:revert', async (_event, repoPath, filePath) =>
    reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
  )
  ipcMain.handle('fulilian:git:review:revParse', async (_event, repoPath, ref) =>
    reviewRevParse(repoPath, ref, resolveGitBinary())
  )
  ipcMain.handle('fulilian:git:review:commit', async (_event, repoPath, message, push) =>
    reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
  )
  ipcMain.handle('fulilian:git:review:commitContext', async (_event, repoPath) =>
  ...
  ipcMain.handle('fulilian:git:review:fetchPrComment', async (_event, repoPath, url) =>
    reviewFetchPrComment(repoPath, resolveGhBinary(), url)
  )
  ipcMain.handle('fulilian:git:review:createPr', async (_event, repoPath) =>
  ...
  ipcMain.handle('fulilian:git:scanRepos', async (_event, roots, options) => {
    try {
