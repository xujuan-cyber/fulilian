// Profile-delete routing logic for the `fulilian:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_fulilian_home() and
...
/** Parse a `fulilian:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its FULILIAN_HOME.
 *
...
/**
 * Route the next `fulilian:api` request away from the primary/window backend
 ...
 * ensure_fulilian_home() recreates the directory the delete just removed.
 */
