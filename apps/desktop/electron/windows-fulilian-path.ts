/**
 * windows-fulilian-path.ts
 *
 * Pure, dependency-injected pieces of Windows `fulilian` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvFulilianCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `fulilian` shim shadowed the real
 *      fulilian.cmd/fulilian.exe; the shim then failed the --version probe and
 ...
 *      and Fulilian launcher. Marker-only or partial runtimes must use --repair;
 *      only a runnable pair can use --update.
 *   3. resolveVenvFulilianCommand() — unwrapWindowsVenvFulilianCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `fulilian`) must
 * not shadow `fulilian.cmd`/`fulilian.exe`. The empty entry is kept LAST so
 ...
 * venv Python interpreter and the Fulilian launcher that drives `fulilian update`.
...
export interface ResolveVenvFulilianCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportFulilianCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `fulilian`/`fulilian.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/fulilian(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m fulilian_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportFulilianCli(). A
 * venv whose update died mid-`pip install` still has python.exe + fulilian.exe
 ...
 *
 * Returns null when `command` is not a venv fulilian shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvFulilianCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvFulilianCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^fulilian(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Fulilian at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Fulilian Python at ${python}`,
    command: python,
