...
export function resolveDesktopRemoteRoute({
  ...
  if (envUrl) {
    ...
    if (!envToken) {
      throw new Error(
        'FULILIAN_DESKTOP_REMOTE_URL is set but FULILIAN_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Fulilian backend.'
      )
    }
  ...
  if (!modeIsRemoteLike(config.mode)) {
    ...
    // resolves local and spawns a loopback `fulilian serve` the desktop never
