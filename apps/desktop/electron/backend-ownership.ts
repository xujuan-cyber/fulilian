  ...
  return {
    ...
    async reapOrphans(): Promise<number[]> {
      ...
      for (const entry of entries) {
        ...
        // if it reaches reapOrphans (see main.ts startFulilian + #87295).
...
export function backendCommandMatches(command: unknown): boolean {
  return /(?:^|[\s/\\"])(?:fulilian(?:\.exe)?|fulilian_cli\.main|fulilian_cli[/\\]main\.py)"?(?:\s+(?:--profile|-p)\s+\S+)?\s+(?:serve|dashboard)(?:\s|$)/i.test(
    String(command ?? '')
  )
}
