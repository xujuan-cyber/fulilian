...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'fulilian_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'fulilian_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'fulilian_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Fulilian.app/Contents/MacOS/Fulilian', 'darwin'),
    '/Applications/Fulilian.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Fulilian.app/Contents/MacOS/Fulilian', 'darwin'),
    '/Users/x/Applications/Fulilian.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Fulilian\\Fulilian.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Fulilian'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\fulilian-desktop\\Fulilian.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\fulilian-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Fulilian.exe', 'win32'), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Fulilian.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Fulilian.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.fulilian/fulilian-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.fulilian/fulilian-agent',
    uninstallArgs: ['-m', 'fulilian_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/fulilian/linux-unpacked',
    fulilianHome: '/home/x/.fulilian'
  })
  ...
  assert.match(script, /'-m' 'fulilian_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/fulilian\/linux-unpacked'/)
  assert.match(script, /export FULILIAN_HOME='\/home\/x\/\.fulilian'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.fulilian/fulilian-agent',
    agentRoot: '/home/x/.fulilian/fulilian-agent',
    uninstallArgs: ['-m', 'fulilian_cli.uninstall', '--mode', 'full'],
    appPath: null,
    fulilianHome: '/home/x/.fulilian'
  })

  // System python + source on PYTHONPATH so import fulilian_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.fulilian\/fulilian-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'fulilian_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'fulilian_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    fulilianHome: '/h'
  })
  ...
  assert.match(script, /'-m' 'fulilian_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    pythonPath: 'C:\\fulilian',
    agentRoot: 'C:\\fulilian',
    uninstallArgs: ['-m', 'fulilian_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Fulilian',
    fulilianHome: 'C:\\Users\\x\\AppData\\Local\\fulilian'
  })

  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import fulilian_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\fulilian;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "fulilian_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Fulilian" >nul 2>&1/)
