 ...
 * the three user-facing uninstall modes to the `fulilian uninstall` CLI flags,
 ...
 *              `fulilian uninstall --gui --yes`
 *   - 'lite' → remove the GUI + agent code, KEEP user data (config / sessions
 *              / .env) for a future reinstall. `fulilian uninstall --yes`
 *   - 'full' → remove everything: GUI + agent + all user data.
 *              `fulilian uninstall --full --yes`
 *
 * Why a detached cleanup script: 'lite'/'full' delete the very venv the
 * `fulilian` command runs from, and every mode may need to delete the running
...
/**
 * Map an uninstall mode to the `python -m fulilian_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `fulilian_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'fulilian_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Fulilian.app/Contents/MacOS/Fulilian  → …/Fulilian.app
 *   Windows: …\Fulilian\Fulilian.exe                 → …\Fulilian  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Fulilian.app/Contents/MacOS/Fulilian → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Fulilian.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Fulilian.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Fulilian$/i.test(dir) || /[\\/]fulilian-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/fulilian
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import fulilian_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, fulilianHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export FULILIAN_HOME=${q(fulilianHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import fulilian_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Fulilian install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "FULILIAN_HOME=${String(fulilianHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
