 ...
 * wire protocol:
 *
 *   gate dials -> drain every captured scope -> detached `fulilian update`
...
function buildPosixManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const home = validateRemoteValue(target.fulilianHome, 'Fulilian home')
  const fulilianPath = validateRemoteValue(target.fulilianPath, 'launcher path')
  ...
  const launcherWord = expandRemotePath(fulilianPath)

  const updateCommand =
    `env FULILIAN_HOME=${homeWord} ` +
    `FULILIAN_UPDATE_CORRELATION_ID=${shq(correlation)} ` +
    'FULILIAN_UPDATE_ORIGIN_PROFILE=default ' +
    `FULILIAN_UPDATE_ORIGIN_HOME=${homeWord} ` +
    `FULILIAN_UPDATE_OUTPUT_PATH=${outputWord} ` +
...
function buildWindowsManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const wrapper = [
    ...
    `$env:FULILIAN_HOME=${psLiteral(home)}`,
    `$env:FULILIAN_UPDATE_CORRELATION_ID=${psLiteral(correlation)}`,
    '$env:FULILIAN_UPDATE_ORIGIN_PROFILE="default"',
    `$env:FULILIAN_UPDATE_ORIGIN_HOME=${psLiteral(home)}`,
    `$env:FULILIAN_UPDATE_OUTPUT_PATH=${psLiteral(outputPath)}`,
    ...
    `$env:FULILIAN_UPDATE_WINDOWS_DETACHED=${psLiteral(correlation)}`,
    `$env:FULILIAN_UPDATE_TAURI_OUTCOME_PATH=${psLiteral(statusPath)}`,
    `$env:FULILIAN_UPDATE_TAURI_READY_PATH=${psLiteral(readyPath)}`,
    ...
    `& ${psLiteral(fulilianPath)} update --yes *>> ${psLiteral(outputPath)}`,
    '$rc=$LASTEXITCODE',
...
marker_path=install_root/'.fulilian-update-in-progress'
...
async function runManagedSshUpdate<TScope extends ManagedSshScope>(
  ...
  return {
    ...
    message:
      outcome === 'updated'
        ? 'Remote Fulilian updated and every managed SSH profile is ready.'
        : restoreOk
