 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of FuLiLian by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.fulilian-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(fulilianHome) {
  return path.join(fulilianHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.fulilian/fulilian-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(fulilianHome) {
  if (!fulilianHome) {
    return null
  }

  const candidate = path.join(fulilianHome, 'fulilian-agent', 'scripts', installScriptName())
...
function cachedScriptPath(fulilianHome, commit) {
  return path.join(bootstrapCacheDir(fulilianHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/xujuan-cyber/fulilian/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(fulilianHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(fulilianHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, fulilianHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass FULILIAN_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          FULILIAN_HOME: fulilianHome || process.env.FULILIAN_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, fulilianHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, fulilianHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--fulilian-home', fulilianHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, fulilianHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, fulilianHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, fulilianHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(fulilianHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, fulilianHome, emit })
