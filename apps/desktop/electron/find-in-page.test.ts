...
describe('installFoundInPageForwarder', () => {
  ...
  test('two forwarders installed on distinct webContents do not cross-fire', () => {
    ...
    assert.deepEqual(wcA.calls.send, [
      { channel: 'fulilian:found-in-page', payload: { activeMatchOrdinal: 1, count: 1 } }
    ])
...
describe('installFindShortcut', () => {
  ...
  test('sends fulilian:open-find-bar on Ctrl+F (Linux/Windows) and prevents default', () => {
    const wc = makeFakeWebContents()
    ...
    assert.deepEqual(wc.calls.send, [{ channel: 'fulilian:open-find-bar', payload: undefined }])
  ...
  test('sends fulilian:open-find-bar on Cmd+F (meta) on macOS and prevents default', () => {
    const wc = makeFakeWebContents()
    const win = makeFakeWindow(wc)
