...
test('isWindowsBinaryPathInWsl blocks Windows binary types on WSL', () => {
  assert.equal(isWindowsBinaryPathInWsl('/mnt/c/Tools/fulilian.exe', { isWsl: true }), true)
  assert.equal(isWindowsBinaryPathInWsl('/mnt/c/Tools/fulilian.cmd', { isWsl: true }), true)
  assert.equal(isWindowsBinaryPathInWsl('/mnt/c/Tools/fulilian.bat', { isWsl: true }), true)
  assert.equal(isWindowsBinaryPathInWsl('/mnt/c/Tools/install.ps1', { isWsl: true }), true)
  assert.equal(isWindowsBinaryPathInWsl('/usr/local/bin/fulilian', { isWsl: true }), false)
  assert.equal(isWindowsBinaryPathInWsl('/mnt/c/Tools/fulilian.exe', { isWsl: false }), false)
})
...
test('detectRemoteDisplay honors the FULILIAN_DESKTOP_DISABLE_GPU override both ways', () => {
  // Force-on even on a local display.
  assert.match(
    String(detectRemoteDisplay({ env: { FULILIAN_DESKTOP_DISABLE_GPU: '1', DISPLAY: ':0' }, platform: 'linux' })),
    /override/
  )
  // Force-off even over SSH (escape hatch when a remote display has working accel).
  assert.equal(
    detectRemoteDisplay({
      env: { FULILIAN_DESKTOP_DISABLE_GPU: 'false', SSH_CONNECTION: '1.2.3.4 5 6.7.8.9 22' },
      platform: 'linux'
    }),
    null
  )
...
test('resolveLinuxPasswordStore is a no-op when the env var is unset', () => {
  ...
  assert.deepEqual(resolveLinuxPasswordStore({ env: { FULILIAN_DESKTOP_PASSWORD_STORE: '  ' }, platform: 'linux' }), {
    store: null,
    warning: null
  })
})
...
test('resolveLinuxPasswordStore ignores the env var off linux', () => {
  assert.deepEqual(
    resolveLinuxPasswordStore({ env: { FULILIAN_DESKTOP_PASSWORD_STORE: 'gnome-libsecret' }, platform: 'darwin' }),
    { store: null, warning: null }
  )
  assert.deepEqual(
    resolveLinuxPasswordStore({ env: { FULILIAN_DESKTOP_PASSWORD_STORE: 'kwallet6' }, platform: 'win32' }),
    { store: null, warning: null }
  )
})
...
test('resolveLinuxPasswordStore warns on unknown values instead of applying them', () => {
  const result = resolveLinuxPasswordStore({
    env: { FULILIAN_DESKTOP_PASSWORD_STORE: 'keychain-of-wonders' },
    platform: 'linux'
  })
