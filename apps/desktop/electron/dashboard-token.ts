 ...
 *
 * The desktop main process can pass FULILIAN_DASHBOARD_SESSION_TOKEN when it
...
async function fetchPublicText(url, options: any = {}) {
  ...
  if (protocol !== 'http:' && protocol !== 'https:') {
    throw new Error(`Unsupported Fulilian backend URL protocol: ${protocol}`)
  }
  ...
  const res = await fetch(url, { signal: AbortSignal.timeout(timeoutMs) }).catch(error => {
    if (error.name === 'TimeoutError') {
      throw new Error(`Timed out connecting to Fulilian backend after ${timeoutMs}ms`)
    }
...
function extractInjectedDashboardToken(html) {
  const match = /window\.__FULILIAN_SESSION_TOKEN__\s*=\s*("(?:\\.|[^"\\])*")/.exec(String(html || ''))
 ...
 */
async function adoptServedDashboardToken(baseUrl, spawnToken, { childAlive, label = 'Fulilian backend', ...options }) {
