// Launching the Fulilian TUI in the user's OWN terminal emulator.
//
...
// `fulilian --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's Fulilian runtime is often a venv Python invoked
//     as `python -m fulilian_cli.main`, not a `fulilian` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The Fulilian
...
export function terminalScriptEnv(
  ...
  if (fulilianHome) {
    out.FULILIAN_HOME = fulilianHome
  }
