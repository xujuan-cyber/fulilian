 ...
 * product name (`Fulilian`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Fulilian'
 *
 * This is a hard failure with no obvious cause for the user — `fulilian desktop`
 ...
 * (the mac-unpacked Fulilian.app bundle) and Windows (win-unpacked), so we
 ...
 * Fulilian.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `fulilian desktop --build-only` (fulilian_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Fulilian.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Fulilian'}.exe`
