#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Fulilian icon + version metadata onto the
// built Fulilian.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Fulilian.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `fulilian desktop`, the installer's
...
// path (which rebuilds via `fulilian desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-Fulilian.exe>
//
...
// Stamp the Fulilian icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  console.log('[set-exe-identity] done — Fulilian icon + identity stamped')
}
