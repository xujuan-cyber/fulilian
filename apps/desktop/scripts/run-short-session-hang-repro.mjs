...
const LEGACY_STORAGE_KEY = 'fulilian.desktop.inflightTurnJournal.v1'
const LEGACY_MIGRATION_KEY = 'fulilian.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.fulilianDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `fulilian-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const fulilianHome = join(sandbox, 'fulilian-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(fulilianHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(fulilianHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        FULILIAN_DESKTOP_APP_NAME: `FulilianShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        FULILIAN_DESKTOP_FULILIAN_ROOT: target.targetRoot,
        FULILIAN_DESKTOP_IGNORE_EXISTING: '1',
        FULILIAN_DESKTOP_USER_DATA_DIR: userData,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'fulilian-short-session-ab-'))
  const prepared = []
