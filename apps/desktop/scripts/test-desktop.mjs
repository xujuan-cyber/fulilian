...
// no longer bundles the FuLiLian Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Fulilian.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Fulilian'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Fulilian.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Fulilian'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default FULILIAN_HOME for non-sandboxed runs -- matches main.ts's
// resolveFulilianHome(). On Windows it's %LOCALAPPDATA%\fulilian; elsewhere
// it's ~/.fulilian. The fresh-install sandbox launchFresh() sets its own
// FULILIAN_HOME and never touches this.
const DEFAULT_FULILIAN_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'fulilian')
  }
  return path.join(os.homedir(), '.fulilian')
})()
const VENV_ROOT = path.join(DEFAULT_FULILIAN_HOME, 'fulilian-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'fulilian-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.FULILIAN_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Fulilian-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Fulilian-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Fulilian-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Fulilian-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.FULILIAN_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.FULILIAN_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const fulilianHome = path.join(sandbox, 'fulilian-home')
  ...
  env.FULILIAN_DESKTOP_CWD = cwd
  env.FULILIAN_DESKTOP_IGNORE_EXISTING = '1'
  env.FULILIAN_DESKTOP_TEST_MODE = 'fresh-install'
  env.FULILIAN_DESKTOP_USER_DATA_DIR = userDataDir
  env.FULILIAN_HOME = fulilianHome
  delete env.FULILIAN_DESKTOP_FULILIAN
  delete env.FULILIAN_DESKTOP_FULILIAN_ROOT
  ...
  console.log(`  FULILIAN_HOME: ${fulilianHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(fulilianHome, 'fulilian-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The FuLiLian Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of fulilian_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'fulilian-agent', 'fulilian_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Fulilian
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + FULILIAN_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  FULILIAN_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
