...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `fulilian-dev-mock-${Date.now()}`))
  ...
  return { root, fulilianHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(fulilianHome, mockUrl) {
  fs.writeFileSync(
    path.join(fulilianHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(fulilianHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.fulilianHome, mock.url)
  console.log(`  FULILIAN_HOME: ${sandbox.fulilianHome}`)
  ...
  const env = {
    ...
    FULILIAN_DESKTOP_APP_NAME: `FulilianDevMock-${Date.now()}`,
  }
