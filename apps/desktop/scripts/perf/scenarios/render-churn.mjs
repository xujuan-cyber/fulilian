...
// session per flush via `__FULILIAN_SESSION_TILES__`, no backend, no credits —
  ...
  (() => {
    if (window.__RC__) {
      ...
      for (const { sid, rid } of window.__RC__.ids) {
        const states = window.__FULILIAN_SESSION_TILES__.states()
        window.__FULILIAN_SESSION_TILES__.publish(rid, { ...states[rid], busy: false, streamId: null })
        window.__FULILIAN_SESSION_TILES__.close(sid)
      }
      window.__RC__ = null
    }
