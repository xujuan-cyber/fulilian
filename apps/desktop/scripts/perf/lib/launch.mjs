...
//   · its own FULILIAN_HOME      → its own backend + sessions, no shared state.
//   · its own --remote-debugging-port → a private CDP endpoint.
//   · FULILIAN_DESKTOP_BOOT_FAKE=1 → deterministic boot overlay.
...
// Seed an isolated FULILIAN_HOME with just enough config (NOT sessions) so the
// spawned instance reaches an empty chat view instead of the onboarding wizard.
// A separate FULILIAN_HOME dir means a separate gateway lock — no collision with
...
export async function startIsolatedInstance({
  ...
  const home = fulilianHome ?? mkTemp('fulilian-perf-home-')
  const userData = userDataDir ?? mkTemp('fulilian-perf-ud-')
  const devUrl = prod ? null : `http://127.0.0.1:${devPort}`

  if (seedConfig && !fulilianHome) {
    seedConfigFrom(join(homedir(), '.fulilian'), home)
  }
  ...
  try {
    ...
    // FULILIAN_HOME (backend + sessions). No DEV_SERVER env in prod → dist load.
    const electronBin = require('electron')
    // NB: do NOT set FULILIAN_DESKTOP_BOOT_FAKE here — it injects artificial
    ...
    const env = {
      ...
      FULILIAN_DESKTOP_CDP_PORT: String(port),
      XCURSOR_SIZE: '24'
    }

    if (devUrl) {
      env.FULILIAN_DESKTOP_DEV_SERVER = devUrl
    }
...
export async function coldStartSamples({ runs = 3, port = 9222, devPort = 5174, prod = false, warm = true } = {}) {
  ...
  if (warm) {
    ...
    const home = mkdtempSync(join(tmpdir(), 'fulilian-perf-cold-home-'))
    const userDataDir = mkdtempSync(join(tmpdir(), 'fulilian-perf-cold-ud-'))
    seedConfigFrom(join(homedir(), '.fulilian'), home)
