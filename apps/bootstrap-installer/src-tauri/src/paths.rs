...
//!
//! Mirrors `fulilian_constants.get_fulilian_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\fulilian
//!   macOS:   ~/.fulilian
//!   Linux:   ~/.fulilian  (override via $FULILIAN_HOME)
//!
//! NOTE (macOS): Python's get_fulilian_home(), scripts/install.sh, and the
//! Electron desktop's resolveFulilianHome() ALL use ~/.fulilian on macOS — there
...
/// Returns the canonical Fulilian home directory, respecting $FULILIAN_HOME if set.
pub fn fulilian_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("FULILIAN_HOME") {
    ...
    {
        // %LOCALAPPDATA%\fulilian — matches scripts/install.ps1's $FulilianHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("fulilian");
        }
    }

    // macOS + Linux + fallback: ~/.fulilian (matches Python get_fulilian_home(),
    // install.sh, and the Electron desktop's resolveFulilianHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".fulilian");
    }

    // Last resort — current dir, almost certainly wrong but at least
    // doesn't panic.
    PathBuf::from(".fulilian")
}

pub fn log_dir() -> PathBuf {
    fulilian_home().join("logs")
}
...
pub fn bootstrap_cache_dir() -> PathBuf {
    fulilian_home().join("bootstrap-cache")
}
...
/// FULILIAN_HOME so it survives repo checkout deletion (unlike anything under
/// fulilian-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\fulilian\fulilian-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_fulilian`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under FULILIAN_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves FULILIAN_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    fulilian_home().join(".fulilian-update-in-progress")
}
...
pub fn copy_self_to_fulilian_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to FULILIAN_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_FULILIAN_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".fulilian-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under FULILIAN_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[fulilian-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("FULILIAN_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_fulilian_home() -> String {
    fulilian_home().to_string_lossy().into_owned()
}
