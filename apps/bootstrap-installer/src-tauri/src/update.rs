...
//!
//! Driven when the installer is launched as `Fulilian-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Fulilian desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `fulilian update`
//!      or repair bootstrap can race locked files),
//!   2. run `fulilian update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in fulilian_cli/main.py),
//!   3. run `fulilian desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `fulilian update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_fulilian) and
...
/// `fulilian update` exit code meaning "another fulilian process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// fulilian_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `fulilian update`'s own guard decide.
...
///
/// The marker is also the cross-process update lock: `fulilian update` claims
/// the same file (see `fulilian_cli/update_lock.py`) so a dashboard-spawned
...
/// overwriting — the pre-fix clobber is what let a dashboard `fulilian update`
...
/// UPDATE_MARKER_MAX_AGE_SECONDS in fulilian_cli/update_lock.py — all three read
...
/// a dashboard-spawned `fulilian update`) still blocks.
...
/// the marker name our PID — because a `fulilian update` child that
...
async fn run_update(app: AppHandle) -> Result<()> {
    let fulilian_home = crate::paths::fulilian_home();
    let install_root = fulilian_home.join("fulilian-agent");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_fulilian`
    ...
    //
    // The same marker is the cross-process update lock (fulilian_cli/
    // update_lock.py claims it too), so a live foreign owner means another
    // updater — most often a dashboard-spawned `fulilian update` — is already
    ...
    let _update_marker = match UpdateMarkerGuard::acquire(
        ...
        Err(owner) => {
            ...
            let msg = format!(
                "Another Fulilian update is already running (PID {}, started {} ago). \
    ...
    let fulilian = resolve_fulilian(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the fulilian CLI under {}. Is Fulilian installed? \
    ...
    // async on Windows. If it still holds the venv shim, `fulilian update`
    ...
    // ---- stage 2: fulilian update -----------------------------------------
    // Pass --branch so `fulilian update` targets the branch this installer was
    ...
    // without --branch, `fulilian update` switches the checkout to `main` (a
    ...
    // --force skips `fulilian update`'s Windows running-exe guard (which would
    ...
    // time `fulilian update` runs there is no legitimate fulilian.exe to protect,
    // and the guard would only produce a false "Fulilian is still running" stop.
    //
    ...
    // probe only checks the fulilian.exe shim and app.asar, so an external venv
    ...
    // below surfaces the correct "close all Fulilian windows" message.
    ...
    // Retry-once for the update-boundary crash. `fulilian update` lazily imports
    ...
    // second `fulilian update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Fulilian" state
    // a retry can't fix.
    ...
    // checkout predating the FULILIAN_UPDATE_HANDOFF_PID env fix (8c76fe19f)
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Fulilian is still running. Close all Fulilian windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "fulilian update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::fulilian_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 3: fulilian desktop --build-only ----------------------------
    // `fulilian update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `fulilian update` already does above, and `fulilian update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `fulilian desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Fulilian manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_fulilian_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Fulilian to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            ...
            // Fulilian.exe, so an image-name kill would tear down the app itself.
            emit_log(
                app,
                Some(stage),
                LogStream::Stdout,
                &format!(
                    "[handoff] Fulilian still holding install files ({}); locating backend shims…",
                    format_locked_paths(&locked)
                ),
            );
            let shim = venv_fulilian(install_root);
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_fulilian(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("Fulilian.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("Fulilian.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Find processes running the exact `venv\Scripts\fulilian.exe` shim for this
/// installation. Windows image names are case-insensitive and the desktop is
/// also Fulilian.exe, so matching by image name alone is unsafe.
#[cfg(windows)]
...
/// Spawn `fulilian <args>` from `cwd`, stream stdout/stderr as Log events on the
...
async fn run_streamed(
    ...
    // the same rule about pipe EOF: `fulilian update` is precisely the shape that
...
/// Path to the venv fulilian shim under an install root, regardless of existence.
fn venv_fulilian(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("fulilian.exe")
    } else {
        install_root.join("venv").join("bin").join("fulilian")
    }
}

/// Resolve the fulilian CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `fulilian` on PATH.
fn resolve_fulilian(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_fulilian(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "fulilian.exe" } else { "fulilian" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let fulilian_home = crate::paths::fulilian_home();
    let mut envs = vec![(
        "FULILIAN_HOME".to_string(),
        fulilian_home.as_os_str().to_os_string(),
    )];
    // `fulilian update` is a Python CLI writing to a pipe here, so CPython
    ...
    // `fulilian update` child claims that SAME lock (fulilian_cli/update_lock.py).
    ...
    // refuses its parent's marker with exit 2 ("Fulilian is still running")
    // and no number of retries can ever succeed. Keep the variable name in
    // sync with HANDOFF_PID_ENV in fulilian_cli/update_lock.py.
    envs.push((
        "FULILIAN_UPDATE_HANDOFF_PID".to_string(),
        OsString::from(std::process::id().to_string()),
    ));
    if let Some(path) = path_with_prepended_entries(&[
        fulilian_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_fulilian_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Fulilian.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.fulilian-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.fulilian-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::fulilian_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_fulilian_is_under_install_root() {
        let root = Path::new("/x/fulilian-agent");
        let shim = venv_fulilian(root);
    ...
    fn update_child_env_forces_unbuffered_python() {
        let envs = update_child_env(Path::new("/x/fulilian-agent"));
        assert!(
            envs.iter()
    ...
    fn update_child_env_names_our_pid_for_the_lock_handoff() {
        let envs = update_child_env(Path::new("/x/fulilian-agent"));
        assert!(
            envs.iter().any(|(k, v)| k == "FULILIAN_UPDATE_HANDOFF_PID"
                && v.to_str() == Some(std::process::id().to_string().as_str())),
            "the fulilian update child claims the same marker we hold; without our pid \
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_fulilian(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/fulilian-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn same_windows_path_accepts_case_only_difference() {
        assert!(same_windows_path(
            Path::new(r"C:\Users\tester\.fulilian\fulilian-agent\venv\scripts\FULILIAN.EXE"),
            Path::new(r"c:\users\tester\.fulilian\fulilian-agent\venv\Scripts\fulilian.exe"),
        ));
    }
    ...
    fn same_windows_path_rejects_desktop_binary() {
        assert!(!same_windows_path(
            Path::new(r"C:\Users\tester\.fulilian\fulilian-agent\apps\desktop\Fulilian.exe"),
            Path::new(r"C:\Users\tester\.fulilian\fulilian-agent\venv\Scripts\fulilian.exe"),
        ));
    }
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".fulilian-update-in-progress");
    ...
    fn acquire_refuses_while_a_live_updater_owns_the_marker() {
        ...
        // dashboard `fulilian update` and install-mode bootstrap mutate one tree
    ...
    fn acquire_adopts_a_marker_prewritten_with_our_own_pid() {
        // #74761: desktop writeUpdateMarker(fulilianHome, child.pid) races ahead
    ...
    // checkout's `fulilian update` reads it as a live foreign update and exits
    ...
    fn missing_or_garbage_marker_never_heals() {
        ...
        let garbage = dir.join(".fulilian-update-in-progress");
    ...
    fn heal_end_to_end_marker_lifecycle() {
        ...
        // own exit 2, e.g. a genuinely still-running Fulilian, stays terminal).
        assert!(!should_heal_self_marker_refusal(
            Some(UPDATE_EXIT_CONCURRENT),
            &marker
        ));
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Fulilian.app"]),
            Some(PathBuf::from("/Applications/Fulilian.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Fulilian.app");
        let tmp = base.join("Fulilian.app.fulilian-update-new");
        let old = base.join("Fulilian.app.fulilian-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Fulilian.app");
        let tmp = base.join("Fulilian.app.fulilian-update-new"); // intentionally absent
        let old = base.join("Fulilian.app.fulilian-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Fulilian.app");
        let tmp = base.join("Fulilian.app.fulilian-update-new"); // absent
        let old = base.join("Fulilian.app.fulilian-update-old");
        write_marker(&target, "OLD");
