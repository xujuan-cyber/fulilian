...
export default function ProgressScreen({ bootstrap }: ProgressProps) {
  ...
  const title = bootstrap.status === 'completed' ? 'Done' : isUpdate ? 'Updating Fulilian' : 'Setting up FuLiLian'

  const description = isUpdate
    ? 'Fulilian is updating to the latest version — this only takes a moment.'
    : 'This is a one-time setup. The Fulilian installer is downloading dependencies and configuring your machine. Subsequent launches will skip this step.'

  const pct = Math.round(progress.fraction * 100)

  return (
    <div className="fulilian-fade-in flex h-full flex-col">
